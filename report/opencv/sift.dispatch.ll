...
bb.n:                                             ; preds = %bb.t, %.noexc.i
  ...
  %20 = fmul float %i.np, f0x3B808081             ; 4 uses
  %21 = fmul float %i.nq, f0x3A808081             ; 5 uses
  ...
  %i.of = call float @llvm.fmuladd.f32(float %i.nk, float %20, float %i.oe) ; 2 uses
  %i.og = fmul float %21, %i.od
  %i.oh = extractelement <2 x float> %i.ob, i64 1 ; 7 uses
  %i.oi = call float @llvm.fmuladd.f32(float %i.oh, float %20, float %i.og)
  ...
  %i.on = fmul float %21, %i.om
  %i.oo = call float @llvm.fmuladd.f32(float %i.oh, float %i.oc, float %i.on)
  %i.op = call float @llvm.fmuladd.f32(float %21, float %i.oo, float %i.ol) ; 2 uses
...
bb.o:                                             ; preds = %bb.n
  ...
  %i.pf = call float @llvm.fmuladd.f32(float %i.oh, float %20, float %i.pd)
  %i.pg = extractelement <2 x float> %i.oz, i64 0 ; 3 uses
  %i.ph = call float @llvm.fmuladd.f32(float %i.pg, float %20, float %i.pe) ; 2 uses
