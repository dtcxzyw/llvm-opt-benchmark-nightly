...
begin_hunk_1_@wwunpack:bb.a
  %.391030 = phi ptr [ %.231014, %bb.fi ], [ %.291020, %bb.dw ], [ %.311022, %bb.ei ], [ %.371028, %bb.fh ], [ %.271018, %bb.dk ] ; 4 uses
  %.39986 = phi i32 [ %.23970, %bb.fi ], [ %.29976, %bb.dw ], [ %.31978, %bb.ei ], [ %.37984, %bb.fh ], [ %.27974, %bb.dk ] ; 4 uses
  %.42 = phi i32 [ %.26, %bb.fi ], [ %.32, %bb.dw ], [ %.34, %bb.ei ], [ %.40, %bb.fh ], [ %.30, %bb.dk ] ; 4 uses
  %.0927 = phi i32 [ %i.nw, %bb.fi ], [ %i.lc, %bb.dw ], [ %i.mb, %bb.ei ], [ %i.nv, %bb.fh ], [ %i.kf, %bb.dk ] ; 7 uses
  %i.nx = zext i32 %.0927 to i64                  ; 7 uses
  %i.ny = add nsw i32 %.0927, -1
  %or.cond1410.not = icmp uge i32 %i.ny, %1
  %.not1314 = icmp ult ptr %.010321488, %0
  %or.cond1451 = select i1 %or.cond1410.not, i1 true, i1 %.not1314
  br i1 %or.cond1451, label %.critedge.sink.split, label %bb.fk

bb.fk:                                            ; preds = %bb.fj
  %i.nz = ptrtoint ptr %.010321488 to i64         ; 2 uses
  %i.oa = add i64 %i.nx, %i.nz                    ; 2 uses
  %.not1315 = icmp ule i64 %i.oa, %i.i
  %i.ob = icmp ugt i64 %i.oa, %i.h
  %or.cond1411 = and i1 %.not1315, %i.ob
  %i.oc = icmp ugt i64 %i.i, %i.nz
  %or.cond1412 = and i1 %i.oc, %or.cond1411
  br i1 %or.cond1412, label %bb.fl, label %.critedge.sink.split

bb.fl:                                            ; preds = %bb.fk
  %i.od = zext nneg i32 %.7942 to i64
  %i.oe = sub nsw i64 0, %i.od                    ; 12 uses
  %i.of = getelementptr inbounds i8, ptr %.010321488, i64 %i.oe ; 2 uses
  %.not1316 = icmp ult ptr %i.of, %0
  br i1 %.not1316, label %.critedge.sink.split, label %bb.fm

bb.fm:                                            ; preds = %bb.fl
  %i.og = ptrtoint ptr %i.of to i64               ; 2 uses
  %i.oh = add i64 %i.nx, %i.og                    ; 2 uses
  %.not1317 = icmp ule i64 %i.oh, %i.i
  %i.oi = icmp ugt i64 %i.oh, %i.h
  %or.cond1413 = and i1 %.not1317, %i.oi
  %i.oj = icmp ugt i64 %i.i, %i.og
  %or.cond1414 = and i1 %i.oj, %or.cond1413
  br i1 %or.cond1414, label %iter.check, label %.critedge.sink.split

iter.check:                                       ; preds = %bb.fm
  %min.iters.check = icmp ult i32 %.0927, 4
  %i.ok = add i32 %.7942, -1
  %diff.check = icmp ult i32 %i.ok, 31
  %or.cond1637 = select i1 %min.iters.check, i1 true, i1 %diff.check
  br i1 %or.cond1637, label %.lr.ph.preheader, label %vector.main.loop.iter.check

vector.main.loop.iter.check:                      ; preds = %iter.check
  %min.iters.check1626 = icmp ult i32 %.0927, 32
  br i1 %min.iters.check1626, label %vec.epilog.ph, label %vector.ph

vector.ph:                                        ; preds = %vector.main.loop.iter.check
  %i.ol = and i64 %i.nx, 28
  %n.vec = and i64 %i.nx, 4294967264              ; 5 uses
  %i.om = trunc nuw i64 %n.vec to i32
  %i.on = sub i32 %.0927, %i.om
  %i.oo = getelementptr i8, ptr %.010321488, i64 %n.vec ; 2 uses
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ] ; 2 uses
  %next.gep = getelementptr i8, ptr %.010321488, i64 %index ; 3 uses
  %i.op = getelementptr inbounds i8, ptr %next.gep, i64 %i.oe ; 2 uses
  %i.oq = getelementptr inbounds nuw i8, ptr %i.op, i64 16
  %wide.load = load <16 x i8>, ptr %i.op, align 1, !tbaa !11
  %wide.load1627 = load <16 x i8>, ptr %i.oq, align 1, !tbaa !11
  %i.or = getelementptr i8, ptr %next.gep, i64 16
  store <16 x i8> %wide.load, ptr %next.gep, align 1, !tbaa !11
  store <16 x i8> %wide.load1627, ptr %i.or, align 1, !tbaa !11
  %index.next = add nuw i64 %index, 32            ; 2 uses
  %i.os = icmp eq i64 %index.next, %n.vec
  br i1 %i.os, label %middle.block, label %vector.body, !llvm.loop !12
...
vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  ...
  %n.vec1629 = and i64 %i.nx, 4294967292          ; 4 uses
  %i.ot = trunc nuw i64 %n.vec1629 to i32
  %i.ou = sub i32 %.0927, %i.ot
  %i.ov = getelementptr i8, ptr %.010321488, i64 %n.vec1629 ; 2 uses
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index1630 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next1633, %vec.epilog.vector.body ] ; 2 uses
  %next.gep1631 = getelementptr i8, ptr %.010321488, i64 %index1630 ; 2 uses
...
.loopexit:                                        ; preds = %.lr.ph.prol.loopexit, %.lr.ph, %middle.block, %vec.epilog.middle.block, %bb.av, %bb.o
  ...
  %.61038 = phi ptr [ %i.bw, %bb.o ], [ %i.fh, %bb.av ], [ %i.oo, %middle.block ], [ %i.ov, %vec.epilog.middle.block ], [ %.lcssa1639.unr, %.lr.ph.prol.loopexit ], [ %i.qc, %.lr.ph ]
...
bb.fv:                                            ; preds = %.lr.ph1508.split.a, %bb.fw
  ...
  %i.sf = icmp ugt i64 %i.se, %i.h
  %or.cond1420 = and i1 %.not1344, %i.sf
  %i.sg = icmp ugt i64 %i.i, %i.sd
...
bb.fw:                                            ; preds = %bb.fv
  %i.sh = getelementptr inbounds nuw [36 x i8], ptr %3, i64 %indvars.iv ; 3 uses
  %i.si = getelementptr inbounds nuw i8, ptr %i.sh, i64 4 ; 2 uses
  %i.sj = load i32, ptr %i.si, align 4, !tbaa !20
  %i.sk = getelementptr inbounds nuw i8, ptr %.19901506, i64 8
  store i32 %i.sj, ptr %i.sk, align 1, !tbaa !11
  %i.sl = load i32, ptr %i.sh, align 4, !tbaa !10
  %i.sm = getelementptr inbounds nuw i8, ptr %.19901506, i64 12
  store i32 %i.sl, ptr %i.sm, align 1, !tbaa !11
  %i.sn = load i32, ptr %i.si, align 4, !tbaa !20
  %i.so = getelementptr inbounds nuw i8, ptr %.19901506, i64 16
  store i32 %i.sn, ptr %i.so, align 1, !tbaa !11
  %i.sp = load i32, ptr %i.sh, align 4, !tbaa !10
  %i.sq = getelementptr inbounds nuw i8, ptr %.19901506, i64 20
  store i32 %i.sp, ptr %i.sq, align 1, !tbaa !11
...
._crit_edge:                                      ; preds = %bb.fw, %bb.fu
  ...
  %i.ss = icmp ult i32 %1, 40
  %.not1340 = icmp ult ptr %.1990.lcssa, %0
  %or.cond1453 = select i1 %i.ss, i1 true, i1 %.not1340
  br i1 %or.cond1453, label %bb.fy, label %bb.fx

bb.fx:                                            ; preds = %._crit_edge
  %i.st = ptrtoint ptr %.1990.lcssa to i64        ; 2 uses
  %i.su = add i64 %i.st, 40                       ; 2 uses
  %.not1341 = icmp ule i64 %i.su, %i.i
  %i.sv = icmp ugt i64 %i.su, %i.h
  %or.cond1422 = and i1 %.not1341, %i.sv
  %i.sw = icmp ugt i64 %i.i, %i.st
  %or.cond1423 = and i1 %i.sw, %or.cond1422
...
bb.fz:                                            ; preds = %bb.fx
  ...
  %i.sx = tail call i64 @cli_writen(i32 noundef %6, ptr noundef %0, i64 noundef %i.f) #5
  %.not1342 = icmp eq i64 %i.sx, %i.f
  %spec.select1424 = select i1 %.not1342, i32 0, i32 14
  br label %bb.ga

bb.ga:                                            ; preds = %bb.fz, %.critedge, %bb.fy, %.split1510, %bb.ft, %bb.fq
  %.0926 = phi i32 [ 26, %.split1510 ], [ 26, %bb.fq ], [ 26, %bb.fy ], [ 26, %bb.ft ], [ 1, %.critedge ], [ %spec.select1424, %bb.fz ]
  ret i32 %.0926
}
