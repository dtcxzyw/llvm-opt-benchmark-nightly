...
bb.d:                                             ; preds = %.lr.ph31, %bb.d
  ...
  %i.bm = getelementptr inbounds nuw [24 x i8], ptr %i.bc, i64 %i.bl ; 2 uses
  %i.bn = getelementptr inbounds nuw [8 x i8], ptr %i.be, i64 %.01630 ; 3 uses
  %1 = load <3 x double>, ptr %i.bm, align 8, !tbaa !109 ; 5 uses
  %.sroa.018.0.copyload.a = load double, ptr %i.bm, align 8, !tbaa !109
  %2 = shufflevector <3 x double> %1, <3 x double> poison, <2 x i32> <i32 1, i32 1>
  %i.bo = fmul <2 x double> %i.bf, %2
  %i.bp = shufflevector <3 x double> %1, <3 x double> poison, <2 x i32> zeroinitializer
  %i.bq = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.bg, <2 x double> %i.bp, <2 x double> %i.bo)
  %3 = shufflevector <3 x double> %1, <3 x double> poison, <2 x i32> <i32 2, i32 2>
  %i.br = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.ab, <2 x double> %3, <2 x double> %i.bq) ; 2 uses
  ...
  %4 = extractelement <3 x double> %1, i64 1
  %i.bv = fmul double %i.bh, %4
  %i.bw = tail call double @llvm.fmuladd.f64(double %i.bi, double %.sroa.018.0.copyload.a, double %i.bv)
  %5 = extractelement <3 x double> %1, i64 2
  %i.bx = tail call double @llvm.fmuladd.f64(double %i.v, double %5, double %i.bw)
