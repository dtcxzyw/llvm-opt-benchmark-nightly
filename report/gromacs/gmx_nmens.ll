...
vector.body330:                                   ; preds = %vector.body330, %vector.ph328
  ...
  %i.ns = getelementptr inbounds nuw [4 x i8], ptr %i.bs, i64 %index331
  ...
  %wide.load = load <8 x float>, ptr %i.ns, align 4, !tbaa !82, !alias.scope !107 ; 3 uses
  ...
  %i.nx = call <8 x float> @llvm.fmuladd.v8f32(<8 x float> %i.nw, <8 x float> %wide.load, <8 x float> %strided.vec336)
  %i.ny = fmul <8 x float> %broadcast.splat, %strided.vec333
  %i.nz = call <8 x float> @llvm.fmuladd.v8f32(<8 x float> %i.ny, <8 x float> %wide.load, <8 x float> %strided.vec337)
