...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %i.j = getelementptr inbounds nuw [4 x i8], ptr %i.g, i64 %index
  ...
  %wide.load = load <8 x float>, ptr %i.j, align 4, !tbaa !21, !alias.scope !45 ; 3 uses
  ...
  %i.q = tail call <8 x float> @llvm.fmuladd.v8f32(<8 x float> %i.p, <8 x float> %wide.load, <8 x float> %strided.vec44) ; 2 uses
  ...
  %i.t = tail call <8 x float> @llvm.fmuladd.v8f32(<8 x float> %i.s, <8 x float> %wide.load, <8 x float> %strided.vec45) ; 2 uses
