...
define hidden noundef ptr @pj_natearth(ptr nofree noundef writeonly captures(address_is_null, ret: address, provenance) %0) local_unnamed_addr #0 {
bb.a:
...
bb.b:                                             ; preds = %bb.a
  ...
  store ptr @_ZL18natearth_s_inverse5PJ_XYP8PJconsts, ptr %i.b, align 8, !tbaa !38
  %i.c = getelementptr inbounds nuw i8, ptr %0, i64 104
  store ptr @_ZL18natearth_s_forward5PJ_LPP8PJconsts, ptr %i.c, align 8, !tbaa !39
  br label %bb.e
...
bb.d:                                             ; preds = %bb.c
  ...
  store ptr @_ZL12des_natearth, ptr %i.g, align 8, !tbaa !41
...
define hidden noundef ptr @_Z37pj_projection_specific_setup_natearthP8PJconsts(ptr nofree noundef returned writeonly captures(ret: address, provenance) initializes((104, 120), (216, 224)) %0) local_unnamed_addr #1 {
bb.a:
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 216
  store double 0.000000e+00, ptr %i.a, align 8, !tbaa !8
  %i.b = getelementptr inbounds nuw i8, ptr %0, i64 112
...
; Function Attrs: mustprogress uwtable
define internal { double, double } @_ZL18natearth_s_inverse5PJ_XYP8PJconsts(double %0, double %1, ptr nofree noundef readonly captures(none) %2) #0 {
bb.a:
  %i.a = fcmp ogt double %1, f0x3FF6C2268FB400DA
  br i1 %i.a, label %bb.d, label %bb.b

bb.b:                                             ; preds = %bb.a
  %i.b = fcmp olt double %1, f0xBFF6C2268FB400DA
...
bb.d:                                             ; preds = %bb.a, %bb.b, %bb.c
  %.sroa.2.0 = phi double [ %1, %bb.b ], [ f0xBFF6C2268FB400DA, %bb.c ], [ f0x3FF6C2268FB400DA, %bb.a ] ; 2 uses
  %i.c = fneg double %.sroa.2.0
  %i.d = insertelement <2 x double> <double poison, double f0x3FF01D99029AE4F3>, double %i.c, i64 0
  br label %bb.f

bb.e:                                             ; preds = %bb.f
  %i.e = add nsw i32 %.03543, -1                  ; 2 uses
  %.not = icmp eq i32 %i.e, 0
  br i1 %.not, label %bb.g, label %bb.f, !llvm.loop !45

bb.f:                                             ; preds = %bb.d, %bb.e
  %.03543 = phi i32 [ 100, %bb.d ], [ %i.e, %bb.e ]
  %.03642 = phi double [ %.sroa.2.0, %bb.d ], [ %i.t, %bb.e ] ; 4 uses
  %i.f = fmul double %.03642, %.03642             ; 4 uses
  %i.g = fmul double %i.f, %i.f                   ; 3 uses
  %3 = insertelement <2 x double> poison, double %i.f, i64 0 ; 2 uses
  %4 = shufflevector <2 x double> %3, <2 x double> poison, <2 x i32> zeroinitializer
  %5 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %4, <2 x double> <double 2.887400e-02, double 2.598660e-01>, <2 x double> <double -4.447500e-02, double -3.113250e-01>) ; 2 uses
  %6 = extractelement <2 x double> %5, i64 0
  %7 = tail call double @llvm.fmuladd.f64(double %i.g, double -5.916000e-03, double %6)
  %8 = insertelement <2 x double> poison, double %i.g, i64 0
  %i.h = shufflevector <2 x double> %8, <2 x double> poison, <2 x i32> zeroinitializer
  %i.i = insertelement <2 x double> <double poison, double f0xBFB0A8D21BC126A7>, double %7, i64 0
  %i.j = insertelement <2 x double> %5, double 1.508500e-02, i64 0
  %i.k = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.h, <2 x double> %i.i, <2 x double> %i.j)
  %i.l = insertelement <2 x double> %3, double %i.g, i64 1
  %i.m = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.l, <2 x double> %i.k, <2 x double> <double f0x3FF01D99029AE4F3, double 4.525500e-02>)
  %i.n = insertelement <2 x double> poison, double %.03642, i64 0
  %i.o = insertelement <2 x double> %i.n, double %i.f, i64 1
  %i.p = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.o, <2 x double> %i.m, <2 x double> %i.d) ; 2 uses
  %i.q = extractelement <2 x double> %i.p, i64 0
  %i.r = extractelement <2 x double> %i.p, i64 1
  %i.s = fdiv double %i.q, %i.r                   ; 2 uses
  %i.t = fsub double %.03642, %i.s                ; 4 uses
  %i.u = tail call double @llvm.fabs.f64(double %i.s)
  %i.v = fcmp olt double %i.u, f0x3DA5FD7FE1796495
  br i1 %i.v, label %.loopexit, label %bb.e

bb.g:                                             ; preds = %bb.e
  %i.w = load ptr, ptr %2, align 8, !tbaa !47
  tail call void @_Z22proj_context_errno_setP6pj_ctxi(ptr noundef %i.w, i32 noundef 2050)
  br label %.loopexit

.loopexit:                                        ; preds = %bb.f, %bb.g
  %i.x = fmul double %i.t, %i.t                   ; 6 uses
  %i.y = fmul double %i.x, %i.x
  %i.z = fmul double %i.x, %i.y
  %i.aa = tail call double @llvm.fmuladd.f64(double %i.x, double -1.529000e-03, double 3.971000e-03)
  %i.ab = tail call double @llvm.fmuladd.f64(double %i.z, double %i.aa, double -1.379100e-02)
  %i.ac = tail call double @llvm.fmuladd.f64(double %i.x, double %i.ab, double -1.319790e-01)
  %i.ad = tail call double @llvm.fmuladd.f64(double %i.x, double %i.ac, double 8.707000e-01)
  %i.ae = fdiv double %0, %i.ad
  %.fca.0.insert = insertvalue { double, double } poison, double %i.ae, 0
  %.fca.1.insert = insertvalue { double, double } %.fca.0.insert, double %i.t, 1
  ret { double, double } %.fca.1.insert
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(none) uwtable
define internal { double, double } @_ZL18natearth_s_forward5PJ_LPP8PJconsts(double %0, double %1, ptr nofree readnone captures(none) %2) #3 {
bb.a:
  %i.a = fmul double %1, %1                       ; 4 uses
  %i.b = fmul double %i.a, %i.a                   ; 3 uses
  %i.c = fmul double %i.a, %i.b
  %i.d = insertelement <2 x double> poison, double %i.a, i64 0 ; 3 uses
  %i.e = shufflevector <2 x double> %i.d, <2 x double> poison, <2 x i32> zeroinitializer
  %i.f = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.e, <2 x double> <double -1.529000e-03, double 2.887400e-02>, <2 x double> <double 3.971000e-03, double -4.447500e-02>) ; 2 uses
  %i.g = insertelement <2 x double> poison, double %i.c, i64 0
  %i.h = insertelement <2 x double> %i.g, double %i.b, i64 1
  %i.i = insertelement <2 x double> %i.f, double -5.916000e-03, i64 1
  %i.j = insertelement <2 x double> %i.f, double -1.379100e-02, i64 0
  %i.k = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.h, <2 x double> %i.i, <2 x double> %i.j)
  %i.l = insertelement <2 x double> %i.d, double %i.b, i64 1
  %i.m = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.l, <2 x double> %i.k, <2 x double> <double -1.319790e-01, double 1.508500e-02>)
  %i.n = shufflevector <2 x double> %i.d, <2 x double> poison, <2 x i32> zeroinitializer
  %i.o = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.n, <2 x double> %i.m, <2 x double> <double 8.707000e-01, double f0x3FF01D99029AE4F3>)
  ...
  %vec2struct16 = insertvalue { double, double } %i.s, double %vec2struct.slot.sroa.0.8.vec.extract, 1
  ret { double, double } %vec2struct16
}
