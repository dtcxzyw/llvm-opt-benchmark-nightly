...
bb.a:
  ...
  %5 = fsub double %i.g, %i.z                     ; 6 uses
  %6 = fmul double %5, %5                         ; 5 uses
  %7 = fmul double %3, %3                         ; 4 uses
  %8 = extractelement <8 x double> %i.x, i64 4
  %9 = tail call double @llvm.fmuladd.f64(double %6, double %8, double %i.y)
  %10 = tail call double @llvm.fmuladd.f64(double %6, double %9, double 1.000000e+00)
  %11 = fneg double %7                            ; 2 uses
  %12 = insertelement <2 x double> <double poison, double -0.000000e+00>, double %5, i64 0
  %13 = shufflevector <8 x double> %i.x, <8 x double> poison, <2 x i32> <i32 3, i32 poison>
  %14 = insertelement <2 x double> %13, double 0.000000e+00, i64 1
  %15 = shufflevector <8 x double> %i.x, <8 x double> poison, <2 x i32> <i32 2, i32 7>
  %16 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %12, <2 x double> %14, <2 x double> %15)
  %i.aa = insertelement <2 x double> <double poison, double -0.000000e+00>, double %6, i64 0
  %17 = shufflevector <8 x double> %i.x, <8 x double> poison, <2 x i32> <i32 5, i32 poison>
  %18 = insertelement <2 x double> %17, double 0.000000e+00, i64 1
  %19 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.aa, <2 x double> %18, <2 x double> %16)
  %20 = load <6 x double>, ptr %i.s, align 8, !tbaa !47 ; 3 uses
  %21 = load double, ptr %i.w, align 8, !tbaa !65
  %22 = load double, ptr %i.v, align 8, !tbaa !66
  %23 = load double, ptr %i.r, align 8, !tbaa !48
  ...
  %24 = shufflevector <6 x double> %20, <6 x double> poison, <8 x i32> <i32 poison, i32 poison, i32 2, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %25 = shufflevector <8 x double> %i.x, <8 x double> %24, <2 x i32> <i32 6, i32 10>
  %26 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.ac, <2 x double> %25, <2 x double> %19)
  %27 = extractelement <6 x double> %20, i64 4
  %28 = tail call double @llvm.fmuladd.f64(double %11, double %27, double %23)
  %29 = tail call double @llvm.fmuladd.f64(double %7, double %28, double 1.000000e+00)
  %i.ad = extractelement <6 x double> %20, i64 0
  %30 = tail call double @llvm.fmuladd.f64(double %6, double %i.u, double %i.ad)
  %i.ae = tail call double @llvm.fmuladd.f64(double %6, double %30, double %29)
  %i.af = fmul double %5, %7
  %i.ag = tail call double @llvm.fmuladd.f64(double %5, double %21, double %22)
  %i.ah = tail call double @llvm.fmuladd.f64(double %i.af, double %i.ag, double %i.ae)
  %i.ai = fmul double %5, %i.ah
  %i.aj = insertelement <2 x double> %i.ac, double %11, i64 0
  %i.ak = insertelement <2 x double> poison, double %10, i64 0
  %i.al = insertelement <2 x double> %i.ak, double %i.ai, i64 1
  %i.am = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.aj, <2 x double> %26, <2 x double> %i.al)
