...
bb.z:                                             ; preds = %_ZN5boost4math6lgammaIdNS0_8policies6policyINS2_14default_policyES4_S4_S4_S4_S4_S4_S4_S4_S4_S4_S4_S4_EEEENS0_5tools12promote_argsIT_fffffE4typeES8_RKT0_.exit
  ...
  %i.ii = fmul double %i.ia, %i.ia                ; 4 uses
  %i.ij = fmul double %i.ii, %i.id
  %i.ik = fmul double %i.ii, %i.ii
  %i.il = fmul double %0, 6.000000e+00
  %13 = fadd double %i.ig, -1.700000e+01
  %14 = fadd double %i.hp, -5.000000e+00          ; 2 uses
  %15 = insertelement <2 x double> poison, double %0, i64 0 ; 2 uses
  %16 = shufflevector <2 x double> %15, <2 x double> poison, <2 x i32> zeroinitializer
  %17 = insertelement <2 x double> %15, double 1.000000e+00, i64 1
  %18 = fmul <2 x double> %16, %17                ; 2 uses
  %19 = extractelement <2 x double> %18, i64 0    ; 5 uses
  %i.im = fmul double %19, 3.000000e+00
  %i.in = fmul double %0, 1.300000e+01
  %i.io = fmul double %.scalar, %i.ie             ; 8 uses
  %20 = fsub double %i.in, %i.im
  %21 = fmul double %i.io, %i.io                  ; 6 uses
  %22 = fmul double %21, %21
  %23 = fmul double %0, %19                       ; 2 uses
  %24 = fadd double %i.io, 1.000000e+00
  %i.ip = fmul double %.scalar, %24
  %i.iq = fmul double %21, 5.000000e-01
  %i.ir = fmul double %i.if, %i.io
  %25 = fmul double %14, 5.000000e-01
  %26 = fmul <2 x double> %18, <double 1.100000e+01, double 4.770000e+02> ; 2 uses
  %i.is = fmul double %22, 2.500000e-01
  %27 = fadd double %20, -1.300000e+01
  %28 = fmul double %27, %21
  %29 = fmul double %i.io, %21                    ; 2 uses
  %i.it = fmul double %13, %29
  %i.iu = fmul double %14, %21
  %30 = fdiv double %i.it, 6.000000e+00
  %31 = fsub double %30, %i.is
  %i.iv = fmul double %i.iu, 5.000000e-01
  %i.iw = fdiv double %29, 3.000000e+00
  %i.ix = fadd double %28, %31
  %i.iy = fsub double %i.iw, %i.iv
  %i.iz = fmul double %23, 2.000000e+00
  %i.ja = fmul double %19, 2.500000e+01
  %i.jb = fsub double %i.iz, %i.ja
  %i.jc = fadd double %i.ih, %i.jb
  %i.jd = fadd double %i.jc, -6.100000e+01
  %i.je = fsub double %19, %i.il
  %i.jf = fmul double %i.jd, %i.io
  %i.jg = fadd double %i.je, 7.000000e+00
  %i.jh = fmul double %i.jf, 5.000000e-01
  %i.ji = fmul double %i.jg, %i.io
  %i.jj = fmul double %23, 2.500000e+01
  %i.jk = fmul double %19, 1.950000e+02
  ...
  %i.ke = insertelement <4 x double> %i.kd, double %32, i64 3
  %i.kf = fmul <4 x double> %i.ke, %i.hs          ; 2 uses
  %i.kg = fdiv double %i.ip, %i.ia
  %i.kh = insertelement <4 x double> poison, double %i.ii, i64 0
  %i.ki = insertelement <4 x double> %i.kh, double %i.ij, i64 1
  %i.kj = insertelement <4 x double> %i.ki, double %i.ik, i64 2
