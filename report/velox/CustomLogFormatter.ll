inline.NumInlined: 978
inline.NumDeleted: 414
begin_hunk_0
@.str.5 = private unnamed_addr constant [4 x i8] c"FUN\00", align 1
@.str.6 = private unnamed_addr constant [4 x i8] c"FIL\00", align 1
@.str.7 = private unnamed_addr constant [27 x i8] c"unterminated format string\00", align 1
@.str.8 = private unnamed_addr constant [2 x i8] c" \00", align 1
@.str.9 = private unnamed_addr constant [2 x i8] c"{\00", align 1
@.str.10 = private unnamed_addr constant [2 x i8] c"}\00", align 1
@_ZN12_GLOBAL__N_112messageIndexE = internal constant i64 13, align 8
@.str.11 = private unnamed_addr constant [3 x i8] c"}{\00", align 1
@.str.12 = private unnamed_addr constant [3 x i8] c"}\0A\00", align 1
@.str.13 = private unnamed_addr constant [24 x i8] c"basic_string::_M_create\00", align 1
@.str.14 = private unnamed_addr constant [21 x i8] c"basic_string::append\00", align 1
@.str.15 = private unnamed_addr constant [4 x i8] c"CTX\00", align 1
@.str.16 = private unnamed_addr constant [2 x i8] c"D\00", align 1
@.str.17 = private unnamed_addr constant [2 x i8] c"H\00", align 1
@.str.18 = private unnamed_addr constant [2 x i8] c"L\00", align 1
@.str.19 = private unnamed_addr constant [5 x i8] c"LINE\00", align 1
@.str.20 = private unnamed_addr constant [2 x i8] c"M\00", align 1
@.str.21 = private unnamed_addr constant [2 x i8] c"S\00", align 1
@.str.22 = private unnamed_addr constant [7 x i8] c"THREAD\00", align 1
@.str.23 = private unnamed_addr constant [6 x i8] c"USECS\00", align 1
@.str.24 = private unnamed_addr constant [2 x i8] c"m\00", align 1
@_ZN5folly6detail15to_ascii_powersILm10EmE4dataE = external local_unnamed_addr global %"struct.folly::c_array", align 8
@_ZN5folly6detail14to_ascii_tableILm10ENS_17to_ascii_alphabetILb0EEEE4dataE = external local_unnamed_addr global %"struct.folly::c_array.1", align 2
@_ZZNK5folly17BaseFormatterImplINS_9FormatterILb0EJRKciRiS4_S4_S4_lmRNS_5RangeIPS2_EES7_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES8_SG_S7_S7_EEELb0ESt16integer_sequenceImJLm0ELm1ELm2ELm3ELm4ELm5ELm6ELm7ELm8ELm9ELm10ELm11ELm12ELm13ELm14ELm15EEEJS3_iS4_S4_S4_S4_lmS8_S7_jSG_S8_SG_S7_S7_EEclINS_6detail27BaseFormatterAppendToStringISE_EEEEvRT_E4funsB5cxx11 = linkonce_odr constant %"struct.folly::c_array.19" { [17 x ptr] [ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm0ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm1ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm2ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm3ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm4ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm5ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm6ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm7ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm8ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm9ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm10ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm11ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm12ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm13ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm14ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_SF_S6_S6_EE11doFormatArgILm15ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr null] }, comdat, align 8
@.str.25 = private unnamed_addr constant [31 x i8] c"base prefix not allowed with '\00", align 1
@.str.26 = private unnamed_addr constant [12 x i8] c"' specifier\00", align 1
@.str.27 = private unnamed_addr constant [26 x i8] c"cannot use ',' with the '\00", align 1
@.str.28 = private unnamed_addr constant [4 x i8] c"%ju\00", align 1
@.str.29 = private unnamed_addr constant [45 x i8] c"thousands separator (',') not allowed with '\00", align 1
@.str.30 = private unnamed_addr constant [20 x i8] c"invalid specifier '\00", align 1
@.str.31 = private unnamed_addr constant [2 x i8] c"'\00", align 1
@_ZTIN5folly12BadFormatArgE = linkonce_odr constant { ptr, ptr, ptr } { ptr getelementptr inbounds (ptr, ptr @_ZTVN10__cxxabiv120__si_class_type_infoE, i64 2), ptr @_ZTSN5folly12BadFormatArgE, ptr @_ZTISt16invalid_argument }, comdat, align 8
@_ZTSN5folly12BadFormatArgE = linkonce_odr constant [23 x i8] c"N5folly12BadFormatArgE\00", comdat, align 1
@_ZTISt16invalid_argument = external constant ptr
@_ZTVN5folly12BadFormatArgE = linkonce_odr unnamed_addr constant { [5 x ptr] } { [5 x ptr] [ptr null, ptr @_ZTIN5folly12BadFormatArgE, ptr @_ZNSt16invalid_argumentD2Ev, ptr @_ZN5folly12BadFormatArgD0Ev, ptr @_ZNKSt11logic_error4whatEv] }, comdat, align 8
@.str.32 = private unnamed_addr constant [26 x i8] c"invalid format argument {\00", align 1
@.str.33 = private unnamed_addr constant [4 x i8] c"}: \00", align 1
@_ZN5folly6detail11formatOctalE = external local_unnamed_addr global %"struct.std::array.20", align 1
@_ZN5folly6detail14formatHexLowerE = external local_unnamed_addr global %"struct.std::array.22", align 1
@_ZN5folly6detail14formatHexUpperE = external local_unnamed_addr global %"struct.std::array.22", align 1
@_ZN5folly6detail12formatBinaryE = external local_unnamed_addr global %"struct.std::array.24", align 1
@.str.34 = private unnamed_addr constant [19 x i8] c"index out of range\00", align 1
@_ZTISt12out_of_range = external constant ptr
@_ZTVSt12out_of_range = external unnamed_addr constant { [5 x ptr] }, align 8
@.str.35 = private unnamed_addr constant [29 x i8] c"folly::format: invalid width\00", align 1
@.str.36 = private unnamed_addr constant [33 x i8] c"folly::format: invalid precision\00", align 1
@.str.37 = private unnamed_addr constant [52 x i8] c"sign specifications not allowed for unsigned values\00", align 1
@.str.38 = private unnamed_addr constant [21 x i8] c"integer key required\00", align 1
@.str.40 = private unnamed_addr constant [14 x i8] c"unmatched ']'\00", align 1
@.str.41 = private unnamed_addr constant [43 x i8] c"folly::format: '}' at end of format string\00", align 1
@.str.42 = private unnamed_addr constant [34 x i8] c"folly::format: missing ending '}'\00", align 1
@.str.43 = private unnamed_addr constant [55 x i8] c"cannot provide width arg index without value arg index\00", align 1
@.str.44 = private unnamed_addr constant [46 x i8] c"dynamic field width argument must be integral\00", align 1
@.str.45 = private unnamed_addr constant [55 x i8] c"cannot provide value arg index without width arg index\00", align 1
@.str.46 = private unnamed_addr constant [31 x i8] c"argument index must be integer\00", align 1
@.str.47 = private unnamed_addr constant [36 x i8] c"argument index must be non-negative\00", align 1
@.str.48 = private unnamed_addr constant [66 x i8] c"folly::format: may not have both default and explicit arg indexes\00", align 1
@.str.49 = private unnamed_addr constant [43 x i8] c"folly::format: single '}' in format string\00", align 1
@.str.50 = private unnamed_addr constant [21 x i8] c"integer key expected\00", align 1
@.str.51 = private unnamed_addr constant [34 x i8] c"argument index out of range, max=\00", align 1
@.str.52 = private unnamed_addr constant [8 x i8] c"VERBOSE\00", align 1
@.str.53 = private unnamed_addr constant [5 x i8] c"INFO\00", align 1
@.str.54 = private unnamed_addr constant [8 x i8] c"WARNING\00", align 1
@.str.55 = private unnamed_addr constant [6 x i8] c"ERROR\00", align 1
@.str.56 = private unnamed_addr constant [9 x i8] c"CRITICAL\00", align 1
@.str.57 = private unnamed_addr constant [6 x i8] c"FATAL\00", align 1
@.str.58 = private unnamed_addr constant [8 x i8] c"\1B[1;30m\00", align 1
@.str.59 = private unnamed_addr constant [1 x i8] zeroinitializer, align 1
@.str.60 = private unnamed_addr constant [6 x i8] c"\1B[33m\00", align 1
@.str.61 = private unnamed_addr constant [6 x i8] c"\1B[31m\00", align 1
@.str.62 = private unnamed_addr constant [8 x i8] c"\1B[1;41m\00", align 1
@.str.63 = private unnamed_addr constant [5 x i8] c"\1B[0m\00", align 1
@_ZZNK5folly17BaseFormatterImplINS_9FormatterILb0EJRKciRiS4_S4_S4_lmRNS_5RangeIPS2_EES7_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES8_EEELb0ESt16integer_sequenceImJLm0ELm1ELm2ELm3ELm4ELm5ELm6ELm7ELm8ELm9ELm10ELm11ELm12EEEJS3_iS4_S4_S4_S4_lmS8_S7_jSG_S8_EEclINS_6detail27BaseFormatterAppendToStringISE_EEEEvRT_E4funsB5cxx11 = linkonce_odr constant %"struct.folly::c_array.47" { [14 x ptr] [ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm0ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm1ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm2ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm3ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm4ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm5ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm6ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm7ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm8ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm9ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm10ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm11ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr @_ZN5folly9FormatterILb0EJRKciRiS3_S3_S3_lmRNS_5RangeIPS1_EES6_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES7_EE11doFormatArgILm12ENS_6detail27BaseFormatterAppendToStringISD_EEEEvRKNSI_17BaseFormatterBaseERNS_9FormatArgERT0_, ptr null] }, comdat, align 8

@_ZN5folly18CustomLogFormatterC1ENS_5RangeIPKcEEb = unnamed_addr alias void (ptr, ptr, ptr, i1), ptr @_ZN5folly18CustomLogFormatterC2ENS_5RangeIPKcEEb

; Function Attrs: inlinehint mustprogress nounwind uwtable
define linkonce_odr void @_ZN5folly18CustomLogFormatterD2Ev(ptr noundef nonnull align 8 dereferenceable(105) %0) unnamed_addr #0 comdat align 2 personality ptr @__gxx_personality_v0 {
bb.a:
  store ptr getelementptr inbounds nuw inrange(-16, 24) (i8, ptr @_ZTVN5folly18CustomLogFormatterE, i64 16), ptr %0, align 8, !tbaa !11
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 40
  %i.b = load ptr, ptr %i.a, align 8, !tbaa !13   ; 2 uses
  %i.c = getelementptr inbounds nuw i8, ptr %0, i64 56 ; 2 uses
  %i.d = icmp eq ptr %i.b, %i.c
  br i1 %i.d, label %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit, label %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i

_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i: ; preds = %bb.a
  %i.e = load i64, ptr %i.c, align 8, !tbaa !19
  %i.f = add i64 %i.e, 1
  tail call void @_ZdlPvm(ptr noundef %i.b, i64 noundef %i.f) #22
  br label %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit

_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit: ; preds = %bb.a, %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i
  %i.g = getelementptr inbounds nuw i8, ptr %0, i64 8
  %i.h = load ptr, ptr %i.g, align 8, !tbaa !13   ; 2 uses
  %i.i = getelementptr inbounds nuw i8, ptr %0, i64 24 ; 2 uses
  %i.j = icmp eq ptr %i.h, %i.i
  br i1 %i.j, label %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit3, label %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i1

_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i1: ; preds = %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit
  %i.k = load i64, ptr %i.i, align 8, !tbaa !19
  %i.l = add i64 %i.k, 1
  tail call void @_ZdlPvm(ptr noundef %i.h, i64 noundef %i.l) #22
  br label %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit3

_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit3: ; preds = %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit, %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i1
  ret void
}

; Function Attrs: inlinehint mustprogress nounwind uwtable
define linkonce_odr void @_ZN5folly18CustomLogFormatterD0Ev(ptr noundef nonnull align 8 dereferenceable(105) %0) unnamed_addr #0 comdat align 2 personality ptr @__gxx_personality_v0 {
bb.a:
  store ptr getelementptr inbounds nuw inrange(-16, 24) (i8, ptr @_ZTVN5folly18CustomLogFormatterE, i64 16), ptr %0, align 8, !tbaa !11
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 40
  %i.b = load ptr, ptr %i.a, align 8, !tbaa !13   ; 2 uses
  %i.c = getelementptr inbounds nuw i8, ptr %0, i64 56 ; 2 uses
  %i.d = icmp eq ptr %i.b, %i.c
  br i1 %i.d, label %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit.i, label %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i.i

_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i.i: ; preds = %bb.a
  %i.e = load i64, ptr %i.c, align 8, !tbaa !19
  %i.f = add i64 %i.e, 1
  tail call void @_ZdlPvm(ptr noundef %i.b, i64 noundef %i.f) #22, !inline_history !20
  br label %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit.i

_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit.i: ; preds = %bb.a, %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i.i
  %i.g = getelementptr inbounds nuw i8, ptr %0, i64 8
  %i.h = load ptr, ptr %i.g, align 8, !tbaa !13   ; 2 uses
  %i.i = getelementptr inbounds nuw i8, ptr %0, i64 24 ; 2 uses
  %i.j = icmp eq ptr %i.h, %i.i
  br i1 %i.j, label %_ZN5folly18CustomLogFormatterD2Ev.exit, label %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i1.i

_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i1.i: ; preds = %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit.i
  %i.k = load i64, ptr %i.i, align 8, !tbaa !19
  %i.l = add i64 %i.k, 1
  tail call void @_ZdlPvm(ptr noundef %i.h, i64 noundef %i.l) #22, !inline_history !20
  br label %_ZN5folly18CustomLogFormatterD2Ev.exit

_ZN5folly18CustomLogFormatterD2Ev.exit:           ; preds = %_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED2Ev.exit.i, %_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE11_M_is_localEv.exit.i.i1.i
  tail call void @_ZdlPvm(ptr noundef nonnull %0, i64 noundef 112) #22
  ret void
}

; Function Attrs: mustprogress uwtable
define void @_ZN5folly18CustomLogFormatter13formatMessageB5cxx11ERKNS_10LogMessageEPKNS_11LogCategoryE(ptr dead_on_unwind noalias writable sret(%"class.std::__cxx11::basic_string") align 8 %0, ptr nofree noundef nonnull readonly align 8 captures(none) dereferenceable(105) %1, ptr noundef nonnull align 8 dereferenceable(176) %2, ptr nofree readnone captures(none) %3) unnamed_addr #1 align 2 personality ptr @__gxx_personality_v0 {
bb.a:
  %i.a = alloca [14 x i32], align 16              ; 13 uses
  %4 = alloca %"struct.folly::detail::BaseFormatterAppendToString", align 8 ; 4 uses
  %5 = alloca %"class.folly::Formatter.44", align 8 ; 18 uses
  %6 = alloca %"class.folly::Formatter", align 8  ; 23 uses
  %7 = alloca %struct.tm, align 16                ; 16 uses
  %i.b = alloca i64, align 8                      ; 5 uses
  %8 = alloca %"class.folly::Range", align 8      ; 8 uses
  %9 = alloca %"class.folly::Range", align 8      ; 10 uses
  %10 = alloca %"class.std::__cxx11::basic_string", align 8 ; 11 uses
  %11 = alloca %"class.std::__cxx11::basic_string", align 8 ; 10 uses
  %12 = alloca %"class.std::__cxx11::basic_string", align 8 ; 10 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %7) #23
  %i.c = getelementptr inbounds nuw i8, ptr %2, i64 24
  %.sroa.0.0.copyload.i = load i64, ptr %i.c, align 8, !tbaa !21 ; 2 uses
  %i.d = sdiv i64 %.sroa.0.0.copyload.i, 1000000000 ; 2 uses
  %i.e = sdiv i64 %.sroa.0.0.copyload.i, 1000
  %.neg.i = mul nsw i64 %i.d, -1000000
  %i.f = add nsw i64 %.neg.i, %i.e                ; 3 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.b) #23
  store i64 %i.d, ptr %i.b, align 8, !tbaa !21
  %i.g = call ptr @localtime_r(ptr noundef nonnull %i.b, ptr noundef nonnull %7) #23
  %.not = icmp eq ptr %i.g, null
  br i1 %.not, label %bb.b, label %bb.c

bb.b:                                             ; preds = %bb.a
  call void @llvm.memset.p0.i64(ptr noundef nonnull align 16 dereferenceable(56) %7, i8 0, i64 56, i1 false)
  br label %bb.c

bb.c:                                             ; preds = %bb.b, %bb.a
  call void @llvm.lifetime.start.p0(ptr nonnull %8) #23
  %i.h = call { ptr, ptr } @_ZNK5folly10LogMessage15getFileBaseNameEv(ptr noundef nonnull align 8 dereferenceable(176) %2) ; 2 uses
  %i.i = extractvalue { ptr, ptr } %i.h, 0        ; 6 uses
  store ptr %i.i, ptr %8, align 8
  %i.j = getelementptr inbounds nuw i8, ptr %8, i64 8 ; 2 uses
  %i.k = extractvalue { ptr, ptr } %i.h, 1        ; 3 uses
  store ptr %i.k, ptr %i.j, align 8
  call void @llvm.lifetime.start.p0(ptr nonnull %9) #23
  call void @llvm.memset.p0.i64(ptr noundef nonnull align 8 dereferenceable(16) %9, i8 0, i64 16, i1 false)
  %i.l = getelementptr inbounds nuw i8, ptr %1, i64 88 ; 2 uses
  %i.m = load i64, ptr %i.l, align 8, !tbaa !22
  %.not45 = icmp eq i64 %i.m, 0
  br i1 %.not45, label %bb.f, label %bb.d

bb.d:                                             ; preds = %bb.c
  %i.n = icmp eq ptr %i.i, %i.k
  br i1 %i.n, label %_ZN12_GLOBAL__N_116getBaseNameNoExtEN5folly5RangeIPKcEE.exit, label %bb.e

bb.e:                                             ; preds = %bb.d
  %.pre.i = ptrtoint ptr %i.k to i64
  %.pre11.i = ptrtoint ptr %i.i to i64            ; 2 uses
  %.pre13.i = sub i64 %.pre.i, %.pre11.i          ; 3 uses
  %i.o = call noundef ptr @_ZN5folly7memrchrEPKvim(ptr noundef %i.i, i32 noundef 46, i64 noundef %.pre13.i) #23 ; 2 uses
  %i.p = icmp eq ptr %i.o, null
  %i.q = ptrtoint ptr %i.o to i64
  %i.r = sub i64 %i.q, %.pre11.i
  %i.s = call i64 @llvm.umin.i64(i64 %.pre13.i, i64 %i.r)
  %i.t = select i1 %i.p, i64 %.pre13.i, i64 %i.s
  br label %_ZN12_GLOBAL__N_116getBaseNameNoExtEN5folly5RangeIPKcEE.exit

_ZN12_GLOBAL__N_116getBaseNameNoExtEN5folly5RangeIPKcEE.exit: ; preds = %bb.d, %bb.e
  %.0.i = phi i64 [ %i.t, %bb.e ], [ 0, %bb.d ]
  %i.u = getelementptr inbounds nuw i8, ptr %i.i, i64 %.0.i
  store ptr %i.i, ptr %9, align 8, !tbaa !26
  %.sroa.4.0..sroa_idx = getelementptr inbounds nuw i8, ptr %9, i64 8
  store ptr %i.u, ptr %.sroa.4.0..sroa_idx, align 8, !tbaa !26
  br label %bb.f

bb.f:                                             ; preds = %_ZN12_GLOBAL__N_116getBaseNameNoExtEN5folly5RangeIPKcEE.exit, %bb.c
  %i.v = getelementptr inbounds nuw i8, ptr %2, i64 72
  %i.w = load i64, ptr %i.v, align 8, !tbaa !27
  %.not196 = icmp eq i64 %i.w, 0
  br i1 %.not196, label %bb.g, label %bb.o

bb.g:                                             ; preds = %bb.f
  %i.x = getelementptr inbounds nuw i8, ptr %1, i64 40
  %i.y = load ptr, ptr %i.x, align 8, !tbaa !13   ; 2 uses
  %i.z = getelementptr inbounds nuw i8, ptr %1, i64 48
  %i.aa = load i64, ptr %i.z, align 8, !tbaa !34
  %i.ab = getelementptr inbounds nuw i8, ptr %i.y, i64 %i.aa
  %i.ac = getelementptr inbounds nuw i8, ptr %2, i64 8
  %i.ad = load i32, ptr %i.ac, align 8, !tbaa !35 ; 9 uses
  %i.ae = icmp ult i32 %i.ad, 2000                ; 2 uses
  br i1 %i.ae, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit, label %bb.h

bb.h:                                             ; preds = %bb.g
  %i.af = icmp ult i32 %i.ad, 3000
  br i1 %i.af, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit, label %bb.i

bb.i:                                             ; preds = %bb.h
  %i.ag = icmp ult i32 %i.ad, 4000
  br i1 %i.ag, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit, label %bb.j

bb.j:                                             ; preds = %bb.i
  %i.ah = icmp ult i32 %i.ad, 5000
  br i1 %i.ah, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit, label %bb.k

bb.k:                                             ; preds = %bb.j
  %i.ai = icmp ult i32 %i.ad, 2147483646
  %.str.56..str.57.i = select i1 %i.ai, ptr @.str.56, ptr @.str.57
  br label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit

_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit: ; preds = %bb.g, %bb.h, %bb.i, %bb.j, %bb.k
  %.sroa.0.0.i = phi ptr [ @.str.55, %bb.j ], [ @.str.52, %bb.g ], [ @.str.53, %bb.h ], [ @.str.54, %bb.i ], [ %.str.56..str.57.i, %bb.k ]
  %i.aj = getelementptr inbounds nuw i8, ptr %7, i64 16
  %i.ak = load i32, ptr %i.aj, align 16, !tbaa !36
  %i.al = add nsw i32 %i.ak, 1
  %i.am = getelementptr inbounds nuw i8, ptr %7, i64 12
  %i.an = getelementptr inbounds nuw i8, ptr %7, i64 8
  %i.ao = getelementptr inbounds nuw i8, ptr %7, i64 4
  %i.ap = getelementptr inbounds nuw i8, ptr %2, i64 16
  %i.aq = load i64, ptr %i.ap, align 8, !tbaa !38
  %i.ar = getelementptr inbounds nuw i8, ptr %2, i64 56
  %i.as = load <2 x ptr>, ptr %i.ar, align 8, !tbaa !26
  %i.at = getelementptr inbounds nuw i8, ptr %2, i64 48
  %i.au = load i32, ptr %i.at, align 8, !tbaa !39
  %i.av = getelementptr inbounds nuw i8, ptr %2, i64 80
  %i.aw = getelementptr inbounds nuw i8, ptr %2, i64 152
  %i.ax = load i64, ptr %i.aw, align 8, !tbaa !34
  %i.ay = icmp eq i64 %i.ax, 0
  %.0.v.i = select i1 %i.ay, i64 112, i64 144
  %.0.i58 = getelementptr inbounds nuw i8, ptr %2, i64 %.0.v.i
  br i1 %i.ae, label %_ZN12_GLOBAL__N_116getColorSequenceEN5folly8LogLevelE.exit, label %bb.l

bb.l:                                             ; preds = %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit
  %i.az = icmp ult i32 %i.ad, 3000
  br i1 %i.az, label %_ZN12_GLOBAL__N_116getColorSequenceEN5folly8LogLevelE.exit, label %bb.m

bb.m:                                             ; preds = %bb.l
  %i.ba = icmp ult i32 %i.ad, 4000
  br i1 %i.ba, label %_ZN12_GLOBAL__N_116getColorSequenceEN5folly8LogLevelE.exit, label %bb.n

bb.n:                                             ; preds = %bb.m
  %i.bb = icmp ult i32 %i.ad, 5000                ; 2 uses
  %..i59 = select i1 %i.bb, ptr getelementptr inbounds nuw (i8, ptr @.str.61, i64 5), ptr getelementptr inbounds nuw (i8, ptr @.str.62, i64 7)
  %.str.61..str.62.i = select i1 %i.bb, ptr @.str.61, ptr @.str.62
  br label %_ZN12_GLOBAL__N_116getColorSequenceEN5folly8LogLevelE.exit

_ZN12_GLOBAL__N_116getColorSequenceEN5folly8LogLevelE.exit: ; preds = %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit, %bb.l, %bb.m, %bb.n
  %.sroa.6.0.i = phi ptr [ getelementptr inbounds nuw (i8, ptr @.str.60, i64 5), %bb.m ], [ getelementptr inbounds nuw (i8, ptr @.str.58, i64 7), %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit ], [ @.str.59, %bb.l ], [ %..i59, %bb.n ]
  %.sroa.0.0.i60 = phi ptr [ @.str.60, %bb.m ], [ @.str.58, %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit ], [ @.str.59, %bb.l ], [ %.str.61..str.62.i, %bb.n ]
  %i.bc = add i32 %i.ad, -3000
  %or.cond.i63 = icmp ult i32 %i.bc, -1000        ; 2 uses
  %..str.59.i = select i1 %or.cond.i63, ptr getelementptr inbounds nuw (i8, ptr @.str.63, i64 4), ptr @.str.59
  %.str.63..str.59.i = select i1 %or.cond.i63, ptr @.str.63, ptr @.str.59
  call void @llvm.lifetime.start.p0(ptr nonnull %6) #23, !noalias !40
  store ptr %i.y, ptr %6, align 8, !tbaa !26, !noalias !40
  %.sroa.2.0..sroa_idx.i.i.i = getelementptr inbounds nuw i8, ptr %6, i64 8
  store ptr %i.ab, ptr %.sroa.2.0..sroa_idx.i.i.i, align 8, !tbaa !26, !noalias !40
  %i.bd = getelementptr inbounds nuw i8, ptr %6, i64 16
  store ptr %.sroa.0.0.i, ptr %i.bd, align 8, !tbaa !26, !noalias !40
  %i.be = getelementptr inbounds nuw i8, ptr %6, i64 24
  store i32 %i.al, ptr %i.be, align 8, !tbaa !43, !noalias !40
  %i.bf = getelementptr inbounds nuw i8, ptr %6, i64 32
  store ptr %i.am, ptr %i.bf, align 8, !tbaa !45, !noalias !40
  %i.bg = getelementptr inbounds nuw i8, ptr %6, i64 40
  store ptr %i.an, ptr %i.bg, align 8, !tbaa !45, !noalias !40
  %i.bh = getelementptr inbounds nuw i8, ptr %6, i64 48
  store ptr %i.ao, ptr %i.bh, align 8, !tbaa !45, !noalias !40
  %i.bi = getelementptr inbounds nuw i8, ptr %6, i64 56
  store ptr %7, ptr %i.bi, align 8, !tbaa !45, !noalias !40
  %i.bj = getelementptr inbounds nuw i8, ptr %6, i64 64
  store i64 %i.f, ptr %i.bj, align 8, !tbaa !47, !noalias !40
  %i.bk = getelementptr inbounds nuw i8, ptr %6, i64 72
  store i64 %i.aq, ptr %i.bk, align 8, !tbaa !49, !noalias !40
  %i.bl = getelementptr inbounds nuw i8, ptr %6, i64 80
  store ptr %8, ptr %i.bl, align 8, !tbaa !51, !noalias !40
  %i.bm = getelementptr inbounds nuw i8, ptr %6, i64 88
  store <2 x ptr> %i.as, ptr %i.bm, align 8, !tbaa !26, !noalias !40
  %i.bn = getelementptr inbounds nuw i8, ptr %6, i64 104
  store i32 %i.au, ptr %i.bn, align 8, !tbaa !53, !noalias !40
  %i.bo = getelementptr inbounds nuw i8, ptr %6, i64 112
  store ptr %i.av, ptr %i.bo, align 8, !tbaa !55, !noalias !40
  %i.bp = getelementptr inbounds nuw i8, ptr %6, i64 120
  store ptr %9, ptr %i.bp, align 8, !tbaa !51, !noalias !40
  %i.bq = getelementptr inbounds nuw i8, ptr %6, i64 128
  store ptr %.0.i58, ptr %i.bq, align 8, !tbaa !55, !noalias !40
  %i.br = getelementptr inbounds nuw i8, ptr %6, i64 136
  store ptr %.sroa.0.0.i60, ptr %i.br, align 8, !tbaa !26, !noalias !40
  %.sroa.4176.0..sroa_idx = getelementptr inbounds nuw i8, ptr %6, i64 144
  store ptr %.sroa.6.0.i, ptr %.sroa.4176.0..sroa_idx, align 8, !tbaa !26, !noalias !40
  %i.bs = getelementptr inbounds nuw i8, ptr %6, i64 152
  store ptr %.str.63..str.59.i, ptr %i.bs, align 8, !tbaa !26, !noalias !40
  %.sroa.4174.0..sroa_idx = getelementptr inbounds nuw i8, ptr %6, i64 160
  store ptr %..str.59.i, ptr %.sroa.4174.0..sroa_idx, align 8, !tbaa !26, !noalias !40
  call void @_ZNK5folly17BaseFormatterImplINS_9FormatterILb0EJRKciRiS4_S4_S4_lmRNS_5RangeIPS2_EES7_jRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEES8_SG_S7_S7_EEELb0ESt16integer_sequenceImJLm0ELm1ELm2ELm3ELm4ELm5ELm6ELm7ELm8ELm9ELm10ELm11ELm12ELm13ELm14ELm15EEEJS3_iS4_S4_S4_S4_lmS8_S7_jSG_S8_SG_S7_S7_EE3strEv(ptr dead_on_unwind writable sret(%"class.std::__cxx11::basic_string") align 8 %0, ptr noundef nonnull align 8 dereferenceable(168) %6)
  call void @llvm.lifetime.end.p0(ptr nonnull %6) #23, !noalias !40
  br label %bb.cc

bb.o:                                             ; preds = %bb.f
  call void @llvm.lifetime.start.p0(ptr nonnull %10) #23
  %i.bt = getelementptr inbounds nuw i8, ptr %1, i64 8
  %i.bu = load ptr, ptr %i.bt, align 8, !tbaa !13 ; 2 uses
  %i.bv = getelementptr inbounds nuw i8, ptr %1, i64 16
  %i.bw = load i64, ptr %i.bv, align 8, !tbaa !34
  %i.bx = getelementptr inbounds nuw i8, ptr %i.bu, i64 %i.bw
  %i.by = getelementptr inbounds nuw i8, ptr %2, i64 8 ; 3 uses
  %i.bz = load i32, ptr %i.by, align 8, !tbaa !35 ; 5 uses
  %i.ca = icmp ult i32 %i.bz, 2000
  br i1 %i.ca, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit72, label %bb.p

bb.p:                                             ; preds = %bb.o
  %i.cb = icmp ult i32 %i.bz, 3000
  br i1 %i.cb, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit72, label %bb.q

bb.q:                                             ; preds = %bb.p
  %i.cc = icmp ult i32 %i.bz, 4000
  br i1 %i.cc, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit72, label %bb.r

bb.r:                                             ; preds = %bb.q
  %i.cd = icmp ult i32 %i.bz, 5000
  br i1 %i.cd, label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit72, label %bb.s

bb.s:                                             ; preds = %bb.r
  %i.ce = icmp ult i32 %i.bz, 2147483646
  %.str.56..str.57.i67 = select i1 %i.ce, ptr @.str.56, ptr @.str.57
  br label %_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit72

_ZN12_GLOBAL__N_116getGlogLevelNameEN5folly8LogLevelE.exit72: ; preds = %bb.o, %bb.p, %bb.q, %bb.r, %bb.s
  %.sroa.0.0.i69 = phi ptr [ @.str.55, %bb.r ], [ @.str.52, %bb.o ], [ @.str.53, %bb.p ], [ @.str.54, %bb.q ], [ %.str.56..str.57.i67, %bb.s ] ; 2 uses
  %i.cf = getelementptr inbounds nuw i8, ptr %7, i64 16
  %i.cg = load i32, ptr %i.cf, align 16, !tbaa !36
  %i.ch = add nsw i32 %i.cg, 1                    ; 2 uses
  %i.ci = getelementptr inbounds nuw i8, ptr %7, i64 12
  %i.cj = getelementptr inbounds nuw i8, ptr %7, i64 8
  %i.ck = getelementptr inbounds nuw i8, ptr %7, i64 4
  %i.cl = getelementptr inbounds nuw i8, ptr %2, i64 16
  %i.cm = load i64, ptr %i.cl, align 8, !tbaa !38 ; 2 uses
  %i.cn = getelementptr inbounds nuw i8, ptr %2, i64 56 ; 2 uses
  %i.co = getelementptr inbounds nuw i8, ptr %2, i64 48
  %i.cp = load i32, ptr %i.co, align 8, !tbaa !39 ; 2 uses
  %i.cq = getelementptr inbounds nuw i8, ptr %2, i64 80
  call void @llvm.lifetime.start.p0(ptr nonnull %5) #23, !noalias !57
  store ptr %i.bu, ptr %5, align 8, !tbaa !26, !noalias !57
  %.sroa.2.0..sroa_idx.i.i.i78 = getelementptr inbounds nuw i8, ptr %5, i64 8
  store ptr %i.bx, ptr %.sroa.2.0..sroa_idx.i.i.i78, align 8, !tbaa !26, !noalias !57
  %i.cr = getelementptr inbounds nuw i8, ptr %5, i64 16
  store ptr %.sroa.0.0.i69, ptr %i.cr, align 8, !tbaa !26, !noalias !57
  %i.cs = getelementptr inbounds nuw i8, ptr %5, i64 24
  store i32 %i.ch, ptr %i.cs, align 8, !tbaa !43, !noalias !57
  %i.ct = getelementptr inbounds nuw i8, ptr %5, i64 32
  store ptr %i.ci, ptr %i.ct, align 8, !tbaa !45, !noalias !57
  %i.cu = getelementptr inbounds nuw i8, ptr %5, i64 40
  store ptr %i.cj, ptr %i.cu, align 8, !tbaa !45, !noalias !57
  %i.cv = getelementptr inbounds nuw i8, ptr %5, i64 48
  store ptr %i.ck, ptr %i.cv, align 8, !tbaa !45, !noalias !57
  %i.cw = getelementptr inbounds nuw i8, ptr %5, i64 56
  store ptr %7, ptr %i.cw, align 8, !tbaa !45, !noalias !57
  %i.cx = getelementptr inbounds nuw i8, ptr %5, i64 64
  store i64 %i.f, ptr %i.cx, align 8, !tbaa !47, !noalias !57
  %i.cy = getelementptr inbounds nuw i8, ptr %5, i64 72
  store i64 %i.cm, ptr %i.cy, align 8, !tbaa !49, !noalias !57
  %i.cz = getelementptr inbounds nuw i8, ptr %5, i64 80
  store ptr %8, ptr %i.cz, align 8, !tbaa !51, !noalias !57
  %i.da = getelementptr inbounds nuw i8, ptr %5, i64 88
  %i.db = load <2 x ptr>, ptr %i.cn, align 8, !tbaa !26
  store <2 x ptr> %i.db, ptr %i.da, align 8, !tbaa !26, !noalias !57
  %i.dc = getelementptr inbounds nuw i8, ptr %5, i64 104
end_hunk_0
