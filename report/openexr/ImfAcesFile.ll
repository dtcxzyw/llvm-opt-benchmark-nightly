...
bb.m:                                             ; preds = %bb.l, %bb.k, %bb.j, %_ZN7Imf_3_418acesChromaticitiesEv.exit
  ...
  %13 = shufflevector <4 x float> %i.ew, <4 x float> %i.dn, <4 x i32> <i32 0, i32 5, i32 0, i32 5>
  %14 = fadd <4 x float> %13, <float -0.000000e+00, float 0.000000e+00, float -0.000000e+00, float 0.000000e+00> ; 2 uses
  %15 = shufflevector <4 x float> %i.ez, <4 x float> poison, <4 x i32> <i32 1, i32 poison, i32 3, i32 poison>
  %16 = call float @llvm.fabs.f32(float %i.db)    ; 3 uses
  %17 = fneg float %16
  %i.fe = insertelement <4 x float> %15, float %17, i64 1
  %i.ff = insertelement <4 x float> %i.fe, float %16, i64 3
  %i.fg = call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %14, <4 x float> <float 9.684870e-01, float 9.869930e-01, float 0.000000e+00, float 4.323050e-01>, <4 x float> %i.ff) ; 3 uses
  ...
  %i.fl = shufflevector <4 x float> %i.dn, <4 x float> %14, <4 x i32> <i32 2, i32 5, i32 2, i32 5>
  %i.fm = fadd <4 x float> %i.fl, <float 0.000000e+00, float -0.000000e+00, float 0.000000e+00, float -0.000000e+00> ; 2 uses
  %i.fn = insertelement <4 x float> poison, float %16, i64 0
