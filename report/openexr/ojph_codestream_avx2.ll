...
bb.a:
  ...
  %i.f = shufflevector <4 x i32> %i.e, <4 x i32> poison, <4 x i32> <i32 2, i32 3, i32 2, i32 3>
  %i.g = bitcast <2 x i64> %i.d to <4 x i32>
  %i.h = or <4 x i32> %i.f, %i.g                  ; 2 uses
  %i.i = shufflevector <4 x i32> %i.h, <4 x i32> poison, <4 x i32> <i32 1, i32 poison, i32 poison, i32 poison>
  %i.j = or <4 x i32> %i.i, %i.h
...
bb.b:                                             ; preds = %._crit_edge
  ...
  %i.ak = tail call <8 x i32> @llvm.x86.avx.cvt.ps2dq.256(<8 x float> %i.aj) ; 2 uses
  %.inner62 = and <8 x i32> %i.ak, splat (i32 -2147483648)
  %i.al = tail call <8 x i32> @llvm.abs.v8i32(<8 x i32> %i.ak, i1 false) ; 2 uses
  %i.am = insertelement <8 x i32> poison, i32 %.040.lcssa, i64 0
  %i.an = shufflevector <8 x i32> %i.am, <8 x i32> poison, <8 x i32> zeroinitializer
  %i.ao = icmp samesign ugt <8 x i32> %i.an, <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>
  %i.ap = select <8 x i1> %i.ao, <8 x i32> %i.al, <8 x i32> zeroinitializer
  %i.aq = bitcast <8 x i32> %i.ap to <4 x i64>
  %i.ar = or <4 x i64> %.041.lcssa, %i.aq
  %.inner63 = or <8 x i32> %.inner62, %i.al
  store <8 x i32> %.inner63, ptr %.0.lcssa, align 1, !tbaa !8
end_hunk_0
