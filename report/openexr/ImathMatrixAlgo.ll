...
.preheader223:                                    ; preds = %bb.f, %.preheader223
  ...
  %10 = insertelement <2 x double> poison, double %i.dd, i64 0
  %11 = insertelement <2 x double> %10, double %i.dg, i64 1
  %i.dh = fsub <2 x double> %11, %i.cp            ; 5 uses
  %i.di = load <2 x double>, ptr %i.de, align 8, !tbaa !8
  %i.dj = fsub <2 x double> %i.di, %i.cn          ; 4 uses
  %12 = load <2 x double>, ptr %i.db, align 8, !tbaa !8
  %i.dk = fsub <2 x double> %12, %i.co            ; 3 uses
  %i.dl = shufflevector <2 x double> %i.dk, <2 x double> poison, <2 x i32> zeroinitializer
  %i.dm = fmul <2 x double> %i.dl, %i.dj
  %i.dn = shufflevector <2 x double> %i.dh, <2 x double> %i.dj, <2 x i32> <i32 1, i32 2>
  %i.do = fmul <2 x double> %i.dk, %i.dn
  %i.dp = shufflevector <2 x double> %i.dk, <2 x double> poison, <2 x i32> <i32 1, i32 1>
  %i.dq = shufflevector <2 x double> %i.dj, <2 x double> %i.dh, <2 x i32> <i32 1, i32 3>
  %i.dr = fmul <2 x double> %i.dp, %i.dq
  %i.ds = shufflevector <2 x double> %i.dh, <2 x double> poison, <2 x i32> zeroinitializer
  %i.dt = fmul <2 x double> %i.ds, %i.dj
...
.preheader223:                                    ; preds = %bb.f, %.preheader223
  ...
  %10 = insertelement <2 x float> poison, float %i.ci, i64 0
  %11 = insertelement <2 x float> %10, float %i.cl, i64 1
  %i.cm = fpext <2 x float> %11 to <2 x double>
  %i.cn = fsub <2 x double> %i.cm, %i.bu          ; 5 uses
  %i.co = load <2 x float>, ptr %i.cj, align 4, !tbaa !27
  %i.cp = fpext <2 x float> %i.co to <2 x double>
  %i.cq = fsub <2 x double> %i.cp, %i.bs          ; 4 uses
  %12 = load <2 x float>, ptr %i.cg, align 4, !tbaa !27
  %i.cr = fpext <2 x float> %12 to <2 x double>
  %i.cs = fsub <2 x double> %i.cr, %i.bt          ; 3 uses
  %i.ct = shufflevector <2 x double> %i.cs, <2 x double> poison, <2 x i32> zeroinitializer
  %i.cu = fmul <2 x double> %i.ct, %i.cq
  %i.cv = shufflevector <2 x double> %i.cn, <2 x double> %i.cq, <2 x i32> <i32 1, i32 2>
  %i.cw = fmul <2 x double> %i.cs, %i.cv
  %i.cx = shufflevector <2 x double> %i.cs, <2 x double> poison, <2 x i32> <i32 1, i32 1>
  %i.cy = shufflevector <2 x double> %i.cq, <2 x double> %i.cn, <2 x i32> <i32 1, i32 3>
  %i.cz = fmul <2 x double> %i.cx, %i.cy
  %i.da = shufflevector <2 x double> %i.cn, <2 x double> poison, <2 x i32> zeroinitializer
  %i.db = fmul <2 x double> %i.da, %i.cq
  %shift = shufflevector <2 x double> %i.cn, <2 x double> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop = fmul <2 x double> %i.cn, %shift
