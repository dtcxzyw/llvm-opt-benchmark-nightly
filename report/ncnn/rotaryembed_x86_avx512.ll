...
.lr.ph467:                                        ; preds = %.lr.ph467.preheader, %.lr.ph467
  ...
  %i.my = insertelement <2 x i32> poison, i32 %i.mx, i64 0
  %10 = insertelement <2 x i32> %i.my, i32 %i.mw, i64 1
  %11 = shl nuw <2 x i32> %10, splat (i32 16)
  %i.mz = bitcast <2 x i32> %11 to <2 x float>    ; 2 uses
  %12 = shufflevector <2 x float> %i.mz, <2 x float> poison, <2 x i32> zeroinitializer
  %i.na = fmul fast <2 x float> %12, %i.mv        ; 2 uses
  %13 = shufflevector <2 x float> %i.mz, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.nb = bitcast <2 x i32> %i.mu to <2 x float>
  %14 = shufflevector <2 x float> %i.nb, <2 x float> poison, <2 x i32> <i32 1, i32 0>
  %i.nc = fmul fast <2 x float> %13, %14          ; 2 uses
  %i.nd = fsub fast <2 x float> %i.na, %i.nc
  %i.ne = fadd fast <2 x float> %i.na, %i.nc
