...
.lr.ph467:                                        ; preds = %.lr.ph467.preheader, %.lr.ph467
  ...
  %10 = load <2 x i16>, ptr %.4466, align 2, !tbaa !99
  %11 = zext <2 x i16> %10 to <2 x i32>
  %12 = shl nuw <2 x i32> %11, splat (i32 16)     ; 2 uses
  %13 = bitcast <2 x i32> %12 to <2 x float>
  %14 = zext i16 %i.mr to i32
  %15 = zext i16 %i.mp to i32
  %i.ms = insertelement <2 x i32> poison, i32 %15, i64 0
  %16 = insertelement <2 x i32> %i.ms, i32 %14, i64 1
  %17 = shl nuw <2 x i32> %16, splat (i32 16)
  %i.mt = bitcast <2 x i32> %17 to <2 x float>    ; 2 uses
  %18 = shufflevector <2 x float> %i.mt, <2 x float> poison, <2 x i32> zeroinitializer
  %i.mu = fmul fast <2 x float> %18, %13          ; 2 uses
  %19 = shufflevector <2 x float> %i.mt, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.mv = bitcast <2 x i32> %12 to <2 x float>
  %20 = shufflevector <2 x float> %i.mv, <2 x float> poison, <2 x i32> <i32 1, i32 0>
  %i.mw = fmul fast <2 x float> %19, %20          ; 2 uses
  %i.mx = fsub fast <2 x float> %i.mu, %i.mw
  %i.my = fadd fast <2 x float> %i.mu, %i.mw
