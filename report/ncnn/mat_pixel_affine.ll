...
._crit_edge:                                      ; preds = %._crit_edge.loopexit, %middle.block, %bb.a
  ...
  %i.al = phi <2 x float> [ zeroinitializer, %bb.a ], [ %i.ag, %middle.block ], [ %i.ef, %._crit_edge.loopexit ] ; 6 uses
  %4 = shufflevector <2 x float> %i.al, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 0> ; 3 uses
  %i.am = extractelement <2 x float> %i.al, i64 1 ; 10 uses
  %i.an = fneg fast float %i.am                   ; 4 uses
  %i.ao = sitofp fast i32 %2 to float             ; 6 uses
  %i.ap = fmul fast float %i.ao, %i.ao
  %i.aq = fmul fast float %i.an, %i.ao            ; 3 uses
  %5 = extractelement <2 x float> %i.al, i64 0    ; 5 uses
  %i.ar = fneg fast float %i.ao
  %i.as = fmul fast float %.sroa.087.0.lcssa, %i.ao ; 2 uses
  %i.at = fmul fast float %i.am, %i.am
  %i.au = fsub fast float %i.as, %i.at            ; 2 uses
  %i.av = fmul fast float %.sroa.087.0.lcssa, %i.an ; 2 uses
  %i.aw = fneg fast float %5                      ; 2 uses
  %i.ax = fmul fast float %.sroa.087.0.lcssa, %i.aw ; 2 uses
  %i.ay = fmul fast float %i.ap, %.sroa.087.0.lcssa ; 2 uses
  %i.az = fmul fast float %i.aq, %i.am
  %i.ba = fadd fast float %i.ay, %i.az
  %6 = insertelement <2 x float> poison, float %i.ao, i64 0
  %7 = insertelement <2 x float> %6, float %i.ar, i64 1
  %8 = fmul fast <2 x float> %i.al, %7            ; 4 uses
  %9 = extractelement <2 x float> %8, i64 0       ; 2 uses
  %10 = fneg fast float %9
  %.neg = fmul fast float %9, %i.aw               ; 2 uses
  %i.bb = fadd fast float %i.ba, %.neg            ; 2 uses
  %i.bc = fmul fast float %i.bb, %.sroa.087.0.lcssa
  %11 = shufflevector <2 x float> %i.al, <2 x float> poison, <2 x i32> <i32 1, i32 0> ; 2 uses
  %12 = fmul fast <2 x float> %8, %11             ; 2 uses
  %13 = tail call fast float @llvm.vector.reduce.fadd.v2f32(float 0.000000e+00, <2 x float> %12)
  %14 = extractelement <2 x float> %8, i64 1      ; 2 uses
  %i.bd = fmul fast float %.sroa.087.0.lcssa, %14
  %i.be = fmul fast float %i.aq, %5
  %15 = extractelement <2 x float> %12, i64 0
  %i.bf = fadd fast float %15, %i.be
  %i.bg = fmul fast float %i.au, %i.am
  %i.bh = fmul fast float %14, %i.am
  %i.bi = fadd fast float %i.ay, %i.bh
  %i.bj = fadd fast float %i.bi, %.neg
  %i.bk = fmul fast float %i.av, %i.am
  %i.bl = fmul fast float %i.ax, %i.am
  %i.bm = fmul fast float %i.av, %5
  %i.bn = fmul fast float %i.au, %.sroa.087.0.lcssa
  %i.bo = fmul fast float %i.ax, %5
  ...
  %i.bu = fmul fast float %.sroa.069.0.lcssa, %13
  ...
  %16 = shufflevector <2 x float> %i.al, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 1, i32 0> ; 2 uses
  %17 = insertelement <4 x float> %16, float %i.an, i64 1
  %18 = fmul fast <4 x float> %4, %17             ; 5 uses
  %i.bz = extractelement <4 x float> %18, i64 0
  %i.ca = fsub fast float %i.as, %i.bz            ; 2 uses
  %19 = shufflevector <4 x float> %18, <4 x float> <float poison, float poison, float 0.000000e+00, float poison>, <4 x i32> <i32 1, i32 0, i32 6, i32 1>
  %20 = fsub fast <4 x float> %18, %19            ; 2 uses
  %i.cb = fmul fast <4 x float> %20, %16          ; 3 uses
  %21 = shufflevector <2 x float> %8, <2 x float> poison, <4 x i32> <i32 0, i32 poison, i32 poison, i32 poison>
  %i.cc = insertelement <4 x float> %21, float %i.aq, i64 1
  %i.cd = insertelement <4 x float> %i.cc, float %i.ca, i64 2
  %i.ce = insertelement <4 x float> %i.cd, float %10, i64 3
  %i.cf = insertelement <2 x float> %11, float %.sroa.087.0.lcssa, i64 0
  ...
  %i.ci = fsub fast <4 x float> %i.cb, %i.ch
  ...
  %foldExtExtBinop = fmul fast <4 x float> %i.ck, %4
  %22 = extractelement <4 x float> %foldExtExtBinop, i64 0
  %i.cl = fadd fast float %i.bc, %22
  %i.cm = extractelement <4 x float> %20, i64 0   ; 2 uses
  %i.cn = fmul fast float %i.cm, %i.an
  %i.co = fsub fast float %i.cn, %i.bd            ; 2 uses
  %i.cp = fmul fast float %i.am, %i.co
  %i.cq = fsub fast float %i.cl, %i.cp
  %i.cr = fdiv fast float 1.000000e+00, %i.cq
  %23 = extractelement <4 x float> %18, i64 2     ; 2 uses
  %i.cs = fmul fast float %23, %5
  %i.ct = fsub fast float %i.cs, %i.bg
  %foldExtExtBinop316 = fmul fast <4 x float> %18, %4
  %shift = shufflevector <4 x float> %i.cb, <4 x float> poison, <4 x i32> <i32 2, i32 poison, i32 poison, i32 poison>
  %foldExtExtBinop318 = fadd fast <4 x float> %shift, %foldExtExtBinop316
  %shift320 = shufflevector <4 x float> %i.ch, <4 x float> poison, <4 x i32> <i32 3, i32 poison, i32 poison, i32 poison>
  %foldExtExtBinop321 = fadd fast <4 x float> %foldExtExtBinop318, %shift320
  %24 = extractelement <4 x float> %foldExtExtBinop321, i64 0
  %i.cu = fmul fast float %i.cm, %i.am
  ...
  %i.cz = fmul fast float %.sroa.087.0.lcssa, %23 ; 2 uses
  %i.da = fadd fast float %i.bl, %i.cz
  %i.db = fadd fast float %i.bm, %i.cz
  %i.dc = fmul fast float %i.ct, %.sroa.21.0.lcssa
  %.neg251.a = fmul fast float %24, %.sroa.21.0.lcssa
  %.neg252.a = fmul fast float %i.cy, %.sroa.15.0.lcssa
  %.neg251 = fmul fast float %i.cw, %.sroa.974.0.lcssa
  %.neg252 = fmul fast float %i.da, %.sroa.21.0.lcssa
  %reass.add259 = fadd fast float %.neg251, %.neg252
  %25 = insertelement <4 x float> poison, float %.sroa.069.0.lcssa, i64 0
  %26 = insertelement <4 x float> %25, float %.sroa.15.0.lcssa, i64 1
  %27 = insertelement <4 x float> %26, float %.sroa.974.0.lcssa, i64 3
  %28 = shufflevector <4 x float> %27, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 1, i32 3>
  %29 = fmul fast <4 x float> %i.ck, %28
  %i.dd = fmul fast float %i.co, %.sroa.069.0.lcssa
  %i.de = fmul fast float %i.db, %.sroa.15.0.lcssa
  %reass.add259.a = fadd fast float %i.dd, %i.de
  %i.df = insertelement <4 x float> poison, float %.neg252.a, i64 0
  ...
  %i.dj = fadd fast <4 x float> %i.di, %29        ; 2 uses
  %i.dk = insertelement <4 x float> poison, float %reass.add259, i64 0
  %i.dl = insertelement <4 x float> %i.dk, float %.neg251.a, i64 1
  %i.dm = insertelement <4 x float> %i.dl, float %i.dc, i64 2
  %i.dn = insertelement <4 x float> %i.dm, float %reass.add259.a, i64 3 ; 2 uses
