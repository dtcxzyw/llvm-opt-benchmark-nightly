...
begin_hunk_1_@_ZN4ncnnL46convolution_im2col_gemm_transform_kernel_bf16sERKNS_3MatERS0_iiiiRKNS_6OptionE.omp_outlined:bb.a
  %i.xo = mul nsw i64 %i.xn, %i.aa
  %gep257.i = getelementptr [4 x i8], ptr %invariant.gep256.i, i64 %i.xo ; 6 uses
  br i1 %i.xb, label %iter.check275, label %.preheader.i

iter.check275:                                    ; preds = %bb.f
  br i1 %min.iters.check259, label %.lr.ph242.i.preheader, label %vector.main.loop.iter.check260

vector.main.loop.iter.check260:                   ; preds = %iter.check275
  br i1 %min.iters.check261, label %vec.epilog.ph279, label %vector.ph262

vector.ph262:                                     ; preds = %vector.main.loop.iter.check260
  %i.xp = getelementptr i8, ptr %gep257.i, i64 %i.xh ; 2 uses
  %i.xq = getelementptr i8, ptr %.12252.i, i64 %i.xi ; 2 uses
  br label %vector.body264

vector.body264:                                   ; preds = %vector.body264, %vector.ph262
  %index265 = phi i64 [ 0, %vector.ph262 ], [ %index.next269, %vector.body264 ] ; 3 uses
  %i.xr = shl i64 %index265, 3
  %next.gep266 = getelementptr i8, ptr %gep257.i, i64 %i.xr
  %i.xs = shl i64 %index265, 2
  %next.gep267 = getelementptr i8, ptr %.12252.i, i64 %i.xs
  %wide.vec = load <32 x i32>, ptr %next.gep266, align 4, !tbaa !74
  %i.xt = lshr <32 x i32> %wide.vec, splat (i32 16)
  %interleaved.vec = trunc nuw <32 x i32> %i.xt to <32 x i16>
  store <32 x i16> %interleaved.vec, ptr %next.gep267, align 2, !tbaa !76
  %index.next269 = add nuw i64 %index265, 16      ; 2 uses
  %i.xu = icmp eq i64 %index.next269, %n.vec263
  br i1 %i.xu, label %middle.block270, label %vector.body264, !llvm.loop !389

middle.block270:                                  ; preds = %vector.body264
  br i1 %cmp.n271, label %.preheader.i, label %vec.epilog.iter.check277

vec.epilog.iter.check277:                         ; preds = %middle.block270
  br i1 %min.epilog.iters.check278, label %.lr.ph242.i.preheader, label %vec.epilog.ph279, !prof !81

vec.epilog.ph279:                                 ; preds = %vector.main.loop.iter.check260, %vec.epilog.iter.check277
  %vec.epilog.resume.val272 = phi i64 [ %n.vec263, %vec.epilog.iter.check277 ], [ 0, %vector.main.loop.iter.check260 ]
  %i.xv = getelementptr i8, ptr %gep257.i, i64 %i.xl ; 2 uses
  %i.xw = getelementptr i8, ptr %.12252.i, i64 %i.xm ; 2 uses
  br label %vec.epilog.vector.body281

vec.epilog.vector.body281:                        ; preds = %vec.epilog.vector.body281, %vec.epilog.ph279
  %index282 = phi i64 [ %vec.epilog.resume.val272, %vec.epilog.ph279 ], [ %index.next289, %vec.epilog.vector.body281 ] ; 3 uses
  %i.xx = shl i64 %index282, 3
  %next.gep283 = getelementptr i8, ptr %gep257.i, i64 %i.xx
  %i.xy = shl i64 %index282, 2
  %next.gep284 = getelementptr i8, ptr %.12252.i, i64 %i.xy
  %wide.vec285 = load <8 x i32>, ptr %next.gep283, align 4, !tbaa !74
  %i.xz = lshr <8 x i32> %wide.vec285, splat (i32 16)
  %interleaved.vec288 = trunc nuw <8 x i32> %i.xz to <8 x i16>
  store <8 x i16> %interleaved.vec288, ptr %next.gep284, align 2, !tbaa !76
  %index.next289 = add nuw i64 %index282, 4       ; 2 uses
  %i.ya = icmp eq i64 %index.next289, %n.vec280
  br i1 %i.ya, label %vec.epilog.middle.block290, label %vec.epilog.vector.body281, !llvm.loop !390

vec.epilog.middle.block290:                       ; preds = %vec.epilog.vector.body281
  br i1 %cmp.n291, label %.preheader.i, label %.lr.ph242.i.preheader

.lr.ph242.i.preheader:                            ; preds = %iter.check275, %vec.epilog.iter.check277, %vec.epilog.middle.block290
  %.0240.i.ph = phi i32 [ 0, %iter.check275 ], [ %i.xg, %vec.epilog.iter.check277 ], [ %i.xk, %vec.epilog.middle.block290 ]
  %.0459239.i.ph = phi ptr [ %gep257.i, %iter.check275 ], [ %i.xp, %vec.epilog.iter.check277 ], [ %i.xv, %vec.epilog.middle.block290 ]
  %.13238.i.ph = phi ptr [ %.12252.i, %iter.check275 ], [ %i.xq, %vec.epilog.iter.check277 ], [ %i.xw, %vec.epilog.middle.block290 ]
  br label %.lr.ph242.i

.preheader.i:                                     ; preds = %.lr.ph242.i, %middle.block270, %vec.epilog.middle.block290, %bb.f
  %.13.lcssa.i = phi ptr [ %.12252.i, %bb.f ], [ %i.xw, %vec.epilog.middle.block290 ], [ %i.xq, %middle.block270 ], [ %i.zs, %.lr.ph242.i ] ; 6 uses
  %.0459.lcssa.i = phi ptr [ %gep257.i, %bb.f ], [ %i.xv, %vec.epilog.middle.block290 ], [ %i.xp, %middle.block270 ], [ %i.zt, %.lr.ph242.i ] ; 5 uses
  %.0.lcssa.i = phi i32 [ 0, %bb.f ], [ %i.xc, %vec.epilog.middle.block290 ], [ %i.xc, %middle.block270 ], [ %i.xc, %.lr.ph242.i ] ; 5 uses
  %i.yb = icmp slt i32 %.0.lcssa.i, %.sroa.speculated
  br i1 %i.yb, label %iter.check, label %._crit_edge250.i

iter.check:                                       ; preds = %.preheader.i
  %i.yc = xor i32 %.0.lcssa.i, -1
  %i.yd = add i32 %smin, %i.yc                    ; 3 uses
  %i.ye = zext i32 %i.yd to i64
  %i.yf = add nuw nsw i64 %i.ye, 1                ; 5 uses
  %min.iters.check = icmp ult i32 %i.yd, 7
  br i1 %min.iters.check, label %.lr.ph249.i.preheader, label %vector.main.loop.iter.check

vector.main.loop.iter.check:                      ; preds = %iter.check
  %min.iters.check241 = icmp ult i32 %i.yd, 63
  br i1 %min.iters.check241, label %vec.epilog.ph, label %vector.ph

vector.ph:                                        ; preds = %vector.main.loop.iter.check
  %i.yg = and i64 %i.yf, 56
  %n.vec = and i64 %i.yf, 8589934528              ; 6 uses
  %i.yh = trunc i64 %n.vec to i32
  %i.yi = add i32 %.0.lcssa.i, %i.yh
  %i.yj = shl nuw nsw i64 %n.vec, 2
  %i.yk = getelementptr i8, ptr %.0459.lcssa.i, i64 %i.yj
  %i.yl = shl nuw nsw i64 %n.vec, 1
  %i.ym = getelementptr i8, ptr %.13.lcssa.i, i64 %i.yl ; 2 uses
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ] ; 3 uses
  %i.yn = shl i64 %index, 2
  %next.gep = getelementptr i8, ptr %.0459.lcssa.i, i64 %i.yn ; 4 uses
  %i.yo = shl i64 %index, 1
  %next.gep242 = getelementptr i8, ptr %.13.lcssa.i, i64 %i.yo ; 4 uses
  %i.yp = getelementptr i8, ptr %next.gep, i64 64
  %i.yq = getelementptr i8, ptr %next.gep, i64 128
  %i.yr = getelementptr i8, ptr %next.gep, i64 192
  %wide.load = load <16 x i32>, ptr %next.gep, align 4, !tbaa !74
  %wide.load243 = load <16 x i32>, ptr %i.yp, align 4, !tbaa !74
  %wide.load244 = load <16 x i32>, ptr %i.yq, align 4, !tbaa !74
  %wide.load245 = load <16 x i32>, ptr %i.yr, align 4, !tbaa !74
  %i.ys = lshr <16 x i32> %wide.load, splat (i32 16)
  %i.yt = lshr <16 x i32> %wide.load243, splat (i32 16)
  %i.yu = lshr <16 x i32> %wide.load244, splat (i32 16)
  %i.yv = lshr <16 x i32> %wide.load245, splat (i32 16)
  %i.yw = trunc nuw <16 x i32> %i.ys to <16 x i16>
  %i.yx = trunc nuw <16 x i32> %i.yt to <16 x i16>
  %i.yy = trunc nuw <16 x i32> %i.yu to <16 x i16>
  %i.yz = trunc nuw <16 x i32> %i.yv to <16 x i16>
  %i.za = getelementptr i8, ptr %next.gep242, i64 32
  %i.zb = getelementptr i8, ptr %next.gep242, i64 64
  %i.zc = getelementptr i8, ptr %next.gep242, i64 96
  store <16 x i16> %i.yw, ptr %next.gep242, align 2, !tbaa !76
  store <16 x i16> %i.yx, ptr %i.za, align 2, !tbaa !76
  store <16 x i16> %i.yy, ptr %i.zb, align 2, !tbaa !76
  store <16 x i16> %i.yz, ptr %i.zc, align 2, !tbaa !76
  %index.next = add nuw i64 %index, 64            ; 2 uses
  %i.zd = icmp eq i64 %index.next, %n.vec
  br i1 %i.zd, label %middle.block, label %vector.body, !llvm.loop !391

middle.block:                                     ; preds = %vector.body
  %cmp.n = icmp eq i64 %i.yf, %n.vec
  br i1 %cmp.n, label %._crit_edge250.i, label %vec.epilog.iter.check

vec.epilog.iter.check:                            ; preds = %middle.block
  %min.epilog.iters.check = icmp eq i64 %i.yg, 0
  br i1 %min.epilog.iters.check, label %.lr.ph249.i.preheader, label %vec.epilog.ph, !prof !108

vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  %vec.epilog.resume.val = phi i64 [ %n.vec, %vec.epilog.iter.check ], [ 0, %vector.main.loop.iter.check ]
  %n.vec248 = and i64 %i.yf, 8589934584           ; 5 uses
  %i.ze = trunc i64 %n.vec248 to i32
  %i.zf = add i32 %.0.lcssa.i, %i.ze
  %i.zg = shl nuw nsw i64 %n.vec248, 2
  %i.zh = getelementptr i8, ptr %.0459.lcssa.i, i64 %i.zg
  %i.zi = shl nuw nsw i64 %n.vec248, 1
  %i.zj = getelementptr i8, ptr %.13.lcssa.i, i64 %i.zi ; 2 uses
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index249 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next253, %vec.epilog.vector.body ] ; 3 uses
  %i.zk = shl i64 %index249, 2
  %next.gep250 = getelementptr i8, ptr %.0459.lcssa.i, i64 %i.zk
  %i.zl = shl i64 %index249, 1
  %next.gep251 = getelementptr i8, ptr %.13.lcssa.i, i64 %i.zl
  %wide.load252 = load <8 x i32>, ptr %next.gep250, align 4, !tbaa !74
  %i.zm = lshr <8 x i32> %wide.load252, splat (i32 16)
  %i.zn = trunc nuw <8 x i32> %i.zm to <8 x i16>
  store <8 x i16> %i.zn, ptr %next.gep251, align 2, !tbaa !76
  %index.next253 = add nuw i64 %index249, 8       ; 2 uses
  %i.zo = icmp eq i64 %index.next253, %n.vec248
  br i1 %i.zo, label %vec.epilog.middle.block, label %vec.epilog.vector.body, !llvm.loop !392

vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  %cmp.n254 = icmp eq i64 %i.yf, %n.vec248
...
.lr.ph249.i.preheader:                            ; preds = %iter.check, %vec.epilog.iter.check, %vec.epilog.middle.block
  %.1248.i.ph = phi i32 [ %.0.lcssa.i, %iter.check ], [ %i.yi, %vec.epilog.iter.check ], [ %i.zf, %vec.epilog.middle.block ]
  %.1460247.i.ph = phi ptr [ %.0459.lcssa.i, %iter.check ], [ %i.yk, %vec.epilog.iter.check ], [ %i.zh, %vec.epilog.middle.block ]
  %.14246.i.ph = phi ptr [ %.13.lcssa.i, %iter.check ], [ %i.ym, %vec.epilog.iter.check ], [ %i.zj, %vec.epilog.middle.block ]
  br label %.lr.ph249.i

.lr.ph242.i:                                      ; preds = %.lr.ph242.i.preheader, %.lr.ph242.i
  %.0240.i = phi i32 [ %i.zu, %.lr.ph242.i ], [ %.0240.i.ph, %.lr.ph242.i.preheader ]
  %.0459239.i = phi ptr [ %i.zt, %.lr.ph242.i ], [ %.0459239.i.ph, %.lr.ph242.i.preheader ] ; 2 uses
  %.13238.i = phi ptr [ %i.zs, %.lr.ph242.i ], [ %.13238.i.ph, %.lr.ph242.i.preheader ] ; 2 uses
  %i.zp = load <2 x i32>, ptr %.0459239.i, align 4, !tbaa !74
  %i.zq = lshr <2 x i32> %i.zp, splat (i32 16)
  %i.zr = trunc nuw <2 x i32> %i.zq to <2 x i16>
  store <2 x i16> %i.zr, ptr %.13238.i, align 2, !tbaa !76
  %i.zs = getelementptr inbounds nuw i8, ptr %.13238.i, i64 4 ; 2 uses
  %i.zt = getelementptr inbounds nuw i8, ptr %.0459239.i, i64 8 ; 2 uses
  %i.zu = add nuw nsw i32 %.0240.i, 2             ; 2 uses
  %i.zv = or disjoint i32 %i.zu, 1
  %i.zw = icmp slt i32 %i.zv, %.sroa.speculated
  br i1 %i.zw, label %.lr.ph242.i, label %.preheader.i, !llvm.loop !393

.lr.ph249.i:                                      ; preds = %.lr.ph249.i.preheader, %.lr.ph249.i
  %.1248.i = phi i32 [ %i.aac, %.lr.ph249.i ], [ %.1248.i.ph, %.lr.ph249.i.preheader ]
  %.1460247.i = phi ptr [ %i.aab, %.lr.ph249.i ], [ %.1460247.i.ph, %.lr.ph249.i.preheader ] ; 2 uses
  %.14246.i = phi ptr [ %i.aaa, %.lr.ph249.i ], [ %.14246.i.ph, %.lr.ph249.i.preheader ] ; 2 uses
  %i.zx = load i32, ptr %.1460247.i, align 4, !tbaa !74
  %i.zy = lshr i32 %i.zx, 16
  %i.zz = trunc nuw i32 %i.zy to i16
  store i16 %i.zz, ptr %.14246.i, align 2, !tbaa !76
  %i.aaa = getelementptr inbounds nuw i8, ptr %.14246.i, i64 2 ; 2 uses
  %i.aab = getelementptr inbounds nuw i8, ptr %.1460247.i, i64 4
  %i.aac = add nuw nsw i32 %.1248.i, 1            ; 2 uses
  %exitcond328.not.i = icmp eq i32 %i.aac, %.sroa.speculated
...
._crit_edge250.i:                                 ; preds = %.lr.ph249.i, %middle.block, %vec.epilog.middle.block, %.preheader.i
  %.14.lcssa.i = phi ptr [ %.13.lcssa.i, %.preheader.i ], [ %i.zj, %vec.epilog.middle.block ], [ %i.ym, %middle.block ], [ %i.aaa, %.lr.ph249.i ]
  %indvars.iv.next330.i = add nuw nsw i64 %indvars.iv329.i, 1 ; 2 uses
  %exitcond332.not.i = icmp eq i64 %indvars.iv.next330.i, %i.aw
...
_ZN4ncnn3MatD2Ev.exit:                            ; preds = %._crit_edge250.i, %.preheader3.i
  ...
  %i.aad = icmp slt i64 %indvars.iv.next, %i.ac
  %indvar.next = add i32 %indvar, 1
  br i1 %i.aad, label %.noexc, label %._crit_edge, !llvm.loop !396
...
.loopexit.i.i:                                    ; preds = %.loopexit.i.i.loopexit236.unr-lcssa, %bb.d, %.loopexit.i.i.loopexit235.unr-lcssa, %.lr.ph709.i.i.epil, %.loopexit.i.i.loopexit234.unr-lcssa, %.lr.ph704.i.i.epil, %.loopexit.i.i.loopexit.unr-lcssa, %.lr.ph699.i.i.epil, %_ZN4ncnn3MatD2Ev.exit.i.i, %_ZN4ncnn3MatD2Ev.exit763.i.i, %_ZN4ncnn3MatD2Ev.exit764.i.i, %_ZN4ncnn3MatD2Ev.exit765.i.i, %.split839.i.i
  ...
  %indvars.iv.next769.i.i = add nuw nsw i64 %indvars.iv768.i.i, 1 ; 2 uses
...
._crit_edge9705:                                  ; preds = %bb.hv, %.preheader8884
  ...
  %indvars.iv.next10040 = add nuw nsw i64 %indvars.iv10039, 1 ; 2 uses
...
._crit_edge2213:                                  ; preds = %bb.fx, %bb.de
  ...
  %indvars.iv.next2310 = add nuw nsw i64 %indvars.iv2309, 1 ; 2 uses
...
._crit_edge3915:                                  ; preds = %bb.ff, %bb.fc
  ...
  %indvars.iv.next4012 = add nuw nsw i64 %indvars.iv4011, 1 ; 2 uses
