...
.lr.ph:                                           ; preds = %.thread
  ...
  %8 = getelementptr inbounds nuw i8, ptr %i.bcb, i64 4
  %i.bcc = load ptr, ptr %7, align 8
  %i.bcd = load ptr, ptr %7, align 8              ; 2 uses
  %i.bce = getelementptr inbounds nuw i8, ptr %i.bcd, i64 4
  %i.bcf = load ptr, ptr %7, align 8              ; 2 uses
  %9 = getelementptr inbounds nuw i8, ptr %i.bcf, i64 4
...
_ZL13activation_ssfiRKN4ncnn3MatE.exit804.thread714: ; preds = %bb.aq
  %10 = load float, ptr %8, align 4, !tbaa !22    ; 4 uses
  %11 = load float, ptr %i.bcb, align 4, !tbaa !22 ; 3 uses
  %12 = insertelement <2 x float> poison, float %11, i64 0
  %i.bff = shufflevector <2 x float> %12, <2 x float> poison, <2 x i32> zeroinitializer
  ...
  %i.bfh = fcmp fast ogt <2 x float> %i.bfg, %14
  %i.bfi = select <2 x i1> %i.bfh, <2 x float> %14, <2 x float> %i.bfg
  %i.bfj = extractelement <2 x float> %i.bet, i64 0
  %.0647 = tail call nnan ninf nsz float @llvm.maxnum.f32(float %i.bfj, float %11) ; 2 uses
  %i.bfk = fcmp fast ogt float %.0647, %10
  %.1648779 = select i1 %i.bfk, float %10, float %.0647
  %i.bfl = extractelement <2 x float> %i.bet, i64 1
  %.0649 = tail call nnan ninf nsz float @llvm.maxnum.f32(float %i.bfl, float %11)
  %spec.select829 = tail call nnan ninf nsz float @llvm.minnum.f32(float %.0649, float %10)
...
_ZL13activation_ssfiRKN4ncnn3MatE.exit804.thread714.1: ; preds = %bb.bd
  %15 = load float, ptr %9, align 4, !tbaa !22    ; 4 uses
  %16 = load float, ptr %i.bcf, align 4, !tbaa !22 ; 3 uses
  %17 = insertelement <2 x float> poison, float %16, i64 0
  %i.bmm = shufflevector <2 x float> %17, <2 x float> poison, <2 x i32> zeroinitializer
  ...
  %i.bmo = fcmp fast ogt <2 x float> %i.bmn, %19
  %i.bmp = select <2 x i1> %i.bmo, <2 x float> %19, <2 x float> %i.bmn
  %i.bmq = extractelement <2 x float> %i.bjk, i64 0
  %.0647.1 = tail call nnan ninf nsz float @llvm.maxnum.f32(float %i.bmq, float %16) ; 2 uses
  ...
  %.0649.1 = tail call nnan ninf nsz float @llvm.maxnum.f32(float %i.bms, float %16)
