...
begin_hunk_0_@_ZN4ncnnL24convolution_packed_bf16sERKNS_3MatERS0_S2_S2_iiiiiiiS2_RKNS_6OptionE:bb.a
  %invariant.op6868 = add <16 x i32> %invariant.op6866, %i.ba
  %cmp.n = icmp eq i64 %n.vec, %i.aw
  %min.epilog.iters.check = icmp eq i64 %i.ax, 0
  %n.vec6238 = and i64 %i.aw, 2147483640          ; 4 uses
  %i.bc = trunc nuw nsw i64 %n.vec6238 to i32
  %i.bd = mul i32 %6, %i.bc
  %broadcast.splatinsert6239 = insertelement <8 x i32> poison, i32 %i.n, i64 0
  %broadcast.splat6240 = shufflevector <8 x i32> %broadcast.splatinsert6239, <8 x i32> poison, <8 x i32> zeroinitializer
  %broadcast.splatinsert6243.a = insertelement <8 x i32> poison, i32 %6, i64 0
  %broadcast.splat6244.a = shufflevector <8 x i32> %broadcast.splatinsert6243.a, <8 x i32> poison, <8 x i32> zeroinitializer
  %i.be = mul nsw <8 x i32> %broadcast.splat6244.a, <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>
  %i.bf = shl nsw i32 %6, 3
  %broadcast.splatinsert6246 = insertelement <8 x i32> poison, i32 %i.bf, i64 0
  %broadcast.splat6247 = shufflevector <8 x i32> %broadcast.splatinsert6246, <8 x i32> poison, <8 x i32> zeroinitializer
  %cmp.n6252 = icmp eq i64 %n.vec6238, %i.aw
  br label %iter.check
...
vector.main.loop.iter.check:                      ; preds = %iter.check
  br i1 %min.iters.check6232, label %vec.epilog.ph, label %vector.ph

vector.ph:                                        ; preds = %vector.main.loop.iter.check
  %i.bi = add nsw i64 %n.vec, %i.bg
  %i.bj = add i32 %.016324408, %i.az              ; 3 uses
  %broadcast.splatinsert6235 = insertelement <16 x i32> poison, i32 %.016324408, i64 0
  %broadcast.splat6236 = shufflevector <16 x i32> %broadcast.splatinsert6235, <16 x i32> poison, <16 x i32> zeroinitializer
  %induction = add nsw <16 x i32> %broadcast.splat6236, %i.bb
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %step.add.3.reass = add <16 x i32> %vec.ind, %invariant.op6866
  %i.bk = mul nsw <16 x i32> %vec.ind, %broadcast.splat6234
  %i.bl = mul nsw <16 x i32> %step.add, %broadcast.splat6234
  %i.bm = mul nsw <16 x i32> %step.add.2.reass, %broadcast.splat6234
  %i.bn = mul nsw <16 x i32> %step.add.3.reass, %broadcast.splat6234
  %gep = getelementptr [4 x i8], ptr %invariant.gep6867, i64 %index ; 4 uses
  ...
  %vec.ind.next.reass = add <16 x i32> %vec.ind, %invariant.op6868
...
vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  ...
  %bc.resume.val6237 = phi i32 [ %i.bj, %vec.epilog.iter.check ], [ %.016324408, %vector.main.loop.iter.check ]
  %i.bs = add nsw i64 %n.vec6238, %i.bg
  %i.bt = add i32 %.016324408, %i.bd              ; 2 uses
  %broadcast.splatinsert6241 = insertelement <8 x i32> poison, i32 %bc.resume.val6237, i64 0
  %broadcast.splat6242 = shufflevector <8 x i32> %broadcast.splatinsert6241, <8 x i32> poison, <8 x i32> zeroinitializer
  %induction6245 = add nsw <8 x i32> %broadcast.splat6242, %i.be
  %invariant.gep6869 = getelementptr [4 x i8], ptr %.sroa.03778.0, i64 %i.bg
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index6248 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next6250, %vec.epilog.vector.body ] ; 2 uses
  %vec.ind6249 = phi <8 x i32> [ %induction6245, %vec.epilog.ph ], [ %vec.ind.next6251, %vec.epilog.vector.body ] ; 2 uses
  %i.bu = mul nsw <8 x i32> %vec.ind6249, %broadcast.splat6240
  %gep6870 = getelementptr [4 x i8], ptr %invariant.gep6869, i64 %index6248
  store <8 x i32> %i.bu, ptr %gep6870, align 4, !tbaa !113
  %index.next6250 = add nuw i64 %index6248, 8     ; 2 uses
  %vec.ind.next6251 = add nsw <8 x i32> %vec.ind6249, %broadcast.splat6247
  %i.bv = icmp eq i64 %index.next6250, %n.vec6238
  br i1 %i.bv, label %vec.epilog.middle.block, label %vec.epilog.vector.body, !llvm.loop !121

vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  br i1 %cmp.n6252, label %._crit_edge, label %vec.epilog.scalar.ph.preheader
...
._crit_edge:                                      ; preds = %vec.epilog.scalar.ph, %vec.epilog.middle.block, %middle.block
  %.lcssa6231 = phi i32 [ %i.bt, %vec.epilog.middle.block ], [ %i.bj, %middle.block ], [ %i.co, %vec.epilog.scalar.ph ]
  %i.ck = add nsw i32 %i.at, %.lcssa6231
...
vec.epilog.scalar.ph:                             ; preds = %vec.epilog.scalar.ph.preheader, %vec.epilog.scalar.ph
  ...
  %i.co = add nsw i32 %.116334404, %6             ; 2 uses
...
._crit_edge4627:                                  ; preds = %._crit_edge4625.split, %._crit_edge4410.split
  ...
  br i1 %i.ct, label %.noexc2328.lr.ph, label %._crit_edge4850

.noexc2328.lr.ph:                                 ; preds = %._crit_edge4627
  ...
  %wide.trip.count5614 = zext nneg i32 %i.cs to i64
  br label %.noexc2328

.noexc2330:                                       ; preds = %.noexc2330.lr.ph, %._crit_edge4625.split
  %indvars.iv5516 = phi i64 [ 0, %.noexc2330.lr.ph ], [ %indvars.iv.next5517, %._crit_edge4625.split ] ; 2 uses
  ...
  br i1 %i.df, label %.preheader4399.lr.ph, label %._crit_edge4625.split

.preheader4399.lr.ph:                             ; preds = %.noexc2330
  %i.dg = shl nuw nsw i64 %indvars.iv5516, 3
  ...
  br i1 %i.dj, label %.preheader4399.preheader, label %._crit_edge4625.split

.preheader4399.preheader:                         ; preds = %.preheader4399.lr.ph
  ...
  %wide.trip.count5513 = zext nneg i32 %i.dc to i64
  br label %.preheader4399

.preheader4399:                                   ; preds = %.preheader4399.preheader, %._crit_edge4615
  %.016234624 = phi i32 [ %i.eq, %._crit_edge4615 ], [ 0, %.preheader4399.preheader ] ; 2 uses
  %.016244623 = phi ptr [ %.41628, %._crit_edge4615 ], [ %i.ec, %.preheader4399.preheader ]
  %i.ee = load i32, ptr %i.b, align 4
  %i.ef = mul nsw i32 %i.ee, %.016234624
  %i.eg = sext i32 %i.ef to i64                   ; 5 uses
  %i.eh = load i32, ptr %i.a, align 4             ; 2 uses
  %factor.op.mul4617 = mul i32 %i.cz, %i.eh       ; 3 uses
  ...
  %wide.trip.count5495 = zext i32 %i.ei to i64    ; 3 uses
  %xtraiter = and i64 %wide.trip.count5495, 1
  %i.ep = icmp eq i32 %i.ei, 1
  %unroll_iter = and i64 %wide.trip.count5495, 2147483646
  %lcmp.mod.not = icmp eq i64 %xtraiter, 0
  %lcmp.mod6640 = trunc i32 %i.ei to i1
  br label %bb.b

._crit_edge4625.split:                            ; preds = %._crit_edge4615, %.preheader4399.lr.ph, %.noexc2330
  %indvars.iv.next5517 = add nuw nsw i64 %indvars.iv5516, 1 ; 2 uses
  %exitcond5520.not = icmp eq i64 %indvars.iv.next5517, %wide.trip.count5519
  br i1 %exitcond5520.not, label %._crit_edge4627, label %.noexc2330, !llvm.loop !132

._crit_edge4615:                                  ; preds = %.thread4292
  %i.eq = add nuw nsw i32 %.016234624, 1          ; 2 uses
  %exitcond5515.not = icmp eq i32 %i.eq, %i.dd
  br i1 %exitcond5515.not, label %._crit_edge4625.split, label %.preheader4399, !llvm.loop !133

bb.b:                                             ; preds = %.preheader4399, %.thread4292
  %indvars.iv5510 = phi i64 [ 0, %.preheader4399 ], [ %indvars.iv.next5511, %.thread4292 ] ; 6 uses
  %.116254612 = phi ptr [ %.016244623, %.preheader4399 ], [ %.41628, %.thread4292 ] ; 15 uses
...
_ZN4ncnn3MatD2Ev.exit2358:                        ; preds = %bb.c, %bb.b
  ...
  br i1 %i.dp, label %_ZN4ncnn3MatD2Ev.exit2357.lr.ph, label %.preheader4398
...
_ZN4ncnn3MatD2Ev.exit2357.lr.ph:                  ; preds = %_ZN4ncnn3MatD2Ev.exit2358
  ...
  %i.fh = trunc nuw nsw i64 %indvars.iv5510 to i32
  %.reass4618 = mul i32 %factor.op.mul4617, %i.fh
  %i.fi = sext i32 %.reass4618 to i64
  %invariant.gep4472 = getelementptr [2 x i8], ptr %invariant.gep, i64 %i.fi
  ...
  %wide.trip.count5453 = zext nneg i32 %i.fj to i64
  %wide.trip.count5446 = zext nneg i32 %i.fj to i64
  %wide.trip.count5439 = zext nneg i32 %i.fj to i64
  %wide.trip.count = zext nneg i32 %i.fj to i64
  br label %_ZN4ncnn3MatD2Ev.exit2357

.preheader4398:                                   ; preds = %.loopexit4390, %_ZN4ncnn3MatD2Ev.exit2358
  %.04179.lcssa = phi <8 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2358 ], [ %.84187, %.loopexit4390 ] ; 2 uses
  %.04158.lcssa = phi <8 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2358 ], [ %.84166, %.loopexit4390 ] ; 2 uses
  %.04135.lcssa = phi <8 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2358 ], [ %.84143, %.loopexit4390 ] ; 2 uses
  %.14111.lcssa = phi <8 x float> [ %.04110, %_ZN4ncnn3MatD2Ev.exit2358 ], [ %.94119, %.loopexit4390 ] ; 2 uses
  %.01597.lcssa = phi ptr [ %i.ez, %_ZN4ncnn3MatD2Ev.exit2358 ], [ %.81605, %.loopexit4390 ] ; 2 uses
  %.01592.lcssa = phi i32 [ 0, %_ZN4ncnn3MatD2Ev.exit2358 ], [ %i.ed, %.loopexit4390 ] ; 3 uses
  %i.fq = or disjoint i32 %.01592.lcssa, 7
  %i.fr = icmp slt i32 %i.fq, %i.db
  br i1 %i.fr, label %_ZN4ncnn3MatD2Ev.exit2356.lr.ph, label %.preheader4397

_ZN4ncnn3MatD2Ev.exit2356.lr.ph:                  ; preds = %.preheader4398
  ...
  %factor.op.mul4523 = mul i64 %i.fu, %i.fv
  %i.fw = sext i32 %i.fs to i64
  %i.fx = mul i64 %i.fv, %i.fw
  %i.fy = mul i64 %i.fx, %i.eg
  %invariant.gep4525 = getelementptr i8, ptr %i.ft, i64 %i.fy
  %i.fz = trunc nuw nsw i64 %indvars.iv5510 to i32
  %.reass4620 = mul i32 %factor.op.mul4617, %i.fz
  %i.ga = sext i32 %.reass4620 to i64
  %invariant.gep4526 = getelementptr [2 x i8], ptr %invariant.gep4525, i64 %i.ga
  ...
  %wide.trip.count5474 = zext nneg i32 %i.gb to i64
  %wide.trip.count5467 = zext nneg i32 %i.gb to i64
  %wide.trip.count5460 = zext nneg i32 %i.gb to i64
  br label %_ZN4ncnn3MatD2Ev.exit2356

_ZN4ncnn3MatD2Ev.exit2357:                        ; preds = %_ZN4ncnn3MatD2Ev.exit2357.lr.ph, %.loopexit4390
  %.015924465 = phi i32 [ 0, %_ZN4ncnn3MatD2Ev.exit2357.lr.ph ], [ %i.aal, %.loopexit4390 ] ; 2 uses
  %.015974464 = phi ptr [ %i.ez, %_ZN4ncnn3MatD2Ev.exit2357.lr.ph ], [ %.81605, %.loopexit4390 ] ; 13 uses
  %.141114463 = phi <8 x float> [ %.04110, %_ZN4ncnn3MatD2Ev.exit2357.lr.ph ], [ %.94119, %.loopexit4390 ] ; 9 uses
  %.041354462 = phi <8 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2357.lr.ph ], [ %.84143, %.loopexit4390 ] ; 9 uses
  %.041584461 = phi <8 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2357.lr.ph ], [ %.84166, %.loopexit4390 ] ; 9 uses
  %.041794460 = phi <8 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2357.lr.ph ], [ %.84187, %.loopexit4390 ] ; 9 uses
  %i.gi = sdiv i32 %.015924465, %i.cz
  %i.gj = sext i32 %i.gi to i64
  %.reass = mul i64 %factor.op.mul, %i.gj
  %gep4473 = getelementptr i8, ptr %invariant.gep4472, i64 %.reass ; 23 uses
  br i1 %i.dq, label %.preheader4393, label %.loopexit4394

.preheader4393:                                   ; preds = %_ZN4ncnn3MatD2Ev.exit2357
  br i1 %i.fk, label %.lr.ph, label %.loopexit4390

.lr.ph:                                           ; preds = %.preheader4393, %.lr.ph
  %indvars.iv5429 = phi i64 [ %indvars.iv.next5430, %.lr.ph ], [ 0, %.preheader4393 ] ; 2 uses
  %.115984415 = phi ptr [ %i.ki, %.lr.ph ], [ %.015974464, %.preheader4393 ] ; 17 uses
  %.241124414 = phi <8 x float> [ %i.jy, %.lr.ph ], [ %.141114463, %.preheader4393 ]
  %.141364413 = phi <8 x float> [ %i.kb, %.lr.ph ], [ %.041354462, %.preheader4393 ]
  %.141594412 = phi <8 x float> [ %i.ke, %.lr.ph ], [ %.041584461, %.preheader4393 ]
  %.141804411 = phi <8 x float> [ %i.kh, %.lr.ph ], [ %.041794460, %.preheader4393 ]
  %i.gk = getelementptr inbounds nuw [4 x i8], ptr %i.fl, i64 %indvars.iv5429
  %i.gl = load i32, ptr %i.gk, align 4, !tbaa !113
  %i.gm = sext i32 %i.gl to i64
  %i.gn = getelementptr inbounds [2 x i8], ptr %gep4473, i64 %i.gm
  %i.go = load <8 x bfloat>, ptr %.115984415, align 16, !tbaa !20
  %i.gp = fpext fast <8 x bfloat> %i.go to <8 x float>
  %i.gq = getelementptr inbounds nuw i8, ptr %.115984415, i64 16
  %i.gr = load <8 x bfloat>, ptr %i.gq, align 16, !tbaa !20
  %i.gs = fpext fast <8 x bfloat> %i.gr to <8 x float>
  %i.gt = getelementptr inbounds nuw i8, ptr %.115984415, i64 32
  %i.gu = load <8 x bfloat>, ptr %i.gt, align 16, !tbaa !20
  %i.gv = fpext fast <8 x bfloat> %i.gu to <8 x float>
  %i.gw = getelementptr inbounds nuw i8, ptr %.115984415, i64 48
  %i.gx = load <8 x bfloat>, ptr %i.gw, align 16, !tbaa !20
  %i.gy = fpext fast <8 x bfloat> %i.gx to <8 x float>
  %i.gz = getelementptr inbounds nuw i8, ptr %.115984415, i64 64
  %i.ha = load <8 x bfloat>, ptr %i.gz, align 16, !tbaa !20
  %i.hb = fpext fast <8 x bfloat> %i.ha to <8 x float>
  %i.hc = getelementptr inbounds nuw i8, ptr %.115984415, i64 80
  %i.hd = load <8 x bfloat>, ptr %i.hc, align 16, !tbaa !20
  %i.he = fpext fast <8 x bfloat> %i.hd to <8 x float>
  %i.hf = getelementptr inbounds nuw i8, ptr %.115984415, i64 96
  %i.hg = load <8 x bfloat>, ptr %i.hf, align 16, !tbaa !20
  %i.hh = fpext fast <8 x bfloat> %i.hg to <8 x float>
  %i.hi = getelementptr inbounds nuw i8, ptr %.115984415, i64 112
  %i.hj = load <8 x bfloat>, ptr %i.hi, align 16, !tbaa !20
  %i.hk = fpext fast <8 x bfloat> %i.hj to <8 x float>
  %i.hl = getelementptr inbounds nuw i8, ptr %.115984415, i64 128
  %i.hm = load <8 x bfloat>, ptr %i.hl, align 16, !tbaa !20
  %i.hn = fpext fast <8 x bfloat> %i.hm to <8 x float>
  %i.ho = getelementptr inbounds nuw i8, ptr %.115984415, i64 144
  %i.hp = load <8 x bfloat>, ptr %i.ho, align 16, !tbaa !20
  %i.hq = fpext fast <8 x bfloat> %i.hp to <8 x float>
  %i.hr = getelementptr inbounds nuw i8, ptr %.115984415, i64 160
...
begin_hunk_1_@_ZN4ncnnL24convolution_packed_bf16sERKNS_3MatERS0_S2_S2_iiiiiiiS2_RKNS_6OptionE:bb.a
  ...
  %i.aev = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.ads, <8 x float> nofpclass(nan inf) %i.aeu, <8 x float> nofpclass(nan inf) %.1341234489)
  ...
  %i.aey = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.adv, <8 x float> nofpclass(nan inf) %i.aex, <8 x float> nofpclass(nan inf) %.1241474488)
  ...
  %i.afb = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.ady, <8 x float> nofpclass(nan inf) %i.afa, <8 x float> nofpclass(nan inf) %.1241704487)
  ...
  %i.afe = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.aeb, <8 x float> nofpclass(nan inf) %i.afd, <8 x float> nofpclass(nan inf) %.1241914486)
  ...
  %i.afr = getelementptr inbounds nuw i8, ptr %.1216094490, i64 128
  %indvars.iv.next5463 = add nuw nsw i64 %indvars.iv5462, 1 ; 2 uses
  %exitcond5468.not = icmp eq i64 %indvars.iv.next5463, %wide.trip.count5467
  br i1 %exitcond5468.not, label %.loopexit4386.loopexit, label %.lr.ph4493, !llvm.loop !152

.loopexit4386.loopexit:                           ; preds = %.lr.ph4493
  %scevgep5464 = getelementptr i8, ptr %.916064515, i64 128
  %scevgep5465 = getelementptr i8, ptr %scevgep5464, i64 %i.gh
  br label %.loopexit4385

.loopexit4386:                                    ; preds = %.loopexit4388
  br i1 %i.dt, label %.preheader4384, label %.loopexit4385

.preheader4384:                                   ; preds = %.loopexit4386
  br i1 %i.gc, label %.lr.ph4505, label %.loopexit4385

.lr.ph4505:                                       ; preds = %.preheader4384
  %i.afs = getelementptr [2 x i8], ptr %gep4527, i64 %i.ge
  %i.aft = getelementptr i8, ptr %gep4527, i64 %.idx2200
  %i.afu = getelementptr i8, ptr %gep4527, i64 %.idx2201
  %i.afv = getelementptr i8, ptr %gep4527, i64 %.idx2202
  %i.afw = getelementptr i8, ptr %gep4527, i64 %.idx2203
  %i.afx = getelementptr i8, ptr %gep4527, i64 %.idx2204
  %i.afy = getelementptr i8, ptr %gep4527, i64 %.idx2205
  br label %bb.h

bb.h:                                             ; preds = %.lr.ph4505, %bb.h
  %indvars.iv5469 = phi i64 [ 0, %.lr.ph4505 ], [ %indvars.iv.next5470, %bb.h ] ; 2 uses
  %.1416114503 = phi ptr [ %.916064515, %.lr.ph4505 ], [ %i.ajl, %bb.h ] ; 9 uses
  %.1541254502 = phi <8 x float> [ %.1041204514, %.lr.ph4505 ], [ %i.aim, %bb.h ]
  %.1441494501 = phi <8 x float> [ %.941444513, %.lr.ph4505 ], [ %i.aiu, %bb.h ]
  %.1441724500 = phi <8 x float> [ %.941674512, %.lr.ph4505 ], [ %i.ajc, %bb.h ]
  %.1441934499 = phi <8 x float> [ %.941884511, %.lr.ph4505 ], [ %i.ajk, %bb.h ]
  %i.afz = getelementptr inbounds nuw [4 x i8], ptr %i.gd, i64 %indvars.iv5469
  %i.aga = load i32, ptr %i.afz, align 4, !tbaa !113
  %i.agb = load <8 x bfloat>, ptr %.1416114503, align 16, !tbaa !20
  %i.agc = fpext fast <8 x bfloat> %i.agb to <8 x float>
  %i.agd = getelementptr inbounds nuw i8, ptr %.1416114503, i64 16
  %i.age = load <8 x bfloat>, ptr %i.agd, align 16, !tbaa !20
  %i.agf = fpext fast <8 x bfloat> %i.age to <8 x float>
  %i.agg = getelementptr inbounds nuw i8, ptr %.1416114503, i64 32
  %i.agh = load <8 x bfloat>, ptr %i.agg, align 16, !tbaa !20
  %i.agi = fpext fast <8 x bfloat> %i.agh to <8 x float>
  %i.agj = getelementptr inbounds nuw i8, ptr %.1416114503, i64 48
  %i.agk = load <8 x bfloat>, ptr %i.agj, align 16, !tbaa !20
  %i.agl = fpext fast <8 x bfloat> %i.agk to <8 x float>
  %i.agm = getelementptr inbounds nuw i8, ptr %.1416114503, i64 64
  %i.agn = load <8 x bfloat>, ptr %i.agm, align 16, !tbaa !20
  %i.ago = fpext fast <8 x bfloat> %i.agn to <8 x float>
  %i.agp = getelementptr inbounds nuw i8, ptr %.1416114503, i64 80
  %i.agq = load <8 x bfloat>, ptr %i.agp, align 16, !tbaa !20
  %i.agr = fpext fast <8 x bfloat> %i.agq to <8 x float>
  %i.ags = getelementptr inbounds nuw i8, ptr %.1416114503, i64 96
  %i.agt = load <8 x bfloat>, ptr %i.ags, align 16, !tbaa !20
  %i.agu = fpext fast <8 x bfloat> %i.agt to <8 x float>
  %i.agv = getelementptr inbounds nuw i8, ptr %.1416114503, i64 112
  ...
  %i.agz = getelementptr inbounds [2 x i8], ptr %gep4527, i64 %i.agy
  ...
  %i.ahg = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.agc, <8 x float> nofpclass(nan inf) %i.ahf, <8 x float> nofpclass(nan inf) %.1541254502)
  ...
  %i.aho = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.agf, <8 x float> nofpclass(nan inf) %i.ahn, <8 x float> nofpclass(nan inf) %.1441494501)
  ...
  %i.ahw = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.agi, <8 x float> nofpclass(nan inf) %i.ahv, <8 x float> nofpclass(nan inf) %.1441724500)
  ...
  %i.aie = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.agl, <8 x float> nofpclass(nan inf) %i.aid, <8 x float> nofpclass(nan inf) %.1441934499)
  ...
  %i.ajl = getelementptr inbounds nuw i8, ptr %.1416114503, i64 128
  %indvars.iv.next5470 = add nuw nsw i64 %indvars.iv5469, 1 ; 2 uses
  %exitcond5475.not = icmp eq i64 %indvars.iv.next5470, %wide.trip.count5474
  br i1 %exitcond5475.not, label %.loopexit4385.loopexit, label %bb.h, !llvm.loop !153

.loopexit4385.loopexit:                           ; preds = %bb.h
  %scevgep5471 = getelementptr i8, ptr %.916064515, i64 128
  %scevgep5472 = getelementptr i8, ptr %scevgep5471, i64 %i.gh
  br label %.loopexit4385

.loopexit4385:                                    ; preds = %.preheader4387, %.loopexit4388.loopexit, %bb.g, %.loopexit4386.loopexit, %.loopexit4385.loopexit, %.preheader4384, %.loopexit4386
  %.154194 = phi nsz <8 x float> [ %.941884511, %.loopexit4386 ], [ %.941884511, %.preheader4384 ], [ %i.ajk, %.loopexit4385.loopexit ], [ %.941884511, %bb.g ], [ %i.afq, %.loopexit4386.loopexit ], [ %i.adj, %.loopexit4388.loopexit ], [ %.941884511, %.preheader4387 ] ; 2 uses
  %.154173 = phi nsz <8 x float> [ %.941674512, %.loopexit4386 ], [ %.941674512, %.preheader4384 ], [ %i.ajc, %.loopexit4385.loopexit ], [ %.941674512, %bb.g ], [ %i.afn, %.loopexit4386.loopexit ], [ %i.adg, %.loopexit4388.loopexit ], [ %.941674512, %.preheader4387 ] ; 2 uses
  %.154150 = phi nsz <8 x float> [ %.941444513, %.loopexit4386 ], [ %.941444513, %.preheader4384 ], [ %i.aiu, %.loopexit4385.loopexit ], [ %.941444513, %bb.g ], [ %i.afk, %.loopexit4386.loopexit ], [ %i.add, %.loopexit4388.loopexit ], [ %.941444513, %.preheader4387 ] ; 2 uses
  %.164126 = phi nsz <8 x float> [ %.1041204514, %.loopexit4386 ], [ %.1041204514, %.preheader4384 ], [ %i.aim, %.loopexit4385.loopexit ], [ %.1041204514, %bb.g ], [ %i.afh, %.loopexit4386.loopexit ], [ %i.ada, %.loopexit4388.loopexit ], [ %.1041204514, %.preheader4387 ] ; 2 uses
  %.151612 = phi ptr [ %.916064515, %.loopexit4386 ], [ %.916064515, %.preheader4384 ], [ %scevgep5472, %.loopexit4385.loopexit ], [ %.916064515, %bb.g ], [ %scevgep5465, %.loopexit4386.loopexit ], [ %scevgep5458, %.loopexit4388.loopexit ], [ %.916064515, %.preheader4387 ] ; 2 uses
  %i.ajm = add nuw nsw i32 %.115934516, 8         ; 3 uses
  %i.ajn = or disjoint i32 %i.ajm, 7
  %i.ajo = icmp slt i32 %i.ajn, %i.db
  br i1 %i.ajo, label %_ZN4ncnn3MatD2Ev.exit2356, label %.preheader4397, !llvm.loop !154

.preheader4396:                                   ; preds = %.loopexit4381, %.preheader4397
  %.164195.lcssa = phi <8 x float> [ %.94188.lcssa, %.preheader4397 ], [ %.204199, %.loopexit4381 ]
  %.164174.lcssa = phi <8 x float> [ %.94167.lcssa, %.preheader4397 ], [ %.204178, %.loopexit4381 ]
  %.164151.lcssa = phi <8 x float> [ %.94144.lcssa, %.preheader4397 ], [ %.204155, %.loopexit4381 ] ; 3 uses
  %.174127.lcssa = phi <8 x float> [ %.104120.lcssa, %.preheader4397 ], [ %.214131, %.loopexit4381 ] ; 3 uses
  %.161613.lcssa = phi ptr [ %.91606.lcssa, %.preheader4397 ], [ %.201617, %.loopexit4381 ] ; 3 uses
  %.21594.lcssa = phi i32 [ %.11593.lcssa, %.preheader4397 ], [ %i.aoo, %.loopexit4381 ] ; 6 uses
  %i.ajp = or disjoint i32 %.21594.lcssa, 1
  %i.ajq = icmp slt i32 %i.ajp, %i.db
  br i1 %i.ajq, label %_ZN4ncnn3MatD2Ev.exit2354.lr.ph, label %.preheader4395

_ZN4ncnn3MatD2Ev.exit2354.lr.ph:                  ; preds = %.preheader4396
  ...
  %factor.op.mul4586 = mul i64 %i.ajt, %i.aju
  %i.ajv = sext i32 %i.ajr to i64
  %i.ajw = mul i64 %i.aju, %i.ajv
  %i.ajx = mul i64 %i.ajw, %i.eg
  %invariant.gep4588 = getelementptr i8, ptr %i.ajs, i64 %i.ajx
  %i.ajy = mul nsw i64 %indvars.iv5510, %i.eo
  %invariant.gep4589 = getelementptr [2 x i8], ptr %invariant.gep4588, i64 %i.ajy
...
_ZN4ncnn3MatD2Ev.exit2354.preheader:              ; preds = %_ZN4ncnn3MatD2Ev.exit2354.lr.ph
  ...
  %13 = add i32 %.21594.lcssa, 3
  %smax = call i32 @llvm.smax.i32(i32 %i.db, i32 %13)
  %14 = add i32 %smax, -2
  %i.akc = sub i32 %14, %.21594.lcssa
  %i.akd = and i32 %i.akc, -2
  %i.ake = add i32 %i.akb, %i.akd
  br label %.preheader4395
...
_ZN4ncnn3MatD2Ev.exit2354.us:                     ; preds = %_ZN4ncnn3MatD2Ev.exit2354.us.preheader, %._crit_edge4574.us
  %indvars.iv5497 = phi i64 [ %i.akf, %_ZN4ncnn3MatD2Ev.exit2354.us.preheader ], [ %indvars.iv.next5498, %._crit_edge4574.us ] ; 2 uses
  %.2116184580.us = phi ptr [ %.161613.lcssa, %_ZN4ncnn3MatD2Ev.exit2354.us.preheader ], [ %scevgep5493, %._crit_edge4574.us ] ; 2 uses
  %.2241324579.us = phi <8 x float> [ %.174127.lcssa, %_ZN4ncnn3MatD2Ev.exit2354.us.preheader ], [ %i.akw, %._crit_edge4574.us ]
  %.2141564578.us = phi <8 x float> [ %.164151.lcssa, %_ZN4ncnn3MatD2Ev.exit2354.us.preheader ], [ %i.ale, %._crit_edge4574.us ]
  %.reass4587.us = mul i64 %factor.op.mul4586, %indvars.iv5497
  %gep4590.us = getelementptr i8, ptr %invariant.gep4589, i64 %.reass4587.us ; 2 uses
  %i.akg = getelementptr [2 x i8], ptr %gep4590.us, i64 %i.aka
  br label %bb.i

bb.i:                                             ; preds = %_ZN4ncnn3MatD2Ev.exit2354.us, %bb.i
  %indvars.iv5490 = phi i64 [ 0, %_ZN4ncnn3MatD2Ev.exit2354.us ], [ %indvars.iv.next5491, %bb.i ] ; 2 uses
  %.2216194571.us = phi ptr [ %.2116184580.us, %_ZN4ncnn3MatD2Ev.exit2354.us ], [ %i.alf, %bb.i ] ; 3 uses
  %.2341334570.us = phi <8 x float> [ %.2241324579.us, %_ZN4ncnn3MatD2Ev.exit2354.us ], [ %i.akw, %bb.i ]
  %.2241574569.us = phi <8 x float> [ %.2141564578.us, %_ZN4ncnn3MatD2Ev.exit2354.us ], [ %i.ale, %bb.i ]
  %i.akh = getelementptr inbounds nuw [4 x i8], ptr %i.ajz, i64 %indvars.iv5490
  %i.aki = load i32, ptr %i.akh, align 4, !tbaa !113
  %i.akj = load <8 x bfloat>, ptr %.2216194571.us, align 16, !tbaa !20
  %i.akk = fpext fast <8 x bfloat> %i.akj to <8 x float>
  %i.akl = getelementptr inbounds nuw i8, ptr %.2216194571.us, i64 16
  ...
  %i.akp = getelementptr inbounds [2 x i8], ptr %gep4590.us, i64 %i.ako
  ...
  %i.akw = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.akk, <8 x float> nofpclass(nan inf) %i.akv, <8 x float> nofpclass(nan inf) %.2341334570.us) ; 3 uses
  ...
  %i.ale = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.akn, <8 x float> nofpclass(nan inf) %i.ald, <8 x float> nofpclass(nan inf) %.2241574569.us) ; 3 uses
  %i.alf = getelementptr inbounds nuw i8, ptr %.2216194571.us, i64 32
  %indvars.iv.next5491 = add nuw nsw i64 %indvars.iv5490, 1 ; 2 uses
  %exitcond5496.not = icmp eq i64 %indvars.iv.next5491, %wide.trip.count5495
  br i1 %exitcond5496.not, label %._crit_edge4574.us, label %bb.i, !llvm.loop !158

._crit_edge4574.us:                               ; preds = %bb.i
  %scevgep5492 = getelementptr i8, ptr %.2116184580.us, i64 32
  %scevgep5493 = getelementptr i8, ptr %scevgep5492, i64 %i.em ; 2 uses
  %indvars.iv.next5498 = add nuw nsw i64 %indvars.iv5497, 2 ; 3 uses
  %i.alg = trunc i64 %indvars.iv.next5498 to i32
  %i.alh = or i32 %i.alg, 1
  %i.ali = icmp slt i32 %i.alh, %i.db
  br i1 %i.ali, label %_ZN4ncnn3MatD2Ev.exit2354.us, label %.preheader4395.loopexit, !llvm.loop !159

_ZN4ncnn3MatD2Ev.exit2355:                        ; preds = %_ZN4ncnn3MatD2Ev.exit2355.lr.ph, %.loopexit4381
  %.215944557 = phi i32 [ %.11593.lcssa, %_ZN4ncnn3MatD2Ev.exit2355.lr.ph ], [ %i.aoo, %.loopexit4381 ] ; 2 uses
  %.1616134556 = phi ptr [ %.91606.lcssa, %_ZN4ncnn3MatD2Ev.exit2355.lr.ph ], [ %.201617, %.loopexit4381 ] ; 7 uses
  %.1741274555 = phi <8 x float> [ %.104120.lcssa, %_ZN4ncnn3MatD2Ev.exit2355.lr.ph ], [ %.214131, %.loopexit4381 ] ; 5 uses
  %.1641514554 = phi <8 x float> [ %.94144.lcssa, %_ZN4ncnn3MatD2Ev.exit2355.lr.ph ], [ %.204155, %.loopexit4381 ] ; 5 uses
  %.1641744553 = phi <8 x float> [ %.94167.lcssa, %_ZN4ncnn3MatD2Ev.exit2355.lr.ph ], [ %.204178, %.loopexit4381 ] ; 5 uses
  %.1641954552 = phi <8 x float> [ %.94188.lcssa, %_ZN4ncnn3MatD2Ev.exit2355.lr.ph ], [ %.204199, %.loopexit4381 ] ; 5 uses
  %i.alj = sdiv i32 %.215944557, %i.cz
  %i.alk = sext i32 %i.alj to i64
  %.reass4565 = mul i64 %factor.op.mul4564, %i.alk
  %gep4568 = getelementptr i8, ptr %invariant.gep4567, i64 %.reass4565 ; 5 uses
  br i1 %i.ds, label %.preheader4382, label %.loopexit4383

.preheader4382:                                   ; preds = %_ZN4ncnn3MatD2Ev.exit2355
  br i1 %i.aba, label %.lr.ph4534, label %.loopexit4381

.lr.ph4534:                                       ; preds = %.preheader4382, %.lr.ph4534
  %indvars.iv5476 = phi i64 [ %indvars.iv.next5477, %.lr.ph4534 ], [ 0, %.preheader4382 ] ; 2 uses
  %.1716144532 = phi ptr [ %i.amp, %.lr.ph4534 ], [ %.1616134556, %.preheader4382 ] ; 5 uses
  %.1841284531 = phi <8 x float> [ %i.amf, %.lr.ph4534 ], [ %.1741274555, %.preheader4382 ]
  %.1741524530 = phi <8 x float> [ %i.ami, %.lr.ph4534 ], [ %.1641514554, %.preheader4382 ]
  %.1741754529 = phi <8 x float> [ %i.aml, %.lr.ph4534 ], [ %.1641744553, %.preheader4382 ]
  %.1741964528 = phi <8 x float> [ %i.amo, %.lr.ph4534 ], [ %.1641954552, %.preheader4382 ]
  %i.all = getelementptr inbounds nuw [4 x i8], ptr %i.abb, i64 %indvars.iv5476
  %i.alm = load i32, ptr %i.all, align 4, !tbaa !113
  %i.aln = sext i32 %i.alm to i64
  %i.alo = getelementptr inbounds [2 x i8], ptr %gep4568, i64 %i.aln
  %i.alp = load <8 x bfloat>, ptr %.1716144532, align 16, !tbaa !20
  %i.alq = fpext fast <8 x bfloat> %i.alp to <8 x float>
  %i.alr = getelementptr inbounds nuw i8, ptr %.1716144532, i64 16
  %i.als = load <8 x bfloat>, ptr %i.alr, align 16, !tbaa !20
  %i.alt = fpext fast <8 x bfloat> %i.als to <8 x float>
  %i.alu = getelementptr inbounds nuw i8, ptr %.1716144532, i64 32
  %i.alv = load <8 x bfloat>, ptr %i.alu, align 16, !tbaa !20
  %i.alw = fpext fast <8 x bfloat> %i.alv to <8 x float>
  %i.alx = getelementptr inbounds nuw i8, ptr %.1716144532, i64 48
  ...
  %i.amf = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.alq, <8 x float> nofpclass(nan inf) %i.ame, <8 x float> nofpclass(nan inf) %.1841284531) ; 2 uses
  ...
  %i.ami = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.alt, <8 x float> nofpclass(nan inf) %i.amh, <8 x float> nofpclass(nan inf) %.1741524530) ; 2 uses
  ...
  %i.aml = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.alw, <8 x float> nofpclass(nan inf) %i.amk, <8 x float> nofpclass(nan inf) %.1741754529) ; 2 uses
  ...
  %i.amo = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.alz, <8 x float> nofpclass(nan inf) %i.amn, <8 x float> nofpclass(nan inf) %.1741964528) ; 2 uses
  %i.amp = getelementptr inbounds nuw i8, ptr %.1716144532, i64 64
  %indvars.iv.next5477 = add nuw nsw i64 %indvars.iv5476, 1 ; 2 uses
  %exitcond5482.not = icmp eq i64 %indvars.iv.next5477, %wide.trip.count5481
  br i1 %exitcond5482.not, label %.loopexit4383.loopexit, label %.lr.ph4534, !llvm.loop !160

.loopexit4383.loopexit:                           ; preds = %.lr.ph4534
  %scevgep5478 = getelementptr i8, ptr %.1616134556, i64 64
  %scevgep5479 = getelementptr i8, ptr %scevgep5478, i64 %i.abf
  br label %.loopexit4381

.loopexit4383:                                    ; preds = %_ZN4ncnn3MatD2Ev.exit2355
  br i1 %i.dt, label %.preheader4380, label %.loopexit4381

.preheader4380:                                   ; preds = %.loopexit4383
  br i1 %i.aba, label %.lr.ph4546, label %.loopexit4381

.lr.ph4546:                                       ; preds = %.preheader4380
  %i.amq = getelementptr [2 x i8], ptr %gep4568, i64 %i.abc
  %i.amr = getelementptr i8, ptr %gep4568, i64 %.idx2187
  %i.ams = getelementptr i8, ptr %gep4568, i64 %.idx2188
  br label %bb.j

bb.j:                                             ; preds = %.lr.ph4546, %bb.j
  %indvars.iv5483 = phi i64 [ 0, %.lr.ph4546 ], [ %indvars.iv.next5484, %bb.j ] ; 2 uses
  %.1916164544 = phi ptr [ %.1616134556, %.lr.ph4546 ], [ %i.aon, %bb.j ] ; 5 uses
  %.2041304543 = phi <8 x float> [ %.1741274555, %.lr.ph4546 ], [ %i.ano, %bb.j ]
  %.1941544542 = phi <8 x float> [ %.1641514554, %.lr.ph4546 ], [ %i.anw, %bb.j ]
  %.1941774541 = phi <8 x float> [ %.1641744553, %.lr.ph4546 ], [ %i.aoe, %bb.j ]
  %.1941984540 = phi <8 x float> [ %.1641954552, %.lr.ph4546 ], [ %i.aom, %bb.j ]
  %i.amt = getelementptr inbounds nuw [4 x i8], ptr %i.abb, i64 %indvars.iv5483
  %i.amu = load i32, ptr %i.amt, align 4, !tbaa !113
  %i.amv = load <8 x bfloat>, ptr %.1916164544, align 16, !tbaa !20
  %i.amw = fpext fast <8 x bfloat> %i.amv to <8 x float>
  %i.amx = getelementptr inbounds nuw i8, ptr %.1916164544, i64 16
  %i.amy = load <8 x bfloat>, ptr %i.amx, align 16, !tbaa !20
  %i.amz = fpext fast <8 x bfloat> %i.amy to <8 x float>
  %i.ana = getelementptr inbounds nuw i8, ptr %.1916164544, i64 32
  %i.anb = load <8 x bfloat>, ptr %i.ana, align 16, !tbaa !20
  %i.anc = fpext fast <8 x bfloat> %i.anb to <8 x float>
  %i.and = getelementptr inbounds nuw i8, ptr %.1916164544, i64 48
  ...
  %i.anh = getelementptr inbounds [2 x i8], ptr %gep4568, i64 %i.ang
  ...
  %i.ano = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.amw, <8 x float> nofpclass(nan inf) %i.ann, <8 x float> nofpclass(nan inf) %.2041304543) ; 2 uses
  ...
  %i.anw = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.amz, <8 x float> nofpclass(nan inf) %i.anv, <8 x float> nofpclass(nan inf) %.1941544542) ; 2 uses
  ...
  %i.aoe = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.anc, <8 x float> nofpclass(nan inf) %i.aod, <8 x float> nofpclass(nan inf) %.1941774541) ; 2 uses
  ...
  %i.aom = call fast noundef nofpclass(nan inf) <8 x float> @llvm.fma.v8f32(<8 x float> nofpclass(nan inf) %i.anf, <8 x float> nofpclass(nan inf) %i.aol, <8 x float> nofpclass(nan inf) %.1941984540) ; 2 uses
  %i.aon = getelementptr inbounds nuw i8, ptr %.1916164544, i64 64
  %indvars.iv.next5484 = add nuw nsw i64 %indvars.iv5483, 1 ; 2 uses
  %exitcond5489.not = icmp eq i64 %indvars.iv.next5484, %wide.trip.count5488
  br i1 %exitcond5489.not, label %.loopexit4381.loopexit, label %bb.j, !llvm.loop !161

.loopexit4381.loopexit:                           ; preds = %bb.j
  %scevgep5485 = getelementptr i8, ptr %.1616134556, i64 64
  %scevgep5486 = getelementptr i8, ptr %scevgep5485, i64 %i.abf
  br label %.loopexit4381

.loopexit4381:                                    ; preds = %.preheader4382, %.loopexit4383.loopexit, %.loopexit4381.loopexit, %.preheader4380, %.loopexit4383
end_hunk_1
begin_hunk_2_@_ZN4ncnnL24convolution_packed_bf16sERKNS_3MatERS0_S2_S2_iiiiiiiS2_RKNS_6OptionE:bb.a
  ...
  %.neg4326 = fmul fast <8 x float> %i.aut, splat (float -2.000000e+00)
  %i.auu = select fast <8 x i1> %i.atp, <8 x float> splat (float +nan(0x3FFFFF)), <8 x float> %.neg4326
...
.noexc2556:                                       ; preds = %._crit_edge4604
...
_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit:      ; preds = %.noexc2556, %.noexc2555, %.noexc2554, %bb.l, %.noexc2553, %bb.k, %._crit_edge4604
  %.0.i = phi nsz <8 x float> [ %i.awh, %.noexc2556 ], [ %i.aqy, %bb.k ], [ %i.arg, %.noexc2553 ], [ %i.arq, %bb.l ], [ %i.asq, %.noexc2554 ], [ %i.avv, %.noexc2555 ], [ %i.aqx, %._crit_edge4604 ] ; 4 uses
...
.thread:                                          ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit
  ...
  store <8 x bfloat> %i.awi, ptr %.116254612, align 1, !tbaa !20
  %i.awj = getelementptr inbounds nuw i8, ptr %.116254612, i64 16
  br label %.thread4292
...
bb.m:                                             ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit
  ...
  store i64 %i.awn, ptr %.116254612, align 1, !tbaa !20
  %i.awo = load i64, ptr %i.e, align 8, !tbaa !117
  %i.awp = getelementptr inbounds nuw [2 x i8], ptr %.116254612, i64 %i.awo
  ...
  %i.awu = getelementptr inbounds nuw i8, ptr %.116254612, i64 8
  br label %.thread4292
...
bb.n:                                             ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit
  ...
  store i16 %i.aww, ptr %.116254612, align 2, !tbaa !76
  %i.awx = extractelement <16 x i16> %i.awv, i64 3
  %i.awy = load i64, ptr %i.e, align 8, !tbaa !117 ; 7 uses
  %i.awz = getelementptr inbounds nuw [2 x i8], ptr %.116254612, i64 %i.awy
  store i16 %i.awx, ptr %i.awz, align 2, !tbaa !76
  %i.axa = extractelement <16 x i16> %i.awv, i64 5
  %.idx2174 = shl i64 %i.awy, 2
  %i.axb = getelementptr inbounds nuw i8, ptr %.116254612, i64 %.idx2174
  store i16 %i.axa, ptr %i.axb, align 2, !tbaa !76
  %i.axc = extractelement <16 x i16> %i.awv, i64 7
  %.idx2175 = mul i64 %i.awy, 6
  %i.axd = getelementptr inbounds nuw i8, ptr %.116254612, i64 %.idx2175
  store i16 %i.axc, ptr %i.axd, align 2, !tbaa !76
  %i.axe = extractelement <16 x i16> %i.awv, i64 9
  %.idx2176 = shl i64 %i.awy, 3
  %i.axf = getelementptr inbounds nuw i8, ptr %.116254612, i64 %.idx2176
  store i16 %i.axe, ptr %i.axf, align 2, !tbaa !76
  %i.axg = extractelement <16 x i16> %i.awv, i64 11
  %.idx2177 = mul i64 %i.awy, 10
  %i.axh = getelementptr inbounds nuw i8, ptr %.116254612, i64 %.idx2177
  store i16 %i.axg, ptr %i.axh, align 2, !tbaa !76
  %i.axi = extractelement <16 x i16> %i.awv, i64 13
  %.idx2178 = mul i64 %i.awy, 12
  %i.axj = getelementptr inbounds nuw i8, ptr %.116254612, i64 %.idx2178
  store i16 %i.axi, ptr %i.axj, align 2, !tbaa !76
  %i.axk = extractelement <16 x i16> %i.awv, i64 15
  %.idx2179 = mul i64 %i.awy, 14
  %i.axl = getelementptr inbounds nuw i8, ptr %.116254612, i64 %.idx2179
  store i16 %i.axk, ptr %i.axl, align 2, !tbaa !76
  %i.axm = getelementptr inbounds nuw i8, ptr %.116254612, i64 2
  br label %.thread4292

.thread4292:                                      ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit, %bb.m, %.thread, %bb.n
  %.41628 = phi ptr [ %i.axm, %bb.n ], [ %.116254612, %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit ], [ %i.awj, %.thread ], [ %i.awu, %bb.m ] ; 2 uses
  %indvars.iv.next5511 = add nuw nsw i64 %indvars.iv5510, 1 ; 2 uses
  %exitcond5514.not = icmp eq i64 %indvars.iv.next5511, %wide.trip.count5513
  br i1 %exitcond5514.not, label %._crit_edge4615, label %bb.b, !llvm.loop !168

._crit_edge4850:                                  ; preds = %._crit_edge4848.split, %._crit_edge4627
  ...
  br i1 %i.axr, label %.noexc2326.lr.ph, label %._crit_edge5036.split

.noexc2326.lr.ph:                                 ; preds = %._crit_edge4850
  ...
  %factor.op.mul5038 = mul i64 %i.axy, %i.aya     ; 2 uses
  ...
  br i1 %brmerge, label %._crit_edge5036.split, label %.noexc2326.lr.ph.split.split
...
.noexc2326.lr.ph.split.split:                     ; preds = %.noexc2326.lr.ph
  ...
  %factor.op.mul5044 = mul i64 %i.ayo, %i.ayp
  %i.ayq = and i32 %i.axu, -16
  %i.ayr = sext i32 %i.axo to i64
  %wide.trip.count5709 = zext nneg i32 %i.axq to i64
  %wide.trip.count5703 = zext nneg i32 %i.axv to i64
  br label %.noexc2326

.noexc2328:                                       ; preds = %.noexc2328.lr.ph, %._crit_edge4848.split
  %indvars.iv5611 = phi i64 [ 0, %.noexc2328.lr.ph ], [ %indvars.iv.next5612, %._crit_edge4848.split ] ; 2 uses
  ...
  br i1 %i.ayy, label %.preheader4379.lr.ph, label %._crit_edge4848.split

.preheader4379.lr.ph:                             ; preds = %.noexc2328
  %i.ayz = shl nuw nsw i64 %indvars.iv5611, 2
  ...
  br i1 %i.azc, label %.preheader4379.preheader, label %._crit_edge4848.split

.preheader4379.preheader:                         ; preds = %.preheader4379.lr.ph
  ...
  %wide.trip.count5608 = zext nneg i32 %i.ayv to i64
  br label %.preheader4379

.preheader4379:                                   ; preds = %.preheader4379.preheader, %._crit_edge4838
  %.015754847 = phi i32 [ %i.bap, %._crit_edge4838 ], [ 0, %.preheader4379.preheader ] ; 2 uses
  %.015764846 = phi ptr [ %.31579, %._crit_edge4838 ], [ %i.bab, %.preheader4379.preheader ]
  %i.bad = load i32, ptr %i.b, align 4
  %i.bae = mul nsw i32 %i.bad, %.015754847
  %i.baf = sext i32 %i.bae to i64                 ; 5 uses
  %i.bag = load i32, ptr %i.a, align 4            ; 2 uses
  %factor.op.mul4840 = mul i32 %i.ays, %i.bag     ; 3 uses
  ...
  %wide.trip.count5590 = zext i32 %i.bah to i64   ; 3 uses
  %xtraiter6642 = and i64 %wide.trip.count5590, 1
  %i.bao = icmp eq i32 %i.bah, 1
  %unroll_iter6646 = and i64 %wide.trip.count5590, 2147483646
  %lcmp.mod6643.not = icmp eq i64 %xtraiter6642, 0
  %lcmp.mod6645 = trunc i32 %i.bah to i1
  br label %bb.o

._crit_edge4848.split:                            ; preds = %._crit_edge4838, %.preheader4379.lr.ph, %.noexc2328
  %indvars.iv.next5612 = add nuw nsw i64 %indvars.iv5611, 1 ; 2 uses
  %exitcond5615.not = icmp eq i64 %indvars.iv.next5612, %wide.trip.count5614
  br i1 %exitcond5615.not, label %._crit_edge4850, label %.noexc2328, !llvm.loop !178

._crit_edge4838:                                  ; preds = %bb.ab
  %i.bap = add nuw nsw i32 %.015754847, 1         ; 2 uses
  %exitcond5610.not = icmp eq i32 %i.bap, %i.ayw
  br i1 %exitcond5610.not, label %._crit_edge4848.split, label %.preheader4379, !llvm.loop !179

bb.o:                                             ; preds = %.preheader4379, %bb.ab
  %indvars.iv5605 = phi i64 [ 0, %.preheader4379 ], [ %indvars.iv.next5606, %bb.ab ] ; 6 uses
  %.115774835 = phi ptr [ %.015764846, %.preheader4379 ], [ %.31579, %bb.ab ] ; 8 uses
...
_ZN4ncnn3MatD2Ev.exit2351:                        ; preds = %bb.p, %bb.o
  ...
  br i1 %i.azo, label %_ZN4ncnn3MatD2Ev.exit2350.lr.ph, label %.preheader4378
...
_ZN4ncnn3MatD2Ev.exit2350.lr.ph:                  ; preds = %_ZN4ncnn3MatD2Ev.exit2351
  ...
  %factor.op.mul4691 = mul i64 %i.bbb, %i.bbc
  %i.bbd = sext i32 %i.baz to i64
  %i.bbe = mul i64 %i.bbc, %i.bbd
  %i.bbf = mul i64 %i.bbe, %i.baf
  %invariant.gep4693 = getelementptr i8, ptr %i.bba, i64 %i.bbf
  %i.bbg = trunc nuw nsw i64 %indvars.iv5605 to i32
  %.reass4841 = mul i32 %factor.op.mul4840, %i.bbg
  %i.bbh = sext i32 %.reass4841 to i64
  %invariant.gep4694 = getelementptr [2 x i8], ptr %invariant.gep4693, i64 %i.bbh
  ...
  %wide.trip.count5547 = zext nneg i32 %i.bbi to i64
  %wide.trip.count5540 = zext nneg i32 %i.bbi to i64
  %wide.trip.count5533 = zext nneg i32 %i.bbi to i64
  %wide.trip.count5526 = zext nneg i32 %i.bbi to i64
  br label %_ZN4ncnn3MatD2Ev.exit2350

.preheader4378:                                   ; preds = %.loopexit4370, %_ZN4ncnn3MatD2Ev.exit2351
  %.14266.lcssa = phi <4 x float> [ %.04265, %_ZN4ncnn3MatD2Ev.exit2351 ], [ %.94274, %.loopexit4370 ] ; 2 uses
  %.04242.lcssa = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2351 ], [ %.84250, %.loopexit4370 ] ; 2 uses
  %.04221.lcssa = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2351 ], [ %.84229, %.loopexit4370 ] ; 2 uses
  %.04200.lcssa = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2351 ], [ %.84208, %.loopexit4370 ] ; 2 uses
  %.01549.lcssa = phi ptr [ %i.bay, %_ZN4ncnn3MatD2Ev.exit2351 ], [ %.81557, %.loopexit4370 ] ; 2 uses
  %.01544.lcssa = phi i32 [ 0, %_ZN4ncnn3MatD2Ev.exit2351 ], [ %i.bac, %.loopexit4370 ] ; 3 uses
  %i.bbp = or disjoint i32 %.01544.lcssa, 7
  %i.bbq = icmp slt i32 %i.bbp, %i.ayu
  br i1 %i.bbq, label %_ZN4ncnn3MatD2Ev.exit2349.lr.ph, label %.preheader4377

_ZN4ncnn3MatD2Ev.exit2349.lr.ph:                  ; preds = %.preheader4378
  ...
  %factor.op.mul4745 = mul i64 %i.bbt, %i.bbu
  %i.bbv = sext i32 %i.bbr to i64
  %i.bbw = mul i64 %i.bbu, %i.bbv
  %i.bbx = mul i64 %i.bbw, %i.baf
  %invariant.gep4747 = getelementptr i8, ptr %i.bbs, i64 %i.bbx
  %i.bby = trunc nuw nsw i64 %indvars.iv5605 to i32
  %.reass4843 = mul i32 %factor.op.mul4840, %i.bby
  %i.bbz = sext i32 %.reass4843 to i64
  %invariant.gep4748 = getelementptr [2 x i8], ptr %invariant.gep4747, i64 %i.bbz
  ...
  %wide.trip.count5568 = zext nneg i32 %i.bca to i64
  %wide.trip.count5561 = zext nneg i32 %i.bca to i64
  %wide.trip.count5554 = zext nneg i32 %i.bca to i64
  br label %_ZN4ncnn3MatD2Ev.exit2349

_ZN4ncnn3MatD2Ev.exit2350:                        ; preds = %_ZN4ncnn3MatD2Ev.exit2350.lr.ph, %.loopexit4370
  %.015444684 = phi i32 [ 0, %_ZN4ncnn3MatD2Ev.exit2350.lr.ph ], [ %i.cdu, %.loopexit4370 ] ; 2 uses
  %.015494683 = phi ptr [ %i.bay, %_ZN4ncnn3MatD2Ev.exit2350.lr.ph ], [ %.81557, %.loopexit4370 ] ; 13 uses
  %.042004682 = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2350.lr.ph ], [ %.84208, %.loopexit4370 ] ; 9 uses
  %.042214681 = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2350.lr.ph ], [ %.84229, %.loopexit4370 ] ; 9 uses
  %.042424680 = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2350.lr.ph ], [ %.84250, %.loopexit4370 ] ; 9 uses
  %.142664679 = phi <4 x float> [ %.04265, %_ZN4ncnn3MatD2Ev.exit2350.lr.ph ], [ %.94274, %.loopexit4370 ] ; 9 uses
  %i.bch = sdiv i32 %.015444684, %i.ays
  %i.bci = sext i32 %i.bch to i64
  %.reass4692 = mul i64 %factor.op.mul4691, %i.bci
  %gep4695 = getelementptr i8, ptr %invariant.gep4694, i64 %.reass4692 ; 23 uses
  br i1 %i.azp, label %.preheader4373, label %.loopexit4374

.preheader4373:                                   ; preds = %_ZN4ncnn3MatD2Ev.exit2350
  br i1 %i.bbj, label %.lr.ph4635, label %.loopexit4370

.lr.ph4635:                                       ; preds = %.preheader4373, %.lr.ph4635
  %indvars.iv5521 = phi i64 [ %indvars.iv.next5522, %.lr.ph4635 ], [ 0, %.preheader4373 ] ; 2 uses
  %.115504633 = phi ptr [ %i.bid, %.lr.ph4635 ], [ %.015494683, %.preheader4373 ] ; 17 uses
  %.142014632 = phi <4 x float> [ %i.bic, %.lr.ph4635 ], [ %.042004682, %.preheader4373 ]
  %.142224631 = phi <4 x float> [ %i.bhz, %.lr.ph4635 ], [ %.042214681, %.preheader4373 ]
  %.142434630 = phi <4 x float> [ %i.bhw, %.lr.ph4635 ], [ %.042424680, %.preheader4373 ]
  %.242674629 = phi <4 x float> [ %i.bht, %.lr.ph4635 ], [ %.142664679, %.preheader4373 ]
  %i.bcj = getelementptr inbounds nuw [4 x i8], ptr %i.bbk, i64 %indvars.iv5521
  %i.bck = load i32, ptr %i.bcj, align 4, !tbaa !113
  %i.bcl = sext i32 %i.bck to i64
  %i.bcm = getelementptr inbounds [2 x i8], ptr %gep4695, i64 %i.bcl
  %i.bcn = load i64, ptr %.115504633, align 1, !tbaa !20
  ...
  %i.bcs = getelementptr inbounds nuw i8, ptr %.115504633, i64 8
  ...
  %i.bcy = getelementptr inbounds nuw i8, ptr %.115504633, i64 16
  ...
  %i.bde = getelementptr inbounds nuw i8, ptr %.115504633, i64 24
  ...
  %i.bdk = getelementptr inbounds nuw i8, ptr %.115504633, i64 32
  ...
  %i.bdq = getelementptr inbounds nuw i8, ptr %.115504633, i64 40
...
begin_hunk_3_@_ZN4ncnnL24convolution_packed_bf16sERKNS_3MatERS0_S2_S2_iiiiiiiS2_RKNS_6OptionE:bb.a
  ...
  %i.cka = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.cic, <4 x float> nofpclass(nan inf) %i.cjz, <4 x float> nofpclass(nan inf) %.1342784708)
  ...
  %i.ckd = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.cii, <4 x float> nofpclass(nan inf) %i.ckc, <4 x float> nofpclass(nan inf) %.1242544709)
  ...
  %i.ckg = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.cio, <4 x float> nofpclass(nan inf) %i.ckf, <4 x float> nofpclass(nan inf) %.1242334710)
  ...
  %i.ckj = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.ciu, <4 x float> nofpclass(nan inf) %i.cki, <4 x float> nofpclass(nan inf) %.1242124711)
  ...
  %i.ckw = getelementptr inbounds nuw i8, ptr %.1215614712, i64 64
  %indvars.iv.next5557 = add nuw nsw i64 %indvars.iv5556, 1 ; 2 uses
  %exitcond5562.not = icmp eq i64 %indvars.iv.next5557, %wide.trip.count5561
  br i1 %exitcond5562.not, label %.loopexit4366.loopexit, label %.lr.ph4715, !llvm.loop !198

.loopexit4366.loopexit:                           ; preds = %.lr.ph4715
  %scevgep5558 = getelementptr i8, ptr %.915584737, i64 64
  %scevgep5559 = getelementptr i8, ptr %scevgep5558, i64 %i.bcg
  br label %.loopexit4365

.loopexit4366:                                    ; preds = %.loopexit4368
  br i1 %i.azs, label %.preheader4364, label %.loopexit4365

.preheader4364:                                   ; preds = %.loopexit4366
  br i1 %i.bcb, label %.lr.ph4727, label %.loopexit4365

.lr.ph4727:                                       ; preds = %.preheader4364
  %i.ckx = getelementptr [2 x i8], ptr %gep4749, i64 %i.bcd
  %i.cky = getelementptr i8, ptr %gep4749, i64 %.idx2066
  %i.ckz = getelementptr i8, ptr %gep4749, i64 %.idx2067
  %i.cla = getelementptr i8, ptr %gep4749, i64 %.idx2068
  %i.clb = getelementptr i8, ptr %gep4749, i64 %.idx2069
  %i.clc = getelementptr i8, ptr %gep4749, i64 %.idx2070
  %i.cld = getelementptr i8, ptr %gep4749, i64 %.idx2071
  br label %bb.u

bb.u:                                             ; preds = %.lr.ph4727, %bb.u
  %indvars.iv5563 = phi i64 [ 0, %.lr.ph4727 ], [ %indvars.iv.next5564, %bb.u ] ; 2 uses
  %.1415634725 = phi ptr [ %.915584737, %.lr.ph4727 ], [ %i.cpo, %bb.u ] ; 9 uses
  %.1442144724 = phi <4 x float> [ %.942094736, %.lr.ph4727 ], [ %i.cpn, %bb.u ]
  %.1442354723 = phi <4 x float> [ %.942304735, %.lr.ph4727 ], [ %i.cpf, %bb.u ]
  %.1442564722 = phi <4 x float> [ %.942514734, %.lr.ph4727 ], [ %i.cox, %bb.u ]
  %.1542804721 = phi <4 x float> [ %.1042754733, %.lr.ph4727 ], [ %i.cop, %bb.u ]
  %i.cle = getelementptr inbounds nuw [4 x i8], ptr %i.bcc, i64 %indvars.iv5563
  %i.clf = load i32, ptr %i.cle, align 4, !tbaa !113
  %i.clg = load i64, ptr %.1415634725, align 1, !tbaa !20
  ...
  %i.cll = getelementptr inbounds nuw i8, ptr %.1415634725, i64 8
  ...
  %i.clr = getelementptr inbounds nuw i8, ptr %.1415634725, i64 16
  ...
  %i.clx = getelementptr inbounds nuw i8, ptr %.1415634725, i64 24
  ...
  %i.cmd = getelementptr inbounds nuw i8, ptr %.1415634725, i64 32
  ...
  %i.cmj = getelementptr inbounds nuw i8, ptr %.1415634725, i64 40
  ...
  %i.cmp = getelementptr inbounds nuw i8, ptr %.1415634725, i64 48
  ...
  %i.cmv = getelementptr inbounds nuw i8, ptr %.1415634725, i64 56
  ...
  %i.cnc = getelementptr inbounds [2 x i8], ptr %gep4749, i64 %i.cnb
  ...
  %i.cnj = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.clk, <4 x float> nofpclass(nan inf) %i.cni, <4 x float> nofpclass(nan inf) %.1542804721)
  ...
  %i.cnr = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.clq, <4 x float> nofpclass(nan inf) %i.cnq, <4 x float> nofpclass(nan inf) %.1442564722)
  ...
  %i.cnz = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.clw, <4 x float> nofpclass(nan inf) %i.cny, <4 x float> nofpclass(nan inf) %.1442354723)
  ...
  %i.coh = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.cmc, <4 x float> nofpclass(nan inf) %i.cog, <4 x float> nofpclass(nan inf) %.1442144724)
  ...
  %i.cpo = getelementptr inbounds nuw i8, ptr %.1415634725, i64 64
  %indvars.iv.next5564 = add nuw nsw i64 %indvars.iv5563, 1 ; 2 uses
  %exitcond5569.not = icmp eq i64 %indvars.iv.next5564, %wide.trip.count5568
  br i1 %exitcond5569.not, label %.loopexit4365.loopexit, label %bb.u, !llvm.loop !199

.loopexit4365.loopexit:                           ; preds = %bb.u
  %scevgep5565 = getelementptr i8, ptr %.915584737, i64 64
  %scevgep5566 = getelementptr i8, ptr %scevgep5565, i64 %i.bcg
  br label %.loopexit4365

.loopexit4365:                                    ; preds = %.preheader4367, %.loopexit4368.loopexit, %bb.t, %.loopexit4366.loopexit, %.loopexit4365.loopexit, %.preheader4364, %.loopexit4366
  %.164281 = phi nsz <4 x float> [ %.1042754733, %.loopexit4366 ], [ %.1042754733, %.preheader4364 ], [ %i.cop, %.loopexit4365.loopexit ], [ %.1042754733, %bb.t ], [ %i.ckm, %.loopexit4366.loopexit ], [ %i.chh, %.loopexit4368.loopexit ], [ %.1042754733, %.preheader4367 ] ; 2 uses
  %.154257 = phi nsz <4 x float> [ %.942514734, %.loopexit4366 ], [ %.942514734, %.preheader4364 ], [ %i.cox, %.loopexit4365.loopexit ], [ %.942514734, %bb.t ], [ %i.ckp, %.loopexit4366.loopexit ], [ %i.chk, %.loopexit4368.loopexit ], [ %.942514734, %.preheader4367 ] ; 2 uses
  %.154236 = phi nsz <4 x float> [ %.942304735, %.loopexit4366 ], [ %.942304735, %.preheader4364 ], [ %i.cpf, %.loopexit4365.loopexit ], [ %.942304735, %bb.t ], [ %i.cks, %.loopexit4366.loopexit ], [ %i.chn, %.loopexit4368.loopexit ], [ %.942304735, %.preheader4367 ] ; 2 uses
  %.154215 = phi nsz <4 x float> [ %.942094736, %.loopexit4366 ], [ %.942094736, %.preheader4364 ], [ %i.cpn, %.loopexit4365.loopexit ], [ %.942094736, %bb.t ], [ %i.ckv, %.loopexit4366.loopexit ], [ %i.chq, %.loopexit4368.loopexit ], [ %.942094736, %.preheader4367 ] ; 2 uses
  %.151564 = phi ptr [ %.915584737, %.loopexit4366 ], [ %.915584737, %.preheader4364 ], [ %scevgep5566, %.loopexit4365.loopexit ], [ %.915584737, %bb.t ], [ %scevgep5559, %.loopexit4366.loopexit ], [ %scevgep5552, %.loopexit4368.loopexit ], [ %.915584737, %.preheader4367 ] ; 2 uses
  %i.cpp = add nuw nsw i32 %.115454738, 8         ; 3 uses
  %i.cpq = or disjoint i32 %i.cpp, 7
  %i.cpr = icmp slt i32 %i.cpq, %i.ayu
  br i1 %i.cpr, label %_ZN4ncnn3MatD2Ev.exit2349, label %.preheader4377, !llvm.loop !200

.preheader4376:                                   ; preds = %.loopexit4361, %.preheader4377
  %.174282.lcssa = phi <4 x float> [ %.104275.lcssa, %.preheader4377 ], [ %.214286, %.loopexit4361 ] ; 3 uses
  %.164258.lcssa = phi <4 x float> [ %.94251.lcssa, %.preheader4377 ], [ %.204262, %.loopexit4361 ] ; 3 uses
  %.164237.lcssa = phi <4 x float> [ %.94230.lcssa, %.preheader4377 ], [ %.204241, %.loopexit4361 ]
  %.164216.lcssa = phi <4 x float> [ %.94209.lcssa, %.preheader4377 ], [ %.204220, %.loopexit4361 ]
  %.161565.lcssa = phi ptr [ %.91558.lcssa, %.preheader4377 ], [ %.201569, %.loopexit4361 ] ; 3 uses
  %.21546.lcssa = phi i32 [ %.11545.lcssa, %.preheader4377 ], [ %i.cvv, %.loopexit4361 ] ; 6 uses
  %i.cps = or disjoint i32 %.21546.lcssa, 1
  %i.cpt = icmp slt i32 %i.cps, %i.ayu
  br i1 %i.cpt, label %_ZN4ncnn3MatD2Ev.exit2347.lr.ph, label %.preheader4375

_ZN4ncnn3MatD2Ev.exit2347.lr.ph:                  ; preds = %.preheader4376
  ...
  %factor.op.mul4808 = mul i64 %i.cpw, %i.cpx
  %i.cpy = sext i32 %i.cpu to i64
  %i.cpz = mul i64 %i.cpx, %i.cpy
  %i.cqa = mul i64 %i.cpz, %i.baf
  %invariant.gep4810 = getelementptr i8, ptr %i.cpv, i64 %i.cqa
  %i.cqb = mul nsw i64 %indvars.iv5605, %i.ban
  %invariant.gep4811 = getelementptr [2 x i8], ptr %invariant.gep4810, i64 %i.cqb
...
_ZN4ncnn3MatD2Ev.exit2347.preheader:              ; preds = %_ZN4ncnn3MatD2Ev.exit2347.lr.ph
  ...
  %15 = add i32 %.21546.lcssa, 3
  %smax5584 = call i32 @llvm.smax.i32(i32 %i.ayu, i32 %15)
  %16 = add i32 %smax5584, -2
  %i.cqf = sub i32 %16, %.21546.lcssa
  %i.cqg = and i32 %i.cqf, -2
  %i.cqh = add i32 %i.cqe, %i.cqg
  br label %.preheader4375
...
_ZN4ncnn3MatD2Ev.exit2347.us:                     ; preds = %_ZN4ncnn3MatD2Ev.exit2347.us.preheader, %._crit_edge4796.us
  %indvars.iv5592 = phi i64 [ %i.cqi, %_ZN4ncnn3MatD2Ev.exit2347.us.preheader ], [ %indvars.iv.next5593, %._crit_edge4796.us ] ; 2 uses
  %.2115704802.us = phi ptr [ %.161565.lcssa, %_ZN4ncnn3MatD2Ev.exit2347.us.preheader ], [ %scevgep5588, %._crit_edge4796.us ] ; 2 uses
  %.2142634801.us = phi <4 x float> [ %.164258.lcssa, %_ZN4ncnn3MatD2Ev.exit2347.us.preheader ], [ %i.crn, %._crit_edge4796.us ]
  %.2242874800.us = phi <4 x float> [ %.174282.lcssa, %_ZN4ncnn3MatD2Ev.exit2347.us.preheader ], [ %i.crf, %._crit_edge4796.us ]
  %.reass4809.us = mul i64 %factor.op.mul4808, %indvars.iv5592
  %gep4812.us = getelementptr i8, ptr %invariant.gep4811, i64 %.reass4809.us ; 2 uses
  %i.cqj = getelementptr [2 x i8], ptr %gep4812.us, i64 %i.cqd
  br label %bb.v

bb.v:                                             ; preds = %_ZN4ncnn3MatD2Ev.exit2347.us, %bb.v
  %indvars.iv5585 = phi i64 [ 0, %_ZN4ncnn3MatD2Ev.exit2347.us ], [ %indvars.iv.next5586, %bb.v ] ; 2 uses
  %.2215714793.us = phi ptr [ %.2115704802.us, %_ZN4ncnn3MatD2Ev.exit2347.us ], [ %i.cro, %bb.v ] ; 3 uses
  %.2242644792.us = phi <4 x float> [ %.2142634801.us, %_ZN4ncnn3MatD2Ev.exit2347.us ], [ %i.crn, %bb.v ]
  %.2342884791.us = phi <4 x float> [ %.2242874800.us, %_ZN4ncnn3MatD2Ev.exit2347.us ], [ %i.crf, %bb.v ]
  %i.cqk = getelementptr inbounds nuw [4 x i8], ptr %i.cqc, i64 %indvars.iv5585
  %i.cql = load i32, ptr %i.cqk, align 4, !tbaa !113
  %i.cqm = load i64, ptr %.2215714793.us, align 1, !tbaa !20
  ...
  %i.cqr = getelementptr inbounds nuw i8, ptr %.2215714793.us, i64 8
  ...
  %i.cqy = getelementptr inbounds [2 x i8], ptr %gep4812.us, i64 %i.cqx
  ...
  %i.crf = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.cqq, <4 x float> nofpclass(nan inf) %i.cre, <4 x float> nofpclass(nan inf) %.2342884791.us) ; 3 uses
  ...
  %i.crn = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.cqw, <4 x float> nofpclass(nan inf) %i.crm, <4 x float> nofpclass(nan inf) %.2242644792.us) ; 3 uses
  %i.cro = getelementptr inbounds nuw i8, ptr %.2215714793.us, i64 16
  %indvars.iv.next5586 = add nuw nsw i64 %indvars.iv5585, 1 ; 2 uses
  %exitcond5591.not = icmp eq i64 %indvars.iv.next5586, %wide.trip.count5590
  br i1 %exitcond5591.not, label %._crit_edge4796.us, label %bb.v, !llvm.loop !204

._crit_edge4796.us:                               ; preds = %bb.v
  %scevgep5587.a = getelementptr i8, ptr %.2115704802.us, i64 16
  %scevgep5588 = getelementptr i8, ptr %scevgep5587.a, i64 %i.bal ; 2 uses
  %indvars.iv.next5593 = add nuw nsw i64 %indvars.iv5592, 2 ; 3 uses
  %i.crp = trunc i64 %indvars.iv.next5593 to i32
  %i.crq = or i32 %i.crp, 1
  %i.crr = icmp slt i32 %i.crq, %i.ayu
  br i1 %i.crr, label %_ZN4ncnn3MatD2Ev.exit2347.us, label %.preheader4375.loopexit, !llvm.loop !205

_ZN4ncnn3MatD2Ev.exit2348:                        ; preds = %_ZN4ncnn3MatD2Ev.exit2348.lr.ph, %.loopexit4361
  %.215464779 = phi i32 [ %.11545.lcssa, %_ZN4ncnn3MatD2Ev.exit2348.lr.ph ], [ %i.cvv, %.loopexit4361 ] ; 2 uses
  %.1615654778 = phi ptr [ %.91558.lcssa, %_ZN4ncnn3MatD2Ev.exit2348.lr.ph ], [ %.201569, %.loopexit4361 ] ; 7 uses
  %.1642164777 = phi <4 x float> [ %.94209.lcssa, %_ZN4ncnn3MatD2Ev.exit2348.lr.ph ], [ %.204220, %.loopexit4361 ] ; 5 uses
  %.1642374776 = phi <4 x float> [ %.94230.lcssa, %_ZN4ncnn3MatD2Ev.exit2348.lr.ph ], [ %.204241, %.loopexit4361 ] ; 5 uses
  %.1642584775 = phi <4 x float> [ %.94251.lcssa, %_ZN4ncnn3MatD2Ev.exit2348.lr.ph ], [ %.204262, %.loopexit4361 ] ; 5 uses
  %.1742824774 = phi <4 x float> [ %.104275.lcssa, %_ZN4ncnn3MatD2Ev.exit2348.lr.ph ], [ %.214286, %.loopexit4361 ] ; 5 uses
  %i.crs = sdiv i32 %.215464779, %i.ays
  %i.crt = sext i32 %i.crs to i64
  %.reass4787 = mul i64 %factor.op.mul4786, %i.crt
  %gep4790 = getelementptr i8, ptr %invariant.gep4789, i64 %.reass4787 ; 5 uses
  br i1 %i.azr, label %.preheader4362, label %.loopexit4363

.preheader4362:                                   ; preds = %_ZN4ncnn3MatD2Ev.exit2348
  br i1 %i.cej, label %.lr.ph4756, label %.loopexit4361

.lr.ph4756:                                       ; preds = %.preheader4362, %.lr.ph4756
  %indvars.iv5570 = phi i64 [ %indvars.iv.next5571, %.lr.ph4756 ], [ 0, %.preheader4362 ] ; 2 uses
  %.1715664754 = phi ptr [ %i.ctk, %.lr.ph4756 ], [ %.1615654778, %.preheader4362 ] ; 5 uses
  %.1742174753 = phi <4 x float> [ %i.ctj, %.lr.ph4756 ], [ %.1642164777, %.preheader4362 ]
  %.1742384752 = phi <4 x float> [ %i.ctg, %.lr.ph4756 ], [ %.1642374776, %.preheader4362 ]
  %.1742594751 = phi <4 x float> [ %i.ctd, %.lr.ph4756 ], [ %.1642584775, %.preheader4362 ]
  %.1842834750 = phi <4 x float> [ %i.cta, %.lr.ph4756 ], [ %.1742824774, %.preheader4362 ]
  %i.cru = getelementptr inbounds nuw [4 x i8], ptr %i.cek, i64 %indvars.iv5570
  %i.crv = load i32, ptr %i.cru, align 4, !tbaa !113
  %i.crw = sext i32 %i.crv to i64
  %i.crx = getelementptr inbounds [2 x i8], ptr %gep4790, i64 %i.crw
  %i.cry = load i64, ptr %.1715664754, align 1, !tbaa !20
  ...
  %i.csd = getelementptr inbounds nuw i8, ptr %.1715664754, i64 8
  ...
  %i.csj = getelementptr inbounds nuw i8, ptr %.1715664754, i64 16
  ...
  %i.csp = getelementptr inbounds nuw i8, ptr %.1715664754, i64 24
  ...
  %i.cta = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.csc, <4 x float> nofpclass(nan inf) %i.csz, <4 x float> nofpclass(nan inf) %.1842834750) ; 2 uses
  ...
  %i.ctd = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.csi, <4 x float> nofpclass(nan inf) %i.ctc, <4 x float> nofpclass(nan inf) %.1742594751) ; 2 uses
  ...
  %i.ctg = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.cso, <4 x float> nofpclass(nan inf) %i.ctf, <4 x float> nofpclass(nan inf) %.1742384752) ; 2 uses
  ...
  %i.ctj = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.csu, <4 x float> nofpclass(nan inf) %i.cti, <4 x float> nofpclass(nan inf) %.1742174753) ; 2 uses
  %i.ctk = getelementptr inbounds nuw i8, ptr %.1715664754, i64 32
  %indvars.iv.next5571 = add nuw nsw i64 %indvars.iv5570, 1 ; 2 uses
  %exitcond5576.not = icmp eq i64 %indvars.iv.next5571, %wide.trip.count5575
  br i1 %exitcond5576.not, label %.loopexit4363.loopexit, label %.lr.ph4756, !llvm.loop !206

.loopexit4363.loopexit:                           ; preds = %.lr.ph4756
  %scevgep5572 = getelementptr i8, ptr %.1615654778, i64 32
  %scevgep5573 = getelementptr i8, ptr %scevgep5572, i64 %i.ceo
  br label %.loopexit4361

.loopexit4363:                                    ; preds = %_ZN4ncnn3MatD2Ev.exit2348
  br i1 %i.azs, label %.preheader4360, label %.loopexit4361

.preheader4360:                                   ; preds = %.loopexit4363
  br i1 %i.cej, label %.lr.ph4768, label %.loopexit4361

.lr.ph4768:                                       ; preds = %.preheader4360
  %i.ctl = getelementptr [2 x i8], ptr %gep4790, i64 %i.cel
  %i.ctm = getelementptr i8, ptr %gep4790, i64 %.idx2051
  %i.ctn = getelementptr i8, ptr %gep4790, i64 %.idx2052
  br label %bb.w

bb.w:                                             ; preds = %.lr.ph4768, %bb.w
  %indvars.iv5577 = phi i64 [ 0, %.lr.ph4768 ], [ %indvars.iv.next5578, %bb.w ] ; 2 uses
  %.1915684766 = phi ptr [ %.1615654778, %.lr.ph4768 ], [ %i.cvu, %bb.w ] ; 5 uses
  %.1942194765 = phi <4 x float> [ %.1642164777, %.lr.ph4768 ], [ %i.cvt, %bb.w ]
  %.1942404764 = phi <4 x float> [ %.1642374776, %.lr.ph4768 ], [ %i.cvl, %bb.w ]
  %.1942614763 = phi <4 x float> [ %.1642584775, %.lr.ph4768 ], [ %i.cvd, %bb.w ]
  %.2042854762 = phi <4 x float> [ %.1742824774, %.lr.ph4768 ], [ %i.cuv, %bb.w ]
  %i.cto = getelementptr inbounds nuw [4 x i8], ptr %i.cek, i64 %indvars.iv5577
  %i.ctp = load i32, ptr %i.cto, align 4, !tbaa !113
  %i.ctq = load i64, ptr %.1915684766, align 1, !tbaa !20
  ...
  %i.ctv = getelementptr inbounds nuw i8, ptr %.1915684766, i64 8
  ...
  %i.cub = getelementptr inbounds nuw i8, ptr %.1915684766, i64 16
  ...
  %i.cuh = getelementptr inbounds nuw i8, ptr %.1915684766, i64 24
  ...
  %i.cuo = getelementptr inbounds [2 x i8], ptr %gep4790, i64 %i.cun
  ...
  %i.cuv = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.ctu, <4 x float> nofpclass(nan inf) %i.cuu, <4 x float> nofpclass(nan inf) %.2042854762) ; 2 uses
...
.noexc2560:                                       ; preds = %._crit_edge4827
...
_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit:      ; preds = %bb.z, %.noexc2560, %.noexc2559, %bb.y, %.noexc2558, %bb.x, %._crit_edge4827
  %.0.i2557 = phi nsz <4 x float> [ %i.dee, %bb.z ], [ %i.cyo, %bb.x ], [ %i.cyw, %.noexc2558 ], [ %i.czg, %bb.y ], [ %i.dai, %.noexc2559 ], [ %i.ddr, %.noexc2560 ], [ %i.cyn, %._crit_edge4827 ] ; 2 uses
...
.thread4294:                                      ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit
  ...
  store i64 %i.deh, ptr %.115774835, align 1, !tbaa !20
  %i.dei = getelementptr inbounds nuw i8, ptr %.115774835, i64 8
  br label %bb.ab
...
bb.aa:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit
  ...
  store i16 %i.dek, ptr %.115774835, align 2, !tbaa !76
  %i.del = extractelement <8 x i16> %i.dej, i64 3
  %i.dem = load i64, ptr %i.e, align 8, !tbaa !117 ; 3 uses
  %i.den = getelementptr inbounds nuw [2 x i8], ptr %.115774835, i64 %i.dem
  store i16 %i.del, ptr %i.den, align 2, !tbaa !76
  %i.deo = extractelement <8 x i16> %i.dej, i64 5
  %.idx2040 = shl i64 %i.dem, 2
  %i.dep = getelementptr inbounds nuw i8, ptr %.115774835, i64 %.idx2040
  store i16 %i.deo, ptr %i.dep, align 2, !tbaa !76
  %i.deq = extractelement <8 x i16> %i.dej, i64 7
  %.idx2041 = mul i64 %i.dem, 6
  %i.der = getelementptr inbounds nuw i8, ptr %.115774835, i64 %.idx2041
  store i16 %i.deq, ptr %i.der, align 2, !tbaa !76
  %i.des = getelementptr inbounds nuw i8, ptr %.115774835, i64 2
  br label %bb.ab

bb.ab:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit, %.thread4294, %bb.aa
  %.31579 = phi ptr [ %i.des, %bb.aa ], [ %.115774835, %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit ], [ %i.dei, %.thread4294 ] ; 2 uses
  %indvars.iv.next5606 = add nuw nsw i64 %indvars.iv5605, 1 ; 2 uses
  %exitcond5609.not = icmp eq i64 %indvars.iv.next5606, %wide.trip.count5608
  br i1 %exitcond5609.not, label %._crit_edge4838, label %bb.o, !llvm.loop !214

._crit_edge5036.split:                            ; preds = %._crit_edge5034, %.noexc2326.lr.ph, %._crit_edge4850
  %i.det = shl nsw i32 %i.axq, 1
  %i.deu = add nsw i32 %i.det, %i.axo             ; 2 uses
  %i.dev = icmp slt i32 %i.deu, %i.ad
  br i1 %i.dev, label %.noexc.lr.ph, label %._crit_edge5193.split

.noexc.lr.ph:                                     ; preds = %._crit_edge5036.split
  ...
  %factor.op.mul5194 = mul i64 %i.dex, %i.dez
  ...
  %brmerge5203 = select i1 %i.dfa, i1 true, i1 %i.dfb
  br i1 %brmerge5203, label %._crit_edge5193.split, label %.noexc.lr.ph.split.split
...
.noexc.lr.ph.split.split:                         ; preds = %.noexc.lr.ph
  ...
  %factor.op.mul5197 = mul i64 %i.dfn, %i.dfo
  %i.dfp = and i32 %i.q, -16
  %i.dfq = sext i32 %i.deu to i64
  %wide.trip.count5804 = sext i32 %i.ad to i64
  %wide.trip.count5798 = zext nneg i32 %i.w to i64
  br label %.noexc

.noexc2326:                                       ; preds = %.noexc2326.lr.ph.split.split, %._crit_edge5034
  %indvars.iv5706 = phi i64 [ 0, %.noexc2326.lr.ph.split.split ], [ %indvars.iv.next5707, %._crit_edge5034 ] ; 2 uses
  %i.dfr = shl nuw nsw i64 %indvars.iv5706, 1
  %i.dfs = add nsw i64 %i.dfr, %i.ayr             ; 4 uses
  %.reass5039 = mul i64 %factor.op.mul5038, %i.dfs
  %i.dft = getelementptr inbounds nuw i8, ptr %i.axx, i64 %.reass5039
  %i.dfu = or disjoint i64 %i.dfs, 1
  %.reass5041 = mul i64 %factor.op.mul5038, %i.dfu
  %i.dfv = getelementptr inbounds nuw i8, ptr %i.axx, i64 %.reass5041
  ...
  %.reass5045 = mul i64 %factor.op.mul5044, %i.dgm
  %i.dgo = getelementptr inbounds nuw i8, ptr %i.ayn, i64 %.reass5045 ; 2 uses
  br label %.preheader4359

.preheader4359:                                   ; preds = %.noexc2326, %._crit_edge5022
  %.015275033 = phi i32 [ 0, %.noexc2326 ], [ %i.dhg, %._crit_edge5022 ] ; 2 uses
  %.015285032 = phi ptr [ %i.dfv, %.noexc2326 ], [ %i.fmq, %._crit_edge5022 ]
  %.015305031 = phi ptr [ %i.dft, %.noexc2326 ], [ %i.fmp, %._crit_edge5022 ]
  %i.dgp = load i32, ptr %i.b, align 4
  %i.dgq = mul nsw i32 %i.dgp, %.015275033
  %i.dgr = sext i32 %i.dgq to i64                 ; 5 uses
  %i.dgs = load i32, ptr %i.a, align 4            ; 2 uses
  %factor.op.mul5025 = mul i32 %i.axs, %i.dgs     ; 3 uses
  ...
  %wide.trip.count5685 = zext i32 %i.dgt to i64   ; 11 uses
  %wide.trip.count5695 = zext nneg i32 %i.dgt to i64
  %min.iters.check6317 = icmp ult i32 %i.dgt, 8
  %min.iters.check6319 = icmp ult i32 %i.dgt, 32
  %i.dha = and i64 %wide.trip.count5685, 24
  %n.vec6321 = and i64 %wide.trip.count5685, 2147483616 ; 5 uses
  %i.dhb = shl nuw nsw i64 %n.vec6321, 3
  %cmp.n6346 = icmp eq i64 %n.vec6321, %wide.trip.count5685
  %min.epilog.iters.check6353 = icmp eq i64 %i.dha, 0
  %n.vec6355 = and i64 %wide.trip.count5685, 2147483640 ; 4 uses
  %i.dhc = shl nuw nsw i64 %n.vec6355, 3
  %cmp.n6369 = icmp eq i64 %n.vec6355, %wide.trip.count5685
  %min.iters.check6255 = icmp ult i32 %i.dgt, 8
  %min.iters.check6257 = icmp ult i32 %i.dgt, 64
  %i.dhd = and i64 %wide.trip.count5685, 56
  %n.vec6259 = and i64 %wide.trip.count5685, 2147483584 ; 5 uses
  %i.dhe = shl nuw nsw i64 %n.vec6259, 2
  %cmp.n6292 = icmp eq i64 %n.vec6259, %wide.trip.count5685
  %min.epilog.iters.check6298 = icmp eq i64 %i.dhd, 0
  %n.vec6300 = and i64 %wide.trip.count5685, 2147483640 ; 4 uses
  %i.dhf = shl nuw nsw i64 %n.vec6300, 2
  %cmp.n6312 = icmp eq i64 %n.vec6300, %wide.trip.count5685
  br label %bb.ac

._crit_edge5034:                                  ; preds = %._crit_edge5022
  %indvars.iv.next5707 = add nuw nsw i64 %indvars.iv5706, 1 ; 2 uses
  %exitcond5710.not = icmp eq i64 %indvars.iv.next5707, %wide.trip.count5709
  br i1 %exitcond5710.not, label %._crit_edge5036.split, label %.noexc2326, !llvm.loop !221

._crit_edge5022:                                  ; preds = %_ZL13activation_ssfiRKN4ncnn3MatE.exit2566
  %i.dhg = add nuw nsw i32 %.015275033, 1         ; 2 uses
  %exitcond5705.not = icmp eq i32 %i.dhg, %i.axw
  br i1 %exitcond5705.not, label %._crit_edge5034, label %.preheader4359, !llvm.loop !222

bb.ac:                                            ; preds = %.preheader4359, %_ZL13activation_ssfiRKN4ncnn3MatE.exit2566
  %indvars.iv5700 = phi i64 [ 0, %.preheader4359 ], [ %indvars.iv.next5701, %_ZL13activation_ssfiRKN4ncnn3MatE.exit2566 ] ; 6 uses
  %.115295019 = phi ptr [ %.015285032, %.preheader4359 ], [ %i.fmq, %_ZL13activation_ssfiRKN4ncnn3MatE.exit2566 ] ; 2 uses
  %.115315018 = phi ptr [ %.015305031, %.preheader4359 ], [ %i.fmp, %_ZL13activation_ssfiRKN4ncnn3MatE.exit2566 ] ; 2 uses
...
_ZN4ncnn3MatD2Ev.exit2343:                        ; preds = %bb.ad, %bb.ac
  ...
  br i1 %i.ayd, label %_ZN4ncnn3MatD2Ev.exit2342.lr.ph, label %._crit_edge4890
...
_ZN4ncnn3MatD2Ev.exit2342.lr.ph:                  ; preds = %_ZN4ncnn3MatD2Ev.exit2343
  ...
  %factor.op.mul4895 = mul i64 %i.dhl, %i.dhm
  %i.dhn = sext i32 %i.dhj to i64
  %i.dho = mul i64 %i.dhm, %i.dhn
  %i.dhp = mul i64 %i.dho, %i.dgr
  %invariant.gep4897 = getelementptr i8, ptr %i.dhk, i64 %i.dhp
  %i.dhq = trunc nuw nsw i64 %indvars.iv5700 to i32
  %.reass5026 = mul i32 %factor.op.mul5025, %i.dhq
  %i.dhr = sext i32 %.reass5026 to i64
  %invariant.gep4898 = getelementptr [2 x i8], ptr %invariant.gep4897, i64 %i.dhr
  ...
  %brmerge5206 = select i1 %i.ayf, i1 true, i1 %i.dht
  %wide.trip.count5621 = zext i32 %i.dhs to i64   ; 5 uses
  %brmerge5209 = select i1 %i.ayi, i1 true, i1 %i.dht
  %wide.trip.count5642 = zext nneg i32 %i.dhs to i64
  %wide.trip.count5635 = zext nneg i32 %i.dhs to i64
  %i.dia = add nsw i64 %wide.trip.count5621, -1   ; 2 uses
  %xtraiter6648 = and i64 %wide.trip.count5621, 1
  %i.dib = icmp eq i64 %i.dia, 0
  %unroll_iter6653 = and i64 %wide.trip.count5621, 2147483646
  %lcmp.mod6649.not = icmp eq i64 %xtraiter6648, 0
  %lcmp.mod6652 = trunc i32 %i.dhs to i1
  %xtraiter6655 = and i64 %wide.trip.count5621, 1
  %i.dic = icmp eq i64 %i.dia, 0
  %unroll_iter6660 = and i64 %wide.trip.count5621, 2147483646
  %lcmp.mod6656.not = icmp eq i64 %xtraiter6655, 0
  %lcmp.mod6659 = trunc i32 %i.dhs to i1
  br label %_ZN4ncnn3MatD2Ev.exit2342

_ZN4ncnn3MatD2Ev.exit2342:                        ; preds = %_ZN4ncnn3MatD2Ev.exit2342.lr.ph, %.loopexit4353
  %.014864889 = phi i32 [ 0, %_ZN4ncnn3MatD2Ev.exit2342.lr.ph ], [ %i.dqs, %.loopexit4353 ] ; 2 uses
  %.014914888 = phi ptr [ %i.dgo, %_ZN4ncnn3MatD2Ev.exit2342.lr.ph ], [ %.81499, %.loopexit4353 ] ; 13 uses
  %.040864887 = phi <16 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2342.lr.ph ], [ %.84094, %.loopexit4353 ] ; 9 uses
  %.040954886 = phi <16 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2342.lr.ph ], [ %.84103, %.loopexit4353 ] ; 9 uses
  %i.did = sdiv i32 %.014864889, %i.axs
  %i.die = sext i32 %i.did to i64
  %.reass4896 = mul i64 %factor.op.mul4895, %i.die
  %gep4899 = getelementptr i8, ptr %invariant.gep4898, i64 %.reass4896 ; 27 uses
  br i1 %brmerge5206, label %.loopexit4357, label %.lr.ph4856.preheader

.lr.ph4856.preheader:                             ; preds = %_ZN4ncnn3MatD2Ev.exit2342
  br i1 %i.dib, label %.lr.ph4856.epil.preheader, label %.lr.ph4856

.lr.ph4856:                                       ; preds = %.lr.ph4856.preheader, %.lr.ph4856
  %indvars.iv5616 = phi i64 [ %indvars.iv.next5617.1, %.lr.ph4856 ], [ 0, %.lr.ph4856.preheader ] ; 3 uses
  %.114924854 = phi ptr [ %i.djh, %.lr.ph4856 ], [ %.014914888, %.lr.ph4856.preheader ] ; 5 uses
  %.140874853 = phi <16 x float> [ %i.djg, %.lr.ph4856 ], [ %.040864887, %.lr.ph4856.preheader ]
  %.140964852 = phi <16 x float> [ %i.djf, %.lr.ph4856 ], [ %.040954886, %.lr.ph4856.preheader ]
  %niter6654 = phi i64 [ %niter6654.next.1, %.lr.ph4856 ], [ 0, %.lr.ph4856.preheader ]
  %i.dif = getelementptr inbounds nuw [4 x i8], ptr %i.dhu, i64 %indvars.iv5616
  %i.dig = load i32, ptr %i.dif, align 4, !tbaa !113
  %i.dih = sext i32 %i.dig to i64
  %i.dii = getelementptr inbounds [2 x i8], ptr %gep4899, i64 %i.dih
  %i.dij = load <16 x bfloat>, ptr %i.dii, align 1, !tbaa !20
  %i.dik = fpext fast <16 x bfloat> %i.dij to <16 x float> ; 2 uses
  %i.dil = load <16 x bfloat>, ptr %.114924854, align 32, !tbaa !20
  %i.dim = fpext fast <16 x bfloat> %i.dil to <16 x float>
  %i.din = getelementptr inbounds nuw i8, ptr %.114924854, i64 32
  %i.dio = load <16 x bfloat>, ptr %i.din, align 32, !tbaa !20
  %i.dip = fpext fast <16 x bfloat> %i.dio to <16 x float>
  %i.diq = call fast noundef nofpclass(nan inf) <16 x float> @llvm.fma.v16f32(<16 x float> nofpclass(nan inf) %i.dik, <16 x float> nofpclass(nan inf) %i.dim, <16 x float> nofpclass(nan inf) %.140964852)
  %i.dir = call fast noundef nofpclass(nan inf) <16 x float> @llvm.fma.v16f32(<16 x float> nofpclass(nan inf) %i.dik, <16 x float> nofpclass(nan inf) %i.dip, <16 x float> nofpclass(nan inf) %.140874853)
  %i.dis = getelementptr inbounds nuw i8, ptr %.114924854, i64 64
  %i.dit = getelementptr inbounds nuw [4 x i8], ptr %i.dhu, i64 %indvars.iv5616
  %i.diu = getelementptr inbounds nuw i8, ptr %i.dit, i64 4
  %i.div = load i32, ptr %i.diu, align 4, !tbaa !113
  %i.diw = sext i32 %i.div to i64
  %i.dix = getelementptr inbounds [2 x i8], ptr %gep4899, i64 %i.diw
...
begin_hunk_5_@_ZN4ncnnL24convolution_packed_bf16sERKNS_3MatERS0_S2_S2_iiiiiiiS2_RKNS_6OptionE:bb.a
  ...
  %brmerge5218 = select i1 %i.ayk, i1 true, i1 %i.dxi
  %wide.trip.count5670 = zext i32 %i.dxh to i64   ; 2 uses
  %brmerge5221 = select i1 %i.ayi, i1 true, i1 %i.dxi
  %wide.trip.count5677 = zext nneg i32 %i.dxh to i64
  %xtraiter6669 = and i64 %wide.trip.count5670, 1
  %i.dxo = icmp eq i32 %i.dxh, 1
  %unroll_iter6674 = and i64 %wide.trip.count5670, 2147483646
  %lcmp.mod6670.not = icmp eq i64 %xtraiter6669, 0
  %lcmp.mod6673 = trunc i32 %i.dxh to i1
  br label %_ZN4ncnn3MatD2Ev.exit2340

_ZN4ncnn3MatD2Ev.exit2340:                        ; preds = %_ZN4ncnn3MatD2Ev.exit2340.lr.ph, %.loopexit4344
  %.214884958 = phi i32 [ %.11487.lcssa, %_ZN4ncnn3MatD2Ev.exit2340.lr.ph ], [ %i.ebu, %.loopexit4344 ] ; 2 uses
  %.1615074957 = phi ptr [ %.91500.lcssa, %_ZN4ncnn3MatD2Ev.exit2340.lr.ph ], [ %.201511, %.loopexit4344 ] ; 6 uses
  %.040624956 = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2340.lr.ph ], [ %.44066, %.loopexit4344 ] ; 4 uses
  %.040674955 = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2340.lr.ph ], [ %.44071, %.loopexit4344 ] ; 4 uses
  %i.dxp = sdiv i32 %.214884958, %i.axs
  %i.dxq = sext i32 %i.dxp to i64
  %.reass4965 = mul i64 %factor.op.mul4964, %i.dxq
  %gep4968 = getelementptr i8, ptr %invariant.gep4967, i64 %.reass4965 ; 7 uses
  br i1 %brmerge5218, label %.loopexit4346, label %.lr.ph4943.preheader

.lr.ph4943.preheader:                             ; preds = %_ZN4ncnn3MatD2Ev.exit2340
  br i1 %i.dxo, label %.lr.ph4943.epil.preheader, label %.lr.ph4943

.lr.ph4943:                                       ; preds = %.lr.ph4943.preheader, %.lr.ph4943
  %indvars.iv5665 = phi i64 [ %indvars.iv.next5666.1, %.lr.ph4943 ], [ 0, %.lr.ph4943.preheader ] ; 3 uses
  %.1715084941 = phi ptr [ %i.dzl, %.lr.ph4943 ], [ %.1615074957, %.lr.ph4943.preheader ] ; 5 uses
  %.140634940 = phi <4 x float> [ %i.dzk, %.lr.ph4943 ], [ %.040624956, %.lr.ph4943.preheader ]
  %.140684939 = phi <4 x float> [ %i.dzj, %.lr.ph4943 ], [ %.040674955, %.lr.ph4943.preheader ]
  %niter6675 = phi i64 [ %niter6675.next.1, %.lr.ph4943 ], [ 0, %.lr.ph4943.preheader ]
  %i.dxr = getelementptr inbounds nuw [4 x i8], ptr %i.dxj, i64 %indvars.iv5665
  %i.dxs = load i32, ptr %i.dxr, align 4, !tbaa !113
  %i.dxt = sext i32 %i.dxs to i64
  %i.dxu = getelementptr inbounds [2 x i8], ptr %gep4968, i64 %i.dxt
  ...
  %i.dya = load i64, ptr %.1715084941, align 1, !tbaa !20
  ...
  %i.dyf = getelementptr inbounds nuw i8, ptr %.1715084941, i64 8
  ...
  %i.dyl = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.dxz, <4 x float> nofpclass(nan inf) %i.dye, <4 x float> nofpclass(nan inf) %.140684939)
  %i.dym = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.dxz, <4 x float> nofpclass(nan inf) %i.dyk, <4 x float> nofpclass(nan inf) %.140634940)
  %i.dyn = getelementptr inbounds nuw i8, ptr %.1715084941, i64 16
  %i.dyo = getelementptr inbounds nuw [4 x i8], ptr %i.dxj, i64 %indvars.iv5665
  %i.dyp = getelementptr inbounds nuw i8, ptr %i.dyo, i64 4
  %i.dyq = load i32, ptr %i.dyp, align 4, !tbaa !113
  %i.dyr = sext i32 %i.dyq to i64
  %i.dys = getelementptr inbounds [2 x i8], ptr %gep4968, i64 %i.dyr
  ...
  %i.dzd = getelementptr inbounds nuw i8, ptr %.1715084941, i64 24
  ...
  %i.dzl = getelementptr inbounds nuw i8, ptr %.1715084941, i64 32 ; 2 uses
  %indvars.iv.next5666.1 = add nuw nsw i64 %indvars.iv5665, 2 ; 2 uses
  %niter6675.next.1 = add i64 %niter6675, 2       ; 2 uses
  %niter6675.ncmp.1 = icmp eq i64 %niter6675.next.1, %unroll_iter6674
  br i1 %niter6675.ncmp.1, label %.loopexit4346.thread.unr-lcssa, label %.lr.ph4943, !llvm.loop !241

.loopexit4346.thread.unr-lcssa:                   ; preds = %.lr.ph4943
  br i1 %lcmp.mod6670.not, label %.loopexit4346.thread, label %.lr.ph4943.epil.preheader

.lr.ph4943.epil.preheader:                        ; preds = %.loopexit4346.thread.unr-lcssa, %.lr.ph4943.preheader
  %indvars.iv5665.epil.init = phi i64 [ 0, %.lr.ph4943.preheader ], [ %indvars.iv.next5666.1, %.loopexit4346.thread.unr-lcssa ]
  %.1715084941.epil.init = phi ptr [ %.1615074957, %.lr.ph4943.preheader ], [ %i.dzl, %.loopexit4346.thread.unr-lcssa ] ; 2 uses
  %.140634940.epil.init = phi <4 x float> [ %.040624956, %.lr.ph4943.preheader ], [ %i.dzk, %.loopexit4346.thread.unr-lcssa ]
  %.140684939.epil.init = phi <4 x float> [ %.040674955, %.lr.ph4943.preheader ], [ %i.dzj, %.loopexit4346.thread.unr-lcssa ]
  call void @llvm.assume(i1 %lcmp.mod6673)
  %i.dzm = getelementptr inbounds nuw [4 x i8], ptr %i.dxj, i64 %indvars.iv5665.epil.init
  %i.dzn = load i32, ptr %i.dzm, align 4, !tbaa !113
  %i.dzo = sext i32 %i.dzn to i64
  %i.dzp = getelementptr inbounds [2 x i8], ptr %gep4968, i64 %i.dzo
  ...
  %i.dzv = load i64, ptr %.1715084941.epil.init, align 1, !tbaa !20
  ...
  %i.eaa = getelementptr inbounds nuw i8, ptr %.1715084941.epil.init, i64 8
  ...
  %i.eag = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.dzu, <4 x float> nofpclass(nan inf) %i.dzz, <4 x float> nofpclass(nan inf) %.140684939.epil.init)
  %i.eah = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.dzu, <4 x float> nofpclass(nan inf) %i.eaf, <4 x float> nofpclass(nan inf) %.140634940.epil.init)
  br label %.loopexit4346.thread

.loopexit4346.thread:                             ; preds = %.loopexit4346.thread.unr-lcssa, %.lr.ph4943.epil.preheader
  %.lcssa6530 = phi <4 x float> [ %i.dzj, %.loopexit4346.thread.unr-lcssa ], [ %i.eag, %.lr.ph4943.epil.preheader ]
  %.lcssa6529 = phi <4 x float> [ %i.dzk, %.loopexit4346.thread.unr-lcssa ], [ %i.eah, %.lr.ph4943.epil.preheader ]
  %scevgep5667.a = getelementptr i8, ptr %.1615074957, i64 16
  %scevgep5668 = getelementptr i8, ptr %scevgep5667.a, i64 %i.dxn
  br label %.loopexit4344

.loopexit4346:                                    ; preds = %_ZN4ncnn3MatD2Ev.exit2340
  br i1 %brmerge5221, label %.loopexit4344, label %.lr.ph4951

.lr.ph4951:                                       ; preds = %.loopexit4346
  %i.eai = getelementptr i8, ptr %gep4968, i64 %.idx1974
  %i.eaj = getelementptr i8, ptr %gep4968, i64 %.idx1975
  %i.eak = getelementptr [2 x i8], ptr %gep4968, i64 %i.dxk
  br label %bb.aj

bb.aj:                                            ; preds = %.lr.ph4951, %bb.aj
  %indvars.iv5672 = phi i64 [ 0, %.lr.ph4951 ], [ %indvars.iv.next5673, %bb.aj ] ; 2 uses
  %.1915104949 = phi ptr [ %.1615074957, %.lr.ph4951 ], [ %i.ebt, %bb.aj ] ; 3 uses
  %.340654948 = phi <4 x float> [ %.040624956, %.lr.ph4951 ], [ %i.ebs, %bb.aj ]
  %.340704947 = phi <4 x float> [ %.040674955, %.lr.ph4951 ], [ %i.ebr, %bb.aj ]
  %i.eal = getelementptr inbounds nuw [4 x i8], ptr %i.dxj, i64 %indvars.iv5672
  ...
  %i.eau = getelementptr inbounds [2 x i8], ptr %gep4968, i64 %i.ean
  ...
  %i.ebg = load i64, ptr %.1915104949, align 1, !tbaa !20
  ...
  %i.ebl = getelementptr inbounds nuw i8, ptr %.1915104949, i64 8
  ...
  %i.ebr = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.ebf, <4 x float> nofpclass(nan inf) %i.ebk, <4 x float> nofpclass(nan inf) %.340704947) ; 2 uses
  %i.ebs = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.ebf, <4 x float> nofpclass(nan inf) %i.ebq, <4 x float> nofpclass(nan inf) %.340654948) ; 2 uses
  %i.ebt = getelementptr inbounds nuw i8, ptr %.1915104949, i64 16
  %indvars.iv.next5673 = add nuw nsw i64 %indvars.iv5672, 1 ; 2 uses
  %exitcond5678.not = icmp eq i64 %indvars.iv.next5673, %wide.trip.count5677
  br i1 %exitcond5678.not, label %.loopexit4344.loopexit, label %bb.aj, !llvm.loop !242

.loopexit4344.loopexit:                           ; preds = %bb.aj
  %scevgep5674.a = getelementptr i8, ptr %.1615074957, i64 16
  %scevgep5675 = getelementptr i8, ptr %scevgep5674.a, i64 %i.dxn
  br label %.loopexit4344

.loopexit4344:                                    ; preds = %.loopexit4346.thread, %.loopexit4344.loopexit, %.loopexit4346
  %.44071 = phi nsz <4 x float> [ %.040674955, %.loopexit4346 ], [ %i.ebr, %.loopexit4344.loopexit ], [ %.lcssa6530, %.loopexit4346.thread ] ; 2 uses
  %.44066 = phi nsz <4 x float> [ %.040624956, %.loopexit4346 ], [ %i.ebs, %.loopexit4344.loopexit ], [ %.lcssa6529, %.loopexit4346.thread ] ; 2 uses
  %.201511 = phi ptr [ %.1615074957, %.loopexit4346 ], [ %scevgep5675, %.loopexit4344.loopexit ], [ %scevgep5668, %.loopexit4346.thread ] ; 2 uses
  %i.ebu = add nuw nsw i32 %.214884958, 4         ; 3 uses
  %i.ebv = or disjoint i32 %i.ebu, 3
  %i.ebw = icmp slt i32 %i.ebv, %i.axu
  br i1 %i.ebw, label %_ZN4ncnn3MatD2Ev.exit2340, label %._crit_edge4959, !llvm.loop !243

._crit_edge4959:                                  ; preds = %.loopexit4344, %._crit_edge4929
  %.04067.lcssa = phi <4 x float> [ zeroinitializer, %._crit_edge4929 ], [ %.44071, %.loopexit4344 ] ; 4 uses
  %.04062.lcssa = phi <4 x float> [ zeroinitializer, %._crit_edge4929 ], [ %.44066, %.loopexit4344 ] ; 4 uses
  %.161507.lcssa = phi ptr [ %.91500.lcssa, %._crit_edge4929 ], [ %.201511, %.loopexit4344 ] ; 3 uses
  %.21488.lcssa = phi i32 [ %.11487.lcssa, %._crit_edge4929 ], [ %i.ebu, %.loopexit4344 ] ; 6 uses
  ...
  br i1 %i.ecw, label %_ZN4ncnn3MatD2Ev.exit2339.lr.ph, label %.preheader4358

_ZN4ncnn3MatD2Ev.exit2339.lr.ph:                  ; preds = %._crit_edge4959
  ...
  %factor.op.mul4986 = mul i64 %i.ecz, %i.eda
  %i.edb = sext i32 %i.ecx to i64
  %i.edc = mul i64 %i.eda, %i.edb
  %i.edd = mul i64 %i.edc, %i.dgr
  %invariant.gep4988 = getelementptr i8, ptr %i.ecy, i64 %i.edd
  %i.ede = mul nsw i64 %indvars.iv5700, %i.dgz
  %invariant.gep4989 = getelementptr [2 x i8], ptr %invariant.gep4988, i64 %i.ede
...
_ZN4ncnn3MatD2Ev.exit2339.preheader:              ; preds = %_ZN4ncnn3MatD2Ev.exit2339.lr.ph
  ...
  %17 = add i32 %.21488.lcssa, 3
  %smax5679 = call i32 @llvm.smax.i32(i32 %i.axu, i32 %17)
  %18 = add i32 %smax5679, -2
  %i.edi = sub i32 %18, %.21488.lcssa
  %i.edj = and i32 %i.edi, -2
  %i.edk = add i32 %i.edh, %i.edj
  br label %.preheader4358

_ZN4ncnn3MatD2Ev.exit2339.us.preheader:           ; preds = %_ZN4ncnn3MatD2Ev.exit2339.lr.ph
  %i.edl = zext i32 %.21488.lcssa to i64
  br label %iter.check6350

iter.check6350:                                   ; preds = %_ZN4ncnn3MatD2Ev.exit2339.us.preheader, %._crit_edge4974.us
  %indvars.iv5687 = phi i64 [ %i.edl, %_ZN4ncnn3MatD2Ev.exit2339.us.preheader ], [ %indvars.iv.next5688, %._crit_edge4974.us ] ; 2 uses
  %.2115124980.us = phi ptr [ %.161507.lcssa, %_ZN4ncnn3MatD2Ev.exit2339.us.preheader ], [ %scevgep5683, %._crit_edge4974.us ] ; 7 uses
  %i.edm = phi <2 x float> [ %i.ecu, %_ZN4ncnn3MatD2Ev.exit2339.us.preheader ], [ %i.etq, %._crit_edge4974.us ] ; 4 uses
  %.reass4987.us = mul i64 %factor.op.mul4986, %indvars.iv5687
  %gep4990.us = getelementptr i8, ptr %invariant.gep4989, i64 %.reass4987.us ; 42 uses
  %i.edn = getelementptr [2 x i8], ptr %gep4990.us, i64 %i.edg ; 41 uses
  br i1 %min.iters.check6317, label %vec.epilog.scalar.ph6351.preheader, label %vector.main.loop.iter.check6318

vector.main.loop.iter.check6318:                  ; preds = %iter.check6350
  %i.edo = extractelement <2 x float> %i.edm, i64 0
  %i.edp = extractelement <2 x float> %i.edm, i64 1
  br i1 %min.iters.check6319, label %vec.epilog.ph6354, label %vector.ph6320

vector.ph6320:                                    ; preds = %vector.main.loop.iter.check6318
  %i.edq = getelementptr i8, ptr %.2115124980.us, i64 %i.dhb
  ...
  br label %vector.body6322

vector.body6322:                                  ; preds = %vector.body6322, %vector.ph6320
  %index6323 = phi i64 [ 0, %vector.ph6320 ], [ %index.next6342, %vector.body6322 ] ; 3 uses
  %vec.phi6324.a = phi <16 x float> [ %i.eds, %vector.ph6320 ], [ %i.eoo, %vector.body6322 ]
  %vec.phi6325 = phi <16 x float> [ zeroinitializer, %vector.ph6320 ], [ %i.eop, %vector.body6322 ]
  %vec.phi6326 = phi <16 x float> [ %i.edt, %vector.ph6320 ], [ %i.eoe, %vector.body6322 ]
  %vec.phi6327 = phi <16 x float> [ zeroinitializer, %vector.ph6320 ], [ %i.eof, %vector.body6322 ]
  %i.edu = shl i64 %index6323, 3                  ; 2 uses
  %next.gep6328 = getelementptr i8, ptr %.2115124980.us, i64 %i.edu
  %i.edv = getelementptr i8, ptr %.2115124980.us, i64 %i.edu
  %next.gep6329 = getelementptr i8, ptr %i.edv, i64 128
  %i.edw = getelementptr inbounds nuw [4 x i8], ptr %i.edf, i64 %index6323 ; 2 uses
  %i.edx = getelementptr inbounds nuw i8, ptr %i.edw, i64 64
  %wide.load6330 = load <16 x i32>, ptr %i.edw, align 4, !tbaa !113
  %wide.load6331 = load <16 x i32>, ptr %i.edx, align 4, !tbaa !113
  %i.edy = sext <16 x i32> %wide.load6330 to <16 x i64> ; 16 uses
  %i.edz = sext <16 x i32> %wide.load6331 to <16 x i64> ; 16 uses
  %i.eea = extractelement <16 x i64> %i.edy, i64 0 ; 2 uses
  %i.eeb = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eea
  %i.eec = extractelement <16 x i64> %i.edy, i64 1 ; 2 uses
  %i.eed = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eec
  %i.eee = extractelement <16 x i64> %i.edy, i64 2 ; 2 uses
  %i.eef = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eee
  %i.eeg = extractelement <16 x i64> %i.edy, i64 3 ; 2 uses
  %i.eeh = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eeg
  %i.eei = extractelement <16 x i64> %i.edy, i64 4 ; 2 uses
  %i.eej = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eei
  %i.eek = extractelement <16 x i64> %i.edy, i64 5 ; 2 uses
  %i.eel = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eek
  %i.eem = extractelement <16 x i64> %i.edy, i64 6 ; 2 uses
  %i.een = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eem
  %i.eeo = extractelement <16 x i64> %i.edy, i64 7 ; 2 uses
  %i.eep = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eeo
  %i.eeq = extractelement <16 x i64> %i.edy, i64 8 ; 2 uses
  %i.eer = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eeq
  %i.ees = extractelement <16 x i64> %i.edy, i64 9 ; 2 uses
  %i.eet = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.ees
  %i.eeu = extractelement <16 x i64> %i.edy, i64 10 ; 2 uses
  %i.eev = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eeu
  %i.eew = extractelement <16 x i64> %i.edy, i64 11 ; 2 uses
  %i.eex = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eew
  %i.eey = extractelement <16 x i64> %i.edy, i64 12 ; 2 uses
  %i.eez = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.eey
  %i.efa = extractelement <16 x i64> %i.edy, i64 13 ; 2 uses
  %i.efb = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efa
  %i.efc = extractelement <16 x i64> %i.edy, i64 14 ; 2 uses
  %i.efd = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efc
  %i.efe = extractelement <16 x i64> %i.edy, i64 15 ; 2 uses
  %i.eff = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efe
  %i.efg = extractelement <16 x i64> %i.edz, i64 0 ; 2 uses
  %i.efh = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efg
  %i.efi = extractelement <16 x i64> %i.edz, i64 1 ; 2 uses
  %i.efj = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efi
  %i.efk = extractelement <16 x i64> %i.edz, i64 2 ; 2 uses
  %i.efl = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efk
  %i.efm = extractelement <16 x i64> %i.edz, i64 3 ; 2 uses
  %i.efn = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efm
  %i.efo = extractelement <16 x i64> %i.edz, i64 4 ; 2 uses
  %i.efp = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efo
  %i.efq = extractelement <16 x i64> %i.edz, i64 5 ; 2 uses
  %i.efr = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efq
  %i.efs = extractelement <16 x i64> %i.edz, i64 6 ; 2 uses
  %i.eft = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efs
  %i.efu = extractelement <16 x i64> %i.edz, i64 7 ; 2 uses
  %i.efv = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efu
  %i.efw = extractelement <16 x i64> %i.edz, i64 8 ; 2 uses
  %i.efx = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efw
  %i.efy = extractelement <16 x i64> %i.edz, i64 9 ; 2 uses
  %i.efz = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.efy
  %i.ega = extractelement <16 x i64> %i.edz, i64 10 ; 2 uses
  %i.egb = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.ega
  %i.egc = extractelement <16 x i64> %i.edz, i64 11 ; 2 uses
  %i.egd = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.egc
  %i.ege = extractelement <16 x i64> %i.edz, i64 12 ; 2 uses
  %i.egf = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.ege
  %i.egg = extractelement <16 x i64> %i.edz, i64 13 ; 2 uses
  %i.egh = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.egg
  %i.egi = extractelement <16 x i64> %i.edz, i64 14 ; 2 uses
  %i.egj = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.egi
  %i.egk = extractelement <16 x i64> %i.edz, i64 15 ; 2 uses
  %i.egl = getelementptr inbounds [2 x i8], ptr %gep4990.us, i64 %i.egk
  ...
  %wide.vec6332 = load <64 x i16>, ptr %next.gep6328, align 2, !tbaa !76 ; 4 uses
  %strided.vec6333.a = shufflevector <64 x i16> %wide.vec6332, <64 x i16> poison, <16 x i32> <i32 0, i32 4, i32 8, i32 12, i32 16, i32 20, i32 24, i32 28, i32 32, i32 36, i32 40, i32 44, i32 48, i32 52, i32 56, i32 60>
  %strided.vec6334 = shufflevector <64 x i16> %wide.vec6332, <64 x i16> poison, <16 x i32> <i32 1, i32 5, i32 9, i32 13, i32 17, i32 21, i32 25, i32 29, i32 33, i32 37, i32 41, i32 45, i32 49, i32 53, i32 57, i32 61>
  %strided.vec6335.a = shufflevector <64 x i16> %wide.vec6332, <64 x i16> poison, <16 x i32> <i32 2, i32 6, i32 10, i32 14, i32 18, i32 22, i32 26, i32 30, i32 34, i32 38, i32 42, i32 46, i32 50, i32 54, i32 58, i32 62>
  %strided.vec6336.a = shufflevector <64 x i16> %wide.vec6332, <64 x i16> poison, <16 x i32> <i32 3, i32 7, i32 11, i32 15, i32 19, i32 23, i32 27, i32 31, i32 35, i32 39, i32 43, i32 47, i32 51, i32 55, i32 59, i32 63>
  %wide.vec6337 = load <64 x i16>, ptr %next.gep6329, align 2, !tbaa !76 ; 4 uses
  %strided.vec6338.a = shufflevector <64 x i16> %wide.vec6337, <64 x i16> poison, <16 x i32> <i32 0, i32 4, i32 8, i32 12, i32 16, i32 20, i32 24, i32 28, i32 32, i32 36, i32 40, i32 44, i32 48, i32 52, i32 56, i32 60>
  %strided.vec6339 = shufflevector <64 x i16> %wide.vec6337, <64 x i16> poison, <16 x i32> <i32 1, i32 5, i32 9, i32 13, i32 17, i32 21, i32 25, i32 29, i32 33, i32 37, i32 41, i32 45, i32 49, i32 53, i32 57, i32 61>
  %strided.vec6340 = shufflevector <64 x i16> %wide.vec6337, <64 x i16> poison, <16 x i32> <i32 2, i32 6, i32 10, i32 14, i32 18, i32 22, i32 26, i32 30, i32 34, i32 38, i32 42, i32 46, i32 50, i32 54, i32 58, i32 62>
  %strided.vec6341 = shufflevector <64 x i16> %wide.vec6337, <64 x i16> poison, <16 x i32> <i32 3, i32 7, i32 11, i32 15, i32 19, i32 23, i32 27, i32 31, i32 35, i32 39, i32 43, i32 47, i32 51, i32 55, i32 59, i32 63>
  %i.eje = zext <16 x i16> %strided.vec6333.a to <16 x i32>
  %i.ejf = zext <16 x i16> %strided.vec6338.a to <16 x i32>
  ...
  %i.ejm = fadd fast <16 x float> %vec.phi6326, %i.ejk
  %i.ejn = fadd fast <16 x float> %vec.phi6327, %i.ejl
  %i.ejo = zext <16 x i16> %strided.vec6334 to <16 x i32>
end_hunk_5
begin_hunk_6_@_ZN4ncnnL24convolution_packed_bf16sERKNS_3MatERS0_S2_S2_iiiiiiiS2_RKNS_6OptionE:bb.a
  %.64052 = phi nsz <8 x float> [ %.040465103, %.loopexit4332 ], [ %i.gcx, %.loopexit4331.loopexit ], [ %.lcssa6493, %.loopexit4334.thread ], [ %.lcssa6495, %.loopexit4332.loopexit ], [ %.040465103, %bb.aw ] ; 2 uses
  %.15 = phi ptr [ %.95104, %.loopexit4332 ], [ %scevgep5756, %.loopexit4331.loopexit ], [ %scevgep5742, %.loopexit4334.thread ], [ %scevgep5749, %.loopexit4332.loopexit ], [ %.95104, %bb.aw ] ; 2 uses
  %i.gcz = add nuw nsw i32 %.15105, 8             ; 3 uses
  %i.gda = or disjoint i32 %i.gcz, 7
  %i.gdb = icmp slt i32 %i.gda, %i.q
  br i1 %i.gdb, label %_ZN4ncnn3MatD2Ev.exit2334, label %._crit_edge5106, !llvm.loop !275

._crit_edge5106:                                  ; preds = %.loopexit4331, %._crit_edge5075
  %.04046.lcssa = phi <8 x float> [ zeroinitializer, %._crit_edge5075 ], [ %.64052, %.loopexit4331 ] ; 2 uses
  %.9.lcssa = phi ptr [ %.01460.lcssa, %._crit_edge5075 ], [ %.15, %.loopexit4331 ] ; 2 uses
  %.1.lcssa = phi i32 [ %.01459.lcssa, %._crit_edge5075 ], [ %i.gcz, %.loopexit4331 ] ; 3 uses
  %i.gdc = shufflevector <8 x float> %.04046.lcssa, <8 x float> poison, <4 x i32> <i32 4, i32 5, i32 6, i32 7>
  %i.gdd = shufflevector <8 x float> %.04046.lcssa, <8 x float> poison, <4 x i32> <i32 0, i32 1, i32 2, i32 3>
  %i.gde = fadd fast <4 x float> %i.gdc, %i.gdd   ; 2 uses
  %i.gdf = or disjoint i32 %.1.lcssa, 3
  %i.gdg = icmp slt i32 %i.gdf, %i.q
  br i1 %i.gdg, label %_ZN4ncnn3MatD2Ev.exit2333.lr.ph, label %._crit_edge5130

_ZN4ncnn3MatD2Ev.exit2333.lr.ph:                  ; preds = %._crit_edge5106
  %i.gdh = load i32, ptr %i.k, align 4, !tbaa !114, !noalias !276
  %i.gdi = load ptr, ptr %0, align 8, !tbaa !9, !noalias !276
  %i.gdj = load i64, ptr %i.r, align 8, !tbaa !18, !noalias !276
  %i.gdk = load i64, ptr %i.dfd, align 8, !tbaa !19, !noalias !276 ; 2 uses
  %factor.op.mul5134 = mul i64 %i.gdj, %i.gdk
  %i.gdl = sext i32 %i.gdh to i64
  %i.gdm = mul i64 %i.gdk, %i.gdl
  %i.gdn = mul i64 %i.gdm, %i.fns
  %invariant.gep5136 = getelementptr i8, ptr %i.gdi, i64 %i.gdn
  %i.gdo = trunc nuw nsw i64 %indvars.iv5795 to i32
  %.reass5188 = mul i32 %factor.op.mul5183, %i.gdo
  %i.gdp = sext i32 %.reass5188 to i64
  %invariant.gep5137 = getelementptr [2 x i8], ptr %invariant.gep5136, i64 %i.gdp
  %i.gdq = load i32, ptr %i.f, align 4            ; 6 uses
  %i.gdr = icmp slt i32 %i.gdq, 1                 ; 2 uses
  %i.gds = load ptr, ptr %i.g, align 8            ; 4 uses
  %i.gdt = load i64, ptr %i.d, align 8            ; 3 uses
  %.idx = mul i64 %i.gdt, 6
  %.idx1923 = shl i64 %i.gdt, 2
  %i.gdu = add i32 %i.gdq, -1
  %i.gdv = zext i32 %i.gdu to i64
  %i.gdw = shl nuw nsw i64 %i.gdv, 3              ; 2 uses
  %brmerge5236 = select i1 %i.dfj, i1 true, i1 %i.gdr
  %wide.trip.count5765 = zext i32 %i.gdq to i64   ; 2 uses
  %brmerge5239 = select i1 %i.dfh, i1 true, i1 %i.gdr
  %wide.trip.count5772 = zext nneg i32 %i.gdq to i64
  %xtraiter6700 = and i64 %wide.trip.count5765, 1
  %i.gdx = icmp eq i32 %i.gdq, 1
  %unroll_iter6704 = and i64 %wide.trip.count5765, 2147483646
  %lcmp.mod6701.not = icmp eq i64 %xtraiter6700, 0
  %lcmp.mod6703 = trunc i32 %i.gdq to i1
  br label %_ZN4ncnn3MatD2Ev.exit2333

_ZN4ncnn3MatD2Ev.exit2333:                        ; preds = %_ZN4ncnn3MatD2Ev.exit2333.lr.ph, %.loopexit
  %.25129 = phi i32 [ %.1.lcssa, %_ZN4ncnn3MatD2Ev.exit2333.lr.ph ], [ %i.ghb, %.loopexit ] ; 2 uses
  %.165128 = phi ptr [ %.9.lcssa, %_ZN4ncnn3MatD2Ev.exit2333.lr.ph ], [ %.20, %.loopexit ] ; 6 uses
  %.040415127 = phi <4 x float> [ zeroinitializer, %_ZN4ncnn3MatD2Ev.exit2333.lr.ph ], [ %.44045, %.loopexit ] ; 4 uses
  %i.gdy = sdiv i32 %.25129, %i.n
  %i.gdz = sext i32 %i.gdy to i64
  %.reass5135 = mul i64 %factor.op.mul5134, %i.gdz
  %gep5138 = getelementptr i8, ptr %invariant.gep5137, i64 %.reass5135 ; 7 uses
  br i1 %brmerge5236, label %.loopexit4329, label %.lr.ph5118.preheader

.lr.ph5118.preheader:                             ; preds = %_ZN4ncnn3MatD2Ev.exit2333
  br i1 %i.gdx, label %.lr.ph5118.epil.preheader, label %.lr.ph5118

.lr.ph5118:                                       ; preds = %.lr.ph5118.preheader, %.lr.ph5118
  %indvars.iv5760 = phi i64 [ %indvars.iv.next5761.1, %.lr.ph5118 ], [ 0, %.lr.ph5118.preheader ] ; 3 uses
  %.175116 = phi ptr [ %i.gfg, %.lr.ph5118 ], [ %.165128, %.lr.ph5118.preheader ] ; 3 uses
  %.140425115 = phi <4 x float> [ %i.gff, %.lr.ph5118 ], [ %.040415127, %.lr.ph5118.preheader ]
  %niter6705 = phi i64 [ %niter6705.next.1, %.lr.ph5118 ], [ 0, %.lr.ph5118.preheader ]
  %i.gea = getelementptr inbounds nuw [4 x i8], ptr %i.gds, i64 %indvars.iv5760
  %i.geb = load i32, ptr %i.gea, align 4, !tbaa !113
  %i.gec = sext i32 %i.geb to i64
  %i.ged = getelementptr inbounds [2 x i8], ptr %gep5138, i64 %i.gec
  %i.gee = load i64, ptr %i.ged, align 1, !tbaa !20
  %i.gef = insertelement <2 x i64> poison, i64 %i.gee, i64 0
  %i.geg = bitcast <2 x i64> %i.gef to <8 x i16>
  %i.geh = shufflevector <8 x i16> <i16 0, i16 0, i16 0, i16 0, i16 poison, i16 poison, i16 poison, i16 poison>, <8 x i16> %i.geg, <8 x i32> <i32 0, i32 8, i32 1, i32 9, i32 2, i32 10, i32 3, i32 11>
  %i.gei = bitcast <8 x i16> %i.geh to <4 x float>
  %i.gej = load i64, ptr %.175116, align 1, !tbaa !20
  %i.gek = insertelement <2 x i64> poison, i64 %i.gej, i64 0
  %i.gel = bitcast <2 x i64> %i.gek to <8 x i16>
  %i.gem = shufflevector <8 x i16> <i16 0, i16 0, i16 0, i16 0, i16 poison, i16 poison, i16 poison, i16 poison>, <8 x i16> %i.gel, <8 x i32> <i32 0, i32 8, i32 1, i32 9, i32 2, i32 10, i32 3, i32 11>
  %i.gen = bitcast <8 x i16> %i.gem to <4 x float>
  %i.geo = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.gei, <4 x float> nofpclass(nan inf) %i.gen, <4 x float> nofpclass(nan inf) %.140425115)
  %i.gep = getelementptr inbounds nuw i8, ptr %.175116, i64 8
  %i.geq = getelementptr inbounds nuw [4 x i8], ptr %i.gds, i64 %indvars.iv5760
  %i.ger = getelementptr inbounds nuw i8, ptr %i.geq, i64 4
  %i.ges = load i32, ptr %i.ger, align 4, !tbaa !113
  %i.get = sext i32 %i.ges to i64
  %i.geu = getelementptr inbounds [2 x i8], ptr %gep5138, i64 %i.get
  %i.gev = load i64, ptr %i.geu, align 1, !tbaa !20
  %i.gew = insertelement <2 x i64> poison, i64 %i.gev, i64 0
  %i.gex = bitcast <2 x i64> %i.gew to <8 x i16>
  %i.gey = shufflevector <8 x i16> <i16 0, i16 0, i16 0, i16 0, i16 poison, i16 poison, i16 poison, i16 poison>, <8 x i16> %i.gex, <8 x i32> <i32 0, i32 8, i32 1, i32 9, i32 2, i32 10, i32 3, i32 11>
  %i.gez = bitcast <8 x i16> %i.gey to <4 x float>
  %i.gfa = load i64, ptr %i.gep, align 1, !tbaa !20
  %i.gfb = insertelement <2 x i64> poison, i64 %i.gfa, i64 0
  %i.gfc = bitcast <2 x i64> %i.gfb to <8 x i16>
  %i.gfd = shufflevector <8 x i16> <i16 0, i16 0, i16 0, i16 0, i16 poison, i16 poison, i16 poison, i16 poison>, <8 x i16> %i.gfc, <8 x i32> <i32 0, i32 8, i32 1, i32 9, i32 2, i32 10, i32 3, i32 11>
  %i.gfe = bitcast <8 x i16> %i.gfd to <4 x float>
  %i.gff = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.gez, <4 x float> nofpclass(nan inf) %i.gfe, <4 x float> nofpclass(nan inf) %i.geo) ; 3 uses
  %i.gfg = getelementptr inbounds nuw i8, ptr %.175116, i64 16 ; 2 uses
  %indvars.iv.next5761.1 = add nuw nsw i64 %indvars.iv5760, 2 ; 2 uses
  %niter6705.next.1 = add i64 %niter6705, 2       ; 2 uses
  %niter6705.ncmp.1 = icmp eq i64 %niter6705.next.1, %unroll_iter6704
  br i1 %niter6705.ncmp.1, label %.loopexit4329.thread.unr-lcssa, label %.lr.ph5118, !llvm.loop !279

.loopexit4329.thread.unr-lcssa:                   ; preds = %.lr.ph5118
  br i1 %lcmp.mod6701.not, label %.loopexit4329.thread, label %.lr.ph5118.epil.preheader

.lr.ph5118.epil.preheader:                        ; preds = %.loopexit4329.thread.unr-lcssa, %.lr.ph5118.preheader
  %indvars.iv5760.epil.init = phi i64 [ 0, %.lr.ph5118.preheader ], [ %indvars.iv.next5761.1, %.loopexit4329.thread.unr-lcssa ]
  %.175116.epil.init = phi ptr [ %.165128, %.lr.ph5118.preheader ], [ %i.gfg, %.loopexit4329.thread.unr-lcssa ]
  %.140425115.epil.init = phi <4 x float> [ %.040415127, %.lr.ph5118.preheader ], [ %i.gff, %.loopexit4329.thread.unr-lcssa ]
  call void @llvm.assume(i1 %lcmp.mod6703)
  %i.gfh = getelementptr inbounds nuw [4 x i8], ptr %i.gds, i64 %indvars.iv5760.epil.init
  %i.gfi = load i32, ptr %i.gfh, align 4, !tbaa !113
  %i.gfj = sext i32 %i.gfi to i64
  %i.gfk = getelementptr inbounds [2 x i8], ptr %gep5138, i64 %i.gfj
  %i.gfl = load i64, ptr %i.gfk, align 1, !tbaa !20
  %i.gfm = insertelement <2 x i64> poison, i64 %i.gfl, i64 0
  %i.gfn = bitcast <2 x i64> %i.gfm to <8 x i16>
  %i.gfo = shufflevector <8 x i16> <i16 0, i16 0, i16 0, i16 0, i16 poison, i16 poison, i16 poison, i16 poison>, <8 x i16> %i.gfn, <8 x i32> <i32 0, i32 8, i32 1, i32 9, i32 2, i32 10, i32 3, i32 11>
  %i.gfp = bitcast <8 x i16> %i.gfo to <4 x float>
  %i.gfq = load i64, ptr %.175116.epil.init, align 1, !tbaa !20
  ...
  %i.gfv = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.gfp, <4 x float> nofpclass(nan inf) %i.gfu, <4 x float> nofpclass(nan inf) %.140425115.epil.init)
  br label %.loopexit4329.thread

.loopexit4329.thread:                             ; preds = %.loopexit4329.thread.unr-lcssa, %.lr.ph5118.epil.preheader
  %.lcssa6497 = phi <4 x float> [ %i.gff, %.loopexit4329.thread.unr-lcssa ], [ %i.gfv, %.lr.ph5118.epil.preheader ]
  %scevgep5762 = getelementptr i8, ptr %.165128, i64 8
  %scevgep5763 = getelementptr i8, ptr %scevgep5762, i64 %i.gdw
  br label %.loopexit

.loopexit4329:                                    ; preds = %_ZN4ncnn3MatD2Ev.exit2333
  br i1 %brmerge5239, label %.loopexit, label %.lr.ph5124

.lr.ph5124:                                       ; preds = %.loopexit4329
  %i.gfw = getelementptr i8, ptr %gep5138, i64 %.idx
  %i.gfx = getelementptr i8, ptr %gep5138, i64 %.idx1923
  %i.gfy = getelementptr [2 x i8], ptr %gep5138, i64 %i.gdt
  br label %bb.ay

bb.ay:                                            ; preds = %.lr.ph5124, %bb.ay
  %indvars.iv5767 = phi i64 [ 0, %.lr.ph5124 ], [ %indvars.iv.next5768, %bb.ay ] ; 2 uses
  %.195122 = phi ptr [ %.165128, %.lr.ph5124 ], [ %i.gha, %bb.ay ] ; 2 uses
  %.340445121 = phi <4 x float> [ %.040415127, %.lr.ph5124 ], [ %i.ggz, %bb.ay ]
  %i.gfz = getelementptr inbounds nuw [4 x i8], ptr %i.gds, i64 %indvars.iv5767
  %i.gga = load i32, ptr %i.gfz, align 4, !tbaa !113
  %i.ggb = sext i32 %i.gga to i64                 ; 4 uses
  %i.ggc = getelementptr [2 x i8], ptr %i.gfw, i64 %i.ggb
  %i.ggd = load i16, ptr %i.ggc, align 2, !tbaa !76
  %i.gge = getelementptr [2 x i8], ptr %i.gfx, i64 %i.ggb
  %i.ggf = load i16, ptr %i.gge, align 2, !tbaa !76
  %i.ggg = getelementptr [2 x i8], ptr %i.gfy, i64 %i.ggb
  %i.ggh = load i16, ptr %i.ggg, align 2, !tbaa !76
  %i.ggi = getelementptr inbounds [2 x i8], ptr %gep5138, i64 %i.ggb
  %i.ggj = load i16, ptr %i.ggi, align 2, !tbaa !76
  %i.ggk = zext i16 %i.ggd to i32
  %i.ggl = zext i16 %i.ggf to i32
  %i.ggm = zext i16 %i.ggh to i32
  %i.ggn = zext i16 %i.ggj to i32
  %i.ggo = insertelement <4 x i32> poison, i32 %i.ggn, i64 0
  %i.ggp = insertelement <4 x i32> %i.ggo, i32 %i.ggm, i64 1
  %i.ggq = insertelement <4 x i32> %i.ggp, i32 %i.ggl, i64 2
  %i.ggr = insertelement <4 x i32> %i.ggq, i32 %i.ggk, i64 3
  %i.ggs = shl nuw <4 x i32> %i.ggr, splat (i32 16)
  %i.ggt = bitcast <4 x i32> %i.ggs to <4 x float>
  %i.ggu = load i64, ptr %.195122, align 1, !tbaa !20
  %i.ggv = insertelement <2 x i64> poison, i64 %i.ggu, i64 0
  %i.ggw = bitcast <2 x i64> %i.ggv to <8 x i16>
  %i.ggx = shufflevector <8 x i16> <i16 0, i16 0, i16 0, i16 0, i16 poison, i16 poison, i16 poison, i16 poison>, <8 x i16> %i.ggw, <8 x i32> <i32 0, i32 8, i32 1, i32 9, i32 2, i32 10, i32 3, i32 11>
  %i.ggy = bitcast <8 x i16> %i.ggx to <4 x float>
  %i.ggz = call fast noundef nofpclass(nan inf) <4 x float> @llvm.fma.v4f32(<4 x float> nofpclass(nan inf) %i.ggt, <4 x float> nofpclass(nan inf) %i.ggy, <4 x float> nofpclass(nan inf) %.340445121) ; 2 uses
  %i.gha = getelementptr inbounds nuw i8, ptr %.195122, i64 8
  %indvars.iv.next5768 = add nuw nsw i64 %indvars.iv5767, 1 ; 2 uses
  %exitcond5773.not = icmp eq i64 %indvars.iv.next5768, %wide.trip.count5772
  br i1 %exitcond5773.not, label %.loopexit.loopexit, label %bb.ay, !llvm.loop !280

.loopexit.loopexit:                               ; preds = %bb.ay
  %scevgep5769 = getelementptr i8, ptr %.165128, i64 8
  %scevgep5770 = getelementptr i8, ptr %scevgep5769, i64 %i.gdw
  br label %.loopexit

.loopexit:                                        ; preds = %.loopexit4329.thread, %.loopexit.loopexit, %.loopexit4329
  %.44045 = phi nsz <4 x float> [ %.040415127, %.loopexit4329 ], [ %i.ggz, %.loopexit.loopexit ], [ %.lcssa6497, %.loopexit4329.thread ] ; 2 uses
  %.20 = phi ptr [ %.165128, %.loopexit4329 ], [ %scevgep5770, %.loopexit.loopexit ], [ %scevgep5763, %.loopexit4329.thread ] ; 2 uses
  %i.ghb = add nuw nsw i32 %.25129, 4             ; 3 uses
  %i.ghc = or disjoint i32 %i.ghb, 3
  %i.ghd = icmp slt i32 %i.ghc, %i.q
  br i1 %i.ghd, label %_ZN4ncnn3MatD2Ev.exit2333, label %._crit_edge5130, !llvm.loop !281

._crit_edge5130:                                  ; preds = %.loopexit, %._crit_edge5106
  %.04041.lcssa = phi <4 x float> [ zeroinitializer, %._crit_edge5106 ], [ %.44045, %.loopexit ] ; 2 uses
  %.16.lcssa = phi ptr [ %.9.lcssa, %._crit_edge5106 ], [ %.20, %.loopexit ] ; 3 uses
  %.2.lcssa = phi i32 [ %.1.lcssa, %._crit_edge5106 ], [ %i.ghb, %.loopexit ] ; 6 uses
  %i.ghe = shufflevector <4 x float> %.04041.lcssa, <4 x float> %i.gde, <4 x i32> <i32 3, i32 2, i32 7, i32 6>
  %i.ghf = shufflevector <4 x float> %.04041.lcssa, <4 x float> %i.gde, <4 x i32> <i32 1, i32 0, i32 5, i32 4>
  %i.ghg = fadd fast <4 x float> %i.ghe, %i.ghf
  %op.rdx = call fast float @llvm.vector.reduce.fadd.v4f32(float %i.fwx, <4 x float> %i.ghg)
  %op.rdx6479 = call fast float @llvm.vector.reduce.fadd.v8f32(float %.01465, <8 x float> %i.fwy)
  %op.rdx6480 = fadd fast float %op.rdx, %op.rdx6479 ; 3 uses
  %i.ghh = or disjoint i32 %.2.lcssa, 1
  %i.ghi = icmp slt i32 %i.ghh, %i.q
  br i1 %i.ghi, label %_ZN4ncnn3MatD2Ev.exit2332.lr.ph, label %.preheader4341

_ZN4ncnn3MatD2Ev.exit2332.lr.ph:                  ; preds = %._crit_edge5130
  %i.ghj = load i32, ptr %i.k, align 4, !tbaa !114, !noalias !282
  %i.ghk = load ptr, ptr %0, align 8, !tbaa !9, !noalias !282
  %i.ghl = load i64, ptr %i.r, align 8, !tbaa !18, !noalias !282
  %i.ghm = load i64, ptr %i.dfd, align 8, !tbaa !19, !noalias !282 ; 2 uses
  %factor.op.mul5152 = mul i64 %i.ghl, %i.ghm
  %i.ghn = sext i32 %i.ghj to i64
  %i.gho = mul i64 %i.ghm, %i.ghn
  %i.ghp = mul i64 %i.gho, %i.fns
  %invariant.gep5154 = getelementptr i8, ptr %i.ghk, i64 %i.ghp
  %i.ghq = mul nsw i64 %indvars.iv5795, %i.foa
  %invariant.gep5155 = getelementptr [2 x i8], ptr %invariant.gep5154, i64 %i.ghq
  %i.ghr = load ptr, ptr %i.g, align 8            ; 3 uses
  %i.ghs = load i64, ptr %i.d, align 8
  br i1 %i.fnv, label %_ZN4ncnn3MatD2Ev.exit2332.us.preheader, label %_ZN4ncnn3MatD2Ev.exit2332.preheader

_ZN4ncnn3MatD2Ev.exit2332.preheader:              ; preds = %_ZN4ncnn3MatD2Ev.exit2332.lr.ph
  %i.ght = add i32 %.2.lcssa, 2
  %19 = add i32 %.2.lcssa, 3
  %smax5774 = call i32 @llvm.smax.i32(i32 %i.q, i32 %19)
  %20 = add i32 %smax5774, -2
  %i.ghu = sub i32 %20, %.2.lcssa
  %i.ghv = and i32 %i.ghu, -2
  %i.ghw = add i32 %i.ght, %i.ghv
  br label %.preheader4341

_ZN4ncnn3MatD2Ev.exit2332.us.preheader:           ; preds = %_ZN4ncnn3MatD2Ev.exit2332.lr.ph
  %i.ghx = zext i32 %.2.lcssa to i64
  br label %iter.check6459

iter.check6459:                                   ; preds = %_ZN4ncnn3MatD2Ev.exit2332.us.preheader, %._crit_edge5143.us
  %indvars.iv5782.a = phi i64 [ %i.ghx, %_ZN4ncnn3MatD2Ev.exit2332.us.preheader ], [ %indvars.iv.next5783.a, %._crit_edge5143.us ] ; 2 uses
  %.215147.us = phi ptr [ %.16.lcssa, %_ZN4ncnn3MatD2Ev.exit2332.us.preheader ], [ %scevgep5778, %._crit_edge5143.us ] ; 9 uses
  %.114665146.us = phi float [ %op.rdx6480, %_ZN4ncnn3MatD2Ev.exit2332.us.preheader ], [ %.lcssa6120, %._crit_edge5143.us ] ; 3 uses
  %.reass5153.us = mul i64 %factor.op.mul5152, %indvars.iv5782.a
  %gep5156.us = getelementptr i8, ptr %invariant.gep5155, i64 %.reass5153.us ; 74 uses
  %i.ghy = getelementptr [2 x i8], ptr %gep5156.us, i64 %i.ghs ; 73 uses
  br i1 %min.iters.check6420, label %vec.epilog.scalar.ph6460.preheader, label %vector.main.loop.iter.check6421

vector.main.loop.iter.check6421:                  ; preds = %iter.check6459
  br i1 %min.iters.check6422, label %vec.epilog.ph6463, label %vector.ph6423

vector.ph6423:                                    ; preds = %vector.main.loop.iter.check6421
  %i.ghz = getelementptr i8, ptr %.215147.us, i64 %i.foc
  %i.gia = insertelement <16 x float> <float poison, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00>, float %.114665146.us, i64 0
  br label %vector.body6425

vector.body6425:                                  ; preds = %vector.body6425, %vector.ph6423
  %index6426 = phi i64 [ 0, %vector.ph6423 ], [ %index.next6451, %vector.body6425 ] ; 3 uses
  %vec.phi6427.a = phi <16 x float> [ %i.gia, %vector.ph6423 ], [ %i.hcf, %vector.body6425 ]
  %vec.phi6428 = phi <16 x float> [ zeroinitializer, %vector.ph6423 ], [ %i.hcg, %vector.body6425 ]
  %vec.phi6429 = phi <16 x float> [ zeroinitializer, %vector.ph6423 ], [ %i.hch, %vector.body6425 ]
  %vec.phi6430 = phi <16 x float> [ zeroinitializer, %vector.ph6423 ], [ %i.hci, %vector.body6425 ]
  %i.gib = shl i64 %index6426, 2                  ; 4 uses
  %next.gep6431.a = getelementptr i8, ptr %.215147.us, i64 %i.gib
  %i.gic = getelementptr i8, ptr %.215147.us, i64 %i.gib
  %next.gep6432 = getelementptr i8, ptr %i.gic, i64 64
  %i.gid = getelementptr i8, ptr %.215147.us, i64 %i.gib
  %next.gep6433 = getelementptr i8, ptr %i.gid, i64 128
  %i.gie = getelementptr i8, ptr %.215147.us, i64 %i.gib
  %next.gep6434 = getelementptr i8, ptr %i.gie, i64 192
  %i.gif = getelementptr inbounds nuw [4 x i8], ptr %i.ghr, i64 %index6426 ; 4 uses
  %i.gig = getelementptr inbounds nuw i8, ptr %i.gif, i64 64
  %i.gih = getelementptr inbounds nuw i8, ptr %i.gif, i64 128
  %i.gii = getelementptr inbounds nuw i8, ptr %i.gif, i64 192
  %wide.load6435.a = load <16 x i32>, ptr %i.gif, align 4, !tbaa !113
  %wide.load6436 = load <16 x i32>, ptr %i.gig, align 4, !tbaa !113
  %wide.load6437 = load <16 x i32>, ptr %i.gih, align 4, !tbaa !113
  %wide.load6438 = load <16 x i32>, ptr %i.gii, align 4, !tbaa !113
  %i.gij = sext <16 x i32> %wide.load6435.a to <16 x i64> ; 16 uses
  %i.gik = sext <16 x i32> %wide.load6436 to <16 x i64> ; 16 uses
  %i.gil = sext <16 x i32> %wide.load6437 to <16 x i64> ; 16 uses
  %i.gim = sext <16 x i32> %wide.load6438 to <16 x i64> ; 16 uses
  %i.gin = extractelement <16 x i64> %i.gij, i64 0 ; 2 uses
  %i.gio = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gin
  %i.gip = extractelement <16 x i64> %i.gij, i64 1 ; 2 uses
  %i.giq = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gip
  %i.gir = extractelement <16 x i64> %i.gij, i64 2 ; 2 uses
  %i.gis = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gir
  %i.git = extractelement <16 x i64> %i.gij, i64 3 ; 2 uses
  %i.giu = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.git
  %i.giv = extractelement <16 x i64> %i.gij, i64 4 ; 2 uses
  %i.giw = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.giv
  %i.gix = extractelement <16 x i64> %i.gij, i64 5 ; 2 uses
  %i.giy = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gix
  %i.giz = extractelement <16 x i64> %i.gij, i64 6 ; 2 uses
  %i.gja = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.giz
  %i.gjb = extractelement <16 x i64> %i.gij, i64 7 ; 2 uses
  %i.gjc = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjb
  %i.gjd = extractelement <16 x i64> %i.gij, i64 8 ; 2 uses
  %i.gje = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjd
  %i.gjf = extractelement <16 x i64> %i.gij, i64 9 ; 2 uses
  %i.gjg = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjf
  %i.gjh = extractelement <16 x i64> %i.gij, i64 10 ; 2 uses
  %i.gji = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjh
  %i.gjj = extractelement <16 x i64> %i.gij, i64 11 ; 2 uses
  %i.gjk = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjj
  %i.gjl = extractelement <16 x i64> %i.gij, i64 12 ; 2 uses
  %i.gjm = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjl
  %i.gjn = extractelement <16 x i64> %i.gij, i64 13 ; 2 uses
  %i.gjo = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjn
  %i.gjp = extractelement <16 x i64> %i.gij, i64 14 ; 2 uses
  %i.gjq = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjp
  %i.gjr = extractelement <16 x i64> %i.gij, i64 15 ; 2 uses
  %i.gjs = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjr
  %i.gjt = extractelement <16 x i64> %i.gik, i64 0 ; 2 uses
  %i.gju = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjt
  %i.gjv = extractelement <16 x i64> %i.gik, i64 1 ; 2 uses
  %i.gjw = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjv
  %i.gjx = extractelement <16 x i64> %i.gik, i64 2 ; 2 uses
  %i.gjy = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjx
  %i.gjz = extractelement <16 x i64> %i.gik, i64 3 ; 2 uses
  %i.gka = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gjz
  %i.gkb = extractelement <16 x i64> %i.gik, i64 4 ; 2 uses
  %i.gkc = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkb
  %i.gkd = extractelement <16 x i64> %i.gik, i64 5 ; 2 uses
  %i.gke = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkd
  %i.gkf = extractelement <16 x i64> %i.gik, i64 6 ; 2 uses
  %i.gkg = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkf
  %i.gkh = extractelement <16 x i64> %i.gik, i64 7 ; 2 uses
  %i.gki = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkh
  %i.gkj = extractelement <16 x i64> %i.gik, i64 8 ; 2 uses
  %i.gkk = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkj
  %i.gkl = extractelement <16 x i64> %i.gik, i64 9 ; 2 uses
  %i.gkm = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkl
  %i.gkn = extractelement <16 x i64> %i.gik, i64 10 ; 2 uses
  %i.gko = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkn
  %i.gkp = extractelement <16 x i64> %i.gik, i64 11 ; 2 uses
  %i.gkq = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkp
  %i.gkr = extractelement <16 x i64> %i.gik, i64 12 ; 2 uses
  %i.gks = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkr
  %i.gkt = extractelement <16 x i64> %i.gik, i64 13 ; 2 uses
  %i.gku = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkt
  %i.gkv = extractelement <16 x i64> %i.gik, i64 14 ; 2 uses
  %i.gkw = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkv
  %i.gkx = extractelement <16 x i64> %i.gik, i64 15 ; 2 uses
  %i.gky = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkx
  %i.gkz = extractelement <16 x i64> %i.gil, i64 0 ; 2 uses
  %i.gla = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gkz
  %i.glb = extractelement <16 x i64> %i.gil, i64 1 ; 2 uses
  %i.glc = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glb
  %i.gld = extractelement <16 x i64> %i.gil, i64 2 ; 2 uses
  %i.gle = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gld
  %i.glf = extractelement <16 x i64> %i.gil, i64 3 ; 2 uses
  %i.glg = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glf
  %i.glh = extractelement <16 x i64> %i.gil, i64 4 ; 2 uses
  %i.gli = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glh
  %i.glj = extractelement <16 x i64> %i.gil, i64 5 ; 2 uses
  %i.glk = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glj
  %i.gll = extractelement <16 x i64> %i.gil, i64 6 ; 2 uses
  %i.glm = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gll
  %i.gln = extractelement <16 x i64> %i.gil, i64 7 ; 2 uses
  %i.glo = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gln
  %i.glp = extractelement <16 x i64> %i.gil, i64 8 ; 2 uses
  %i.glq = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glp
  %i.glr = extractelement <16 x i64> %i.gil, i64 9 ; 2 uses
  %i.gls = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glr
  %i.glt = extractelement <16 x i64> %i.gil, i64 10 ; 2 uses
  %i.glu = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glt
  %i.glv = extractelement <16 x i64> %i.gil, i64 11 ; 2 uses
  %i.glw = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glv
  %i.glx = extractelement <16 x i64> %i.gil, i64 12 ; 2 uses
  %i.gly = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glx
  %i.glz = extractelement <16 x i64> %i.gil, i64 13 ; 2 uses
  %i.gma = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.glz
  %i.gmb = extractelement <16 x i64> %i.gil, i64 14 ; 2 uses
  %i.gmc = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmb
  %i.gmd = extractelement <16 x i64> %i.gil, i64 15 ; 2 uses
  %i.gme = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmd
  %i.gmf = extractelement <16 x i64> %i.gim, i64 0 ; 2 uses
  %i.gmg = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmf
  %i.gmh = extractelement <16 x i64> %i.gim, i64 1 ; 2 uses
  %i.gmi = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmh
  %i.gmj = extractelement <16 x i64> %i.gim, i64 2 ; 2 uses
  %i.gmk = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmj
  %i.gml = extractelement <16 x i64> %i.gim, i64 3 ; 2 uses
  %i.gmm = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gml
  %i.gmn = extractelement <16 x i64> %i.gim, i64 4 ; 2 uses
  %i.gmo = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmn
  %i.gmp = extractelement <16 x i64> %i.gim, i64 5 ; 2 uses
  %i.gmq = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmp
  %i.gmr = extractelement <16 x i64> %i.gim, i64 6 ; 2 uses
  %i.gms = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmr
  %i.gmt = extractelement <16 x i64> %i.gim, i64 7 ; 2 uses
  %i.gmu = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmt
  %i.gmv = extractelement <16 x i64> %i.gim, i64 8 ; 2 uses
  %i.gmw = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmv
  %i.gmx = extractelement <16 x i64> %i.gim, i64 9 ; 2 uses
  %i.gmy = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmx
  %i.gmz = extractelement <16 x i64> %i.gim, i64 10 ; 2 uses
  %i.gna = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gmz
  %i.gnb = extractelement <16 x i64> %i.gim, i64 11 ; 2 uses
  %i.gnc = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gnb
  %i.gnd = extractelement <16 x i64> %i.gim, i64 12 ; 2 uses
  %i.gne = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gnd
  %i.gnf = extractelement <16 x i64> %i.gim, i64 13 ; 2 uses
  %i.gng = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gnf
  %i.gnh = extractelement <16 x i64> %i.gim, i64 14 ; 2 uses
  %i.gni = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gnh
  %i.gnj = extractelement <16 x i64> %i.gim, i64 15 ; 2 uses
  %i.gnk = getelementptr inbounds [2 x i8], ptr %gep5156.us, i64 %i.gnj
  %i.gnl = load i16, ptr %i.gio, align 2, !tbaa !76
  %i.gnm = load i16, ptr %i.giq, align 2, !tbaa !76
  %i.gnn = load i16, ptr %i.gis, align 2, !tbaa !76
  %i.gno = load i16, ptr %i.giu, align 2, !tbaa !76
  %i.gnp = load i16, ptr %i.giw, align 2, !tbaa !76
  %i.gnq = load i16, ptr %i.giy, align 2, !tbaa !76
  %i.gnr = load i16, ptr %i.gja, align 2, !tbaa !76
  %i.gns = load i16, ptr %i.gjc, align 2, !tbaa !76
  %i.gnt = load i16, ptr %i.gje, align 2, !tbaa !76
  %i.gnu = load i16, ptr %i.gjg, align 2, !tbaa !76
  %i.gnv = load i16, ptr %i.gji, align 2, !tbaa !76
  %i.gnw = load i16, ptr %i.gjk, align 2, !tbaa !76
  %i.gnx = load i16, ptr %i.gjm, align 2, !tbaa !76
  %i.gny = load i16, ptr %i.gjo, align 2, !tbaa !76
  %i.gnz = load i16, ptr %i.gjq, align 2, !tbaa !76
  %i.goa = load i16, ptr %i.gjs, align 2, !tbaa !76
  %i.gob = insertelement <16 x i16> poison, i16 %i.gnl, i64 0
  %i.goc = insertelement <16 x i16> %i.gob, i16 %i.gnm, i64 1
  %i.god = insertelement <16 x i16> %i.goc, i16 %i.gnn, i64 2
  %i.goe = insertelement <16 x i16> %i.god, i16 %i.gno, i64 3
  %i.gof = insertelement <16 x i16> %i.goe, i16 %i.gnp, i64 4
end_hunk_6
begin_hunk_7_@_ZN4ncnnL24convolution_packed_bf16sERKNS_3MatERS0_S2_S2_iiiiiiiS2_RKNS_6OptionE:bb.a
  %i.huv = zext i16 %i.huu to i32
  %i.huw = shl nuw i32 %i.huv, 16
  %i.hux = bitcast i32 %i.huw to float
  %i.huy = load i16, ptr %.245161.us, align 2, !tbaa !76
  %i.huz = zext i16 %i.huy to i32
  %i.hva = shl nuw i32 %i.huz, 16
  %i.hvb = bitcast i32 %i.hva to float
  %i.hvc = fmul fast float %i.hvb, %i.hux
  %i.hvd = fadd fast float %i.hvc, %.414695160.us ; 2 uses
  %i.hve = getelementptr inbounds nuw i8, ptr %.245161.us, i64 2
  %indvars.iv.next5786 = add nuw nsw i64 %indvars.iv5785, 1 ; 2 uses
  %exitcond5791.not = icmp eq i64 %indvars.iv.next5786, %wide.trip.count5790
  br i1 %exitcond5791.not, label %._crit_edge5164.us, label %vec.epilog.scalar.ph6403, !llvm.loop !294

._crit_edge5164.us:                               ; preds = %vec.epilog.scalar.ph6403, %vec.epilog.middle.block6415, %middle.block6395
  %.lcssa6121 = phi float [ %i.hup, %vec.epilog.middle.block6415 ], [ %i.hsu, %middle.block6395 ], [ %i.hvd, %vec.epilog.scalar.ph6403 ] ; 2 uses
  %scevgep5787 = getelementptr i8, ptr %.235168.us, i64 2
  %scevgep5788 = getelementptr i8, ptr %scevgep5787, i64 %i.fnz
  %indvars.iv.next5793.a = add nuw nsw i64 %indvars.iv5792, 1 ; 2 uses
  %i.hvf = trunc nuw i64 %indvars.iv.next5793.a to i32
  %i.hvg = icmp sgt i32 %i.q, %i.hvf
  br i1 %i.hvg, label %iter.check6402, label %._crit_edge5170, !llvm.loop !295

._crit_edge5170:                                  ; preds = %._crit_edge5164.us, %_ZN4ncnn3MatD2Ev.exit.lr.ph, %.preheader4341
  %.31468.lcssa = phi float [ %.11466.lcssa, %.preheader4341 ], [ %.11466.lcssa, %_ZN4ncnn3MatD2Ev.exit.lr.ph ], [ %.lcssa6121, %._crit_edge5164.us ] ; 13 uses
  switch i32 %i.fno, label %_ZL13activation_ssfiRKN4ncnn3MatE.exit [
    i32 1, label %bb.az
    i32 2, label %bb.ba
    i32 3, label %bb.bb
    i32 4, label %bb.bd
...
bb.az:                                            ; preds = %._crit_edge5170
  %i.hvh = call fast float @llvm.maxnum.f32(float nofpclass(nan inf) %.31468.lcssa, float 0.000000e+00)
  br label %_ZL13activation_ssfiRKN4ncnn3MatE.exit

bb.ba:                                            ; preds = %._crit_edge5170
  %i.hvi = load ptr, ptr %11, align 8, !tbaa !9
  %i.hvj = load float, ptr %i.hvi, align 4, !tbaa !74
  %i.hvk = fcmp fast ogt float %.31468.lcssa, 0.000000e+00
  %i.hvl = select fast i1 %i.hvk, float 1.000000e+00, float %i.hvj
  %i.hvm = fmul fast float %i.hvl, %.31468.lcssa
  br label %_ZL13activation_ssfiRKN4ncnn3MatE.exit

bb.bb:                                            ; preds = %._crit_edge5170
  %i.hvn = load ptr, ptr %11, align 8, !tbaa !9   ; 2 uses
  %i.hvo = load float, ptr %i.hvn, align 4, !tbaa !74
  %i.hvp = getelementptr inbounds nuw i8, ptr %i.hvn, i64 4
  %i.hvq = load float, ptr %i.hvp, align 4, !tbaa !74 ; 2 uses
  %.04108 = call nnan ninf nsz float @llvm.maxnum.f32(float %.31468.lcssa, float %i.hvo) ; 2 uses
  %i.hvr = fcmp fast ogt float %.04108, %i.hvq
  br i1 %i.hvr, label %bb.bc, label %_ZL13activation_ssfiRKN4ncnn3MatE.exit

bb.bc:                                            ; preds = %bb.bb
  br label %_ZL13activation_ssfiRKN4ncnn3MatE.exit

bb.bd:                                            ; preds = %._crit_edge5170
  ...
  %i.hvs = fneg fast float %.sroa.speculated
  %i.hvt = call fast float @llvm.exp.f32(float %i.hvs)
  %i.hvu = fadd fast float %i.hvt, 1.000000e+00
  %i.hvv = fdiv fast float 1.000000e+00, %i.hvu
  br label %_ZL13activation_ssfiRKN4ncnn3MatE.exit

bb.be:                                            ; preds = %._crit_edge5170
  %i.hvw = call fast float @llvm.exp.f32(float nofpclass(nan inf) %.31468.lcssa)
  %i.hvx = fadd fast float %i.hvw, 1.000000e+00
  %i.hvy = call fast float @llvm.log.f32(float %i.hvx)
  %i.hvz = call fast float @llvm.tanh.f32(float %i.hvy)
  %i.hwa = fmul fast float %i.hvz, %.31468.lcssa
  br label %_ZL13activation_ssfiRKN4ncnn3MatE.exit

bb.bf:                                            ; preds = %._crit_edge5170
  %i.hwb = load ptr, ptr %11, align 8, !tbaa !9   ; 2 uses
  %i.hwc = load float, ptr %i.hwb, align 4, !tbaa !74 ; 3 uses
  %i.hwd = getelementptr inbounds nuw i8, ptr %i.hwb, i64 4
  %i.hwe = load float, ptr %i.hwd, align 4, !tbaa !74 ; 2 uses
  %i.hwf = fneg fast float %i.hwe
  %i.hwg = fdiv fast float %i.hwf, %i.hwc         ; 2 uses
  %i.hwh = fcmp fast olt float %.31468.lcssa, %i.hwg
  br i1 %i.hwh, label %_ZL13activation_ssfiRKN4ncnn3MatE.exit, label %bb.bg

bb.bg:                                            ; preds = %bb.bf
  %i.hwi = fdiv fast float 1.000000e+00, %i.hwc
  %i.hwj = fadd fast float %i.hwg, %i.hwi
  %i.hwk = fcmp fast ogt float %.31468.lcssa, %i.hwj
  br i1 %i.hwk, label %_ZL13activation_ssfiRKN4ncnn3MatE.exit, label %bb.bh

bb.bh:                                            ; preds = %bb.bg
  %i.hwl = fmul fast float %i.hwc, %.31468.lcssa
  %i.hwm = fadd fast float %i.hwl, %i.hwe
  %i.hwn = fmul fast float %i.hwm, %.31468.lcssa
  br label %_ZL13activation_ssfiRKN4ncnn3MatE.exit

_ZL13activation_ssfiRKN4ncnn3MatE.exit:           ; preds = %bb.bh, %bb.bg, %bb.be, %bb.bd, %bb.bc, %bb.bb, %bb.ba, %bb.az, %._crit_edge5170, %bb.bf
  %.14109 = phi nsz float [ %.31468.lcssa, %._crit_edge5170 ], [ %i.hvh, %bb.az ], [ %i.hvm, %bb.ba ], [ %i.hvq, %bb.bc ], [ %.04108, %bb.bb ], [ %i.hvv, %bb.bd ], [ %i.hwa, %bb.be ], [ %i.hwn, %bb.bh ], [ %.31468.lcssa, %bb.bg ], [ 0.000000e+00, %bb.bf ]
  %i.hwo = bitcast float %.14109 to i32
  %i.hwp = lshr i32 %i.hwo, 16
  %i.hwq = trunc nuw i32 %i.hwp to i16
  store i16 %i.hwq, ptr %.114735178, align 2, !tbaa !76
  %i.hwr = getelementptr inbounds nuw i8, ptr %.114735178, i64 2 ; 2 uses
  %indvars.iv.next5796 = add nuw nsw i64 %indvars.iv5795, 1 ; 2 uses
  %exitcond5799.not = icmp eq i64 %indvars.iv.next5796, %wide.trip.count5798
  br i1 %exitcond5799.not, label %._crit_edge5181, label %bb.ar, !llvm.loop !296
}
...
.preheader1082:                                   ; preds = %.loopexit, %.preheader1083
  ...
  %.2.lcssa = phi i32 [ %.1.lcssa, %.preheader1083 ], [ %i.anm, %.loopexit ] ; 6 uses
...
_ZN4ncnn3MatD2Ev.exit724.preheader:               ; preds = %_ZN4ncnn3MatD2Ev.exit724.lr.ph
  ...
  %15 = add i32 %.2.lcssa, 3
  %smax = call i32 @llvm.smax.i32(i32 %i.ae, i32 %15)
  %16 = add i32 %smax, -2
  %i.aii = sub i32 %16, %.2.lcssa
...
bb.bd:                                            ; preds = %bb.ba
  ...
  call void @__clang_call_terminate(ptr %i.ir) #25
  unreachable
...
bb.bl:                                            ; preds = %bb.bi
  ...
  call void @__clang_call_terminate(ptr %i.jc) #25
  unreachable
...
.preheader5.i:                                    ; preds = %.preheader5.loopexit.i, %.preheader7.i
  ...
  %.2.lcssa.i = phi i32 [ %.1530.lcssa.i, %.preheader7.i ], [ %i.nu, %.preheader5.loopexit.i ] ; 7 uses
...
.preheader4.preheader.i:                          ; preds = %.lr.ph213.split.i
  %9 = add i32 %.2.lcssa.i, 3
  %smax.i = call i32 @llvm.smax.i32(i32 %.sroa.speculated60, i32 %9)
  %reass.sub = sub i32 %smax.i, %.2.lcssa.i
  %10 = and i32 %reass.sub, -2
  %i.qw = add i32 %.2.lcssa.i, %10
...
bb.i:                                             ; preds = %bb.h
  %i.on = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 16
...
bb.k:                                             ; preds = %.split.us._crit_edge, %bb.j, %bb.g
  %.pre-phi1031 = phi <16 x bfloat> [ %.pre1030, %.split.us._crit_edge ], [ %.pre-phi, %bb.j ], [ %i.og, %bb.g ] ; 3 uses
  %i.ot = getelementptr inbounds nuw [2 x i8], ptr %.0711853.us, i64 %i.m ; 2 uses
  %i.ou = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx743 ; 2 uses
  %i.ov = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx744 ; 2 uses
  %i.ow = bitcast <16 x bfloat> %.pre-phi1031 to <4 x i64> ; 2 uses
  %i.ox = extractelement <4 x i64> %i.ow, i64 0
  store i64 %i.ox, ptr %.0711853.us, align 1, !tbaa !20
  %i.oy = bitcast <16 x bfloat> %.pre-phi1031 to <32 x i8>
  ...
  %i.pd = bitcast <16 x bfloat> %.pre-phi1031 to <32 x i8>
...
bb.l:                                             ; preds = %bb.k
  %i.ph = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 8
...
bb.n:                                             ; preds = %bb.m, %bb.j, %bb.g, %.split.us
  %i.px = getelementptr inbounds nuw [2 x i8], ptr %.0711853.us, i64 %i.m ; 2 uses
  %i.py = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx743 ; 2 uses
  %i.pz = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx744 ; 2 uses
  %i.qa = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx747 ; 2 uses
  %i.qb = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx748 ; 2 uses
  %i.qc = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx749 ; 2 uses
  %i.qd = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx750 ; 2 uses
  %i.qe = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx751 ; 2 uses
  %i.qf = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx752 ; 2 uses
  %i.qg = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx753 ; 2 uses
  %i.qh = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx754 ; 2 uses
  %i.qi = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx755 ; 2 uses
  %i.qj = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx756 ; 2 uses
  %i.qk = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx757 ; 2 uses
  %i.ql = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 %.idx758 ; 2 uses
  %i.qm = bitcast <16 x float> %.0.i792657.us to <32 x i16> ; 16 uses
  %i.qn = extractelement <32 x i16> %i.qm, i64 1
  store i16 %i.qn, ptr %.0711853.us, align 2, !tbaa !76
...
bb.o:                                             ; preds = %bb.n
  ...
  %i.rf = getelementptr inbounds nuw i8, ptr %.0711853.us, i64 2
...
bb.p:                                             ; preds = %bb.o, %bb.n, %bb.m, %bb.j, %bb.g, %.split.us, %_ZL17activation_avx512Dv16_fiRKN4ncnn3MatE.exit.us
  %i.sk = getelementptr inbounds [2 x i8], ptr %.0711853.us, i64 %i.ah
  br label %bb.q

bb.q:                                             ; preds = %bb.p, %bb.c
  %.1712.us = phi ptr [ %.0711853.us, %bb.c ], [ %i.sk, %bb.p ]
  br i1 %i.dt, label %bb.c, label %bb.r, !llvm.loop !655

bb.r:                                             ; preds = %bb.q
  %indvars.iv.next951 = add nuw nsw i64 %indvars.iv950, 1 ; 2 uses
  %exitcond.not = icmp eq i64 %indvars.iv.next951, %wide.trip.count
  br i1 %exitcond.not, label %._crit_edge.us, label %_ZN4ncnn3MatD2Ev.exit766.us, !llvm.loop !656

._crit_edge.us:                                   ; preds = %bb.r
  call void @llvm.lifetime.end.p0(ptr nonnull %i.a) #10
  %indvars.iv.next954 = add nuw nsw i64 %indvars.iv953, 16 ; 3 uses
  %i.sl = or disjoint i64 %indvars.iv.next954, 15
  %i.sm = icmp samesign ult i64 %i.sl, %i.ai
  br i1 %i.sm, label %.lr.ph859.split.us, label %.preheader847.loopexit, !llvm.loop !657

.lr.ph859.split:                                  ; preds = %.lr.ph859
  %i.sn = and i32 %3, 2147483632
  br label %.preheader847

.preheader847.loopexit:                           ; preds = %._crit_edge.us
  %i.so = trunc nuw nsw i64 %indvars.iv.next954 to i32
  br label %.preheader847

.preheader847:                                    ; preds = %.lr.ph859.split, %.preheader847.loopexit, %bb.a
  %.0700.lcssa = phi i32 [ 0, %bb.a ], [ %i.sn, %.lr.ph859.split ], [ %i.so, %.preheader847.loopexit ] ; 6 uses
  %i.sp = or disjoint i32 %.0700.lcssa, 7
  %i.sq = icmp slt i32 %i.sp, %3
  br i1 %i.sq, label %.lr.ph873, label %.preheader846

.lr.ph873:                                        ; preds = %.preheader847
  ...
  %i.sw = sext i32 %factor.op.mul856 to i64
  ...
  br i1 %i.st, label %.lr.ph873.split.us.preheader, label %.lr.ph873.split

.lr.ph873.split.us.preheader:                     ; preds = %.lr.ph873
  %i.tf = sext i32 %.0700.lcssa to i64
  %i.tg = sext i32 %3 to i64
  %wide.trip.count967 = zext nneg i32 %5 to i64
  ...
  br label %.lr.ph873.split.us

.lr.ph873.split.us:                               ; preds = %.lr.ph873.split.us.preheader, %._crit_edge.us877
  %indvars.iv969 = phi i64 [ %i.tf, %.lr.ph873.split.us.preheader ], [ %indvars.iv.next970, %._crit_edge.us877 ] ; 4 uses
  br i1 %.not733, label %.lr.ph.us876, label %bb.s

bb.s:                                             ; preds = %.lr.ph873.split.us
  %i.tn = getelementptr inbounds nuw [4 x i8], ptr %i.ss, i64 %indvars.iv969
  %i.to = load <8 x float>, ptr %i.tn, align 1, !tbaa !20
  br label %.lr.ph.us876

.lr.ph.us876:                                     ; preds = %bb.s, %.lr.ph873.split.us
  %i.tp = phi fast <8 x float> [ %i.to, %bb.s ], [ zeroinitializer, %.lr.ph873.split.us ] ; 2 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.b) #10
  %i.tq = trunc nsw i64 %indvars.iv969 to i32
  %factor.op.mul.reass.us875 = mul i32 %factor.op.mul856, %i.tq
  %i.tr = sext i32 %factor.op.mul.reass.us875 to i64
  %i.ts = trunc i64 %indvars.iv969 to i32
  %i.tt = add i32 %2, %i.ts
  %i.tu = sdiv i32 %i.tt, %i.i
  %i.tv = sext i32 %i.tu to i64
  br label %_ZN4ncnn3MatD2Ev.exit765.us

_ZN4ncnn3MatD2Ev.exit765.us:                      ; preds = %.lr.ph.us876, %bb.ad
  %indvars.iv964 = phi i64 [ 0, %.lr.ph.us876 ], [ %indvars.iv.next965, %bb.ad ] ; 3 uses
  %i.tw = load ptr, ptr %0, align 8, !tbaa !9
  %i.tx = getelementptr inbounds [4 x i8], ptr %i.tw, i64 %i.tr
  %.idx1067 = shl nuw nsw i64 %indvars.iv964, 5
  %i.ty = getelementptr inbounds nuw i8, ptr %i.tx, i64 %.idx1067 ; 5 uses
  ...
  %i.vu = trunc i64 %indvars.iv964 to i32
...
bb.t:                                             ; preds = %bb.ac, %_ZN4ncnn3MatD2Ev.exit765.us
  ...
  %indvars.iv961.sroa.phi = phi ptr [ %indvars.iv961.sroa.gep, %bb.ac ], [ %i.b, %_ZN4ncnn3MatD2Ev.exit765.us ] ; 4 uses
  %indvars.iv961 = phi i32 [ 1, %bb.ac ], [ 0, %_ZN4ncnn3MatD2Ev.exit765.us ]
  %.0698868.us = phi ptr [ %.1699.us, %bb.ac ], [ %i.wo, %_ZN4ncnn3MatD2Ev.exit765.us ] ; 16 uses
  %i.ws = or disjoint i32 %i.wg, %indvars.iv961
  %.not734.us = icmp slt i32 %i.ws, %i.g
  br i1 %.not734.us, label %bb.u, label %bb.ac

bb.u:                                             ; preds = %bb.t
  %i.wt = load <8 x float>, ptr %indvars.iv961.sroa.phi, align 32, !tbaa !20
  %i.wu = getelementptr inbounds nuw i8, ptr %indvars.iv961.sroa.phi, i64 32
  %i.wv = load <8 x float>, ptr %i.wu, align 32, !tbaa !20 ; 2 uses
  %i.ww = getelementptr inbounds nuw i8, ptr %indvars.iv961.sroa.phi, i64 64
  %i.wx = load <8 x float>, ptr %i.ww, align 32, !tbaa !20 ; 2 uses
  %i.wy = getelementptr inbounds nuw i8, ptr %indvars.iv961.sroa.phi, i64 96
...
bb.v:                                             ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  %i.ahg = getelementptr inbounds nuw [2 x i8], ptr %.0698868.us, i64 %i.m ; 2 uses
  %i.ahh = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 %.idx735 ; 2 uses
  %i.ahi = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 %.idx736 ; 2 uses
  %i.ahj = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 %.idx737 ; 2 uses
  %i.ahk = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 %.idx738 ; 2 uses
  %i.ahl = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 %.idx739 ; 2 uses
  %i.ahm = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 %.idx740 ; 2 uses
  %i.ahn = bitcast <8 x float> %.0.i795675.us to <16 x i16> ; 8 uses
  %i.aho = extractelement <16 x i16> %i.ahn, i64 1
  store i16 %i.aho, ptr %.0698868.us, align 2, !tbaa !76
...
bb.w:                                             ; preds = %bb.v
  ...
  %i.ahy = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 2
...
bb.x:                                             ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  %i.ain = getelementptr inbounds nuw [2 x i8], ptr %.0698868.us, i64 %i.m ; 2 uses
  ...
  store i64 %i.aiq, ptr %.0698868.us, align 1, !tbaa !20
...
bb.y:                                             ; preds = %bb.x
  %i.ait = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 8
...
bb.z:                                             ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  ...
  store <8 x bfloat> %i.aja, ptr %.0698868.us, align 1, !tbaa !20
  br i1 %i.wq, label %bb.aa, label %bb.ab

bb.aa:                                            ; preds = %bb.z
  %i.ajb = getelementptr inbounds nuw i8, ptr %.0698868.us, i64 16
...
bb.ab:                                            ; preds = %bb.aa, %bb.z, %bb.y, %bb.x, %bb.w, %bb.v, %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  %i.ajd = getelementptr inbounds [2 x i8], ptr %.0698868.us, i64 %i.te
  br label %bb.ac

bb.ac:                                            ; preds = %bb.ab, %bb.t
  %.1699.us = phi ptr [ %.0698868.us, %bb.t ], [ %i.ajd, %bb.ab ]
...
bb.ad:                                            ; preds = %bb.ac
  ...
  br i1 %exitcond968.not, label %._crit_edge.us877, label %_ZN4ncnn3MatD2Ev.exit765.us, !llvm.loop !662

._crit_edge.us877:                                ; preds = %bb.ad
  call void @llvm.lifetime.end.p0(ptr nonnull %i.b) #10
  %indvars.iv.next970 = add nuw nsw i64 %indvars.iv969, 8 ; 3 uses
  %i.aje = icmp slt i64 %indvars.iv.next970, %invariant.op
  br i1 %i.aje, label %.lr.ph873.split.us, label %.preheader846.loopexit, !llvm.loop !663

.lr.ph873.split:                                  ; preds = %.lr.ph873
  %8 = add i32 %.0700.lcssa, 15
  %smax956 = tail call i32 @llvm.smax.i32(i32 %3, i32 %8)
  %i.ajf = add i32 %smax956, -8
...
.preheader846.loopexit:                           ; preds = %._crit_edge.us877
  %i.ajk = trunc nsw i64 %indvars.iv.next970 to i32
  br label %.preheader846

.preheader846:                                    ; preds = %.lr.ph873.split, %.preheader846.loopexit, %.preheader847
  %.1701.lcssa = phi i32 [ %.0700.lcssa, %.preheader847 ], [ %i.ajj, %.lr.ph873.split ], [ %i.ajk, %.preheader846.loopexit ] ; 6 uses
  %i.ajl = or disjoint i32 %.1701.lcssa, 3
  %i.ajm = icmp slt i32 %i.ajl, %3
  br i1 %i.ajm, label %.lr.ph892, label %.preheader845

.lr.ph892:                                        ; preds = %.preheader846
  ...
  %i.ajx = sext i32 %factor.op.mul856 to i64      ; 12 uses
  %i.ajy = getelementptr inbounds nuw i8, ptr %1, i64 16
  %.idx = shl i64 %i.m, 2
  %.idx732 = mul i64 %i.m, 6
  %i.ajz = mul nsw i32 %i.i, %i.e
  %i.aka = sext i32 %i.ajz to i64
  br i1 %i.ajp, label %.lr.ph892.split.us.preheader, label %.lr.ph892.split

.lr.ph892.split.us.preheader:                     ; preds = %.lr.ph892
  ...
  %invariant.op1070 = add nsw i64 %i.akc, -3
  br label %.lr.ph892.split.us

.lr.ph892.split.us:                               ; preds = %.lr.ph892.split.us.preheader, %._crit_edge.us896
  %indvars.iv986 = phi i64 [ %i.akb, %.lr.ph892.split.us.preheader ], [ %indvars.iv.next987, %._crit_edge.us896 ] ; 4 uses
  br i1 %.not730, label %.lr.ph.us895, label %bb.ae

bb.ae:                                            ; preds = %.lr.ph892.split.us
  %i.akj = getelementptr inbounds nuw [4 x i8], ptr %i.ajo, i64 %indvars.iv986
  %i.akk = load <4 x float>, ptr %i.akj, align 1, !tbaa !20
  br label %.lr.ph.us895

.lr.ph.us895:                                     ; preds = %bb.ae, %.lr.ph892.split.us
  %i.akl = phi fast <4 x float> [ %i.akk, %bb.ae ], [ zeroinitializer, %.lr.ph892.split.us ] ; 2 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.c) #10
  %i.akm = trunc nsw i64 %indvars.iv986 to i32
  %factor.op.mul.reass.us894 = mul i32 %factor.op.mul856, %i.akm
  %i.akn = sext i32 %factor.op.mul.reass.us894 to i64
  %i.ako = trunc i64 %indvars.iv986 to i32
  %i.akp = add i32 %2, %i.ako
  %i.akq = sdiv i32 %i.akp, %i.i
  %i.akr = sext i32 %i.akq to i64
  br label %_ZN4ncnn3MatD2Ev.exit764.us

_ZN4ncnn3MatD2Ev.exit764.us:                      ; preds = %.lr.ph.us895, %bb.an
  %indvars.iv981 = phi i64 [ 0, %.lr.ph.us895 ], [ %indvars.iv.next982, %bb.an ] ; 3 uses
  %i.aks = load ptr, ptr %0, align 8, !tbaa !9
  %i.akt = getelementptr inbounds [4 x i8], ptr %i.aks, i64 %i.akn
  %.idx1068 = shl nuw nsw i64 %indvars.iv981, 4
  %i.aku = getelementptr inbounds nuw i8, ptr %i.akt, i64 %.idx1068 ; 5 uses
  ...
  %i.amq = trunc i64 %indvars.iv981 to i32
...
bb.af:                                            ; preds = %bb.am, %_ZN4ncnn3MatD2Ev.exit764.us
  ...
  %indvars.iv978.sroa.phi = phi ptr [ %indvars.iv978.sroa.gep, %bb.am ], [ %i.c, %_ZN4ncnn3MatD2Ev.exit764.us ] ; 4 uses
  %indvars.iv978 = phi i32 [ 1, %bb.am ], [ 0, %_ZN4ncnn3MatD2Ev.exit764.us ]
  %.0689887.us = phi ptr [ %.1690.us, %bb.am ], [ %i.ank, %_ZN4ncnn3MatD2Ev.exit764.us ] ; 9 uses
  %i.ano = or disjoint i32 %i.anc, %indvars.iv978
  %.not731.us = icmp slt i32 %i.ano, %i.g
  br i1 %.not731.us, label %bb.ag, label %bb.am

bb.ag:                                            ; preds = %bb.af
  %i.anp = load <4 x float>, ptr %indvars.iv978.sroa.phi, align 16, !tbaa !20
  %i.anq = getelementptr inbounds nuw i8, ptr %indvars.iv978.sroa.phi, i64 16
  %i.anr = load <4 x float>, ptr %i.anq, align 16, !tbaa !20 ; 2 uses
  %i.ans = getelementptr inbounds nuw i8, ptr %indvars.iv978.sroa.phi, i64 32
  %i.ant = load <4 x float>, ptr %i.ans, align 16, !tbaa !20 ; 2 uses
  %i.anu = getelementptr inbounds nuw i8, ptr %indvars.iv978.sroa.phi, i64 48
...
bb.ah:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  %i.ayq = getelementptr inbounds nuw [2 x i8], ptr %.0689887.us, i64 %i.m ; 2 uses
  %i.ayr = getelementptr inbounds nuw i8, ptr %.0689887.us, i64 %.idx ; 2 uses
  %i.ays = getelementptr inbounds nuw i8, ptr %.0689887.us, i64 %.idx732 ; 2 uses
  %i.ayt = bitcast <4 x float> %.0.i798693.us to <8 x i16> ; 4 uses
  %i.ayu = extractelement <8 x i16> %i.ayt, i64 1
  store i16 %i.ayu, ptr %.0689887.us, align 2, !tbaa !76
...
bb.ai:                                            ; preds = %bb.ah
  ...
  %i.aza = getelementptr inbounds nuw i8, ptr %.0689887.us, i64 2
...
bb.aj:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  ...
  store i64 %i.azl, ptr %.0689887.us, align 1, !tbaa !20
  br i1 %i.anm, label %bb.ak, label %bb.al

bb.ak:                                            ; preds = %bb.aj
  %i.azm = getelementptr inbounds nuw i8, ptr %.0689887.us, i64 8
...
bb.al:                                            ; preds = %bb.ak, %bb.aj, %bb.ai, %bb.ah, %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  %i.azs = getelementptr inbounds [2 x i8], ptr %.0689887.us, i64 %i.aka
  br label %bb.am

bb.am:                                            ; preds = %bb.al, %bb.af
  %.1690.us = phi ptr [ %.0689887.us, %bb.af ], [ %i.azs, %bb.al ]
  br i1 %i.ann, label %bb.af, label %bb.an, !llvm.loop !667

bb.an:                                            ; preds = %bb.am
  %indvars.iv.next982 = add nuw nsw i64 %indvars.iv981, 1 ; 2 uses
  %exitcond985.not = icmp eq i64 %indvars.iv.next982, %wide.trip.count984
  br i1 %exitcond985.not, label %._crit_edge.us896, label %_ZN4ncnn3MatD2Ev.exit764.us, !llvm.loop !668

._crit_edge.us896:                                ; preds = %bb.an
  call void @llvm.lifetime.end.p0(ptr nonnull %i.c) #10
  %indvars.iv.next987 = add nuw nsw i64 %indvars.iv986, 4 ; 3 uses
  %i.azt = icmp slt i64 %indvars.iv.next987, %invariant.op1070
  br i1 %i.azt, label %.lr.ph892.split.us, label %.preheader845.loopexit, !llvm.loop !669

.lr.ph892.split:                                  ; preds = %.lr.ph892
  %9 = add i32 %.1701.lcssa, 7
  %smax973 = tail call i32 @llvm.smax.i32(i32 %3, i32 %9)
  %i.azu = add i32 %smax973, -4
...
.preheader845.loopexit:                           ; preds = %._crit_edge.us896
  %i.azz = trunc nsw i64 %indvars.iv.next987 to i32
  br label %.preheader845

.preheader845:                                    ; preds = %.lr.ph892.split, %.preheader845.loopexit, %.preheader846
  %.2.lcssa = phi i32 [ %.1701.lcssa, %.preheader846 ], [ %i.azy, %.lr.ph892.split ], [ %i.azz, %.preheader845.loopexit ] ; 3 uses
  %i.baa = or disjoint i32 %.2.lcssa, 1
  %i.bab = icmp slt i32 %i.baa, %3
  br i1 %i.bab, label %.lr.ph913, label %.preheader

.lr.ph913:                                        ; preds = %.preheader845
  ...
  %wide.trip.count999 = zext nneg i32 %5 to i64
  %invariant.op1071 = add nsw i64 %i.bao, -1
  ...
  %factor.op.mul909 = mul i64 %i.bau, %i.bav
  br label %bb.ao

.preheader.loopexit:                              ; preds = %._crit_edge
  %i.baw = trunc nsw i64 %indvars.iv.next1002 to i32
  br label %.preheader

.preheader:                                       ; preds = %.preheader.loopexit, %.preheader845
  %.3.lcssa = phi i32 [ %.2.lcssa, %.preheader845 ], [ %i.baw, %.preheader.loopexit ] ; 2 uses
  %i.bax = icmp slt i32 %.3.lcssa, %3
  br i1 %i.bax, label %.lr.ph931, label %._crit_edge932

.lr.ph931:                                        ; preds = %.preheader
  ...
  %wide.trip.count1019 = sext i32 %3 to i64
  %invariant.gep1072 = getelementptr [4 x i8], ptr %.0.val, i64 %i.bbj
  %wide.trip.count1014 = zext nneg i32 %5 to i64
  ...
  %factor.op.mul1077 = mul i64 %i.bbn, %i.bbo
  %i.bbp = sext i32 %i.bbl to i64
  %factor.op.mul926 = mul i64 %i.bbo, %i.bbp
  br label %bb.br

bb.ao:                                            ; preds = %.lr.ph913, %._crit_edge
  %indvars.iv1001 = phi i64 [ %i.ban, %.lr.ph913 ], [ %indvars.iv.next1002, %._crit_edge ] ; 4 uses
  br i1 %.not728, label %.thread, label %bb.ap

bb.ap:                                            ; preds = %bb.ao
  %gep = getelementptr [4 x i8], ptr %invariant.gep, i64 %indvars.iv1001
...
.lr.ph:                                           ; preds = %.thread
  %i.bbs = trunc nsw i64 %indvars.iv1001 to i32
  %factor.op.mul.reass = mul i32 %factor.op.mul856, %i.bbs
  %i.bbt = sext i32 %factor.op.mul.reass to i64
  %i.bbu = getelementptr inbounds [4 x i8], ptr %i.baq, i64 %i.bbt
  %i.bbv = add nsw i64 %indvars.iv1001, %i.bap
...
_ZN4ncnn3MatD2Ev.exit763:                         ; preds = %.lr.ph, %bb.bq
  %indvars.iv996 = phi i64 [ 0, %.lr.ph ], [ %indvars.iv.next997, %bb.bq ] ; 3 uses
  %.idx1069 = shl nuw nsw i64 %indvars.iv996, 3
  %i.bch = getelementptr inbounds nuw i8, ptr %i.bbu, i64 %.idx1069 ; 5 uses
  ...
  %i.bdo = trunc i64 %indvars.iv996 to i32
  ...
  %.reass910 = mul i64 %factor.op.mul909, %i.bdt
  %i.bdu = getelementptr inbounds nuw i8, ptr %i.bbw, i64 %.reass910
  ...
  br i1 %.not729, label %bb.aq, label %bb.bd
...
.preheader497:                                    ; preds = %._crit_edge1097, %.preheader498
  ...
  %.1899.lcssa = phi i32 [ %.0898.lcssa, %.preheader498 ], [ %i.bno, %._crit_edge1097 ] ; 9 uses
...
.lr.ph1120.split.split.preheader:                 ; preds = %.lr.ph1120.split
  ...
  %7 = add i32 %.1899.lcssa, 7
  %smax = tail call i32 @llvm.smax.i32(i32 %4, i32 %7)
  %8 = add i32 %smax, -4
  %i.blr = sub i32 %8, %.1899.lcssa               ; 2 uses
...
.lr.ph1120.split.split.us.preheader:              ; preds = %.lr.ph1120.split
  %9 = add i32 %.1899.lcssa, 7
  %smax1477 = tail call i32 @llvm.smax.i32(i32 %4, i32 %9)
  %10 = add i32 %smax1477, -4
  %i.bly = sub i32 %10, %.1899.lcssa              ; 2 uses
...
bb.aa:                                            ; preds = %bb.z
  ...
  %i.ahk = getelementptr inbounds nuw i8, ptr %.011482134.us, i64 2
...
bb.ac:                                            ; preds = %bb.ab
  ...
  %i.air = getelementptr inbounds nuw i8, ptr %.011482134.us, i64 4
...
bb.ae:                                            ; preds = %bb.ad
  ...
  %i.ajy = getelementptr inbounds nuw i8, ptr %.011482134.us, i64 6
...
bb.af:                                            ; preds = %bb.ae, %bb.ad, %bb.y, %bb.r, %bb.k, %.split.us, %_ZL17activation_avx512Dv16_fiRKN4ncnn3MatE.exit.us
  %i.ald = getelementptr inbounds [2 x i8], ptr %.011482134.us, i64 %i.ap
  br label %bb.ag

bb.ag:                                            ; preds = %bb.af, %bb.c
  %.11149.us = phi ptr [ %.011482134.us, %bb.c ], [ %i.ald, %bb.af ]
...
bb.ah:                                            ; preds = %bb.ag
  ...
  %exitcond2241.not = icmp eq i64 %indvars.iv.next2239, %wide.trip.count
  br i1 %exitcond2241.not, label %._crit_edge.us, label %_ZN4ncnn3MatD2Ev.exit1208.us, !llvm.loop !842

._crit_edge.us:                                   ; preds = %bb.ah
  call void @llvm.lifetime.end.p0(ptr nonnull %i.a) #10
  %indvars.iv.next2243 = add nuw nsw i64 %indvars.iv2242, 16 ; 3 uses
  %i.ale = or disjoint i64 %indvars.iv.next2243, 15
  %i.alf = icmp samesign ult i64 %i.ale, %i.aq
  br i1 %i.alf, label %.lr.ph2139.split.us, label %.preheader2125.loopexit, !llvm.loop !843

.lr.ph2139.split:                                 ; preds = %.lr.ph2139
  %i.alg = and i32 %3, 2147483632
  br label %.preheader2125

.preheader2125.loopexit:                          ; preds = %._crit_edge.us
  %i.alh = trunc nuw nsw i64 %indvars.iv.next2243 to i32
  br label %.preheader2125

.preheader2125:                                   ; preds = %.lr.ph2139.split, %.preheader2125.loopexit, %bb.a
  %.01116.lcssa = phi i32 [ 0, %bb.a ], [ %i.alg, %.lr.ph2139.split ], [ %i.alh, %.preheader2125.loopexit ] ; 6 uses
  %i.ali = or disjoint i32 %.01116.lcssa, 7
  %i.alj = icmp slt i32 %i.ali, %3
  br i1 %i.alj, label %.lr.ph2155, label %.preheader2124

.lr.ph2155:                                       ; preds = %.preheader2125
  ...
  br i1 %i.alm, label %.lr.ph2155.split.us.preheader, label %.lr.ph2155.split

.lr.ph2155.split.us.preheader:                    ; preds = %.lr.ph2155
  %i.amf = sext i32 %.01116.lcssa to i64
  %i.amg = sext i32 %3 to i64
  %wide.trip.count2257 = zext nneg i32 %5 to i64
  ...
  br label %.lr.ph2155.split.us

.lr.ph2155.split.us:                              ; preds = %.lr.ph2155.split.us.preheader, %._crit_edge.us2159
  %indvars.iv2259 = phi i64 [ %i.amf, %.lr.ph2155.split.us.preheader ], [ %indvars.iv.next2260, %._crit_edge.us2159 ] ; 4 uses
  br i1 %.not1175, label %.lr.ph.us2158, label %bb.ai

bb.ai:                                            ; preds = %.lr.ph2155.split.us
  %i.anb = getelementptr inbounds nuw [4 x i8], ptr %i.all, i64 %indvars.iv2259
  %i.anc = load <8 x float>, ptr %i.anb, align 1, !tbaa !20
  br label %.lr.ph.us2158

.lr.ph.us2158:                                    ; preds = %bb.ai, %.lr.ph2155.split.us
  %i.and = phi fast <8 x float> [ %i.anc, %bb.ai ], [ zeroinitializer, %.lr.ph2155.split.us ] ; 4 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.b) #10
  %i.ane = trunc nsw i64 %indvars.iv2259 to i32
  %factor.op.mul.reass.us2157 = mul i32 %factor.op.mul2136, %i.ane
  %i.anf = sext i32 %factor.op.mul.reass.us2157 to i64
  %i.ang = trunc i64 %indvars.iv2259 to i32
  %i.anh = add i32 %2, %i.ang
  %i.ani = sdiv i32 %i.anh, %i.j
  %i.anj = sext i32 %i.ani to i64
  br label %_ZN4ncnn3MatD2Ev.exit1207.us

_ZN4ncnn3MatD2Ev.exit1207.us:                     ; preds = %.lr.ph.us2158, %bb.bf
  %indvars.iv2254 = phi i64 [ 0, %.lr.ph.us2158 ], [ %indvars.iv.next2255, %bb.bf ] ; 3 uses
  %i.ank = load ptr, ptr %0, align 8, !tbaa !9
  %i.anl = getelementptr inbounds [4 x i8], ptr %i.ank, i64 %i.anf
  %.idx2347 = shl nuw nsw i64 %indvars.iv2254, 5
  %i.anm = getelementptr inbounds nuw i8, ptr %i.anl, i64 %.idx2347 ; 7 uses
...
bb.am:                                            ; preds = %bb.al
  ...
  %i.bpb = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 2
...
bb.ao:                                            ; preds = %bb.an
  ...
  %i.bps = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 4
...
bb.aq:                                            ; preds = %bb.ap
  ...
  %i.bqj = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 6
...
bb.ar:                                            ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  %i.bqy = getelementptr inbounds nuw [2 x i8], ptr %.011452150.us, i64 %i.n ; 4 uses
  ...
  store i64 %i.brb, ptr %.011452150.us, align 1, !tbaa !20
...
bb.as:                                            ; preds = %bb.ar
  %i.bre = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 8
...
bb.au:                                            ; preds = %bb.at
  %i.brl = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 16
...
bb.aw:                                            ; preds = %bb.av
  %i.brs = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 24
...
bb.ax:                                            ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  ...
  store <8 x bfloat> %i.brz, ptr %.011452150.us, align 1, !tbaa !20
  br i1 %i.atw, label %bb.ay, label %bb.az

bb.ay:                                            ; preds = %bb.ax
  %i.bsa = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 16
...
bb.ba:                                            ; preds = %bb.az
  %i.bsc = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 32
...
bb.bc:                                            ; preds = %bb.bb
  %i.bse = getelementptr inbounds nuw i8, ptr %.011452150.us, i64 48
...
bb.bd:                                            ; preds = %bb.bc, %bb.bb, %bb.aw, %bb.av, %bb.aq, %bb.ap, %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  %i.bsg = getelementptr inbounds [2 x i8], ptr %.011452150.us, i64 %i.ame
  br label %bb.be

bb.be:                                            ; preds = %bb.bd, %bb.aj
  %.11146.us = phi ptr [ %.011452150.us, %bb.aj ], [ %i.bsg, %bb.bd ]
  %indvars.iv.next2251 = add nuw nsw i64 %indvars.iv2250, 1 ; 2 uses
  %exitcond2253.not = icmp eq i64 %indvars.iv.next2251, 4
  br i1 %exitcond2253.not, label %bb.bf, label %bb.aj, !llvm.loop !847

bb.bf:                                            ; preds = %bb.be
  %indvars.iv.next2255 = add nuw nsw i64 %indvars.iv2254, 1 ; 2 uses
  %exitcond2258.not = icmp eq i64 %indvars.iv.next2255, %wide.trip.count2257
  br i1 %exitcond2258.not, label %._crit_edge.us2159, label %_ZN4ncnn3MatD2Ev.exit1207.us, !llvm.loop !848

._crit_edge.us2159:                               ; preds = %bb.bf
  call void @llvm.lifetime.end.p0(ptr nonnull %i.b) #10
  %indvars.iv.next2260 = add nuw nsw i64 %indvars.iv2259, 8 ; 3 uses
  %i.bsh = icmp slt i64 %indvars.iv.next2260, %invariant.op
  br i1 %i.bsh, label %.lr.ph2155.split.us, label %.preheader2124.loopexit, !llvm.loop !849

.lr.ph2155.split:                                 ; preds = %.lr.ph2155
  %8 = add i32 %.01116.lcssa, 15
  %smax2245 = tail call i32 @llvm.smax.i32(i32 %3, i32 %8)
  %i.bsi = add i32 %smax2245, -8
  %i.bsj = sub i32 %i.bsi, %.01116.lcssa
  %i.bsk = and i32 %i.bsj, -8
  %i.bsl = add i32 %.01116.lcssa, %i.bsk
  %i.bsm = add i32 %i.bsl, 8
  br label %.preheader2124

.preheader2124.loopexit:                          ; preds = %._crit_edge.us2159
  %i.bsn = trunc nsw i64 %indvars.iv.next2260 to i32
  br label %.preheader2124

.preheader2124:                                   ; preds = %.lr.ph2155.split, %.preheader2124.loopexit, %.preheader2125
  %.11117.lcssa = phi i32 [ %.01116.lcssa, %.preheader2125 ], [ %i.bsm, %.lr.ph2155.split ], [ %i.bsn, %.preheader2124.loopexit ] ; 6 uses
  %i.bso = or disjoint i32 %.11117.lcssa, 3
  %i.bsp = icmp slt i32 %i.bso, %3
  br i1 %i.bsp, label %.lr.ph2176, label %.preheader2123

.lr.ph2176:                                       ; preds = %.preheader2124
  ...
  br i1 %i.bss, label %.lr.ph2176.split.us.preheader, label %.lr.ph2176.split

.lr.ph2176.split.us.preheader:                    ; preds = %.lr.ph2176
  %i.btl = sext i32 %.11117.lcssa to i64
  %i.btm = sext i32 %3 to i64
  %wide.trip.count2275 = zext nneg i32 %5 to i64
  ...
  %invariant.op2350 = add nsw i64 %i.btm, -3
  br label %.lr.ph2176.split.us

.lr.ph2176.split.us:                              ; preds = %.lr.ph2176.split.us.preheader, %._crit_edge.us2180
  %indvars.iv2277 = phi i64 [ %i.btl, %.lr.ph2176.split.us.preheader ], [ %indvars.iv.next2278, %._crit_edge.us2180 ] ; 4 uses
  br i1 %.not1172, label %.lr.ph.us2179, label %bb.bg

bb.bg:                                            ; preds = %.lr.ph2176.split.us
  %i.buh = getelementptr inbounds nuw [4 x i8], ptr %i.bsr, i64 %indvars.iv2277
  %i.bui = load <4 x float>, ptr %i.buh, align 1, !tbaa !20
  br label %.lr.ph.us2179

.lr.ph.us2179:                                    ; preds = %bb.bg, %.lr.ph2176.split.us
  %i.buj = phi fast <4 x float> [ %i.bui, %bb.bg ], [ zeroinitializer, %.lr.ph2176.split.us ] ; 4 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.c) #10
  %i.buk = trunc nsw i64 %indvars.iv2277 to i32
  %factor.op.mul.reass.us2178 = mul i32 %factor.op.mul2136, %i.buk
  %i.bul = sext i32 %factor.op.mul.reass.us2178 to i64
  %i.bum = trunc i64 %indvars.iv2277 to i32
  %i.bun = add i32 %2, %i.bum
  %i.buo = sdiv i32 %i.bun, %i.j
  %i.bup = sext i32 %i.buo to i64
  br label %_ZN4ncnn3MatD2Ev.exit1206.us

_ZN4ncnn3MatD2Ev.exit1206.us:                     ; preds = %.lr.ph.us2179, %bb.bx
  %indvars.iv2272 = phi i64 [ 0, %.lr.ph.us2179 ], [ %indvars.iv.next2273, %bb.bx ] ; 3 uses
  %i.buq = load ptr, ptr %0, align 8, !tbaa !9
  %i.bur = getelementptr inbounds [4 x i8], ptr %i.buq, i64 %i.bul
  %.idx2348 = shl nuw nsw i64 %indvars.iv2272, 4
  %i.bus = getelementptr inbounds nuw i8, ptr %i.bur, i64 %.idx2348 ; 7 uses
...
bb.bj:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  %i.cwr = getelementptr inbounds nuw [2 x i8], ptr %.011272171.us, i64 %i.n ; 4 uses
  %i.cws = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 %.idx ; 4 uses
  %i.cwt = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 %.idx1174 ; 4 uses
  %i.cwu = bitcast <4 x float> %.0.i1252150715261561.us to <8 x i16> ; 4 uses
  %i.cwv = extractelement <8 x i16> %i.cwu, i64 1
  store i16 %i.cwv, ptr %.011272171.us, align 2, !tbaa !76
...
bb.bk:                                            ; preds = %bb.bj
  ...
  %i.cxb = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 2
...
bb.bm:                                            ; preds = %bb.bl
  ...
  %i.cxk = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 4
...
bb.bo:                                            ; preds = %bb.bn
  ...
  %i.cxt = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 6
...
bb.bp:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  ...
  store i64 %i.cye, ptr %.011272171.us, align 1, !tbaa !20
  br i1 %i.cbc, label %bb.bq, label %bb.br

bb.bq:                                            ; preds = %bb.bp
  %i.cyf = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 8
...
bb.bs:                                            ; preds = %bb.br
  %i.cyl = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 16
...
bb.bu:                                            ; preds = %bb.bt
  %i.cyr = getelementptr inbounds nuw i8, ptr %.011272171.us, i64 24
...
bb.bv:                                            ; preds = %bb.bu, %bb.bt, %bb.bo, %bb.bn, %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  %i.cyx = getelementptr inbounds [2 x i8], ptr %.011272171.us, i64 %i.btk
  br label %bb.bw

bb.bw:                                            ; preds = %bb.bv, %bb.bh
  %.11128.us = phi ptr [ %.011272171.us, %bb.bh ], [ %i.cyx, %bb.bv ]
  %indvars.iv.next2269 = add nuw nsw i64 %indvars.iv2268, 1 ; 2 uses
  %exitcond2271.not = icmp eq i64 %indvars.iv.next2269, 4
  br i1 %exitcond2271.not, label %bb.bx, label %bb.bh, !llvm.loop !853

bb.bx:                                            ; preds = %bb.bw
  %indvars.iv.next2273 = add nuw nsw i64 %indvars.iv2272, 1 ; 2 uses
  %exitcond2276.not = icmp eq i64 %indvars.iv.next2273, %wide.trip.count2275
  br i1 %exitcond2276.not, label %._crit_edge.us2180, label %_ZN4ncnn3MatD2Ev.exit1206.us, !llvm.loop !854

._crit_edge.us2180:                               ; preds = %bb.bx
  call void @llvm.lifetime.end.p0(ptr nonnull %i.c) #10
  %indvars.iv.next2278 = add nuw nsw i64 %indvars.iv2277, 4 ; 3 uses
  %i.cyy = icmp slt i64 %indvars.iv.next2278, %invariant.op2350
  br i1 %i.cyy, label %.lr.ph2176.split.us, label %.preheader2123.loopexit, !llvm.loop !855

.lr.ph2176.split:                                 ; preds = %.lr.ph2176
  %9 = add i32 %.11117.lcssa, 7
  %smax2263 = tail call i32 @llvm.smax.i32(i32 %3, i32 %9)
  %i.cyz = add i32 %smax2263, -4
  %i.cza = sub i32 %i.cyz, %.11117.lcssa
  %i.czb = and i32 %i.cza, -4
  %i.czc = add i32 %.11117.lcssa, %i.czb
  %i.czd = add i32 %i.czc, 4
  br label %.preheader2123

.preheader2123.loopexit:                          ; preds = %._crit_edge.us2180
  %i.cze = trunc nsw i64 %indvars.iv.next2278 to i32
  br label %.preheader2123

.preheader2123:                                   ; preds = %.lr.ph2176.split, %.preheader2123.loopexit, %.preheader2124
  %.2.lcssa = phi i32 [ %.11117.lcssa, %.preheader2124 ], [ %i.czd, %.lr.ph2176.split ], [ %i.cze, %.preheader2123.loopexit ] ; 3 uses
  %i.czf = or disjoint i32 %.2.lcssa, 1
  %i.czg = icmp slt i32 %i.czf, %3
  br i1 %i.czg, label %.lr.ph2199, label %.preheader

.lr.ph2199:                                       ; preds = %.preheader2123
  ...
  %wide.trip.count2291 = zext nneg i32 %5 to i64
  %invariant.op2351 = add nsw i64 %i.daa, -1
  ...
  %factor.op.mul2195 = mul i64 %i.dag, %i.dah
...
.preheader.loopexit:                              ; preds = %._crit_edge
  %i.dbc = trunc nsw i64 %indvars.iv.next2294 to i32
  br label %.preheader

.preheader:                                       ; preds = %.preheader.loopexit, %.preheader2123
  %.3.lcssa = phi i32 [ %.2.lcssa, %.preheader2123 ], [ %i.dbc, %.preheader.loopexit ] ; 2 uses
  %i.dbd = icmp slt i32 %.3.lcssa, %3
  br i1 %i.dbd, label %.lr.ph2219, label %._crit_edge2220
...
.lr.ph2219:                                       ; preds = %.preheader
  ...
  %wide.trip.count2312 = sext i32 %3 to i64
  %invariant.gep2352 = getelementptr [4 x i8], ptr %.0.val, i64 %i.dbt
  %wide.trip.count2307 = zext nneg i32 %5 to i64
  ...
  %factor.op.mul2379 = mul i64 %i.dbx, %i.dby
  %i.dbz = sext i32 %i.dbv to i64
  %factor.op.mul2214 = mul i64 %i.dby, %i.dbz
  %.sroa.0.64..sroa_idx2375 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 64
  %.sroa.0.20..sroa_idx2365 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 20
  %.sroa.0.4..sroa_idx2362 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 4
  %.sroa.0.8..sroa_idx2363 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 8
  %.sroa.0.12..sroa_idx2364 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 12
  %.sroa.0.24..sroa_idx2366 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 24
  %.sroa.0.44..sroa_idx2370 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 44
  %.sroa.0.28..sroa_idx2367 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 28
  %.sroa.0.32..sroa_idx2368 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 32
  %.sroa.0.36..sroa_idx2369 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 36
  %.sroa.0.48..sroa_idx2371 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 48
  %.sroa.0.68..sroa_idx2376 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 68
  %.sroa.0.52..sroa_idx2372 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 52
  %.sroa.0.56..sroa_idx2373 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 56
  %.sroa.0.60..sroa_idx2374 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 60
  %.sroa.0.72..sroa_idx2377 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 72
  %.sroa.0.76..sroa_idx2378 = getelementptr inbounds nuw i8, ptr %.sroa.0, i64 76
  br label %bb.dc

bb.by:                                            ; preds = %.lr.ph2199, %._crit_edge
  %indvars.iv2293 = phi i64 [ %i.czz, %.lr.ph2199 ], [ %indvars.iv.next2294, %._crit_edge ] ; 4 uses
  br i1 %.not1170, label %.thread, label %bb.bz

bb.bz:                                            ; preds = %bb.by
  %gep = getelementptr [4 x i8], ptr %invariant.gep, i64 %indvars.iv2293
...
.lr.ph:                                           ; preds = %.thread
  %i.dcc = trunc nsw i64 %indvars.iv2293 to i32
  %factor.op.mul.reass = mul i32 %factor.op.mul2136, %i.dcc
  %i.dcd = sext i32 %factor.op.mul.reass to i64
  %i.dce = getelementptr inbounds [4 x i8], ptr %i.dac, i64 %i.dcd
  %i.dcf = add nsw i64 %indvars.iv2293, %i.dab
...
_ZN4ncnn3MatD2Ev.exit1205:                        ; preds = %.lr.ph, %bb.ca
  %indvars.iv2288 = phi i64 [ 0, %.lr.ph ], [ %indvars.iv.next2289, %bb.ca ] ; 3 uses
  %.idx2349 = shl nuw nsw i64 %indvars.iv2288, 3
  %i.dch = getelementptr inbounds nuw i8, ptr %i.dce, i64 %.idx2349 ; 7 uses
...
bb.aq:                                            ; preds = %bb.ap
  ...
  %i.bch = getelementptr inbounds nuw i8, ptr %.016113828.us, i64 6
...
bb.as:                                            ; preds = %bb.ar
  ...
  %i.bdo = getelementptr inbounds nuw i8, ptr %.016113828.us, i64 8
...
bb.au:                                            ; preds = %bb.at
  ...
  %i.bev = getelementptr inbounds nuw i8, ptr %.016113828.us, i64 10
...
bb.av:                                            ; preds = %bb.au, %bb.at, %bb.ak, %bb.z, %bb.o, %.split.us, %_ZL17activation_avx512Dv16_fiRKN4ncnn3MatE.exit.us
  %i.bga = getelementptr inbounds [2 x i8], ptr %.016113828.us, i64 %i.av
  br label %bb.aw

bb.aw:                                            ; preds = %bb.av, %bb.c
  %.11612.us = phi ptr [ %.016113828.us, %bb.c ], [ %i.bga, %bb.av ]
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1 ; 2 uses
  %exitcond.not = icmp eq i64 %indvars.iv.next, 6
  br i1 %exitcond.not, label %bb.ax, label %bb.c, !llvm.loop !929

bb.ax:                                            ; preds = %bb.aw
  %indvars.iv.next3941 = add nuw nsw i64 %indvars.iv3940, 1 ; 2 uses
  %exitcond3943.not = icmp eq i64 %indvars.iv.next3941, %wide.trip.count
  br i1 %exitcond3943.not, label %._crit_edge.us, label %_ZN4ncnn3MatD2Ev.exit1686.us, !llvm.loop !930

._crit_edge.us:                                   ; preds = %bb.ax
  call void @llvm.lifetime.end.p0(ptr nonnull %i.a) #10
  %indvars.iv.next3945 = add nuw nsw i64 %indvars.iv3944, 16 ; 3 uses
  %i.bgb = or disjoint i64 %indvars.iv.next3945, 15
  %i.bgc = icmp samesign ult i64 %i.bgb, %i.ax
  br i1 %i.bgc, label %.lr.ph3833.split.us, label %.preheader3817.loopexit, !llvm.loop !931

.lr.ph3833.split:                                 ; preds = %.lr.ph3833
  %i.bgd = and i32 %3, 2147483632
  br label %.preheader3817

.preheader3817.loopexit:                          ; preds = %._crit_edge.us
  %i.bge = trunc nuw nsw i64 %indvars.iv.next3945 to i32
  br label %.preheader3817

.preheader3817:                                   ; preds = %.lr.ph3833.split, %.preheader3817.loopexit, %bb.a
  %.01586.lcssa = phi i32 [ 0, %bb.a ], [ %i.bgd, %.lr.ph3833.split ], [ %i.bge, %.preheader3817.loopexit ] ; 6 uses
  %i.bgf = or disjoint i32 %.01586.lcssa, 7
  %i.bgg = icmp slt i32 %i.bgf, %3
  br i1 %i.bgg, label %.lr.ph3851, label %.preheader3816

.lr.ph3851:                                       ; preds = %.preheader3817
  ...
  %i.bhd = sext i32 %factor.op.mul3830 to i64     ; 56 uses
  ...
  br i1 %i.bgj, label %.lr.ph3851.split.us.preheader, label %.lr.ph3851.split

.lr.ph3851.split.us.preheader:                    ; preds = %.lr.ph3851
  %i.bhh = sext i32 %i.i to i64
  %i.bhi = sext i32 %.01586.lcssa to i64
  %i.bhj = sext i32 %3 to i64
  %wide.trip.count3959 = zext nneg i32 %5 to i64
  ...
  %invariant.op4061.a = add nsw i64 %i.bhj, -7
  br label %.lr.ph3851.split.us

.lr.ph3851.split.us:                              ; preds = %.lr.ph3851.split.us.preheader, %._crit_edge.us3855
  %indvars.iv3961 = phi i64 [ %i.bhi, %.lr.ph3851.split.us.preheader ], [ %indvars.iv.next3962, %._crit_edge.us3855 ] ; 4 uses
  br i1 %.not1653, label %.lr.ph.us3854, label %bb.ay

bb.ay:                                            ; preds = %.lr.ph3851.split.us
  %i.bja = getelementptr inbounds nuw [4 x i8], ptr %i.bgi, i64 %indvars.iv3961
  %i.bjb = load <8 x float>, ptr %i.bja, align 1, !tbaa !20
  br label %.lr.ph.us3854

.lr.ph.us3854:                                    ; preds = %bb.ay, %.lr.ph3851.split.us
  %i.bjc = phi fast <8 x float> [ %i.bjb, %bb.ay ], [ zeroinitializer, %.lr.ph3851.split.us ] ; 6 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.b) #10
  %i.bjd = trunc nsw i64 %indvars.iv3961 to i32
  %factor.op.mul.reass.us3853 = mul i32 %factor.op.mul3830, %i.bjd
  %i.bje = sext i32 %factor.op.mul.reass.us3853 to i64
  %i.bjf = trunc i64 %indvars.iv3961 to i32
  %i.bjg = add i32 %2, %i.bjf
  %i.bjh = sdiv i32 %i.bjg, %i.k
  %i.bji = sext i32 %i.bjh to i64
  br label %_ZN4ncnn3MatD2Ev.exit1685.us

_ZN4ncnn3MatD2Ev.exit1685.us:                     ; preds = %.lr.ph.us3854, %bb.ch
  %indvars.iv3956 = phi i64 [ 0, %.lr.ph.us3854 ], [ %indvars.iv.next3957, %bb.ch ] ; 3 uses
  %i.bjj = load ptr, ptr %0, align 8, !tbaa !9
  %i.bjk = getelementptr inbounds [4 x i8], ptr %i.bjj, i64 %i.bje
  %.idx4057 = shl nuw nsw i64 %indvars.iv3956, 5
  %i.bjl = getelementptr inbounds nuw i8, ptr %i.bjk, i64 %.idx4057 ; 9 uses
...
bb.bk:                                            ; preds = %bb.bj
  ...
  %i.ddc = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 10
...
bb.bl:                                            ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  %i.ddr = getelementptr inbounds nuw [2 x i8], ptr %.016253846.us, i64 %i.o ; 6 uses
  ...
  store i64 %i.ddu, ptr %.016253846.us, align 1, !tbaa !20
...
bb.bm:                                            ; preds = %bb.bl
  %i.ddx = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 8
...
bb.bo:                                            ; preds = %bb.bn
  %i.dee = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 16
...
bb.bq:                                            ; preds = %bb.bp
  %i.del = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 24
...
bb.bs:                                            ; preds = %bb.br
  %i.des = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 32
...
bb.bu:                                            ; preds = %bb.bt
  %i.dez = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 40
...
bb.bv:                                            ; preds = %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  ...
  store <8 x bfloat> %i.dfg, ptr %.016253846.us, align 1, !tbaa !20
  br i1 %i.bvj, label %bb.bw, label %bb.bx

bb.bw:                                            ; preds = %bb.bv
  %i.dfh = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 16
...
bb.by:                                            ; preds = %bb.bx
  %i.dfj = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 32
...
bb.ca:                                            ; preds = %bb.bz
  %i.dfl = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 48
...
bb.cc:                                            ; preds = %bb.cb
  %i.dfn = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 64
...
bb.ce:                                            ; preds = %bb.cd
  %i.dfp = getelementptr inbounds nuw i8, ptr %.016253846.us, i64 80
...
bb.cf:                                            ; preds = %bb.ce, %bb.cd, %bb.bu, %bb.bt, %bb.bk, %bb.bj, %_ZL14activation_avxDv8_fiRKN4ncnn3MatE.exit.us
  %i.dfr = getelementptr inbounds [2 x i8], ptr %.016253846.us, i64 %i.bhg
  br label %bb.cg

bb.cg:                                            ; preds = %bb.cf, %bb.az
  %.11626.us = phi ptr [ %.016253846.us, %bb.az ], [ %i.dfr, %bb.cf ]
  %indvars.iv.next3953 = add nuw nsw i64 %indvars.iv3952, 1 ; 2 uses
  %exitcond3955.not = icmp eq i64 %indvars.iv.next3953, 6
  br i1 %exitcond3955.not, label %bb.ch, label %bb.az, !llvm.loop !935

bb.ch:                                            ; preds = %bb.cg
  %indvars.iv.next3957 = add nuw nsw i64 %indvars.iv3956, 1 ; 2 uses
  %exitcond3960.not = icmp eq i64 %indvars.iv.next3957, %wide.trip.count3959
  br i1 %exitcond3960.not, label %._crit_edge.us3855, label %_ZN4ncnn3MatD2Ev.exit1685.us, !llvm.loop !936

._crit_edge.us3855:                               ; preds = %bb.ch
  call void @llvm.lifetime.end.p0(ptr nonnull %i.b) #10
  %indvars.iv.next3962 = add nuw nsw i64 %indvars.iv3961, 8 ; 3 uses
  %i.dfs = icmp slt i64 %indvars.iv.next3962, %invariant.op4061.a
  br i1 %i.dfs, label %.lr.ph3851.split.us, label %.preheader3816.loopexit, !llvm.loop !937

.lr.ph3851.split:                                 ; preds = %.lr.ph3851
  %8 = add i32 %.01586.lcssa, 15
  %smax3947 = tail call i32 @llvm.smax.i32(i32 %3, i32 %8)
  %i.dft = add i32 %smax3947, -8
  %i.dfu = sub i32 %i.dft, %.01586.lcssa
  %i.dfv = and i32 %i.dfu, -8
  %i.dfw = add i32 %.01586.lcssa, %i.dfv
  %i.dfx = add i32 %i.dfw, 8
  br label %.preheader3816

.preheader3816.loopexit:                          ; preds = %._crit_edge.us3855
  %i.dfy = trunc nsw i64 %indvars.iv.next3962 to i32
  br label %.preheader3816

.preheader3816:                                   ; preds = %.lr.ph3851.split, %.preheader3816.loopexit, %.preheader3817
  %.11587.lcssa = phi i32 [ %.01586.lcssa, %.preheader3817 ], [ %i.dfx, %.lr.ph3851.split ], [ %i.dfy, %.preheader3816.loopexit ] ; 6 uses
  %i.dfz = or disjoint i32 %.11587.lcssa, 3
  %i.dga = icmp slt i32 %i.dfz, %3
  br i1 %i.dga, label %.lr.ph3874, label %.preheader3815

.lr.ph3874:                                       ; preds = %.preheader3816
  ...
  br i1 %i.dgd, label %.lr.ph3874.split.us.preheader, label %.lr.ph3874.split

.lr.ph3874.split.us.preheader:                    ; preds = %.lr.ph3874
  %i.dhc = sext i32 %i.i to i64
  %i.dhd = sext i32 %.11587.lcssa to i64
  %i.dhe = sext i32 %3 to i64
  %wide.trip.count3977 = zext nneg i32 %5 to i64
  ...
  %invariant.op4063.a = add nsw i64 %i.dhe, -3
  br label %.lr.ph3874.split.us

.lr.ph3874.split.us:                              ; preds = %.lr.ph3874.split.us.preheader, %._crit_edge.us3878
  %indvars.iv3979.a = phi i64 [ %i.dhd, %.lr.ph3874.split.us.preheader ], [ %indvars.iv.next3980.a, %._crit_edge.us3878 ] ; 4 uses
  br i1 %.not1650, label %.lr.ph.us3877, label %bb.ci

bb.ci:                                            ; preds = %.lr.ph3874.split.us
  %i.div = getelementptr inbounds nuw [4 x i8], ptr %i.dgc, i64 %indvars.iv3979.a
  %i.diw = load <4 x float>, ptr %i.div, align 1, !tbaa !20
  br label %.lr.ph.us3877

.lr.ph.us3877:                                    ; preds = %bb.ci, %.lr.ph3874.split.us
  %i.dix = phi fast <4 x float> [ %i.diw, %bb.ci ], [ zeroinitializer, %.lr.ph3874.split.us ] ; 6 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.c) #10
  %i.diy = trunc nsw i64 %indvars.iv3979.a to i32
  %factor.op.mul.reass.us3876 = mul i32 %factor.op.mul3830, %i.diy
  %i.diz = sext i32 %factor.op.mul.reass.us3876 to i64
  %i.dja = trunc i64 %indvars.iv3979.a to i32
  %i.djb = add i32 %2, %i.dja
  %i.djc = sdiv i32 %i.djb, %i.k
  %i.djd = sext i32 %i.djc to i64
  br label %_ZN4ncnn3MatD2Ev.exit1684.us

_ZN4ncnn3MatD2Ev.exit1684.us:                     ; preds = %.lr.ph.us3877, %bb.dh
  %indvars.iv3974 = phi i64 [ 0, %.lr.ph.us3877 ], [ %indvars.iv.next3975, %bb.dh ] ; 3 uses
  %i.dje = load ptr, ptr %0, align 8, !tbaa !9
  %i.djf = getelementptr inbounds [4 x i8], ptr %i.dje, i64 %i.diz
  %.idx4058 = shl nuw nsw i64 %indvars.iv3974, 4
  %i.djg = getelementptr inbounds nuw i8, ptr %i.djf, i64 %.idx4058 ; 9 uses
...
bb.cl:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  %i.fbf = getelementptr inbounds nuw [2 x i8], ptr %.016073869.us, i64 %i.o ; 6 uses
  %i.fbg = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 %.idx ; 6 uses
  %i.fbh = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 %.idx1652 ; 6 uses
  %i.fbi = bitcast <4 x float> %.0.i174224002419245425002561.us to <8 x i16> ; 4 uses
  %i.fbj = extractelement <8 x i16> %i.fbi, i64 1
  store i16 %i.fbj, ptr %.016073869.us, align 2, !tbaa !76
...
bb.cm:                                            ; preds = %bb.cl
  ...
  %i.fbp = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 2
...
bb.co:                                            ; preds = %bb.cn
  ...
  %i.fby = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 4
...
bb.cq:                                            ; preds = %bb.cp
  ...
  %i.fch = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 6
...
bb.cs:                                            ; preds = %bb.cr
  ...
  %i.fcq = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 8
...
bb.cu:                                            ; preds = %bb.ct
  ...
  %i.fcz = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 10
...
bb.cv:                                            ; preds = %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  ...
  store i64 %i.fdk, ptr %.016073869.us, align 1, !tbaa !20
  br i1 %i.dve, label %bb.cw, label %bb.cx

bb.cw:                                            ; preds = %bb.cv
  %i.fdl = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 8
...
bb.cy:                                            ; preds = %bb.cx
  %i.fdr = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 16
...
bb.da:                                            ; preds = %bb.cz
  %i.fdx = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 24
...
bb.dc:                                            ; preds = %bb.db
  %i.fed = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 32
...
bb.de:                                            ; preds = %bb.dd
  %i.fej = getelementptr inbounds nuw i8, ptr %.016073869.us, i64 40
...
bb.df:                                            ; preds = %bb.de, %bb.dd, %bb.cu, %bb.ct, %_ZL14activation_sseDv4_fiRKN4ncnn3MatE.exit.us
  %i.fep = getelementptr inbounds [2 x i8], ptr %.016073869.us, i64 %i.dhb
  br label %bb.dg

bb.dg:                                            ; preds = %bb.df, %bb.cj
  %.11608.us = phi ptr [ %.016073869.us, %bb.cj ], [ %i.fep, %bb.df ]
  %indvars.iv.next3971 = add nuw nsw i64 %indvars.iv3970, 1 ; 2 uses
  %exitcond3973.not = icmp eq i64 %indvars.iv.next3971, 6
  br i1 %exitcond3973.not, label %bb.dh, label %bb.cj, !llvm.loop !941

bb.dh:                                            ; preds = %bb.dg
  %indvars.iv.next3975 = add nuw nsw i64 %indvars.iv3974, 1 ; 2 uses
  %exitcond3978.not = icmp eq i64 %indvars.iv.next3975, %wide.trip.count3977
  br i1 %exitcond3978.not, label %._crit_edge.us3878, label %_ZN4ncnn3MatD2Ev.exit1684.us, !llvm.loop !942

._crit_edge.us3878:                               ; preds = %bb.dh
  call void @llvm.lifetime.end.p0(ptr nonnull %i.c) #10
  %indvars.iv.next3980.a = add nuw nsw i64 %indvars.iv3979.a, 4 ; 3 uses
  %i.feq = icmp slt i64 %indvars.iv.next3980.a, %invariant.op4063.a
  br i1 %i.feq, label %.lr.ph3874.split.us, label %.preheader3815.loopexit, !llvm.loop !943

.lr.ph3874.split:                                 ; preds = %.lr.ph3874
  %9 = add i32 %.11587.lcssa, 7
  %smax3965 = tail call i32 @llvm.smax.i32(i32 %3, i32 %9)
  %i.fer = add i32 %smax3965, -4
  %i.fes = sub i32 %i.fer, %.11587.lcssa
  %i.fet = and i32 %i.fes, -4
  %i.feu = add i32 %.11587.lcssa, %i.fet
  %i.fev = add i32 %i.feu, 4
  br label %.preheader3815

.preheader3815.loopexit:                          ; preds = %._crit_edge.us3878
  %i.few = trunc nsw i64 %indvars.iv.next3980.a to i32
  br label %.preheader3815

.preheader3815:                                   ; preds = %.lr.ph3874.split, %.preheader3815.loopexit, %.preheader3816
  %.2.lcssa = phi i32 [ %.11587.lcssa, %.preheader3816 ], [ %i.fev, %.lr.ph3874.split ], [ %i.few, %.preheader3815.loopexit ] ; 3 uses
  %i.fex = or disjoint i32 %.2.lcssa, 1
  %i.fey = icmp slt i32 %i.fex, %3
  br i1 %i.fey, label %.lr.ph3899, label %.preheader

.lr.ph3899:                                       ; preds = %.preheader3815
  ...
  %wide.trip.count3993 = zext nneg i32 %5 to i64
  %invariant.op4065 = add nsw i64 %i.ffz, -1
  ...
  %factor.op.mul3895 = mul i64 %i.fgf, %i.fgg
  br label %bb.di

.preheader.loopexit:                              ; preds = %._crit_edge
  %i.fgh = trunc nsw i64 %indvars.iv.next3996.a to i32
  br label %.preheader

.preheader:                                       ; preds = %.preheader.loopexit, %.preheader3815
  %.3.lcssa = phi i32 [ %.2.lcssa, %.preheader3815 ], [ %i.fgh, %.preheader.loopexit ] ; 2 uses
  %i.fgi = icmp slt i32 %.3.lcssa, %3
  br i1 %i.fgi, label %.lr.ph3921, label %._crit_edge3922

.lr.ph3921:                                       ; preds = %.preheader
  %.not = icmp eq ptr %.0.val, null
  ...
  %wide.trip.count4014 = sext i32 %3 to i64
  %invariant.gep4067 = getelementptr [4 x i8], ptr %.0.val, i64 %i.fhi
  %wide.trip.count4009 = zext nneg i32 %5 to i64
  ...
  %factor.op.mul4069 = mul i64 %i.fhm, %i.fhn
  %i.fho = sext i32 %i.fhk to i64
  %factor.op.mul3916 = mul i64 %i.fhn, %i.fho
  br label %bb.fa

bb.di:                                            ; preds = %.lr.ph3899, %._crit_edge
  %indvars.iv3995.a = phi i64 [ %i.ffy, %.lr.ph3899 ], [ %indvars.iv.next3996.a, %._crit_edge ] ; 4 uses
  br i1 %.not1648, label %.thread, label %bb.dj

bb.dj:                                            ; preds = %bb.di
  %gep = getelementptr [4 x i8], ptr %invariant.gep, i64 %indvars.iv3995.a
  %i.fhp = load <2 x float>, ptr %gep, align 4, !tbaa !74
  br label %.thread

.thread:                                          ; preds = %bb.di, %bb.dj
  %i.fhq = phi <2 x float> [ %i.fhp, %bb.dj ], [ zeroinitializer, %bb.di ] ; 3 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.d) #10
  br i1 %i.fez, label %.lr.ph, label %._crit_edge

.lr.ph:                                           ; preds = %.thread
  %i.fhr = trunc nsw i64 %indvars.iv3995.a to i32
  %factor.op.mul.reass = mul i32 %factor.op.mul3830, %i.fhr
  %i.fhs = sext i32 %factor.op.mul.reass to i64
  %i.fht = getelementptr inbounds [4 x i8], ptr %i.fgb, i64 %i.fhs
  %i.fhu = add nsw i64 %indvars.iv3995.a, %i.fga
  %.reass = mul i64 %factor.op.mul, %i.fhu
  %i.fhv = getelementptr inbounds nuw i8, ptr %i.fgd, i64 %.reass
  br label %bb.dk

bb.dk:                                            ; preds = %.lr.ph, %bb.dm
  %indvars.iv3990 = phi i64 [ 0, %.lr.ph ], [ %indvars.iv.next3991, %bb.dm ] ; 3 uses
  %.idx4059 = shl nuw nsw i64 %indvars.iv3990, 3
  %i.fhw = getelementptr inbounds nuw i8, ptr %i.fht, i64 %.idx4059 ; 8 uses
...
_ZN4ncnn3MatD2Ev.exit1683:                        ; preds = %bb.dl
  %i.fie = trunc i64 %indvars.iv3990 to i32
  ...
  %.reass3896 = mul i64 %factor.op.mul3895, %i.fij
  %i.fik = getelementptr inbounds nuw i8, ptr %i.fhv, i64 %.reass3896
  ...
  %invariant.op4064 = sub nsw i64 %i.ffx, %i.fij
...
bb.dl:                                            ; preds = %bb.dk, %bb.dl
  %indvars.iv3982 = phi i64 [ 0, %bb.dk ], [ %indvars.iv.next3983, %bb.dl ] ; 7 uses
  %.015973890 = phi ptr [ %i.fid, %bb.dk ], [ %i.flc, %bb.dl ] ; 2 uses
  %.015983889 = phi ptr [ %i.fic, %bb.dk ], [ %i.flb, %bb.dl ] ; 2 uses
  %.015993888 = phi ptr [ %i.fib, %bb.dk ], [ %i.fla, %bb.dl ] ; 2 uses
  %.016003887 = phi ptr [ %i.fia, %bb.dk ], [ %i.fkz, %bb.dl ] ; 2 uses
  %.016013886 = phi ptr [ %i.fhz, %bb.dk ], [ %i.fky, %bb.dl ] ; 2 uses
  %.016023885 = phi ptr [ %i.fhy, %bb.dk ], [ %i.fkx, %bb.dl ] ; 2 uses
  %.016033884 = phi ptr [ %i.fhx, %bb.dk ], [ %i.fkw, %bb.dl ] ; 2 uses
  %.016043883 = phi ptr [ %i.fhw, %bb.dk ], [ %i.fkv, %bb.dl ] ; 2 uses
  %i.fjd = getelementptr inbounds nuw [8 x i8], ptr %i.d, i64 %indvars.iv3982
  %i.fje = load <2 x float>, ptr %.016033884, align 4, !tbaa !74 ; 2 uses
  %i.fjf = load <2 x float>, ptr %.016023885, align 4, !tbaa !74 ; 2 uses
  %i.fjg = fadd fast <2 x float> %i.fjf, %i.fje   ; 3 uses
  %i.fjh = load <2 x float>, ptr %.016013886, align 4, !tbaa !74 ; 2 uses
  %i.fji = load <2 x float>, ptr %.016003887, align 4, !tbaa !74 ; 2 uses
  %i.fjj = fadd fast <2 x float> %i.fji, %i.fjh   ; 3 uses
  %i.fjk = load <2 x float>, ptr %.015993888, align 4, !tbaa !74 ; 2 uses
  %i.fjl = load <2 x float>, ptr %.015983889, align 4, !tbaa !74 ; 2 uses
  %i.fjm = fadd fast <2 x float> %i.fjl, %i.fjk   ; 3 uses
  %i.fjn = load <2 x float>, ptr %.016043883, align 4, !tbaa !74
  %i.fjo = fmul fast <2 x float> %i.fjm, splat (float 3.200000e+01)
  %i.fjp = fadd fast <2 x float> %i.fjj, %i.fjg
  %i.fjq = fadd fast <2 x float> %i.fjo, %i.fjp
end_hunk_21
...
bb.f:                                             ; preds = %bb.c
  ...
  call void @__clang_call_terminate(ptr %i.ev) #25
  unreachable
}
...
!104 = distinct !{!104, !22, !79, !80}
!105 = distinct !{!105, !22, !80, !79}
!106 = distinct !{!106, !22}
end_hunk_22
