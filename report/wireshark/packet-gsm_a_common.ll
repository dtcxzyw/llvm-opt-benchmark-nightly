...
bb.hh:                                            ; preds = %bb.he
  %i.xo = load i32, ptr @hf_gsm_a_ext_tsc_set_cap_support, align 4
  %i.xp = call ptr @proto_tree_add_bits_item(ptr noundef %1, i32 noundef %i.xo, ptr noundef %0, i32 noundef %i.xj, i32 noundef 1, i32 noundef 0) ; 0 uses
  %i.xq = add i32 %.22, 22                        ; 2 uses
  %i.xr = sub i32 %i.bo, %i.xq                    ; 2 uses
  %i.xs = icmp slt i32 %i.xr, 1
  br i1 %i.xs, label %bb.hi, label %bb.hk

bb.hi:                                            ; preds = %bb.hh
  %i.xt = icmp slt i32 %i.xr, 0
  br i1 %i.xt, label %bb.hj, label %bb.ho

bb.hj:                                            ; preds = %bb.hi
  %i.xu = call ptr @proto_tree_add_expert(ptr noundef %1, ptr noundef %2, ptr noundef nonnull @ei_gsm_a_ie_length_too_short, ptr noundef %0, i32 noundef %3, i32 noundef %4) ; 0 uses
  br label %bb.ho

bb.hk:                                            ; preds = %bb.hh
  %i.xv = load i32, ptr @hf_gsm_a_ext_earfcn_value_range, align 4
  %i.xw = call ptr @proto_tree_add_bits_item(ptr noundef %1, i32 noundef %i.xv, ptr noundef %0, i32 noundef %i.xq, i32 noundef 1, i32 noundef 0) ; 0 uses
  %i.xx = add i32 %.22, 23                        ; 3 uses
  %i.xy = sub i32 1, %.22
  %i.xz = and i32 %i.xy, 7                        ; 3 uses
  %.not1387 = icmp eq i32 %i.xz, 0
  br i1 %.not1387, label %bb.hm, label %bb.hl

bb.hl:                                            ; preds = %bb.hk
  %i.ya = load i32, ptr @hf_gsm_a_spare_bits, align 4
  %i.yb = call ptr @proto_tree_add_bits_item(ptr noundef %1, i32 noundef %i.ya, ptr noundef %0, i32 noundef %i.xx, i32 noundef %i.xz, i32 noundef 0) ; 0 uses
  %i.yc = add i32 %i.xz, %i.xx
  br label %bb.hm

bb.hm:                                            ; preds = %bb.hl, %bb.hk
  %.23 = phi i32 [ %i.yc, %bb.hl ], [ %i.xx, %bb.hk ]
  %i.yd = lshr i32 %.23, 3                        ; 2 uses
  %i.ye = sub i32 %i.yd, %3                       ; 2 uses
  %i.yf = icmp ugt i32 %4, %i.ye
  br i1 %i.yf, label %bb.hn, label %bb.ho

bb.hn:                                            ; preds = %bb.hm
  %i.yg = sub nuw i32 %4, %i.ye
  %i.yh = call ptr @proto_tree_add_expert(ptr noundef %1, ptr noundef %2, ptr noundef nonnull @ei_gsm_a_extraneous_data, ptr noundef %0, i32 noundef %i.yd, i32 noundef %i.yg) ; 0 uses
  br label %bb.ho
