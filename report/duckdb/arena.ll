...
.preheader:                                       ; preds = %duckdb_je_arena_slab_dalloc.exit, %bb.ai
  ...
  br i1 %i.ld, label %arena_bin_flush_batch_after_unlock.exit95.loopexit, label %.lr.ph188
...
.lr.ph188:                                        ; preds = %.preheader, %duckdb_je_arena_slab_dalloc.exit120
  ...
  %i.ln = load ptr, ptr %i.lm, align 8, !tbaa !120 ; 3 uses
  %i.lo = icmp eq ptr %i.ln, %.val106187
  br i1 %i.lo, label %edata_list_active_remove.exit, label %.thread.i
...
edata_list_active_remove.exit:                    ; preds = %.lr.ph188, %.thread.i
  %.val106 = phi ptr [ %i.ln, %.thread.i ], [ null, %.lr.ph188 ] ; 2 uses
...
duckdb_je_arena_slab_dalloc.exit120:              ; preds = %edata_list_active_remove.exit, %bb.al
  ...
  br i1 %i.ml, label %arena_bin_flush_batch_after_unlock.exit95.loopexit, label %.lr.ph188
...
.preheader150:                                    ; preds = %duckdb_je_arena_slab_dalloc.exit122, %bb.an
  ...
  br i1 %i.mw, label %arena_bin_flush_batch_after_unlock.exit.loopexit, label %.lr.ph182
...
.lr.ph182:                                        ; preds = %.preheader150, %duckdb_je_arena_slab_dalloc.exit126
  %.val108181 = phi ptr [ %7, %duckdb_je_arena_slab_dalloc.exit126 ], [ %.val108180, %.preheader150 ] ; 6 uses
  ...
  br i1 %i.oq, label %edata_list_active_remove.exit124, label %.thread.i123
...
edata_list_active_remove.exit124:                 ; preds = %.lr.ph182, %.thread.i123
  %7 = phi ptr [ %i.op, %.thread.i123 ], [ null, %.lr.ph182 ] ; 2 uses
...
duckdb_je_arena_slab_dalloc.exit126:              ; preds = %bb.bd, %bb.bc, %bb.bb, %edata_list_active_remove.exit124
  ...
  %i.qt = icmp eq ptr %7, null
  br i1 %i.qt, label %arena_bin_flush_batch_after_unlock.exit.loopexit, label %.lr.ph182
