...
.preheader:                                       ; preds = %duckdb_je_arena_slab_dalloc.exit, %bb.ai
  ...
  br i1 %i.ld, label %arena_bin_flush_batch_after_unlock.exit95, label %.lr.ph189
...
.lr.ph189:                                        ; preds = %.preheader, %duckdb_je_arena_slab_dalloc.exit120
  ...
  %i.lo = load ptr, ptr %i.ln, align 8, !tbaa !120 ; 4 uses
  store ptr %i.lo, ptr %i.ar, align 8, !tbaa !127
  %i.lp = icmp eq ptr %i.lo, %i.lm
  br i1 %i.lp, label %7, label %.thread.i
...
edata_list_active_remove.exit:                    ; preds = %.thread.i, %7
  %i.mg = phi ptr [ %i.lo, %.thread.i ], [ null, %7 ] ; 2 uses
...
duckdb_je_arena_slab_dalloc.exit120:              ; preds = %edata_list_active_remove.exit, %bb.al
  ...
  br i1 %i.mn, label %arena_bin_flush_batch_after_unlock.exit95, label %.lr.ph189
...
.preheader150:                                    ; preds = %duckdb_je_arena_slab_dalloc.exit122, %bb.an
  ...
  br i1 %i.my, label %arena_bin_flush_batch_after_unlock.exit, label %.lr.ph182
...
.lr.ph182:                                        ; preds = %.preheader150, %duckdb_je_arena_slab_dalloc.exit126
  %i.oq = phi ptr [ %.val108, %duckdb_je_arena_slab_dalloc.exit126 ], [ %.promoted180, %.preheader150 ] ; 6 uses
  %i.or = getelementptr inbounds nuw i8, ptr %i.oq, i64 40 ; 3 uses
  %i.os = load ptr, ptr %i.or, align 8, !tbaa !120 ; 3 uses
  store ptr %i.os, ptr %i.ar, align 8, !tbaa !127
  %i.ot = icmp eq ptr %i.os, %i.oq
  br i1 %i.ot, label %8, label %.thread.i123
...
duckdb_je_arena_slab_dalloc.exit126:              ; preds = %bb.bd, %bb.bc, %bb.bb, %edata_list_active_remove.exit124
  ...
  %i.qw = icmp eq ptr %.val108, null
  br i1 %i.qw, label %arena_bin_flush_batch_after_unlock.exit, label %.lr.ph182
