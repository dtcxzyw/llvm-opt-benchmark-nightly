...
.lr.ph.preheader:                                 ; preds = %.preheader112
  ...
  %i.bt = shl nsw i64 %i.bn, 3                    ; 2 uses
  %6 = add i64 %i.bt, %i.bs
  %7 = add i64 %6, 8
  %8 = call i64 @llvm.umax.i64(i64 %i.br, i64 %7)
  %i.bu = xor i64 %i.bs, -1
  %i.bv = add i64 %8, %i.bu
  %i.bw = sub i64 %i.bv, %i.bt                    ; 2 uses
  %i.bx = lshr i64 %i.bw, 3
  %i.by = add nuw nsw i64 %i.bx, 1                ; 2 uses
  %min.iters.check221 = icmp ult i64 %i.bw, 24
  br i1 %min.iters.check221, label %.lr.ph.preheader234, label %vector.memcheck218

vector.memcheck218:                               ; preds = %.lr.ph.preheader
  %i.bz = sub nsw i64 %i.bq, %i.bn
  %i.ca = shl nsw i64 %i.bz, 3
  %i.cb = add nsw i64 %i.ca, -1
  %diff.check219 = icmp ult i64 %i.cb, 31
  br i1 %diff.check219, label %.lr.ph.preheader234, label %vector.ph222

vector.ph222:                                     ; preds = %vector.memcheck218
  %n.vec223 = and i64 %i.by, 4611686018427387900  ; 5 uses
  %i.cc = add nsw i64 %n.vec223, %i.bq            ; 2 uses
  %i.cd = add nsw i64 %n.vec223, %i.bn            ; 2 uses
  %i.ce = shl i64 %n.vec223, 3
  %i.cf = getelementptr i8, ptr %i.bo, i64 %i.ce
...
vector.body224:                                   ; preds = %vector.body224, %vector.ph222
  ...
  %i.cg = shl i64 %index225, 3
  %next.gep = getelementptr i8, ptr %i.bo, i64 %i.cg ; 2 uses
  %i.ch = getelementptr i8, ptr %next.gep, i64 16
  %wide.load226 = load <2 x ptr>, ptr %next.gep, align 8, !tbaa !62
  %wide.load227 = load <2 x ptr>, ptr %i.ch, align 8, !tbaa !62
  ...
  store <2 x ptr> %wide.load227, ptr %i.ci, align 8, !tbaa !62
  %index.next228 = add nuw i64 %index225, 4       ; 2 uses
  %i.cj = icmp eq i64 %index.next228, %n.vec223
  br i1 %i.cj, label %middle.block229, label %vector.body224, !llvm.loop !126

middle.block229:                                  ; preds = %vector.body224
  %cmp.n230 = icmp eq i64 %i.by, %n.vec223
...
.lr.ph.preheader234:                              ; preds = %vector.memcheck218, %.lr.ph.preheader, %middle.block229
  %indvars.iv157.ph = phi i64 [ %i.bq, %vector.memcheck218 ], [ %i.bq, %.lr.ph.preheader ], [ %i.cc, %middle.block229 ]
  %indvars.iv.ph = phi i64 [ %i.bn, %vector.memcheck218 ], [ %i.bn, %.lr.ph.preheader ], [ %i.cd, %middle.block229 ]
  %.ph = phi ptr [ %i.bo, %vector.memcheck218 ], [ %i.bo, %.lr.ph.preheader ], [ %i.cf, %middle.block229 ]
  br label %.lr.ph

._crit_edge144:                                   ; preds = %.loopexit, %.loopexit.thread
  %.469191 = phi i32 [ %i.fx, %.loopexit.thread ], [ %.267, %.loopexit ] ; 2 uses
  %.pre170 = load ptr, ptr %i.aj, align 8, !tbaa !127 ; 2 uses
  %.pre171 = load ptr, ptr %i.am, align 8, !tbaa !128
  %i.ck = icmp eq ptr %.pre171, %.pre170
  br i1 %i.ck, label %_ZNSt6vectorIN10duckdb_re26SpliceESaIS1_EE5clearEv.exit, label %_ZSt8_DestroyIPN10duckdb_re26SpliceES1_EvT_S3_RSaIT0_E.exit.i.i
...
_ZSt8_DestroyIPN10duckdb_re26SpliceES1_EvT_S3_RSaIT0_E.exit.i.i: ; preds = %.preheader113, %._crit_edge144
  ...
  %i.cl = phi ptr [ %.pre170, %._crit_edge144 ], [ %i.al, %.preheader113 ]
  store ptr %i.cl, ptr %i.am, align 8, !tbaa !128
...
.lr.ph:                                           ; preds = %.lr.ph.preheader234, %.lr.ph
  ...
  %i.cm = phi ptr [ %i.cp, %.lr.ph ], [ %.ph, %.lr.ph.preheader234 ]
  %indvars.iv.next = add nsw i64 %indvars.iv, 1   ; 3 uses
  %i.cn = load ptr, ptr %i.cm, align 8, !tbaa !62
  %indvars.iv.next158 = add nsw i64 %indvars.iv157, 1 ; 2 uses
  %i.co = getelementptr inbounds [8 x i8], ptr %i.bk, i64 %indvars.iv157
  store ptr %i.cn, ptr %i.co, align 8, !tbaa !62
  %i.cp = getelementptr inbounds [8 x i8], ptr %i.bk, i64 %indvars.iv.next ; 2 uses
  %i.cq = icmp ult ptr %i.cp, %i.bm
  br i1 %i.cq, label %.lr.ph, label %._crit_edge.loopexit, !llvm.loop !129

._crit_edge.loopexit:                             ; preds = %.lr.ph, %middle.block229
  %indvars.iv.next.lcssa = phi i64 [ %i.cd, %middle.block229 ], [ %indvars.iv.next, %.lr.ph ]
  %indvars.iv.next158.lcssa = phi i64 [ %i.cc, %middle.block229 ], [ %indvars.iv.next158, %.lr.ph ]
  %i.cr = trunc nsw i64 %indvars.iv.next158.lcssa to i32
  %i.cs = trunc nsw i64 %indvars.iv.next.lcssa to i32
  br label %._crit_edge

._crit_edge:                                      ; preds = %._crit_edge.loopexit, %.preheader112
  %.166.lcssa = phi i32 [ %.065142, %.preheader112 ], [ %i.cr, %._crit_edge.loopexit ] ; 5 uses
  %.164.lcssa = phi i32 [ %.063143, %.preheader112 ], [ %i.cs, %._crit_edge.loopexit ] ; 3 uses
  %i.ct = load i32, ptr %i.ai, align 4, !tbaa !3
  switch i32 %i.ct, label %bb.m [
    i32 1, label %bb.h
    i32 2, label %bb.h
    i32 3, label %bb.l
  ]
...
bb.h:                                             ; preds = %._crit_edge, %._crit_edge
  ...
  %i.cu = load ptr, ptr %.sroa.099.0141, align 8, !tbaa !130
  store ptr %i.cu, ptr %i.c, align 16, !tbaa !62
  %i.cv = load ptr, ptr %i.bl, align 8, !tbaa !124
  %i.cw = getelementptr inbounds nuw i8, ptr %.sroa.099.0141, i64 20
  %i.cx = load i32, ptr %i.cw, align 4, !tbaa !131
  %i.cy = invoke noundef ptr @_ZN10duckdb_re26Regexp17AlternateNoFactorEPPS0_iNS0_10ParseFlagsE(ptr noundef %i.cv, i32 noundef %i.cx, i32 noundef %2)
