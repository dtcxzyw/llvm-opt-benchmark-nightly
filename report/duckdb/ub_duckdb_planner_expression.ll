...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %next.gep82 = getelementptr i8, ptr %i.y, i64 %offset.idx ; 4 uses
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %next.gep36 = getelementptr i8, ptr %i.v, i64 %offset.idx ; 4 uses
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %offset.idx = shl i64 %index, 3                 ; 2 uses
  %next.gep = getelementptr i8, ptr %i.aj, i64 %offset.idx ; 2 uses
  %next.gep57 = getelementptr i8, ptr %i.y, i64 %offset.idx ; 4 uses
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %next.gep48 = getelementptr i8, ptr %i.aw, i64 %offset.idx ; 4 uses
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %next.gep36 = getelementptr i8, ptr %i.z, i64 %offset.idx ; 4 uses
...
vector.ph:                                        ; preds = %vector.memcheck
  ...
  %i.cv = getelementptr i8, ptr %i.by, i64 %i.ct
  br label %vector.body
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %next.gep473 = getelementptr i8, ptr %i.by, i64 %offset.idx ; 4 uses
...
vector.ph486:                                     ; preds = %vector.memcheck476
  %n.vec488 = and i64 %i.ew, 4611686018427387900  ; 3 uses
  %i.fa = shl i64 %n.vec488, 3                    ; 2 uses
...
vector.body489:                                   ; preds = %vector.body489, %vector.ph486
  %index490 = phi i64 [ 0, %vector.ph486 ], [ %index.next497, %vector.body489 ] ; 2 uses
  %offset.idx491 = shl i64 %index490, 3           ; 2 uses
  %next.gep492 = getelementptr i8, ptr %i.eq, i64 %offset.idx491 ; 2 uses
  %next.gep494 = getelementptr i8, ptr %i.ef, i64 %offset.idx491 ; 4 uses
...
vector.ph538:                                     ; preds = %vector.memcheck528
  %n.vec540 = and i64 %i.hb, 4611686018427387900  ; 3 uses
  %i.hf = shl i64 %n.vec540, 3                    ; 2 uses
  %i.hg = getelementptr i8, ptr %i.gv, i64 %i.hf  ; 2 uses
  %i.hh = getelementptr i8, ptr %i.gk, i64 %i.hf
  br label %vector.body541

vector.body541:                                   ; preds = %vector.body541, %vector.ph538
  %index542 = phi i64 [ 0, %vector.ph538 ], [ %index.next549, %vector.body541 ] ; 2 uses
  %offset.idx543 = shl i64 %index542, 3           ; 2 uses
  %next.gep544 = getelementptr i8, ptr %i.gv, i64 %offset.idx543 ; 2 uses
  %next.gep546 = getelementptr i8, ptr %i.gk, i64 %offset.idx543 ; 4 uses
...
vector.ph512:                                     ; preds = %vector.memcheck502
  %n.vec514 = and i64 %i.iq, 4611686018427387900  ; 3 uses
  %i.iu = shl i64 %n.vec514, 3                    ; 2 uses
  %i.iv = getelementptr i8, ptr %i.il, i64 %i.iu  ; 2 uses
  %i.iw = getelementptr i8, ptr %i.ia, i64 %i.iu
  br label %vector.body515

vector.body515:                                   ; preds = %vector.body515, %vector.ph512
  %index516 = phi i64 [ 0, %vector.ph512 ], [ %index.next523, %vector.body515 ] ; 2 uses
  %offset.idx517 = shl i64 %index516, 3           ; 2 uses
  %next.gep518 = getelementptr i8, ptr %i.il, i64 %offset.idx517 ; 2 uses
  %next.gep520 = getelementptr i8, ptr %i.ia, i64 %offset.idx517 ; 4 uses
...
vector.ph590:                                     ; preds = %vector.memcheck580
  %n.vec592 = and i64 %i.sp, 4611686018427387900  ; 3 uses
  %i.st = shl i64 %n.vec592, 3                    ; 2 uses
  %i.su = getelementptr i8, ptr %i.sj, i64 %i.st  ; 2 uses
  %i.sv = getelementptr i8, ptr %i.ry, i64 %i.st
  br label %vector.body593

vector.body593:                                   ; preds = %vector.body593, %vector.ph590
  %index594 = phi i64 [ 0, %vector.ph590 ], [ %index.next601, %vector.body593 ] ; 2 uses
  %offset.idx595 = shl i64 %index594, 3           ; 2 uses
  %next.gep596 = getelementptr i8, ptr %i.sj, i64 %offset.idx595 ; 2 uses
  %next.gep598 = getelementptr i8, ptr %i.ry, i64 %offset.idx595 ; 4 uses
...
vector.ph564:                                     ; preds = %vector.memcheck554
  %n.vec566 = and i64 %i.ue, 4611686018427387900  ; 3 uses
  %i.ui = shl i64 %n.vec566, 3                    ; 2 uses
  %i.uj = getelementptr i8, ptr %i.tz, i64 %i.ui  ; 2 uses
  %i.uk = getelementptr i8, ptr %i.to, i64 %i.ui
  br label %vector.body567

vector.body567:                                   ; preds = %vector.body567, %vector.ph564
  %index568 = phi i64 [ 0, %vector.ph564 ], [ %index.next575, %vector.body567 ] ; 2 uses
  %offset.idx569 = shl i64 %index568, 3           ; 2 uses
  %next.gep570 = getelementptr i8, ptr %i.tz, i64 %offset.idx569 ; 2 uses
  %next.gep572 = getelementptr i8, ptr %i.to, i64 %offset.idx569 ; 4 uses
...
vector.ph:                                        ; preds = %vector.memcheck
  ...
  %i.bc = getelementptr i8, ptr %i.k, i64 %i.ba
  br label %vector.body
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %next.gep41 = getelementptr i8, ptr %i.k, i64 %offset.idx ; 4 uses
