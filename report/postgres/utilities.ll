...
select.unfold:                                    ; preds = %select.unfold.outer, %.thread149
  %.071 = phi i32 [ %i.ah, %.thread149 ], [ 0, %select.unfold.outer ] ; 4 uses
  %.069 = phi i32 [ %i.ai, %.thread149 ], [ %2, %select.unfold.outer ] ; 4 uses
  %.066 = phi i32 [ %i.ag, %.thread149 ], [ 0, %select.unfold.outer ] ; 4 uses
  %.064 = phi i32 [ %i.aj, %.thread149 ], [ 0, %select.unfold.outer ] ; 5 uses
  %i.h = sub i32 %.069, %.071
  %i.i = ashr i32 %i.h, 1
  %i.j = add i32 %i.i, %.071                      ; 6 uses
  %i.k = tail call i32 @llvm.smin.i32(i32 %.066, i32 %.064) ; 3 uses
...
.thread149:                                       ; preds = %bb.c, %.split153.thread.loopexit, %._crit_edge, %select.unfold, %.split153.thread
  %i.ag = phi i32 [ %i.ad, %._crit_edge ], [ %.066, %.split153.thread ], [ %i.k, %select.unfold ], [ %.066, %.split153.thread.loopexit ], [ %i.n, %bb.c ] ; 3 uses
  %i.ah = phi i32 [ %i.j, %._crit_edge ], [ %.071, %.split153.thread ], [ %i.j, %select.unfold ], [ %.071, %.split153.thread.loopexit ], [ %i.j, %bb.c ] ; 6 uses
  %i.ai = phi i32 [ %.069, %._crit_edge ], [ %i.j, %.split153.thread ], [ %.069, %select.unfold ], [ %i.j, %.split153.thread.loopexit ], [ %.069, %bb.c ] ; 4 uses
  %i.aj = phi i32 [ %.064, %._crit_edge ], [ %i.ad, %.split153.thread ], [ %.064, %select.unfold ], [ %i.af, %.split153.thread.loopexit ], [ %.064, %bb.c ] ; 2 uses
  %i.ak = sub i32 %i.ai, %i.ah
...
select.unfold.1:                                  ; preds = %bb.d, %.thread149.1
  %.071.1 = phi i32 [ %i.bo, %.thread149.1 ], [ %i.ah, %bb.d ] ; 4 uses
  %.069.1 = phi i32 [ %i.bp, %.thread149.1 ], [ %i.ai, %bb.d ] ; 4 uses
  %.066.1 = phi i32 [ %i.bn, %.thread149.1 ], [ %i.ag, %bb.d ] ; 4 uses
  %.064.1 = phi i32 [ %i.bq, %.thread149.1 ], [ %i.aj, %bb.d ] ; 5 uses
  %i.ao = sub i32 %.069.1, %.071.1
  %i.ap = ashr i32 %i.ao, 1
  %i.aq = add i32 %i.ap, %.071.1                  ; 6 uses
  %i.ar = tail call i32 @llvm.smin.i32(i32 %.066.1, i32 %.064.1) ; 3 uses
...
.thread149.1:                                     ; preds = %bb.g, %.split153.thread.loopexit.1, %.split153.thread.1, %._crit_edge.1, %select.unfold.1
  %i.bn = phi i32 [ %i.bj, %._crit_edge.1 ], [ %.066.1, %.split153.thread.1 ], [ %i.ar, %select.unfold.1 ], [ %.066.1, %.split153.thread.loopexit.1 ], [ %i.au, %bb.g ] ; 2 uses
  %i.bo = phi i32 [ %i.aq, %._crit_edge.1 ], [ %.071.1, %.split153.thread.1 ], [ %i.aq, %select.unfold.1 ], [ %.071.1, %.split153.thread.loopexit.1 ], [ %i.aq, %bb.g ] ; 3 uses
  %i.bp = phi i32 [ %.069.1, %._crit_edge.1 ], [ %i.aq, %.split153.thread.1 ], [ %.069.1, %select.unfold.1 ], [ %i.aq, %.split153.thread.loopexit.1 ], [ %.069.1, %bb.g ] ; 2 uses
  %i.bq = phi i32 [ %.064.1, %._crit_edge.1 ], [ %i.bj, %.split153.thread.1 ], [ %.064.1, %select.unfold.1 ], [ %i.bm, %.split153.thread.loopexit.1 ], [ %.064.1, %bb.g ]
  %i.br = sub i32 %i.bp, %i.bo
...
bb.h:                                             ; preds = %.thread149.1, %bb.d
  ...
  %.lcssa187.lcssa = phi i32 [ %i.ah, %bb.d ], [ %i.bo, %.thread149.1 ]
  %i.bt = sext i32 %.lcssa187.lcssa to i64
  %i.bu = getelementptr inbounds [32 x i8], ptr %1, i64 %i.bt
  %i.bv = getelementptr inbounds nuw i8, ptr %0, i64 28
  br label %bb.i

bb.i:                                             ; preds = %bb.n, %bb.h
  %.068 = phi ptr [ %i.bu, %bb.h ], [ %i.cg, %bb.n ] ; 6 uses
  %i.bw = load i32, ptr %.068, align 8            ; 2 uses
...
bb.j:                                             ; preds = %bb.i
  ...
  %i.by = getelementptr inbounds nuw i8, ptr %.068, i64 24
...
bb.l:                                             ; preds = %bb.k
  %i.cb = load i32, ptr %.068, align 8
  %i.cc = add i32 %i.cb, %i.b
  store i32 %i.cc, ptr %i.a, align 8
  br label %.loopexit.sink.split

bb.m:                                             ; preds = %bb.k, %bb.i
  %i.cd = getelementptr inbounds nuw i8, ptr %.068, i64 16
...
bb.n:                                             ; preds = %bb.m
  ...
  %i.cg = getelementptr inbounds [32 x i8], ptr %.068, i64 %i.cf
  br label %bb.i

.loopexit.sink.split:                             ; preds = %bb.j, %bb.l
  %i.ch = getelementptr inbounds nuw i8, ptr %.068, i64 20
  %i.ci = load i32, ptr %i.ch, align 4
  br label %.loopexit

.loopexit:                                        ; preds = %bb.m, %.loopexit.sink.split
  %.073 = phi i32 [ %i.ci, %.loopexit.sink.split ], [ 0, %bb.m ]
  ret i32 %.073
}
...
select.unfold:                                    ; preds = %select.unfold.outer, %.thread147
  %.071 = phi i32 [ %i.am, %.thread147 ], [ 0, %select.unfold.outer ] ; 4 uses
  %.069 = phi i32 [ %i.an, %.thread147 ], [ %2, %select.unfold.outer ] ; 4 uses
  %.066 = phi i32 [ %i.al, %.thread147 ], [ 0, %select.unfold.outer ] ; 3 uses
  %.064 = phi i32 [ %i.ao, %.thread147 ], [ 0, %select.unfold.outer ] ; 4 uses
  %i.j = sub i32 %.069, %.071
  %i.k = ashr i32 %i.j, 1
  %i.l = add i32 %i.k, %.071                      ; 6 uses
  %i.m = tail call i32 @llvm.smin.i32(i32 %.066, i32 %.064) ; 3 uses
...
.thread147:                                       ; preds = %bb.c, %bb.a, %._crit_edge, %select.unfold, %.split151.thread
  %i.al = phi i32 [ %.06092, %._crit_edge ], [ %.066, %.split151.thread ], [ %i.m, %select.unfold ], [ %i.p, %bb.c ], [ %.066, %bb.a ] ; 3 uses
  %i.am = phi i32 [ %i.l, %._crit_edge ], [ %.071, %.split151.thread ], [ %i.l, %select.unfold ], [ %i.l, %bb.c ], [ %.071, %bb.a ] ; 6 uses
  %i.an = phi i32 [ %.069, %._crit_edge ], [ %i.l, %.split151.thread ], [ %.069, %select.unfold ], [ %.069, %bb.c ], [ %i.l, %bb.a ] ; 4 uses
  %i.ao = phi i32 [ %.064, %._crit_edge ], [ %.06092, %.split151.thread ], [ %.064, %select.unfold ], [ %.064, %bb.c ], [ %i.i, %bb.a ] ; 2 uses
  %i.ap = sub i32 %i.an, %i.am
  %i.aq = icmp slt i32 %i.ap, 2
  br i1 %i.aq, label %bb.d, label %select.unfold

bb.d:                                             ; preds = %.thread147
  %i.ar = icmp slt i32 %i.am, 1
  %i.as = icmp ne i32 %i.an, %i.am
  %or.cond.not89 = and i1 %i.ar, %i.as
  br i1 %or.cond.not89, label %select.unfold.1, label %bb.h

select.unfold.1:                                  ; preds = %bb.d, %.thread147.1
  %.071.1 = phi i32 [ %i.bw, %.thread147.1 ], [ %i.am, %bb.d ] ; 4 uses
  %.069.1 = phi i32 [ %i.bx, %.thread147.1 ], [ %i.an, %bb.d ] ; 4 uses
  %.066.1 = phi i32 [ %i.bv, %.thread147.1 ], [ %i.al, %bb.d ] ; 3 uses
  %.064.1 = phi i32 [ %i.by, %.thread147.1 ], [ %i.ao, %bb.d ] ; 4 uses
  %i.at = sub i32 %.069.1, %.071.1
  %i.au = ashr i32 %i.at, 1
  %i.av = add i32 %i.au, %.071.1                  ; 6 uses
  %i.aw = tail call i32 @llvm.smin.i32(i32 %.066.1, i32 %.064.1) ; 3 uses
