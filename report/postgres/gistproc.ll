...
bb.z:                                             ; preds = %bb.v
  %i.bv = tail call zeroext i1 @errstart_cold(i32 noundef 21, ptr noundef null) #12 ; 0 uses
  %i.bw = tail call i32 (ptr, ...) @errmsg_internal(ptr noundef nonnull @.str.2) #13 ; 0 uses
  tail call void @errfinish(ptr noundef nonnull @.str.1, i32 noundef 1259, ptr noundef nonnull @__func__.computeDistance) #13
  unreachable

bb.aa:                                            ; preds = %bb.q, %bb.p
  call void @llvm.lifetime.start.p0(ptr nonnull %3) #13
  %i.bx = ptrtoint ptr %2 to i64                  ; 4 uses
  %i.by = getelementptr inbounds nuw i8, ptr %1, i64 16
  %i.bz = ptrtoint ptr %i.by to i64
  %i.ca = tail call i64 @DirectFunctionCall2Coll(ptr noundef nonnull @point_distance, i32 noundef 0, i64 noundef %i.bx, i64 noundef %i.bz) #13
  %i.cb = bitcast i64 %i.ca to double             ; 2 uses
  %i.cc = ptrtoint ptr %1 to i64
  %i.cd = tail call i64 @DirectFunctionCall2Coll(ptr noundef nonnull @point_distance, i32 noundef 0, i64 noundef %i.bx, i64 noundef %i.cc) #13
  %i.ce = bitcast i64 %i.cd to double             ; 2 uses
  %i.cf = fcmp ogt double %i.cb, %i.ce
  %.0 = select i1 %i.cf, double %i.ce, double %i.cb ; 2 uses
  %i.cg = getelementptr inbounds nuw i8, ptr %3, i64 8
  %i.ch = load <2 x double>, ptr %i.av, align 8
  %i.ci = shufflevector <2 x double> %i.ch, <2 x double> poison, <2 x i32> <i32 1, i32 0>
  store <2 x double> %i.ci, ptr %3, align 16
  %i.cj = ptrtoint ptr %3 to i64                  ; 2 uses
  %i.ck = call i64 @DirectFunctionCall2Coll(ptr noundef nonnull @point_distance, i32 noundef 0, i64 noundef %i.bx, i64 noundef %i.cj) #13
  %i.cl = bitcast i64 %i.ck to double             ; 2 uses
  %i.cm = fcmp ogt double %.0, %i.cl
  %.1 = select i1 %i.cm, double %i.cl, double %.0 ; 2 uses
  %i.cn = load double, ptr %1, align 8
  store double %i.cn, ptr %3, align 16
  %i.co = getelementptr inbounds nuw i8, ptr %1, i64 24
  %i.cp = load double, ptr %i.co, align 8
  store double %i.cp, ptr %i.cg, align 8
  %i.cq = call i64 @DirectFunctionCall2Coll(ptr noundef nonnull @point_distance, i32 noundef 0, i64 noundef %i.bx, i64 noundef %i.cj) #13
  %i.cr = bitcast i64 %i.cq to double             ; 2 uses
  %i.cs = fcmp ogt double %.1, %i.cr
...
point_zorder_internal.exit:                       ; preds = %ieee_float32_to_uint32.exit.i, %bb.c
  ...
  %i.x = shl nuw nsw <2 x i64> %i.w, splat (i64 4)
  %i.y = or <2 x i64> %i.x, %i.w
  %i.z = and <2 x i64> %i.y, splat (i64 1085102592571150095) ; 2 uses
  %i.aa = shl nuw nsw <2 x i64> %i.z, splat (i64 2)
  %i.ab = or <2 x i64> %i.aa, %i.z
  %i.ac = and <2 x i64> %i.ab, splat (i64 3689348814741910323) ; 3 uses
  %i.ad = shl nuw <2 x i64> %i.ac, <i64 2, i64 1>
  %i.ae = extractelement <2 x i64> %i.ac, i64 0
  %i.af = shl nuw nsw i64 %i.ae, 1
  %i.ag = insertelement <2 x i64> %i.ac, i64 %i.af, i64 0
  %i.ah = or <2 x i64> %i.ad, %i.ag
  %i.ai = and <2 x i64> %i.ah, <i64 -6148914691236517206, i64 6148914691236517205>
  %i.aj = tail call i64 @llvm.vector.reduce.or.v2i64(<2 x i64> %i.ai)
  ret i64 %i.aj
}
...
point_zorder_internal.exit:                       ; preds = %ieee_float32_to_uint32.exit.i, %bb.d
  ...
  %i.aw = tail call i64 @llvm.vector.reduce.or.v2i64(<2 x i64> %i.av) ; 2 uses
  ...
  %i.ba = fcmp uno double %i.f, 0.000000e+00
  br i1 %i.ba, label %ieee_float32_to_uint32.exit.i18, label %bb.e

bb.e:                                             ; preds = %point_zorder_internal.exit
  %i.bb = fptrunc double %i.f to float            ; 2 uses
  %i.bc = bitcast float %i.bb to i32              ; 2 uses
  %i.bd = xor i32 %i.bc, -1
  %i.be = fneg float %i.bb
  %i.bf = bitcast float %i.be to i32
  %.not5.i.i16 = icmp slt i32 %i.bc, 0
  %.sroa.0.0.i.i17 = select i1 %.not5.i.i16, i32 %i.bd, i32 %i.bf
  %i.bg = zext i32 %.sroa.0.0.i.i17 to i64
...
bb.f:                                             ; preds = %ieee_float32_to_uint32.exit.i18
  %i.bi = bitcast float %i.az to i32              ; 2 uses
  %i.bj = xor i32 %i.bi, -1
  %i.bk = fneg float %i.az
  %i.bl = bitcast float %i.bk to i32
  %.not5.i3.i20 = icmp slt i32 %i.bi, 0
  %.sroa.0.0.i4.i21 = select i1 %.not5.i3.i20, i32 %i.bj, i32 %i.bl
  %i.bm = zext i32 %.sroa.0.0.i4.i21 to i64
...
point_zorder_internal.exit23:                     ; preds = %ieee_float32_to_uint32.exit.i18, %bb.f
  ...
  %i.br = shl nuw nsw <2 x i64> %i.bq, splat (i64 4)
  %i.bs = or <2 x i64> %i.br, %i.bq
  %i.bt = and <2 x i64> %i.bs, splat (i64 1085102592571150095) ; 2 uses
  %i.bu = shl nuw nsw <2 x i64> %i.bt, splat (i64 2)
  %i.bv = or <2 x i64> %i.bu, %i.bt
  %i.bw = and <2 x i64> %i.bv, splat (i64 3689348814741910323) ; 3 uses
  %i.bx = shl nuw <2 x i64> %i.bw, <i64 2, i64 1>
  %i.by = extractelement <2 x i64> %i.bw, i64 0
  %i.bz = shl nuw nsw i64 %i.by, 1
  %i.ca = insertelement <2 x i64> %i.bw, i64 %i.bz, i64 0
  %i.cb = or <2 x i64> %i.bx, %i.ca
  %i.cc = and <2 x i64> %i.cb, <i64 -6148914691236517206, i64 6148914691236517205>
  %i.cd = tail call i64 @llvm.vector.reduce.or.v2i64(<2 x i64> %i.cc) ; 2 uses
  %i.ce = icmp ugt i64 %i.aw, %i.cd
  br i1 %i.ce, label %bb.h, label %bb.g

bb.g:                                             ; preds = %point_zorder_internal.exit23
  %i.cf = icmp ult i64 %i.aw, %i.cd
  %. = sext i1 %i.cf to i32
  br label %bb.h
