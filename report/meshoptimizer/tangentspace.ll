...
bb.t:                                             ; preds = %bb.s, %bb.r
  ...
  %12 = fsub float %i.kw, %i.ky                   ; 3 uses
  %13 = fsub float %i.lc, %i.ky                   ; 3 uses
  %i.lo = fneg <2 x float> %i.ln
  %14 = insertelement <2 x float> poison, float %12, i64 0
  %15 = shufflevector <2 x float> %14, <2 x float> poison, <2 x i32> zeroinitializer
  %16 = fmul <2 x float> %15, %i.lo
  %i.lp = fmul float %12, %i.ld
  %i.lq = tail call float @llvm.fmuladd.f32(float %13, float %i.ko, float %i.lp) ; 3 uses
  %i.lr = fmul float %12, %i.le
  %i.ls = tail call float @llvm.fmuladd.f32(float %i.ku, float %13, float %i.lr) ; 2 uses
  ...
  %i.ml = shufflevector <2 x float> %17, <2 x float> poison, <2 x i32> zeroinitializer
  %i.mm = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ml, <2 x float> %i.ll, <2 x float> %16) ; 4 uses
