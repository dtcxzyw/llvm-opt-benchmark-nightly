...
bb.f:                                             ; preds = %.lr.ph94
  %i.y = fmul double %i.d, %i.v
  %i.z = tail call i32 @daxpy_k(i64 noundef %i.x, i64 noundef 0, i64 noundef 0, double noundef %i.y, ptr noundef nonnull %3, i64 noundef 1, ptr noundef %.07892, i64 noundef 1, ptr noundef null, i64 noundef 0) #3 ; 0 uses
...
.lr.ph:                                           ; preds = %.lr.ph.preheader, %.lr.ph._crit_edge
  ...
  %.17990 = phi ptr [ %5, %.lr.ph.preheader ], [ %i.ah, %.lr.ph._crit_edge ] ; 2 uses
  %i.ab = getelementptr inbounds nuw [8 x i8], ptr %3, i64 %indvars.iv ; 2 uses
  %i.ac = load double, ptr %i.ab, align 8, !tbaa !10 ; 2 uses
  %i.ad = fcmp une double %i.ac, 0.000000e+00
  %i.ae = sub nsw i64 %i.t, %indvars.iv           ; 2 uses
  br i1 %i.ad, label %bb.g, label %.lr.ph._crit_edge

bb.g:                                             ; preds = %.lr.ph
  %i.af = fmul double %i.d, %i.ac
  %i.ag = tail call i32 @daxpy_k(i64 noundef %i.ae, i64 noundef 0, i64 noundef 0, double noundef %i.af, ptr noundef nonnull %i.ab, i64 noundef 1, ptr noundef %.17990, i64 noundef 1, ptr noundef null, i64 noundef 0) #3 ; 0 uses
  br label %.lr.ph._crit_edge

.lr.ph._crit_edge:                                ; preds = %.lr.ph, %bb.g
  %i.ah = getelementptr inbounds [8 x i8], ptr %.17990, i64 %i.ae
...
bb.h:                                             ; preds = %bb.d
  ...
  %i.aj = add nsw i32 %i.c, -1
  %i.ak = mul nsw i32 %i.e, %i.aj
  %i.al = sext i32 %i.ak to i64
  %i.am = sub nsw i64 0, %i.al
  %.077.idx = select i1 %i.ai, i64 %i.am, i64 0
  %.077 = getelementptr inbounds [8 x i8], ptr %3, i64 %.077.idx ; 2 uses
  %i.an = tail call ptr @blas_memory_alloc(i32 noundef 1) #3 ; 3 uses
  %i.ao = load atomic i32, ptr @blas_cpu_number seq_cst, align 4, !tbaa !8
  %i.ap = icmp eq i32 %i.ao, 1
  br i1 %i.ap, label %num_cpu_avail.exit, label %bb.i

bb.i:                                             ; preds = %bb.h
  %i.aq = load atomic i32, ptr @blas_cpu_number seq_cst, align 4, !tbaa !8
  br label %num_cpu_avail.exit

num_cpu_avail.exit:                               ; preds = %bb.h, %bb.i
  %.0.i = phi i32 [ %i.aq, %bb.i ], [ 1, %bb.h ]
  %i.ar = icmp slt i32 %i.c, 50
  %spec.store.select3 = select i1 %i.ar, i32 1, i32 %.0.i ; 2 uses
  %i.as = icmp sgt i32 %spec.store.select3, 2
  %i.at = icmp slt i32 %i.c, 500
  %or.cond5 = select i1 %i.as, i1 %i.at, i1 false
  %spec.store.select7 = select i1 %or.cond5, i32 2, i32 %spec.store.select3 ; 2 uses
  %i.au = icmp eq i32 %spec.store.select7, 1
  %i.av = sext i32 %spec.store.select6 to i64     ; 2 uses
  %i.aw = sext i32 %i.c to i64                    ; 2 uses
  %i.ax = sext i32 %i.e to i64                    ; 2 uses
...
bb.j:                                             ; preds = %num_cpu_avail.exit
  ...
  %i.ba = tail call i32 %i.az(i64 noundef %i.aw, double noundef %i.d, ptr noundef %.077, i64 noundef %i.ax, ptr noundef %5, ptr noundef %i.an) #3 ; 0 uses
  br label %bb.l
...
bb.k:                                             ; preds = %num_cpu_avail.exit
  ...
  %i.bd = tail call i32 %i.bc(i64 noundef %i.aw, double noundef %i.d, ptr noundef %.077, i64 noundef %i.ax, ptr noundef %5, ptr noundef %i.an, i32 noundef %spec.store.select7) #3 ; 0 uses
  br label %bb.l

bb.l:                                             ; preds = %bb.k, %bb.j
  tail call void @blas_memory_free(ptr noundef %i.an) #3
  br label %.loopexit
