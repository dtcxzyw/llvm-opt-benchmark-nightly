...
.lr.ph27.preheader:                               ; preds = %.preheader
  %i.j = add i64 %i.c, %i.a
  %i.k = add i64 %i.j, -3
  %i.l = ptrtoaddr ptr %.019.lcssa to i64         ; 2 uses
  %i.m = add i64 %i.l, 4
  %i.n = tail call i64 @llvm.umax.i64(i64 %i.k, i64 %i.m)
  %i.o = xor i64 %i.l, -1
  %i.p = add i64 %i.n, %i.o                       ; 2 uses
  %i.q = lshr i64 %i.p, 2
  %i.r = add nuw nsw i64 %i.q, 1                  ; 2 uses
  %min.iters.check = icmp ult i64 %i.p, 28
  br i1 %min.iters.check, label %.lr.ph27.preheader72, label %vector.ph

vector.ph:                                        ; preds = %.lr.ph27.preheader
  %n.vec = and i64 %i.r, 9223372036854775800      ; 3 uses
  %i.s = shl i64 %n.vec, 2
  %i.t = getelementptr i8, ptr %.019.lcssa, i64 %i.s ; 2 uses
  %i.u = insertelement <4 x i32> <i32 poison, i32 0, i32 0, i32 0>, i32 %.0.lcssa, i64 0
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ] ; 2 uses
  %vec.phi = phi <4 x i32> [ %i.u, %vector.ph ], [ %i.x, %vector.body ]
  %vec.phi50 = phi <4 x i32> [ zeroinitializer, %vector.ph ], [ %i.y, %vector.body ]
  %i.v = shl i64 %index, 2
  %next.gep = getelementptr i8, ptr %.019.lcssa, i64 %i.v ; 2 uses
  %i.w = getelementptr i8, ptr %next.gep, i64 16
  %wide.load = load <4 x i32>, ptr %next.gep, align 4, !tbaa !10
  %wide.load51 = load <4 x i32>, ptr %i.w, align 4, !tbaa !10
  %i.x = xor <4 x i32> %wide.load, %vec.phi       ; 2 uses
  %i.y = xor <4 x i32> %wide.load51, %vec.phi50   ; 2 uses
...
middle.block:                                     ; preds = %vector.body
  ...
  %i.aa = tail call i32 @llvm.vector.reduce.xor.v4i32(<4 x i32> %bin.rdx) ; 2 uses
  %cmp.n = icmp eq i64 %i.r, %n.vec
  br i1 %cmp.n, label %._crit_edge, label %.lr.ph27.preheader72

.lr.ph27.preheader72:                             ; preds = %.lr.ph27.preheader, %middle.block
  %.126.ph = phi i32 [ %.0.lcssa, %.lr.ph27.preheader ], [ %i.aa, %middle.block ]
...
.lr.ph27:                                         ; preds = %.lr.ph27.preheader72, %.lr.ph27
  ...
  %.12025 = phi ptr [ %i.ai, %.lr.ph27 ], [ %.12025.ph, %.lr.ph27.preheader72 ] ; 2 uses
  %i.ag = load i32, ptr %.12025, align 4, !tbaa !10
...
._crit_edge:                                      ; preds = %.lr.ph27, %middle.block, %.preheader
  ...
  %.1.lcssa = phi i32 [ %.0.lcssa, %.preheader ], [ %i.aa, %middle.block ], [ %i.ah, %.lr.ph27 ] ; 2 uses
  %i.ak = lshr i32 %.1.lcssa, 16
  %i.al = xor i32 %i.ak, %.1.lcssa                ; 2 uses
  %i.am = lshr i32 %i.al, 8
  %i.an = xor i32 %i.am, %i.al                    ; 3 uses
  %i.ao = icmp ult ptr %.120.lcssa, %i.b
  br i1 %i.ao, label %.lr.ph33.preheader, label %._crit_edge34

.lr.ph33.preheader:                               ; preds = %._crit_edge
  %.120.lcssa40 = ptrtoaddr ptr %.120.lcssa to i64
  %i.ap = add i64 %i.c, %i.a
  %i.aq = sub i64 %i.ap, %.120.lcssa40            ; 4 uses
  %scevgep = getelementptr i8, ptr %.120.lcssa, i64 %i.aq
  %min.iters.check53 = icmp ult i64 %i.aq, 8
  br i1 %min.iters.check53, label %.lr.ph33.preheader69, label %vector.ph54

vector.ph54:                                      ; preds = %.lr.ph33.preheader
  %n.vec55 = and i64 %i.aq, -8                    ; 3 uses
  %i.ar = getelementptr i8, ptr %.120.lcssa, i64 %n.vec55
  %i.as = insertelement <4 x i32> <i32 poison, i32 0, i32 0, i32 0>, i32 %i.an, i64 0
...
vector.body56:                                    ; preds = %vector.body56, %vector.ph54
  ...
  %vec.phi58 = phi <4 x i32> [ %i.as, %vector.ph54 ], [ %i.aw, %vector.body56 ]
  %vec.phi59 = phi <4 x i32> [ zeroinitializer, %vector.ph54 ], [ %i.ax, %vector.body56 ]
  %next.gep60 = getelementptr i8, ptr %.120.lcssa, i64 %index57 ; 2 uses
  %i.at = getelementptr i8, ptr %next.gep60, i64 4
  %wide.load61 = load <4 x i8>, ptr %next.gep60, align 1, !tbaa !9
  %wide.load62 = load <4 x i8>, ptr %i.at, align 1, !tbaa !9
  %i.au = zext <4 x i8> %wide.load61 to <4 x i32>
  %i.av = zext <4 x i8> %wide.load62 to <4 x i32>
  %i.aw = xor <4 x i32> %vec.phi58, %i.au         ; 2 uses
...
middle.block64:                                   ; preds = %vector.body56
  ...
  %i.az = tail call i32 @llvm.vector.reduce.xor.v4i32(<4 x i32> %bin.rdx65) ; 2 uses
  %cmp.n66 = icmp eq i64 %i.aq, %n.vec55
  br i1 %cmp.n66, label %._crit_edge34, label %.lr.ph33.preheader69

.lr.ph33.preheader69:                             ; preds = %.lr.ph33.preheader, %middle.block64
  %.231.ph = phi i32 [ %i.an, %.lr.ph33.preheader ], [ %i.az, %middle.block64 ]
  %.22130.ph = phi ptr [ %.120.lcssa, %.lr.ph33.preheader ], [ %i.ar, %middle.block64 ]
  br label %.lr.ph33

.lr.ph33:                                         ; preds = %.lr.ph33.preheader69, %.lr.ph33
  %.231 = phi i32 [ %i.bc, %.lr.ph33 ], [ %.231.ph, %.lr.ph33.preheader69 ]
  %.22130 = phi ptr [ %i.bd, %.lr.ph33 ], [ %.22130.ph, %.lr.ph33.preheader69 ] ; 2 uses
  %i.ba = load i8, ptr %.22130, align 1, !tbaa !9
