...
scalar.ph:                                        ; preds = %scalar.ph.preheader, %scalar.ph
  ...
  %4 = load <2 x float>, ptr %i.ax, align 4, !tbaa !63 ; 3 uses
  %5 = load <2 x float>, ptr %i.ay, align 4, !tbaa !63 ; 4 uses
  %6 = extractelement <2 x float> %5, i64 1       ; 3 uses
  %i.az = fmul nsz float %6, %6
  %7 = extractelement <2 x float> %5, i64 0       ; 2 uses
  %8 = tail call nsz float @llvm.fmuladd.f32(float %7, float %7, float %i.az)
  %9 = fadd nsz float %i.g, %8
  %i.ba = extractelement <2 x float> %4, i64 0
  %i.bb = fneg nsz float %i.ba
  %i.bc = fmul nsz float %6, %i.bb
  %foldExtExtBinop = fmul nsz <2 x float> %4, %5
  %i.bd = shufflevector <2 x float> %5, <2 x float> poison, <2 x i32> zeroinitializer
  %10 = shufflevector <2 x float> %foldExtExtBinop, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %i.be = insertelement <2 x float> %10, float %i.bc, i64 1
  %i.bf = tail call nsz <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bd, <2 x float> %4, <2 x float> %i.be)
  %i.bg = insertelement <2 x float> poison, float %9, i64 0
