...
bb.t:                                             ; preds = %bb.s
  ...
  %4 = shufflevector <2 x double> %i.ax, <2 x double> %i.bk, <2 x i32> <i32 1, i32 3>
  %i.bl = fmul nsz <2 x double> %4, <double f0x3FF423B88EE23B99, double 2.000000e+00> ; 2 uses
  %i.bm = extractelement <2 x double> %i.bl, i64 0
  %5 = fdiv nsz double %i.bm, f0x3FE6A12DF668776C ; 2 uses
  %i.bn = extractelement <2 x double> %i.bk, i64 0
  %6 = fmul nsz double %i.bn, 2.000000e+00
  %7 = fadd nsz double %5, f0x3FF95B9B16BF11C6
  %i.bo = insertelement <2 x double> %i.az, double %5, i64 1
  %8 = fsub nsz <2 x double> <double 1.000000e+00, double f0x3FF95B9B16BF11C6>, %i.bo
  %i.bp = tail call nsz <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.ax, <2 x double> %i.ax, <2 x double> %8) ; 2 uses
  %9 = shufflevector <2 x double> %i.bp, <2 x double> %i.bl, <2 x i32> <i32 0, i32 3>
  %i.bq = fdiv nsz <2 x double> %9, %i.bb         ; 3 uses
  %10 = tail call nsz double @llvm.fmuladd.f64(double %i.ay, double %i.ay, double %7)
  %11 = shufflevector <2 x double> %i.bp, <2 x double> poison, <2 x i32> <i32 1, i32 poison>
  %i.br = insertelement <2 x double> %11, double %6, i64 1
  %i.bs = shufflevector <2 x double> %i.bb, <2 x double> poison, <2 x i32> <i32 1, i32 1>
  %i.bt = fdiv nsz <2 x double> %i.br, %i.bs      ; 4 uses
  %12 = fdiv nsz double %10, %i.bc                ; 3 uses
  store double %12, ptr %i.ao, align 8, !tbaa !35
  %i.bu = extractelement <2 x double> %i.bt, i64 1 ; 2 uses
  %i.bv = tail call nsz double @llvm.fmuladd.f64(double %12, double -2.000000e+00, double %i.bu)
  store double %i.bv, ptr %i.ap, align 8, !tbaa !35
  %i.bw = fmul nsz double %i.bu, 2.000000e+00
  %13 = extractelement <2 x double> %i.bt, i64 0  ; 2 uses
  %14 = fmul nsz double %13, 2.000000e+00
  %15 = fsub nsz double %12, %i.bw
  %i.bx = insertelement <2 x double> poison, double %15, i64 0
  %i.by = insertelement <2 x double> %i.bx, double %14, i64 1 ; 2 uses
  ...
  store double %13, ptr %i.ar, align 8, !tbaa !35
