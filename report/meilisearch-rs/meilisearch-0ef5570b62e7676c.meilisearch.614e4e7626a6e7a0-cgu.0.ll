...
vector.body2270:                                  ; preds = %.noexc156.i, %vector.body2270
  ...
  %wide.load2273 = load <4 x float>, ptr %i.eci, align 8, !alias.scope !37504, !noalias !37508
  %wide.load2274 = load <4 x float>, ptr %i.ecj, align 8, !alias.scope !37504, !noalias !37508
  %14 = freeze <4 x float> %wide.load2273         ; 2 uses
  %15 = freeze <4 x float> %wide.load2274         ; 2 uses
  %i.eck = call <4 x float> @llvm.minnum.v4f32(<4 x float> %vec.phi, <4 x float> %14) ; 2 uses
  %i.ecl = call <4 x float> @llvm.minnum.v4f32(<4 x float> %vec.phi2272, <4 x float> %15) ; 2 uses
  %index.next2275 = add nuw i64 %index2271, 8     ; 2 uses
  %i.ecm = fcmp uno <4 x float> %14, %15
  %i.ecn = bitcast <4 x i1> %i.ecm to i4
