...
bb.z:                                             ; preds = %bb.u
  ...
  %i.fr = load i16, ptr %i.fq, align 2, !tbaa !379 ; 4 uses
  %i.fs = getelementptr inbounds nuw i8, ptr %i.ed, i64 6
  %i.ft = load i16, ptr %i.fs, align 2, !tbaa !380 ; 5 uses
  %i.fu = getelementptr inbounds nuw i8, ptr %i.ed, i64 2
  %i.fv = load i16, ptr %i.fp, align 2, !tbaa !377 ; 7 uses
  %i.fw = load i16, ptr %i.fo, align 2, !tbaa !375 ; 3 uses
  %i.fx = sext i16 %i.fv to i32
  %i.fy = load i16, ptr %i.fu, align 2, !tbaa !377 ; 7 uses
  %i.fz = load i16, ptr %i.ed, align 2, !tbaa !375 ; 3 uses
...
bb.aa:                                            ; preds = %bb.z
  ...
  %10 = sitofp i16 %i.fr to float                 ; 2 uses
  %11 = sitofp i16 %i.ft to float                 ; 2 uses
  %12 = shufflevector <2 x i16> %i.gl, <2 x i16> %i.gm, <2 x i32> <i32 0, i32 2>
  %i.gn = sitofp <2 x i16> %12 to <2 x float>     ; 4 uses
  %13 = shufflevector <2 x i16> %i.gl, <2 x i16> %i.gm, <2 x i32> <i32 1, i32 3>
  %14 = sitofp <2 x i16> %13 to <2 x float>       ; 4 uses
  %15 = icmp ne i16 %i.fw, %i.fr
  %16 = icmp ne i16 %i.fv, %i.ft
  %narrow.i.not.i = or i1 %15, %16
  br i1 %narrow.i.not.i, label %bb.ab, label %bb.ac

bb.ab:                                            ; preds = %bb.aa
  %17 = icmp ne i16 %i.fr, %i.fz
  %18 = icmp ne i16 %i.ft, %i.fy
  %narrow.i182.not.i = or i1 %17, %18
...
bb.af:                                            ; preds = %bb.ae
  ...
  %20 = extractelement <2 x float> %i.gn, i64 1
  %i.hh = call float @llvm.fmuladd.f32(float %i.he, float %i.hg, float %20)
...
.thread21.i.i:                                    ; preds = %bb.al, %bb.ak, %bb.aj
  ...
  %i.is = call <2 x float> @llvm.copysign.v2f32(<2 x float> zeroinitializer, <2 x float> %14)
  %i.it = call float @llvm.copysign.f32(float 0.000000e+00, float %11)
  %i.iu = fadd float %i.it, %10
  %i.iv = fadd <2 x float> %i.is, %i.gn           ; 2 uses
...
bb.d:                                             ; preds = %bb.c
  ...
  %i.i = load <4 x float>, ptr %0, align 4, !tbaa !8 ; 2 uses
  %i.j = fptosi <4 x float> %i.i to <4 x i32>     ; 2 uses
  %i.k = sitofp <4 x i32> %i.j to <4 x float>     ; 3 uses
  %2 = load <2 x float>, ptr %i.h, align 4, !tbaa !8
  %3 = load <2 x float>, ptr %.sroa.13.0..sroa_idx, align 4, !tbaa !8
  store <4 x float> %i.k, ptr %0, align 4, !tbaa !8
  %i.l = fptosi <2 x float> %2 to <2 x i32>
  %4 = shufflevector <2 x i32> %i.l, <2 x i32> poison, <2 x i32> <i32 1, i32 0> ; 2 uses
  %5 = shufflevector <2 x i32> %4, <2 x i32> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
  %6 = sitofp <4 x i32> %5 to <4 x float>         ; 2 uses
  %7 = shufflevector <4 x float> %i.k, <4 x float> %6, <4 x i32> <i32 1, i32 0, i32 4, i32 5>
  %8 = shufflevector <2 x float> %3, <2 x float> poison, <4 x i32> <i32 1, i32 0, i32 poison, i32 poison>
  %9 = shufflevector <4 x float> %i.i, <4 x float> %8, <4 x i32> <i32 3, i32 2, i32 4, i32 5>
  %10 = fptosi <4 x float> %9 to <4 x i32>
  %11 = sitofp <4 x i32> %10 to <4 x float>
  %12 = fadd <4 x float> %7, %11                  ; 3 uses
  %13 = shufflevector <4 x i32> %i.j, <4 x i32> poison, <2 x i32> <i32 1, i32 0>
  %14 = tail call <2 x i32> @llvm.smax.v2i32(<2 x i32> %4, <2 x i32> %13)
  %15 = sitofp <2 x i32> %14 to <2 x float>       ; 5 uses
  %16 = shufflevector <4 x float> %12, <4 x float> poison, <2 x i32> <i32 2, i32 3> ; 2 uses
  %17 = shufflevector <4 x float> %12, <4 x float> poison, <2 x i32> <i32 0, i32 1> ; 2 uses
  %i.m = fcmp olt <2 x float> %16, %17
  %i.n = select <2 x i1> %i.m, <2 x float> %16, <2 x float> %17
  %i.o = fsub <2 x float> %i.n, %15               ; 2 uses
  %i.p = fcmp ogt <2 x float> %i.o, zeroinitializer
  %i.q = select <2 x i1> %i.p, <2 x float> %i.o, <2 x float> zeroinitializer ; 2 uses
  %18 = shufflevector <4 x float> %6, <4 x float> %i.k, <4 x i32> <i32 0, i32 1, i32 5, i32 4>
  %19 = fcmp ogt <4 x float> %12, %18             ; 4 uses
  %20 = extractelement <4 x i1> %19, i64 1
  %21 = extractelement <4 x i1> %19, i64 3
  %or.cond = select i1 %21, i1 %20, i1 false
  %22 = extractelement <4 x i1> %19, i64 2
  %or.cond56 = select i1 %or.cond, i1 %22, i1 false
  %i.r = extractelement <4 x i1> %19, i64 0
...
bb.f:                                             ; preds = %bb.e
  ...
  %i.x = extractelement <2 x float> %15, i64 1
  %23 = fcmp oge float %i.w, %i.x
  %foldExtExtBinop = fadd <2 x float> %i.q, %15
...
bb.g:                                             ; preds = %bb.f
  ...
  %i.ac = extractelement <2 x float> %15, i64 0
  %24 = fcmp ult float %i.ab, %i.ac
  br i1 %24, label %bb.i, label %bb.h

bb.h:                                             ; preds = %bb.g
  %foldExtExtBinop65 = fadd <2 x float> %i.q, %15
  %i.ad = extractelement <2 x float> %foldExtExtBinop65, i64 0
