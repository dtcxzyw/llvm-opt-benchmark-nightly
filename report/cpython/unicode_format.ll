...
bb.j:                                             ; preds = %_PyUnicode_DATA.exit
  ...
  %i.t = or disjoint i32 %switch.ext252, %i.s     ; 4 uses
...
iter.check:                                       ; preds = %.preheader135
  ...
  %i.bo = or disjoint i32 %switch.ext252, %i.s
...
vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  ...
  %n.vec178 = and i64 %i.bq, -4                   ; 5 uses
  %i.by = getelementptr i8, ptr %.177.lcssa, i64 %n.vec178
  %i.bz = getelementptr i8, ptr %.189.lcssa, i64 %n.vec178 ; 2 uses
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index179 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next183, %vec.epilog.vector.body ] ; 3 uses
  %next.gep180 = getelementptr i8, ptr %.177.lcssa, i64 %index179
  %next.gep181 = getelementptr i8, ptr %.189.lcssa, i64 %index179
  %wide.load182 = load <4 x i8>, ptr %next.gep180, align 1, !tbaa !14
  store <4 x i8> %wide.load182, ptr %next.gep181, align 1, !tbaa !14
  %index.next183 = add nuw i64 %index179, 4       ; 2 uses
  %i.ca = icmp eq i64 %index.next183, %n.vec178
  br i1 %i.ca, label %vec.epilog.middle.block, label %vec.epilog.vector.body, !llvm.loop !29

vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  %cmp.n184 = icmp eq i64 %n.vec178, %i.bq
  br i1 %cmp.n184, label %._crit_edge, label %.lr.ph148.preheader
...
.lr.ph148.preheader:                              ; preds = %iter.check, %vec.epilog.iter.check, %vec.epilog.middle.block
  ...
  %.280146.ph = phi i64 [ 0, %iter.check ], [ %n.vec, %vec.epilog.iter.check ], [ %n.vec178, %vec.epilog.middle.block ] ; 4 uses
  ...
  %xtraiter259 = and i64 %i.cb, 7                 ; 2 uses
  %lcmp.mod260.not = icmp eq i64 %xtraiter259, 0
  br i1 %lcmp.mod260.not, label %.lr.ph148.prol.loopexit, label %.lr.ph148.prol
