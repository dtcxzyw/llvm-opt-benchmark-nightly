...
.preheader91:                                     ; preds = %.lr.ph, %bb.k
  %.079.lcssa = phi ptr [ %.076, %bb.k ], [ %i.ch, %.lr.ph ] ; 3 uses
  %.078.lcssa = phi ptr [ %.0.i, %bb.k ], [ %i.cg, %.lr.ph ] ; 5 uses
...
.lr.ph98.preheader:                               ; preds = %.preheader91
  ...
  %i.bm = ptrtoint ptr %.078.lcssa to i64         ; 2 uses
  %i.bn = add i64 %i.bm, 2
  %i.bo = tail call i64 @llvm.umax.i64(i64 %i.bl, i64 %i.bn)
  %i.bp = xor i64 %i.bm, -1
  %i.bq = add i64 %i.bo, %i.bp                    ; 2 uses
  %i.br = lshr i64 %i.bq, 1
  %i.bs = add nuw i64 %i.br, 1                    ; 2 uses
  %min.iters.check = icmp ult i64 %i.bq, 14
  br i1 %min.iters.check, label %.lr.ph98.preheader151, label %vector.ph

vector.ph:                                        ; preds = %.lr.ph98.preheader
  %n.vec = and i64 %i.bs, -8                      ; 4 uses
  %i.bt = shl i64 %n.vec, 1
  %i.bu = getelementptr i8, ptr %.078.lcssa, i64 %i.bt
  %i.bv = shl i64 %n.vec, 2
  %i.bw = getelementptr i8, ptr %.079.lcssa, i64 %i.bv
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ] ; 3 uses
  %i.bx = shl i64 %index, 1
  %next.gep = getelementptr i8, ptr %.078.lcssa, i64 %i.bx ; 2 uses
  %i.by = shl i64 %index, 2
  %next.gep129 = getelementptr i8, ptr %.079.lcssa, i64 %i.by ; 2 uses
  %i.bz = getelementptr i8, ptr %next.gep, i64 8
  %wide.load = load <4 x i16>, ptr %next.gep, align 2, !tbaa !208
  %wide.load130 = load <4 x i16>, ptr %i.bz, align 2, !tbaa !208
  %i.ca = zext <4 x i16> %wide.load to <4 x i32>
  %i.cb = zext <4 x i16> %wide.load130 to <4 x i32>
  %i.cc = getelementptr i8, ptr %next.gep129, i64 16
  store <4 x i32> %i.ca, ptr %next.gep129, align 4, !tbaa !7
  store <4 x i32> %i.cb, ptr %i.cc, align 4, !tbaa !7
  %index.next = add nuw i64 %index, 8             ; 2 uses
  %i.cd = icmp eq i64 %index.next, %n.vec
  br i1 %i.cd, label %middle.block, label %vector.body, !llvm.loop !343
...
.lr.ph98.preheader151:                            ; preds = %.lr.ph98.preheader, %middle.block
  %.197.ph = phi ptr [ %.078.lcssa, %.lr.ph98.preheader ], [ %i.bu, %middle.block ]
  %.18096.ph = phi ptr [ %.079.lcssa, %.lr.ph98.preheader ], [ %i.bw, %middle.block ]
  br label %.lr.ph98

.lr.ph:                                           ; preds = %bb.k, %.lr.ph
  %.07894 = phi ptr [ %i.cg, %.lr.ph ], [ %.0.i, %bb.k ] ; 2 uses
  %.07993 = phi ptr [ %i.ch, %.lr.ph ], [ %.076, %bb.k ] ; 2 uses
  %i.ce = load <4 x i16>, ptr %.07894, align 2, !tbaa !208
  %i.cf = zext <4 x i16> %i.ce to <4 x i32>
  store <4 x i32> %i.cf, ptr %.07993, align 4, !tbaa !7
  %i.cg = getelementptr i8, ptr %.07894, i64 8    ; 3 uses
  %i.ch = getelementptr i8, ptr %.07993, i64 16   ; 2 uses
  %i.ci = icmp ult ptr %i.cg, %i.bi
  br i1 %i.ci, label %.lr.ph, label %.preheader91, !llvm.loop !344

.lr.ph98:                                         ; preds = %.lr.ph98.preheader151, %.lr.ph98
  %.197 = phi ptr [ %i.cj, %.lr.ph98 ], [ %.197.ph, %.lr.ph98.preheader151 ] ; 2 uses
  %.18096 = phi ptr [ %i.cm, %.lr.ph98 ], [ %.18096.ph, %.lr.ph98.preheader151 ] ; 2 uses
  %i.cj = getelementptr i8, ptr %.197, i64 2      ; 2 uses
  %i.ck = load i16, ptr %.197, align 2, !tbaa !208
  %i.cl = zext i16 %i.ck to i32
  %i.cm = getelementptr i8, ptr %.18096, i64 4
  store i32 %i.cl, ptr %.18096, align 4, !tbaa !7
  %i.cn = icmp ult ptr %i.cj, %i.bf
  br i1 %i.cn, label %.lr.ph98, label %.loopexit, !llvm.loop !345
...
_PyUnicode_DATA.exit49:                           ; preds = %bb.i, %bb.j
  ...
  %.idx89.a = shl i64 %.val39, 1
  %i.r = ashr exact i64 %.idx89.a, 1
  %i.s = and i64 %i.r, -4
