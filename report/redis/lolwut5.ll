...
.lr.ph.split.us:                                  ; preds = %.lr.ph.split.us.preheader, %.lr.ph.split.us
  ...
  %exitcond59.not = icmp eq i32 %i.an, %1
...
._crit_edge:                                      ; preds = %.lr.ph.split, %.lr.ph.split.us
  %i.ao = add nuw nsw i32 %.05256, 1              ; 2 uses
  %exitcond60.not = icmp eq i32 %i.ao, %2
  br i1 %exitcond60.not, label %._crit_edge57.split, label %.preheader, !llvm.loop !16

.lr.ph.split:                                     ; preds = %.preheader, %.lr.ph.split
  %.05155 = phi i32 [ %i.at, %.lr.ph.split ], [ 0, %.preheader ] ; 2 uses
  %i.ap = uitofp nneg i32 %.05155 to float
  %i.aq = tail call float @llvm.fmuladd.f32(float %i.ap, float %i.h, float %i.p)
  %i.ar = fadd float %i.aq, %i.q
  %i.as = fptosi float %i.ar to i32
  tail call void @lwDrawSquare(ptr noundef %i.m, i32 noundef %i.as, i32 noundef %i.u, float noundef %i.h, float noundef 0.000000e+00, i32 noundef 1) #6
  %i.at = add nuw nsw i32 %.05155, 1              ; 2 uses
