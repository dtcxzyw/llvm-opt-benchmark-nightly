...
.preheader.lr.ph:                                 ; preds = %bb.a
  ...
  br i1 %i.o, label %.preheader.us, label %._crit_edge57

.preheader.us:                                    ; preds = %.preheader.lr.ph, %._crit_edge
  %.05256.us = phi i32 [ %3, %._crit_edge ], [ 0, %.preheader.lr.ph ] ; 3 uses
  %i.r = uitofp nneg i32 %.05256.us to float      ; 4 uses
  ...
  %i.v = icmp samesign ugt i32 %.05256.us, 1
  %i.w = sitofp i32 %i.u to float
  br i1 %i.v, label %.lr.ph.split.us.us, label %.lr.ph.split

.lr.ph.split.us.us:                               ; preds = %.preheader.us, %.lr.ph.split.us.us
  %.05155.us.us = phi i32 [ %i.bi, %.lr.ph.split.us.us ], [ 0, %.preheader.us ] ; 2 uses
  %i.x = uitofp nneg i32 %.05155.us.us to float
  ...
  %.047.us.us = select i1 %.not.us.us, float %i.af, float %i.as
  %i.at = tail call i32 @rand() #6
  %i.au = and i32 %i.at, 1
  %.not53.us.us = icmp eq i32 %i.au, 0
  %i.av = fneg float %i.ak
  %.046.us.us = select i1 %.not53.us.us, float %i.ak, float %i.av
  %i.aw = tail call i32 @rand() #6
  %i.ax = and i32 %i.aw, 1
  %.not54.us.us = icmp eq i32 %i.ax, 0
  %i.ay = fneg float %i.ap
  %.0.us.us = select i1 %.not54.us.us, float %i.ap, float %i.ay
  %i.az = fmul float %i.h, %.046.us.us
  %i.ba = fdiv float %i.az, 3.000000e+00
  %i.bb = sitofp i32 %i.aa to float
  %i.bc = fadd float %i.ba, %i.bb
  %i.bd = fptosi float %i.bc to i32
  %i.be = fmul float %i.h, %.0.us.us
  %i.bf = fdiv float %i.be, 3.000000e+00
  %i.bg = fadd float %i.bf, %i.w
  %i.bh = fptosi float %i.bg to i32
  tail call void @lwDrawSquare(ptr noundef %i.m, i32 noundef %i.bd, i32 noundef %i.bh, float noundef %i.h, float noundef %.047.us.us, i32 noundef 1) #6
  %i.bi = add nuw nsw i32 %.05155.us.us, 1        ; 2 uses
  %exitcond63.not = icmp eq i32 %i.bi, %1
  br i1 %exitcond63.not, label %._crit_edge, label %.lr.ph.split.us.us, !llvm.loop !14

._crit_edge57:                                    ; preds = %._crit_edge, %.preheader.lr.ph, %bb.a
  ret ptr %i.m

._crit_edge:                                      ; preds = %.lr.ph.split, %.lr.ph.split.us.us
  %3 = add nuw nsw i32 %.05256.us, 1              ; 2 uses
  %exitcond60.not = icmp eq i32 %3, %2
  br i1 %exitcond60.not, label %._crit_edge57, label %.preheader.us, !llvm.loop !16

.lr.ph.split:                                     ; preds = %.preheader.us, %.lr.ph.split
  %.05155 = phi i32 [ %8, %.lr.ph.split ], [ 0, %.preheader.us ] ; 2 uses
