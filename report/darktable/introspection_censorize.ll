...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %4 = fmul reassoc nnan nsz arcp contract afn <8 x float> %i.bs, splat (float f0x33800000)
  %5 = tail call reassoc nnan nsz arcp contract afn <8 x float> @llvm.maxnum.v8f32(<8 x float> %4, <8 x float> splat (float f0x00800000))
  %i.bt = tail call fast <8 x float> @llvm.log.v8f32(<8 x float> %5)
...
gaussian_noise.exit:                              ; preds = %gaussian_noise.exit.preheader, %gaussian_noise.exit
  ...
  %6 = fmul reassoc nnan nsz arcp contract afn float %i.er, f0x33800000
  %7 = tail call reassoc nnan nsz arcp contract afn float @llvm.maxnum.f32(float %6, float f0x00800000)
  %i.es = tail call fast float @llvm.log.f32(float %7)
