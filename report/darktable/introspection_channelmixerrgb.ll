...
dt_simd_memcpy.exit.i:                            ; preds = %.lr.ph.preheader.i.i, %bb.i
  ...
  %i.cs = select <2 x i1> %i.cn, <2 x float> %i.cp, <2 x float> %i.cr
  %6 = fmul reassoc nsz arcp contract afn <2 x float> %i.cs, <float 9.642000e-01, float f0x3F532CA5> ; 2 uses
  ...
  %7 = shufflevector <2 x float> %i.cx, <2 x float> %6, <2 x i32> <i32 0, i32 2>
  ...
  %i.da = fdiv reassoc nsz arcp contract afn <2 x float> %7, %i.cz ; 7 uses
  %8 = extractelement <2 x float> %6, i64 1
  %i.db = fdiv reassoc nsz arcp contract afn float %8, %i.cw ; 7 uses
