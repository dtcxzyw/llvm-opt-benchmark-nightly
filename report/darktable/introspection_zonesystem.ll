...
vector.body44:                                    ; preds = %vector.body44, %vector.ph42
  %index45 = phi i64 [ 0, %vector.ph42 ], [ %index.next56, %vector.body44 ] ; 2 uses
  %vector.recur47 = phi <8 x float> [ %vector.recur.init46, %vector.ph42 ], [ %wide.load55, %vector.body44 ]
  %vec.ind = phi <8 x i64> [ <i64 0, i64 1, i64 2, i64 3, i64 4, i64 5, i64 6, i64 7>, %vector.ph42 ], [ %vec.ind.next, %vector.body44 ] ; 5 uses
  %vec.ind48 = phi <8 x i32> [ <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7>, %vector.ph42 ], [ %vec.ind.next57, %vector.body44 ] ; 5 uses
  %step.add49.a = add <8 x i32> %vec.ind48, splat (i32 8)
  %step.add.250.a = add <8 x i32> %vec.ind48, splat (i32 16)
  %step.add.351.a = add <8 x i32> %vec.ind48, splat (i32 24)
  %4 = add nuw nsw <8 x i64> %vec.ind, splat (i64 1) ; 2 uses
  %5 = extractelement <8 x i64> %4, i64 0
  %6 = trunc nuw nsw <8 x i64> %4 to <8 x i32>
  %7 = trunc <8 x i64> %vec.ind to <8 x i32>
  %i.cp = add <8 x i32> %7, splat (i32 9)
  %8 = trunc <8 x i64> %vec.ind to <8 x i32>
  %i.cq = add <8 x i32> %8, splat (i32 17)
  %9 = trunc <8 x i64> %vec.ind to <8 x i32>
  %i.cr = add <8 x i32> %9, splat (i32 25)
  %i.cs = uitofp nneg <8 x i32> %6 to <8 x float>
  %i.ct = uitofp nneg <8 x i32> %i.cp to <8 x float>
  %i.cu = uitofp nneg <8 x i32> %i.cq to <8 x float>
  %i.cv = uitofp nneg <8 x i32> %i.cr to <8 x float>
  %i.cw = uitofp nneg <8 x i32> %vec.ind48 to <8 x float>
  %i.cx = uitofp nneg <8 x i32> %step.add49.a to <8 x float>
  %i.cy = uitofp nneg <8 x i32> %step.add.250.a to <8 x float>
  %i.cz = uitofp nneg <8 x i32> %step.add.351.a to <8 x float>
  %i.da = getelementptr inbounds nuw [4 x i8], ptr %i.a, i64 %5 ; 4 uses
  ...
  %wide.load52 = load <8 x float>, ptr %i.da, align 4, !tbaa !54 ; 3 uses
