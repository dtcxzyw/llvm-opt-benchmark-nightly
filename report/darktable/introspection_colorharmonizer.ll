...
bb.c:                                             ; preds = %dt_UCS_JCH_to_xyY.exit.i
  ...
  %3 = insertelement <2 x float> poison, float %i.ay, i64 0
  %4 = insertelement <2 x float> %3, float %i.bg, i64 1
...
dt_UCS_JCH_to_XYZ.exit:                           ; preds = %dt_UCS_JCH_to_xyY.exit.i, %bb.c
  ...
  %5 = phi <2 x float> [ %4, %bb.c ], [ zeroinitializer, %dt_UCS_JCH_to_xyY.exit.i ] ; 2 uses
  %i.bm = fmul reassoc nsz arcp contract afn float %i.bl, f0xBEFF3F82
  %i.bn = fmul reassoc nsz arcp contract afn float %i.bl, 4.155600e-02
  %6 = fmul reassoc nsz arcp contract afn float %i.bl, f0x3F875328
  %7 = shufflevector <2 x float> %5, <2 x float> poison, <4 x i32> <i32 1, i32 1, i32 1, i32 1>
  %i.bo = fmul reassoc nsz arcp contract afn <4 x float> %7, <float f0x404F639A, float f0xBF7821D1, float 5.564340e-02, float 1.000000e+00>
  %i.bp = shufflevector <2 x float> %5, <2 x float> poison, <4 x i32> zeroinitializer
  ...
  %i.bt = fadd reassoc nsz arcp contract afn float %i.bs, %i.bm ; 3 uses
  %i.bu = extractelement <4 x float> %i.br, i64 1
  %i.bv = fadd reassoc nsz arcp contract afn float %i.bu, %i.bn ; 3 uses
  %i.bw = extractelement <4 x float> %i.br, i64 2
  %i.bx = fadd reassoc nsz arcp contract afn float %i.bw, %6 ; 3 uses
