...
bb.bw:                                            ; preds = %_blendif_compute_factor.exit.2.i107, %.lr.ph.i99
  ...
  %7 = fmul reassoc nsz arcp contract afn float %i.xg, %i.xg
  %8 = fmul reassoc nsz arcp contract afn float %7, %i.xg
  %9 = fmul reassoc nsz arcp contract afn float %i.xg, f0x3E038026
  %10 = fadd reassoc nsz arcp contract afn float %9, f0xBC911AA6
  %11 = load <2 x float>, ptr %i.wl, align 4, !tbaa !39, !noalias !182
  %12 = fmul reassoc nsz arcp contract afn <2 x float> %11, <float 2.000000e-03, float 5.000000e-03> ; 2 uses
  %13 = select reassoc nsz arcp contract afn i1 %6, float %8, float %10 ; 3 uses
  %14 = insertelement <2 x float> poison, float %i.xg, i64 0
  %15 = shufflevector <2 x float> %14, <2 x float> poison, <2 x i32> zeroinitializer ; 2 uses
  %16 = fadd reassoc nsz arcp contract afn <2 x float> %15, %12
  %17 = fsub reassoc nsz arcp contract afn <2 x float> %15, %12
  %18 = shufflevector <2 x float> %16, <2 x float> %17, <2 x i32> <i32 0, i32 3> ; 5 uses
  %19 = fcmp reassoc nsz arcp contract afn ogt <2 x float> %18, splat (float f0x3E53DCB1)
  %20 = fmul reassoc nsz arcp contract afn <2 x float> %18, %18
  %i.xh = fmul reassoc nsz arcp contract afn <2 x float> %20, %18
  %21 = fmul reassoc nsz arcp contract afn <2 x float> %18, splat (float f0x3E038026)
  %22 = fadd reassoc nsz arcp contract afn <2 x float> %21, splat (float f0xBC911AA6)
  %i.xi = select <2 x i1> %19, <2 x float> %i.xh, <2 x float> %22 ; 4 uses
  %i.xj = extractelement <2 x float> %i.xi, i64 0
  %i.xk = fmul reassoc nsz arcp contract afn float %13, 2.303930e-02
  ...
  %.reass37.i = fmul reassoc nsz arcp contract afn float %13, f0x3F2AA3C6
  %i.xq = fmul reassoc nsz arcp contract afn <2 x float> %i.xi, <float f0x3C4247BE, float f0x3C3B638A> ; 2 uses
  %i.xr = fadd reassoc nsz arcp contract afn float %.reass37.i, %.reass38.i
  %i.xs = fmul reassoc nsz arcp contract afn float %13, 2.048300e-02
