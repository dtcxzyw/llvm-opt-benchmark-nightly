...
define void @dt_develop_blendif_rgb_jzczhz_make_mask(ptr noundef %0, ptr noalias noundef %1, ptr noalias noundef %2, ptr nofree noundef readonly captures(none) %3, ptr nofree noundef readonly captures(none) %4, ptr noalias noundef %5) local_unnamed_addr #0 {
bb.a:
  ...
  %i.f = alloca [96 x float], align 64            ; 7 uses
  %6 = alloca %struct.dt_iop_order_iccprofile_info_t, align 64 ; 7 uses
...
bb.b:                                             ; preds = %bb.a
  ...
  %.not140 = icmp eq i32 %i.x, 0                  ; 2 uses
  %i.z = select i1 %.not140, i32 0, i32 2013200384
...
bb.d:                                             ; preds = %bb.c, %bb.b
  %.not142 = icmp eq i32 %i.y, 0
  br i1 %.not142, label %bb.e, label %.preheader

.preheader:                                       ; preds = %bb.d
  %.not183 = icmp eq i64 %i.ah, 0
  br i1 %.not183, label %.loopexit, label %iter.check387

iter.check387:                                    ; preds = %.preheader
  %min.iters.check370 = icmp ult i64 %i.ah, 4
  br i1 %min.iters.check370, label %.lr.ph175.preheader, label %vector.main.loop.iter.check371

vector.main.loop.iter.check371:                   ; preds = %iter.check387
  %min.iters.check372 = icmp ult i64 %i.ah, 32
  br i1 %min.iters.check372, label %vec.epilog.ph391, label %vector.ph373

vector.ph373:                                     ; preds = %vector.main.loop.iter.check371
  %i.ap = and i64 %i.ah, 28
  %n.vec374 = and i64 %i.ah, -32                  ; 4 uses
  %broadcast.splatinsert375 = insertelement <8 x float> poison, float %8, i64 0
  %broadcast.splat376 = shufflevector <8 x float> %broadcast.splatinsert375, <8 x float> poison, <8 x i32> zeroinitializer ; 4 uses
  br label %vector.body377

vector.body377:                                   ; preds = %vector.body377, %vector.ph373
  %index378 = phi i64 [ 0, %vector.ph373 ], [ %index.next383, %vector.body377 ] ; 2 uses
  %i.aq = getelementptr inbounds nuw [4 x i8], ptr %5, i64 %index378 ; 5 uses
  ...
  %wide.load379 = load <8 x float>, ptr %i.aq, align 4, !tbaa !39
  %wide.load380 = load <8 x float>, ptr %i.ar, align 4, !tbaa !39
  %wide.load381 = load <8 x float>, ptr %i.as, align 4, !tbaa !39
  %wide.load382 = load <8 x float>, ptr %i.at, align 4, !tbaa !39
  %i.au = fsub reassoc nsz arcp contract afn <8 x float> splat (float 1.000000e+00), %wide.load379
  %i.av = fsub reassoc nsz arcp contract afn <8 x float> splat (float 1.000000e+00), %wide.load380
  %i.aw = fsub reassoc nsz arcp contract afn <8 x float> splat (float 1.000000e+00), %wide.load381
  %i.ax = fsub reassoc nsz arcp contract afn <8 x float> splat (float 1.000000e+00), %wide.load382
  %i.ay = fmul reassoc nsz arcp contract afn <8 x float> %i.au, %broadcast.splat376
  %i.az = fmul reassoc nsz arcp contract afn <8 x float> %i.av, %broadcast.splat376
  %i.ba = fmul reassoc nsz arcp contract afn <8 x float> %i.aw, %broadcast.splat376
  %i.bb = fmul reassoc nsz arcp contract afn <8 x float> %i.ax, %broadcast.splat376
  ...
  %index.next383 = add nuw i64 %index378, 32      ; 2 uses
  %i.bc = icmp eq i64 %index.next383, %n.vec374
  br i1 %i.bc, label %middle.block384, label %vector.body377, !llvm.loop !40

middle.block384:                                  ; preds = %vector.body377
  %cmp.n385 = icmp eq i64 %i.ah, %n.vec374
  br i1 %cmp.n385, label %.loopexit, label %vec.epilog.iter.check389

vec.epilog.iter.check389:                         ; preds = %middle.block384
  %min.epilog.iters.check390 = icmp eq i64 %i.ap, 0
  br i1 %min.epilog.iters.check390, label %.lr.ph175.preheader, label %vec.epilog.ph391, !prof !43

vec.epilog.ph391:                                 ; preds = %vector.main.loop.iter.check371, %vec.epilog.iter.check389
  %vec.epilog.resume.val386 = phi i64 [ %n.vec374, %vec.epilog.iter.check389 ], [ 0, %vector.main.loop.iter.check371 ]
  %n.vec392 = and i64 %i.ah, -4                   ; 3 uses
  %broadcast.splatinsert393 = insertelement <4 x float> poison, float %8, i64 0
  %broadcast.splat394 = shufflevector <4 x float> %broadcast.splatinsert393, <4 x float> poison, <4 x i32> zeroinitializer
  br label %vec.epilog.vector.body395

vec.epilog.vector.body395:                        ; preds = %vec.epilog.vector.body395, %vec.epilog.ph391
  %index396 = phi i64 [ %vec.epilog.resume.val386, %vec.epilog.ph391 ], [ %index.next398, %vec.epilog.vector.body395 ] ; 2 uses
  %i.bd = getelementptr inbounds nuw [4 x i8], ptr %5, i64 %index396 ; 2 uses
  %wide.load397 = load <4 x float>, ptr %i.bd, align 4, !tbaa !39
  %i.be = fsub reassoc nsz arcp contract afn <4 x float> splat (float 1.000000e+00), %wide.load397
  %i.bf = fmul reassoc nsz arcp contract afn <4 x float> %i.be, %broadcast.splat394
  store <4 x float> %i.bf, ptr %i.bd, align 4, !tbaa !39
  %index.next398 = add nuw i64 %index396, 4       ; 2 uses
  %i.bg = icmp eq i64 %index.next398, %n.vec392
  br i1 %i.bg, label %vec.epilog.middle.block399, label %vec.epilog.vector.body395, !llvm.loop !44

vec.epilog.middle.block399:                       ; preds = %vec.epilog.vector.body395
  %cmp.n400 = icmp eq i64 %i.ah, %n.vec392
  br i1 %cmp.n400, label %.loopexit, label %.lr.ph175.preheader

.lr.ph175.preheader:                              ; preds = %iter.check387, %vec.epilog.iter.check389, %vec.epilog.middle.block399
  %.0134174.ph = phi i64 [ 0, %iter.check387 ], [ %n.vec374, %vec.epilog.iter.check389 ], [ %n.vec392, %vec.epilog.middle.block399 ]
  br label %.lr.ph175

.lr.ph175:                                        ; preds = %.lr.ph175.preheader, %.lr.ph175
  %.0134174 = phi i64 [ %i.bl, %.lr.ph175 ], [ %.0134174.ph, %.lr.ph175.preheader ] ; 2 uses
  %i.bh = getelementptr inbounds nuw [4 x i8], ptr %5, i64 %.0134174 ; 2 uses
  ...
  %i.bl = add nuw i64 %.0134174, 1                ; 2 uses
  %exitcond193.not = icmp eq i64 %i.bl, %i.ah
  br i1 %exitcond193.not, label %.loopexit, label %.lr.ph175, !llvm.loop !45

bb.e:                                             ; preds = %bb.d
  tail call void @dt_iop_image_mul_const(ptr noundef %5, float noundef %8, i64 noundef %i.af, i64 noundef %i.ag, i64 noundef 1) #7
  br label %.loopexit
...
bb.g:                                             ; preds = %bb.f
  ...
  %.not143 = icmp eq i32 %i.y, %.lobit
  %i.bn = select reassoc nsz arcp contract afn i1 %.not143, float 0.000000e+00, float %8
  tail call void @dt_iop_image_fill(ptr noundef %5, float noundef %i.bn, i64 noundef %i.af, i64 noundef %i.ag, i64 noundef 1) #7
  br label %.loopexit

bb.h:                                             ; preds = %bb.f
  call void @llvm.lifetime.start.p0(ptr nonnull %i.f) #7
  call void @dt_develop_blendif_process_parameters(ptr noundef nonnull %i.f, ptr noundef nonnull %i.h) #7
  call void @llvm.lifetime.start.p0(ptr nonnull %6) #7
  %i.bo = call i32 @dt_develop_blendif_init_masking_profile(ptr noundef nonnull %0, ptr noundef nonnull %6, i32 noundef 4) #7
  %.not144 = icmp eq i32 %i.bo, 0
  br i1 %.not144, label %.critedge, label %bb.i

bb.i:                                             ; preds = %bb.h
  %i.bp = shl i64 %i.ah, 2
  %i.bq = call ptr @dt_alloc_aligned(i64 noundef %i.bp) #7 ; 44 uses
  call void @llvm.assume(i1 true) [ "align"(ptr %i.bq, i64 64) ]
  %.not145.not = icmp eq ptr %i.bq, null
  br i1 %.not145.not, label %.critedge150, label %bb.j
...
bb.j:                                             ; preds = %bb.i
  ...
  %.not176 = icmp eq i64 %i.ah, 0                 ; 5 uses
  br i1 %.not176, label %.preheader160, label %iter.check
...
vector.main.loop.iter.check:                      ; preds = %iter.check
  %min.iters.check201 = icmp ult i64 %i.ah, 32
...
middle.block:                                     ; preds = %vector.body
  ...
  br i1 %cmp.n, label %.preheader160, label %vec.epilog.iter.check
...
vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  ...
  %n.vec202 = and i64 %i.ah, -4                   ; 3 uses
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index203 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next204, %vec.epilog.vector.body ] ; 2 uses
  %i.cb = getelementptr inbounds nuw [4 x i8], ptr %i.bq, i64 %index203
  store <4 x float> splat (float 1.000000e+00), ptr %i.cb, align 16, !tbaa !39
  %index.next204 = add nuw i64 %index203, 4       ; 2 uses
  %i.cc = icmp eq i64 %index.next204, %n.vec202
end_hunk_0
begin_hunk_1_@_blendif_combine_channels:bb.a
  %i.nl = xor <8 x i1> %i.nk, splat (i1 true)
  %i.nm = select <8 x i1> %i.nj, <8 x i1> %i.nl, <8 x i1> zeroinitializer ; 3 uses
  %wide.masked.gather355.a = tail call <8 x float> @llvm.masked.gather.v8f32.v8p0(<8 x ptr> align 4 %broadcast.splat343, <8 x i1> %i.nm, <8 x float> poison), !tbaa !39, !alias.scope !148, !noalias !151 ; 2 uses
  %i.nn = fcmp reassoc nsz arcp contract afn ugt <8 x float> %strided.vec353, %wide.masked.gather355.a ; 2 uses
  %i.no = select <8 x i1> %i.nm, <8 x i1> %i.nn, <8 x i1> zeroinitializer ; 2 uses
  %wide.masked.gather356.a = tail call <8 x float> @llvm.masked.gather.v8f32.v8p0(<8 x ptr> align 4 %broadcast.splat345, <8 x i1> %i.no, <8 x float> poison), !tbaa !39, !alias.scope !148, !noalias !151
  %i.np = fcmp reassoc nsz arcp contract afn olt <8 x float> %strided.vec353, %wide.masked.gather356.a
  %i.nq = select <8 x i1> %i.no, <8 x i1> %i.np, <8 x i1> zeroinitializer ; 2 uses
  %i.nr = fsub reassoc nsz arcp contract afn <8 x float> %strided.vec353, %wide.masked.gather355.a
  %wide.masked.gather357.a = tail call <8 x float> @llvm.masked.gather.v8f32.v8p0(<8 x ptr> align 4 %broadcast.splat347, <8 x i1> %i.nq, <8 x float> poison), !tbaa !39, !alias.scope !148, !noalias !151
  %i.ns = fmul reassoc nsz arcp contract afn <8 x float> %wide.masked.gather357.a, %i.nr
  %i.nt = fsub reassoc nsz arcp contract afn <8 x float> splat (float 1.000000e+00), %i.ns
  %i.nu = select <8 x i1> %i.nj, <8 x i1> %i.nk, <8 x i1> zeroinitializer ; 2 uses
  %i.nv = fsub reassoc nsz arcp contract afn <8 x float> %strided.vec353, %broadcast.splat339
  %wide.masked.gather358 = tail call <8 x float> @llvm.masked.gather.v8f32.v8p0(<8 x ptr> align 4 %broadcast.splat349, <8 x i1> %i.nu, <8 x float> poison), !tbaa !39, !alias.scope !148, !noalias !151
  %i.nw = fmul reassoc nsz arcp contract afn <8 x float> %wide.masked.gather358, %i.nv
  %i.nx = xor <8 x i1> %i.nn, splat (i1 true)
  %i.ny = select <8 x i1> %i.nm, <8 x i1> %i.nx, <8 x i1> zeroinitializer
  %predphi359.a = select nsz <8 x i1> %i.nq, <8 x float> %i.nt, <8 x float> zeroinitializer
  %predphi360.a = select nsz <8 x i1> %i.ny, <8 x float> splat (float 1.000000e+00), <8 x float> %predphi359.a
  %predphi361 = select nsz <8 x i1> %i.nu, <8 x float> %i.nw, <8 x float> %predphi360.a
...
bb.bf:                                            ; preds = %_blendif_compute_factor.exit.2.i, %.lr.ph.i93
  ...
  %.reass35.i = fmul reassoc nsz arcp contract afn float %i.rm, -1.660080e-02
  %.reass33.i = fmul reassoc nsz arcp contract afn float %i.rq, 2.648000e-01
  %.reass34.i = fmul reassoc nsz arcp contract afn float %i.rk, f0x3F2B2180
  ...
  %i.rv = fmul reassoc nsz arcp contract afn <2 x float> %i.ru, <float -2.015100e-01, float f0x3ED45F51>
  ...
  %i.ry = fmul reassoc nsz arcp contract afn <2 x float> %i.rx, <float f0x3F8F716D, float 5.799990e-01>
  ...
  %i.sb = fmul reassoc nsz arcp contract afn <2 x float> %i.sa, <float 5.310080e-02, float 1.464800e-02>
  ...
  %6 = fmul reassoc nnan nsz arcp contract afn <2 x float> %i.se, splat (float f0x38D1B717)
  %i.sf = tail call reassoc nsz arcp contract afn <2 x float> @llvm.pow.v2f32(<2 x float> %6, <2 x float> splat (float f0x3E232000)) ; 2 uses
  ...
  %7 = fmul reassoc nnan nsz arcp contract afn float %i.sm, f0x38D1B717
  %i.sn = tail call reassoc nsz arcp contract afn float @llvm.pow.f32(float %7, float f0x3E232000) ; 2 uses
...
bb.bw:                                            ; preds = %_blendif_compute_factor.exit.2.i107, %.lr.ph.i99
  ...
  %i.xh = load <2 x float>, ptr %i.wl, align 4, !tbaa !39, !noalias !182
  ...
  %i.xo = fcmp reassoc nsz arcp contract afn ogt float %i.xg, f0x3E53DCB1
  %i.xp = fmul reassoc nsz arcp contract afn float %i.xg, %i.xg
  %i.xq = fmul reassoc nsz arcp contract afn <2 x float> %i.xn, %i.xn
  %i.xr = fmul reassoc nsz arcp contract afn float %i.xp, %i.xg
  %i.xs = fmul reassoc nsz arcp contract afn float %i.xg, f0x3E038026
  %i.xt = fmul reassoc nsz arcp contract afn <2 x float> %i.xn, splat (float f0x3E038026)
  %i.xu = fadd reassoc nsz arcp contract afn float %i.xs, f0xBC911AA6
  %i.xv = fcmp reassoc nsz arcp contract afn ogt <2 x float> %i.xn, splat (float f0x3E53DCB1)
  %i.xw = fmul reassoc nsz arcp contract afn <2 x float> %i.xq, %i.xn
  %i.xx = fadd reassoc nsz arcp contract afn <2 x float> %i.xt, splat (float f0xBC911AA6)
  %i.xy = select reassoc nsz arcp contract afn i1 %i.xo, float %i.xr, float %i.xu ; 3 uses
  %i.xz = select <2 x i1> %i.xv, <2 x float> %i.xw, <2 x float> %i.xx ; 4 uses
  %i.ya = extractelement <2 x float> %i.xz, i64 0
  %i.yb = fmul reassoc nsz arcp contract afn float %i.xy, 2.303930e-02
  ...
  %.reass37.i = fmul reassoc nsz arcp contract afn float %i.xy, f0x3F2AA3C6
  %i.yh = fmul reassoc nsz arcp contract afn <2 x float> %i.xz, <float f0x3C4247BE, float f0x3C3B638A> ; 2 uses
  %i.yi = fadd reassoc nsz arcp contract afn float %.reass38.i, %.reass37.i
  %i.yj = fmul reassoc nsz arcp contract afn float %i.xy, 2.048300e-02
  ...
  %.reass47.i = fmul reassoc nsz arcp contract afn float %i.yw, -1.660080e-02
  %i.yx = fmul reassoc nsz arcp contract afn <2 x float> %i.yt, <float f0x3F2B2180, float 2.648000e-01> ; 2 uses
  ...
  %i.ze = fmul reassoc nsz arcp contract afn <2 x float> %i.zd, <float -2.015100e-01, float f0x3ED45F51>
  %i.zf = shufflevector <2 x float> %i.yt, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.zg = fmul reassoc nsz arcp contract afn <2 x float> %i.zf, <float f0x3F8F716D, float 5.799990e-01>
  %i.zh = shufflevector <2 x float> %i.yt, <2 x float> poison, <2 x i32> zeroinitializer
  %i.zi = fmul reassoc nsz arcp contract afn <2 x float> %i.zh, <float 5.310080e-02, float 1.464800e-02>
  ...
  %8 = fmul reassoc nnan nsz arcp contract afn <2 x float> %i.zl, splat (float f0x38D1B717)
  %i.zm = call reassoc nsz arcp contract afn <2 x float> @llvm.pow.v2f32(<2 x float> %8, <2 x float> splat (float f0x3E232000)) ; 2 uses
  ...
  %9 = fmul reassoc nnan nsz arcp contract afn float %i.zt, f0x38D1B717
  %i.zu = call reassoc nsz arcp contract afn float @llvm.pow.f32(float %9, float f0x3E232000) ; 2 uses
...
bb.b:                                             ; preds = %bb.a
  ...
  %i.l = load float, ptr %i.k, align 8, !tbaa !39 ; 4 uses
  %i.m = fmul reassoc nsz arcp contract afn float %i.l, f0x3E199998
  %i.n = fsub reassoc nsz arcp contract afn float %i.j, %i.m ; 3 uses
  ...
  %i.t = fmul reassoc nsz arcp contract afn float %i.n, f0x3ED45F51
  %i.u = fmul reassoc nsz arcp contract afn float %i.l, 1.464800e-02
  %i.v = fadd reassoc nsz arcp contract afn float %i.t, %i.u
  %i.w = fmul reassoc nsz arcp contract afn float %i.n, -2.015100e-01
  ...
  %i.z = fmul reassoc nsz arcp contract afn <2 x float> %i.y, <float f0x3F8F716D, float 5.799990e-01>
  %i.aa = fmul reassoc nsz arcp contract afn float %i.l, 5.310080e-02
  %i.ab = fadd reassoc nsz arcp contract afn float %i.w, %i.aa
  %i.ac = fmul reassoc nsz arcp contract afn float %i.n, -1.660080e-02
  %i.ad = fmul reassoc nsz arcp contract afn float %i.s, 2.648000e-01
  %i.ae = fmul reassoc nsz arcp contract afn float %i.l, f0x3F2B2180
  %i.af = fadd reassoc nsz arcp contract afn float %i.ac, %i.ae
  %i.ag = fadd reassoc nsz arcp contract afn float %i.af, %i.ad
  %3 = tail call reassoc nsz arcp contract afn float @llvm.maxnum.f32(float %i.ag, float 0.000000e+00)
  %4 = fmul reassoc nnan nsz arcp contract afn float %3, f0x38D1B717
  %i.ah = tail call reassoc nsz arcp contract afn float @llvm.pow.f32(float %4, float f0x3E232000) ; 2 uses
  %i.ai = fmul reassoc nsz arcp contract afn float %i.ah, f0x4196D000
  %i.aj = fadd reassoc nsz arcp contract afn float %i.ai, f0x3F560000
  %i.ak = fmul reassoc nsz arcp contract afn float %i.ah, 1.868750e+01
  %i.al = fadd reassoc nsz arcp contract afn float %i.ak, 1.000000e+00
  %i.am = fdiv reassoc nsz arcp contract afn float %i.aj, %i.al
  %i.an = tail call reassoc nsz arcp contract afn float @llvm.pow.f32(float %i.am, float f0x430608CD) ; 2 uses
  %i.ao = fmul reassoc nsz arcp contract afn float %i.an, 0.000000e+00
  %i.ap = insertelement <2 x float> poison, float %i.ab, i64 0
  %i.aq = insertelement <2 x float> %i.ap, float %i.v, i64 1
  %i.ar = fadd reassoc nsz arcp contract afn <2 x float> %i.aq, %i.z
  %5 = tail call reassoc nsz arcp contract afn <2 x float> @llvm.maxnum.v2f32(<2 x float> %i.ar, <2 x float> zeroinitializer)
  %6 = fmul reassoc nnan nsz arcp contract afn <2 x float> %5, splat (float f0x38D1B717)
  %i.as = tail call reassoc nsz arcp contract afn <2 x float> @llvm.pow.v2f32(<2 x float> %6, <2 x float> splat (float f0x3E232000)) ; 2 uses
  %i.at = fmul reassoc nsz arcp contract afn <2 x float> %i.as, splat (float f0x4196D000)
  %i.au = fadd reassoc nsz arcp contract afn <2 x float> %i.at, splat (float f0x3F560000)
  %i.av = fmul reassoc nsz arcp contract afn <2 x float> %i.as, splat (float 1.868750e+01)
  %i.aw = fadd reassoc nsz arcp contract afn <2 x float> %i.av, splat (float 1.000000e+00)
  %i.ax = fdiv reassoc nsz arcp contract afn <2 x float> %i.au, %i.aw
  %i.ay = tail call reassoc nsz arcp contract afn <2 x float> @llvm.pow.v2f32(<2 x float> %i.ax, <2 x float> splat (float f0x430608CD)) ; 3 uses
  %i.az = tail call reassoc nsz arcp contract afn float @llvm.vector.reduce.fadd.v2f32(float 0.000000e+00, <2 x float> %i.ay)
  %i.ba = fmul reassoc nsz arcp contract afn float %i.az, 5.000000e-01
  %i.bb = fadd reassoc nsz arcp contract afn float %i.ba, %i.ao ; 2 uses
  %i.bc = fmul reassoc nsz arcp contract afn <2 x float> %i.ay, <float f0xC0822279, float 1.990760e-01>
  %i.bd = shufflevector <2 x float> %i.bc, <2 x float> poison, <2 x i32> <i32 1, i32 0>
  %i.be = fmul reassoc nsz arcp contract afn <2 x float> %i.ay, <float f0x3F8C63E9, float 3.524000e+00>
  %i.bf = fadd reassoc nsz arcp contract afn <2 x float> %i.be, %i.bd
  ...
  %i.bi = fmul reassoc nsz arcp contract afn <2 x float> %i.bh, <float f0xBFA5DF3B, float 5.427080e-01>
  %i.bj = fadd reassoc nsz arcp contract afn <2 x float> %i.bf, %i.bi
  %i.bk = fmul reassoc nsz arcp contract afn float %i.bb, 4.400000e-01
  %i.bl = fmul reassoc nsz arcp contract afn float %i.bb, 5.600000e-01
  %i.bm = fsub reassoc nsz arcp contract afn float 1.000000e+00, %i.bl
  %i.bn = fdiv reassoc nsz arcp contract afn float %i.bk, %i.bm
  %i.bo = fadd reassoc nsz arcp contract afn float %i.bn, -1.629550e-11
  %i.bp = tail call reassoc nsz arcp contract afn float @llvm.maxnum.f32(float %i.bo, float 0.000000e+00)
  call void @llvm.lifetime.end.p0(ptr nonnull %i.a) #7
  br label %bb.d

bb.c:                                             ; preds = %bb.a
  %i.bq = load float, ptr %0, align 4, !tbaa !39  ; 2 uses
  %i.br = fmul reassoc nsz arcp contract afn float %i.bq, 1.150000e+00
  %i.bs = getelementptr inbounds nuw i8, ptr %0, i64 8
  %i.bt = load float, ptr %i.bs, align 4, !tbaa !39 ; 4 uses
  %i.bu = fmul reassoc nsz arcp contract afn float %i.bt, f0x3E199998
  %i.bv = fsub reassoc nsz arcp contract afn float %i.br, %i.bu ; 3 uses
  %i.bw = getelementptr inbounds nuw i8, ptr %0, i64 4
  %i.bx = load float, ptr %i.bw, align 4, !tbaa !39
  %i.by = fmul reassoc nsz arcp contract afn float %i.bx, 6.600000e-01
  %i.bz = fmul reassoc nsz arcp contract afn float %i.bq, f0x3EAE147A
  %i.ca = fadd reassoc nsz arcp contract afn float %i.by, %i.bz ; 2 uses
  %i.cb = fmul reassoc nsz arcp contract afn float %i.bv, f0x3ED45F51
  %i.cc = fmul reassoc nsz arcp contract afn float %i.bt, 1.464800e-02
  %i.cd = fadd reassoc nsz arcp contract afn float %i.cb, %i.cc
  %i.ce = fmul reassoc nsz arcp contract afn float %i.bv, -2.015100e-01
  ...
  %i.ch = fmul reassoc nsz arcp contract afn <2 x float> %i.cg, <float f0x3F8F716D, float 5.799990e-01>
  %i.ci = fmul reassoc nsz arcp contract afn float %i.bt, 5.310080e-02
  %i.cj = fadd reassoc nsz arcp contract afn float %i.ce, %i.ci
  %i.ck = fmul reassoc nsz arcp contract afn float %i.bv, -1.660080e-02
  %i.cl = fmul reassoc nsz arcp contract afn float %i.ca, 2.648000e-01
  %i.cm = fmul reassoc nsz arcp contract afn float %i.bt, f0x3F2B2180
  %i.cn = fadd reassoc nsz arcp contract afn float %i.ck, %i.cm
  %i.co = fadd reassoc nsz arcp contract afn float %i.cn, %i.cl
  %7 = tail call reassoc nsz arcp contract afn float @llvm.maxnum.f32(float %i.co, float 0.000000e+00)
  %8 = fmul reassoc nnan nsz arcp contract afn float %7, f0x38D1B717
  %i.cp = tail call reassoc nsz arcp contract afn float @llvm.pow.f32(float %8, float f0x3E232000) ; 2 uses
  %i.cq = fmul reassoc nsz arcp contract afn float %i.cp, f0x4196D000
  %i.cr = fadd reassoc nsz arcp contract afn float %i.cq, f0x3F560000
  %i.cs = fmul reassoc nsz arcp contract afn float %i.cp, 1.868750e+01
  %i.ct = fadd reassoc nsz arcp contract afn float %i.cs, 1.000000e+00
  %i.cu = fdiv reassoc nsz arcp contract afn float %i.cr, %i.ct
  %i.cv = tail call reassoc nsz arcp contract afn float @llvm.pow.f32(float %i.cu, float f0x430608CD) ; 2 uses
  %i.cw = fmul reassoc nsz arcp contract afn float %i.cv, 0.000000e+00
  %i.cx = insertelement <2 x float> poison, float %i.cj, i64 0
  %i.cy = insertelement <2 x float> %i.cx, float %i.cd, i64 1
  %i.cz = fadd reassoc nsz arcp contract afn <2 x float> %i.cy, %i.ch
  %9 = tail call reassoc nsz arcp contract afn <2 x float> @llvm.maxnum.v2f32(<2 x float> %i.cz, <2 x float> zeroinitializer)
  %10 = fmul reassoc nnan nsz arcp contract afn <2 x float> %9, splat (float f0x38D1B717)
  %i.da = tail call reassoc nsz arcp contract afn <2 x float> @llvm.pow.v2f32(<2 x float> %10, <2 x float> splat (float f0x3E232000)) ; 2 uses
  %i.db = fmul reassoc nsz arcp contract afn <2 x float> %i.da, splat (float f0x4196D000)
  %i.dc = fadd reassoc nsz arcp contract afn <2 x float> %i.db, splat (float f0x3F560000)
  %i.dd = fmul reassoc nsz arcp contract afn <2 x float> %i.da, splat (float 1.868750e+01)
  %i.de = fadd reassoc nsz arcp contract afn <2 x float> %i.dd, splat (float 1.000000e+00)
  %i.df = fdiv reassoc nsz arcp contract afn <2 x float> %i.dc, %i.de
  %i.dg = tail call reassoc nsz arcp contract afn <2 x float> @llvm.pow.v2f32(<2 x float> %i.df, <2 x float> splat (float f0x430608CD)) ; 3 uses
  %i.dh = tail call reassoc nsz arcp contract afn float @llvm.vector.reduce.fadd.v2f32(float 0.000000e+00, <2 x float> %i.dg)
  %i.di = fmul reassoc nsz arcp contract afn float %i.dh, 5.000000e-01
  %i.dj = fadd reassoc nsz arcp contract afn float %i.di, %i.cw ; 2 uses
  %i.dk = fmul reassoc nsz arcp contract afn <2 x float> %i.dg, <float f0xC0822279, float 1.990760e-01>
  %i.dl = shufflevector <2 x float> %i.dk, <2 x float> poison, <2 x i32> <i32 1, i32 0>
  %i.dm = fmul reassoc nsz arcp contract afn <2 x float> %i.dg, <float f0x3F8C63E9, float 3.524000e+00>
  %i.dn = fadd reassoc nsz arcp contract afn <2 x float> %i.dm, %i.dl
  %i.do = insertelement <2 x float> poison, float %i.cv, i64 0
  %i.dp = shufflevector <2 x float> %i.do, <2 x float> poison, <2 x i32> zeroinitializer
  %i.dq = fmul reassoc nsz arcp contract afn <2 x float> %i.dp, <float f0xBFA5DF3B, float 5.427080e-01>
  %i.dr = fadd reassoc nsz arcp contract afn <2 x float> %i.dn, %i.dq
  %i.ds = fmul reassoc nsz arcp contract afn float %i.dj, 4.400000e-01
  %i.dt = fmul reassoc nsz arcp contract afn float %i.dj, 5.600000e-01
  %i.du = fsub reassoc nsz arcp contract afn float 1.000000e+00, %i.dt
  %i.dv = fdiv reassoc nsz arcp contract afn float %i.ds, %i.du
  %i.dw = fadd reassoc nsz arcp contract afn float %i.dv, -1.629550e-11
  %i.dx = tail call reassoc nsz arcp contract afn float @llvm.maxnum.f32(float %i.dw, float 0.000000e+00)
  br label %bb.d

bb.d:                                             ; preds = %bb.c, %bb.b
  %.sroa.0.0 = phi nsz float [ %i.dx, %bb.c ], [ %i.bp, %bb.b ]
  %i.dy = phi <2 x float> [ %i.dr, %bb.c ], [ %i.bj, %bb.b ] ; 2 uses
  %i.dz = extractelement <2 x float> %i.dy, i64 0 ; 2 uses
  %i.ea = extractelement <2 x float> %i.dy, i64 1 ; 2 uses
  %i.eb = tail call reassoc nsz arcp contract afn float @llvm.atan2.f32(float %i.dz, float %i.ea)
  %i.ec = fmul reassoc nsz arcp contract afn float %i.eb, f0x3E22F983 ; 3 uses
  store float %.sroa.0.0, ptr %1, align 4, !tbaa !39
  %i.ed = tail call reassoc nsz arcp contract afn float @hypotf(float noundef %i.ea, float noundef %i.dz) #16
  %i.ee = getelementptr inbounds nuw i8, ptr %1, i64 4
  store float %i.ed, ptr %i.ee, align 4, !tbaa !39
  %i.ef = fcmp reassoc nsz arcp contract afn oge float %i.ec, 0.000000e+00
  %i.eg = fadd reassoc nsz arcp contract afn float %i.ec, 1.000000e+00
  %i.eh = select reassoc nsz arcp contract afn i1 %i.ef, float %i.ec, float %i.eg
  %i.ei = getelementptr inbounds nuw i8, ptr %1, i64 8
  store float %i.eh, ptr %i.ei, align 4, !tbaa !39
  ret void
}
