Download link: https://huggingface.co/buckets/llvm-opt-benchmark/llvm-opt-benchmark/resolve/darktable/original/RawImage?download=true
inline.NumInlined: 589
inline.NumDeleted: 381
loop-unroll.NumCompletelyUnrolled: 1
loop-unroll.NumRuntimeUnrolled: 4
loop-unroll.NumUnrolled: 5
begin_hunk_0_@_ZN8rawspeed12RawImageData22transferBadPixelsToMapEv:bb.a
  %i.t = lshr i64 %i.s, 3                         ; 2 uses
  %i.u = add nuw nsw i64 %i.t, 1                  ; 2 uses
  %i.v = and i64 %i.u, 15                         ; 2 uses
  %i.w = icmp eq i64 %i.v, 0
  %i.x = add nuw nsw i64 %i.t, 17
  %.0.i.i.i = sub nuw nsw i64 %i.x, %i.v
  %spec.select.i = select i1 %i.w, i64 %i.u, i64 %.0.i.i.i
  %i.y = trunc i64 %spec.select.i to i32
  br label %_ZN8rawspeed7roundUpEmm.exit.i

_ZN8rawspeed7roundUpEmm.exit.i:                   ; preds = %_ZN8rawspeed19roundUpDivisionSafeEmm.exit.i, %bb.d
  %i.z = phi i32 [ %i.y, %_ZN8rawspeed19roundUpDivisionSafeEmm.exit.i ], [ 0, %bb.d ] ; 2 uses
  %i.aa = getelementptr inbounds nuw i8, ptr %0, i64 240
  store i32 %i.z, ptr %i.aa, align 8, !tbaa !370
  %i.ab = zext i32 %i.z to i64
  %i.ac = getelementptr inbounds nuw i8, ptr %0, i64 612
  %i.ad = load i32, ptr %i.ac, align 4, !tbaa !371
  %i.ae = sext i32 %i.ad to i64
  %i.af = mul nsw i64 %i.ae, %i.ab                ; 2 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.a) #28
  store i8 0, ptr %i.a, align 1, !tbaa !344
  %.not = icmp eq i64 %i.af, 0
  br i1 %.not, label %_ZN8rawspeed12RawImageData17createBadPixelMapEv.exit, label %bb.e

bb.e:                                             ; preds = %_ZN8rawspeed7roundUpEmm.exit.i
  call void @_ZNSt6vectorIhN8rawspeed16AlignedAllocatorIhLi16EEEE14_M_fill_insertEN9__gnu_cxx17__normal_iteratorIPhS3_EEmRKh(ptr noundef nonnull align 8 dereferenceable(24) %i.g, ptr %i.j, i64 noundef %i.af, ptr noundef nonnull align 1 dereferenceable(1) %i.a)
  %.pre.pre = load ptr, ptr %i.b, align 8, !tbaa !6918
  %.pre16.pre = load ptr, ptr %i.d, align 8, !tbaa !6918
  br label %_ZN8rawspeed12RawImageData17createBadPixelMapEv.exit

_ZN8rawspeed12RawImageData17createBadPixelMapEv.exit: ; preds = %_ZN8rawspeed7roundUpEmm.exit.i, %bb.e
  %.pre16 = phi ptr [ %.pre16.pre, %bb.e ], [ %i.e, %_ZN8rawspeed7roundUpEmm.exit.i ]
  %.pre = phi ptr [ %.pre.pre, %bb.e ], [ %i.c, %_ZN8rawspeed7roundUpEmm.exit.i ]
  call void @llvm.lifetime.end.p0(ptr nonnull %i.a) #28
  br label %bb.f

bb.f:                                             ; preds = %_ZN8rawspeed12RawImageData17createBadPixelMapEv.exit, %bb.b
  %i.ag = phi ptr [ %.pre16, %_ZN8rawspeed12RawImageData17createBadPixelMapEv.exit ], [ %i.e, %bb.b ] ; 2 uses
  %i.ah = phi ptr [ %.pre, %_ZN8rawspeed12RawImageData17createBadPixelMapEv.exit ], [ %i.c, %bb.b ] ; 2 uses
  %i.ai = icmp eq ptr %i.ah, %i.ag
  br i1 %i.ai, label %_ZNSt6vectorIjSaIjEE5clearEv.exit, label %.lr.ph

.lr.ph:                                           ; preds = %bb.f
  %i.aj = getelementptr inbounds nuw i8, ptr %0, i64 240
  br label %bb.g

._crit_edge:                                      ; preds = %bb.g
  %.pre17 = load ptr, ptr %i.b, align 8, !tbaa !351 ; 2 uses
  %.pre18 = load ptr, ptr %i.d, align 8, !tbaa !6919
  %i.ak = icmp eq ptr %.pre18, %.pre17
  br i1 %i.ak, label %_ZNSt6vectorIjSaIjEE5clearEv.exit, label %_ZSt8_DestroyIPjjEvT_S1_RSaIT0_E.exit.i.i

_ZSt8_DestroyIPjjEvT_S1_RSaIT0_E.exit.i.i:        ; preds = %._crit_edge
  store ptr %.pre17, ptr %i.d, align 8, !tbaa !6919
  br label %_ZNSt6vectorIjSaIjEE5clearEv.exit

bb.g:                                             ; preds = %.lr.ph, %bb.g
  %.sroa.011.015 = phi ptr [ %i.ah, %.lr.ph ], [ %i.ba, %bb.g ] ; 2 uses
  %i.al = load i32, ptr %.sroa.011.015, align 4, !tbaa !373 ; 3 uses
  %i.am = lshr i32 %i.al, 16
  %i.an = and i32 %i.al, 7
  %i.ao = shl nuw nsw i32 1, %i.an
  %i.ap = load i32, ptr %i.aj, align 8, !tbaa !370
  %i.aq = mul i32 %i.ap, %i.am
  %i.ar = lshr i32 %i.al, 3
  %i.as = and i32 %i.ar, 8191
  %i.at = add i32 %i.aq, %i.as
  %i.au = zext i32 %i.at to i64
  %i.av = load ptr, ptr %i.g, align 8, !tbaa !348
  %i.aw = getelementptr inbounds nuw i8, ptr %i.av, i64 %i.au ; 2 uses
  %i.ax = load i8, ptr %i.aw, align 1, !tbaa !344
  %i.ay = trunc nuw i32 %i.ao to i8
  %i.az = or i8 %i.ax, %i.ay
  store i8 %i.az, ptr %i.aw, align 1, !tbaa !344
  %i.ba = getelementptr inbounds nuw i8, ptr %.sroa.011.015, i64 4 ; 2 uses
  %i.bb = icmp eq ptr %i.ba, %i.ag
  br i1 %i.bb, label %._crit_edge, label %bb.g

_ZNSt6vectorIjSaIjEE5clearEv.exit:                ; preds = %bb.f, %_ZSt8_DestroyIPjjEvT_S1_RSaIT0_E.exit.i.i, %._crit_edge, %bb.a
  ret void
}

; Function Attrs: mustprogress uwtable
define hidden void @_ZN8rawspeed12RawImageData12fixBadPixelsEv(ptr noundef nonnull align 8 dereferenceable(624) %0) local_unnamed_addr #1 align 2 {
bb.a:
  %1 = alloca %"class.rawspeed::RawImageWorker", align 8 ; 7 uses
  tail call void @_ZN8rawspeed12RawImageData22transferBadPixelsToMapEv(ptr noundef nonnull align 8 dereferenceable(624) %0)
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 216
  %i.b = load ptr, ptr %i.a, align 8, !tbaa !357
  %i.c = getelementptr inbounds nuw i8, ptr %0, i64 224
  %i.d = load ptr, ptr %i.c, align 8, !tbaa !357
  %i.e = icmp eq ptr %i.b, %i.d
  br i1 %i.e, label %_ZN8rawspeed12RawImageData11startWorkerENS_14RawImageWorker18RawImageWorkerTaskEb.exit, label %bb.b

bb.b:                                             ; preds = %bb.a
  %.0.in.i.i = getelementptr inbounds nuw i8, ptr %0, i64 612
  %.0.i.i = load i32, ptr %.0.in.i.i, align 4, !tbaa !373 ; 3 uses
  %i.f = tail call i32 @rawspeed_get_number_of_processor_cores() ; 4 uses
  %i.g = add i32 %.0.i.i, -1
  %i.h = add i32 %i.g, %i.f
  %i.i = sdiv i32 %i.h, %i.f                      ; 2 uses
  %i.j = icmp sgt i32 %i.f, 0
  br i1 %i.j, label %.lr.ph.i, label %_ZN8rawspeed12RawImageData11startWorkerENS_14RawImageWorker18RawImageWorkerTaskEb.exit

.lr.ph.i:                                         ; preds = %bb.b
  %i.k = getelementptr inbounds nuw i8, ptr %1, i64 8
  %i.l = getelementptr inbounds nuw i8, ptr %1, i64 12
  %i.m = getelementptr inbounds nuw i8, ptr %1, i64 16
  br label %bb.c

bb.c:                                             ; preds = %bb.c, %.lr.ph.i
  %.024.i = phi i32 [ 0, %.lr.ph.i ], [ %i.o, %bb.c ] ; 2 uses
  %i.n = mul nsw i32 %.024.i, %i.i
  %.sroa.speculated14.i = tail call i32 @llvm.smin.i32(i32 %.0.i.i, i32 %i.n)
  %i.o = add nuw nsw i32 %.024.i, 1               ; 3 uses
  %i.p = mul nsw i32 %i.o, %i.i
  %.sroa.speculated.i = tail call i32 @llvm.smin.i32(i32 %.0.i.i, i32 %i.p)
  call void @llvm.lifetime.start.p0(ptr nonnull %1) #28
  store ptr %0, ptr %1, align 8, !tbaa !377
  store i16 2, ptr %i.k, align 8, !tbaa !378
  store i32 %.sroa.speculated14.i, ptr %i.l, align 4, !tbaa !379
  store i32 %.sroa.speculated.i, ptr %i.m, align 8, !tbaa !380
  call void @_ZN8rawspeed14RawImageWorker11performTaskEv(ptr noundef nonnull align 8 dereferenceable(24) %1) #28
  call void @llvm.lifetime.end.p0(ptr nonnull %1) #28
  %exitcond.not.i = icmp eq i32 %i.o, %i.f
  br i1 %exitcond.not.i, label %_ZN8rawspeed12RawImageData11startWorkerENS_14RawImageWorker18RawImageWorkerTaskEb.exit, label %bb.c, !llvm.loop !1

_ZN8rawspeed12RawImageData11startWorkerENS_14RawImageWorker18RawImageWorkerTaskEb.exit: ; preds = %bb.c, %bb.b, %bb.a
  ret void
}

; Function Attrs: mustprogress uwtable
define hidden void @_ZN8rawspeed12RawImageData11startWorkerENS_14RawImageWorker18RawImageWorkerTaskEb(ptr noundef nonnull align 8 dereferenceable(624) %0, i16 noundef zeroext %1, i1 noundef zeroext %2) local_unnamed_addr #1 align 2 {
bb.a:
  %3 = alloca %"class.rawspeed::RawImageWorker", align 8 ; 7 uses
  %i.a = and i16 %1, 4096
  %.not.i = icmp eq i16 %i.a, 0
  %i.b = and i1 %2, %.not.i
  %.0.in.v.i = select i1 %i.b, i64 44, i64 612
  %.0.in.i = getelementptr inbounds nuw i8, ptr %0, i64 %.0.in.v.i
  %.0.i = load i32, ptr %.0.in.i, align 4, !tbaa !373 ; 3 uses
  %i.c = tail call i32 @rawspeed_get_number_of_processor_cores() ; 4 uses
  %i.d = add i32 %.0.i, -1
  %i.e = add i32 %i.d, %i.c
  %i.f = sdiv i32 %i.e, %i.c                      ; 2 uses
  %i.g = icmp sgt i32 %i.c, 0
  br i1 %i.g, label %.lr.ph, label %._crit_edge

.lr.ph:                                           ; preds = %bb.a
  %i.h = getelementptr inbounds nuw i8, ptr %3, i64 8
  %i.i = getelementptr inbounds nuw i8, ptr %3, i64 12
  %i.j = getelementptr inbounds nuw i8, ptr %3, i64 16
  br label %bb.b

._crit_edge:                                      ; preds = %bb.b, %bb.a
  ret void

bb.b:                                             ; preds = %.lr.ph, %bb.b
  %.024 = phi i32 [ 0, %.lr.ph ], [ %i.l, %bb.b ] ; 2 uses
  %i.k = mul nsw i32 %.024, %i.f
  %.sroa.speculated14 = tail call i32 @llvm.smin.i32(i32 %.0.i, i32 %i.k)
  %i.l = add nuw nsw i32 %.024, 1                 ; 3 uses
  %i.m = mul nsw i32 %i.l, %i.f
  %.sroa.speculated = tail call i32 @llvm.smin.i32(i32 %.0.i, i32 %i.m)
  call void @llvm.lifetime.start.p0(ptr nonnull %3) #28
  store ptr %0, ptr %3, align 8, !tbaa !377
  store i16 %1, ptr %i.h, align 8, !tbaa !378
  store i32 %.sroa.speculated14, ptr %i.i, align 4, !tbaa !379
  store i32 %.sroa.speculated, ptr %i.j, align 8, !tbaa !380
  call void @_ZN8rawspeed14RawImageWorker11performTaskEv(ptr noundef nonnull align 8 dereferenceable(24) %3) #28
  call void @llvm.lifetime.end.p0(ptr nonnull %3) #28
  %exitcond.not = icmp eq i32 %i.l, %i.c
  br i1 %exitcond.not, label %._crit_edge, label %bb.b, !llvm.loop !1
}

declare i32 @rawspeed_get_number_of_processor_cores() local_unnamed_addr #7

; Function Attrs: mustprogress uwtable
define hidden void @_ZN8rawspeed12RawImageData18fixBadPixelsThreadEii(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %1, i32 noundef %2) local_unnamed_addr #1 align 2 {
bb.a:
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 608
  %i.b = load i32, ptr %i.a, align 8, !tbaa !369  ; 2 uses
  %i.c = getelementptr inbounds nuw i8, ptr %0, i64 216
  %i.d = load ptr, ptr %i.c, align 8, !tbaa !348
  %i.e = getelementptr inbounds nuw i8, ptr %0, i64 240
  %i.f = load i32, ptr %i.e, align 8, !tbaa !370  ; 3 uses
  %i.g = icmp sgt i32 %i.f, -1
  tail call void @llvm.assume(i1 %i.g)
  %i.h = icmp ne i32 %i.f, 0
  tail call void @llvm.assume(i1 %i.h)
  %i.i = icmp slt i32 %1, %2
  %i.j = icmp sgt i32 %i.b, 16
  %or.cond = select i1 %i.i, i1 %i.j, i1 false
  br i1 %or.cond, label %.preheader42.preheader, label %._crit_edge55.split

.preheader42.preheader:                           ; preds = %bb.a
  %i.k = getelementptr inbounds nuw i8, ptr %0, i64 612
  %i.l = load i32, ptr %i.k, align 4, !tbaa !371
  %i.m = add nuw nsw i32 %i.b, 15
  %i.n = lshr i32 %i.m, 5
  %i.o = zext nneg i32 %i.f to i64                ; 3 uses
  %i.p = zext i32 %1 to i64
  %i.q = zext nneg i32 %i.l to i64
  %wide.trip.count = zext nneg i32 %i.n to i64
  br label %.preheader42

.preheader42:                                     ; preds = %.preheader42.preheader, %._crit_edge
  %indvars.iv61 = phi i64 [ %i.p, %.preheader42.preheader ], [ %indvars.iv.next62, %._crit_edge ] ; 11 uses
  %i.r = icmp samesign ult i64 %indvars.iv61, %i.q
  tail call void @llvm.assume(i1 %i.r)
  %i.s = mul nuw nsw i64 %indvars.iv61, %i.o
  %i.t = getelementptr inbounds nuw i8, ptr %i.d, i64 %i.s ; 3 uses
  %i.u = trunc nuw nsw i64 %indvars.iv61 to i32
  %i.v = trunc nuw nsw i64 %indvars.iv61 to i32
  %i.w = trunc nuw nsw i64 %indvars.iv61 to i32
  %i.x = trunc nuw nsw i64 %indvars.iv61 to i32
  %i.y = trunc nuw nsw i64 %indvars.iv61 to i32
  %i.z = trunc nuw nsw i64 %indvars.iv61 to i32
  %i.aa = trunc nuw nsw i64 %indvars.iv61 to i32
  %i.ab = trunc nuw nsw i64 %indvars.iv61 to i32
  br label %.lr.ph.preheader.i.i.i.i

._crit_edge55.split:                              ; preds = %._crit_edge, %bb.a
  ret void

._crit_edge:                                      ; preds = %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit.thread"
  %indvars.iv.next62 = add nuw nsw i64 %indvars.iv61, 1 ; 2 uses
  %i.ac = trunc nuw nsw i64 %indvars.iv.next62 to i32
  %i.ad = icmp sgt i32 %2, %i.ac
  br i1 %i.ad, label %.preheader42, label %._crit_edge55.split, !llvm.loop !6920

.lr.ph.preheader.i.i.i.i:                         ; preds = %.preheader42, %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit.thread"
  %indvars.iv57 = phi i64 [ 0, %.preheader42 ], [ %indvars.iv.next58, %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit.thread" ] ; 3 uses
  %i.ae = shl nuw nsw i64 %indvars.iv57, 2        ; 3 uses
  %i.af = add nuw nsw i64 %i.ae, 4                ; 2 uses
  %i.ag = icmp samesign ule i64 %i.af, %i.o
  tail call void @llvm.assume(i1 %i.ag)
  %i.ah = getelementptr inbounds nuw i8, ptr %i.t, i64 %i.af
  %i.ai = getelementptr inbounds nuw i8, ptr %i.t, i64 %i.ae ; 5 uses
  %.029.val32.i.i.i.i = load i8, ptr %i.ai, align 1, !tbaa !344
  %.not35.i.i.i.i = icmp eq i8 %.029.val32.i.i.i.i, 0
  br i1 %.not35.i.i.i.i, label %bb.b, label %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit"

bb.b:                                             ; preds = %.lr.ph.preheader.i.i.i.i
  %i.aj = getelementptr inbounds nuw i8, ptr %i.ai, i64 1 ; 2 uses
  %.val31.i.i.i.i = load i8, ptr %i.aj, align 1, !tbaa !344
  %.not36.i.i.i.i = icmp eq i8 %.val31.i.i.i.i, 0
  br i1 %.not36.i.i.i.i, label %bb.c, label %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit"

bb.c:                                             ; preds = %bb.b
  %i.ak = getelementptr inbounds nuw i8, ptr %i.ai, i64 2 ; 2 uses
  %.val30.i.i.i.i = load i8, ptr %i.ak, align 1, !tbaa !344
  %.not37.i.i.i.i = icmp eq i8 %.val30.i.i.i.i, 0
  br i1 %.not37.i.i.i.i, label %bb.d, label %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit"

bb.d:                                             ; preds = %bb.c
  %i.al = getelementptr inbounds nuw i8, ptr %i.ai, i64 3 ; 2 uses
  %.val.i.i.i.i = load i8, ptr %i.al, align 1, !tbaa !344
  %.not38.i.i.i.i = icmp eq i8 %.val.i.i.i.i, 0
  br i1 %.not38.i.i.i.i, label %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit.thread", label %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit"

"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit": ; preds = %bb.d, %bb.c, %bb.b, %.lr.ph.preheader.i.i.i.i
  %.028.i.i.i.i = phi ptr [ %i.aj, %bb.b ], [ %i.ai, %.lr.ph.preheader.i.i.i.i ], [ %i.ak, %bb.c ], [ %i.al, %bb.d ]
  %i.am = icmp eq ptr %i.ah, %.028.i.i.i.i
  br i1 %i.am, label %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit.thread", label %.preheader41

.preheader41:                                     ; preds = %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit"
  %i.an = shl nuw nsw i64 %indvars.iv57, 5
  br label %.preheader

.preheader:                                       ; preds = %.preheader41, %bb.t
  %indvars.iv = phi i64 [ 0, %.preheader41 ], [ %indvars.iv.next, %bb.t ] ; 3 uses
  %i.ao = or disjoint i64 %indvars.iv, %i.ae      ; 2 uses
  %i.ap = icmp samesign ule i64 %i.ao, %i.o
  tail call void @llvm.assume(i1 %i.ap)
  %i.aq = getelementptr inbounds nuw i8, ptr %i.t, i64 %i.ao ; 8 uses
  %i.ar = shl nuw nsw i64 %indvars.iv, 3
  %i.as = add nuw nsw i64 %i.ar, %i.an            ; 8 uses
  %i.at = load i8, ptr %i.aq, align 1, !tbaa !344
  %i.au = zext i8 %i.at to i32                    ; 2 uses
  %i.av = and i32 %i.au, 1
  %.not.not = icmp eq i32 %i.av, 0
  br i1 %.not.not, label %bb.f, label %bb.e

bb.e:                                             ; preds = %.preheader
  %i.aw = load ptr, ptr %0, align 8, !tbaa !265
  %i.ax = getelementptr inbounds nuw i8, ptr %i.aw, i64 64
  %i.ay = load ptr, ptr %i.ax, align 8
  %i.az = trunc nuw nsw i64 %i.as to i32
  tail call void %i.ay(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.az, i32 noundef %i.u, i32 noundef 0), !call_target !391
  %.pre = load i8, ptr %i.aq, align 1, !tbaa !344
  %.pre70 = zext i8 %.pre to i32
  br label %bb.f

bb.f:                                             ; preds = %.preheader, %bb.e
  %.pre-phi = phi i32 [ %i.au, %.preheader ], [ %.pre70, %bb.e ] ; 2 uses
  %i.ba = and i32 %.pre-phi, 2
  %.not.not.1 = icmp eq i32 %i.ba, 0
  br i1 %.not.not.1, label %bb.h, label %bb.g

bb.g:                                             ; preds = %bb.f
  %i.bb = load ptr, ptr %0, align 8, !tbaa !265
  %i.bc = getelementptr inbounds nuw i8, ptr %i.bb, i64 64
  %i.bd = load ptr, ptr %i.bc, align 8
  %i.be = trunc i64 %i.as to i32
  %i.bf = or disjoint i32 %i.be, 1
  tail call void %i.bd(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.bf, i32 noundef %i.v, i32 noundef 0), !call_target !391
  %.pre64 = load i8, ptr %i.aq, align 1, !tbaa !344
  %.pre71 = zext i8 %.pre64 to i32
  br label %bb.h

bb.h:                                             ; preds = %bb.g, %bb.f
  %.pre-phi72 = phi i32 [ %.pre71, %bb.g ], [ %.pre-phi, %bb.f ] ; 2 uses
  %i.bg = and i32 %.pre-phi72, 4
  %.not.not.2 = icmp eq i32 %i.bg, 0
  br i1 %.not.not.2, label %bb.j, label %bb.i

bb.i:                                             ; preds = %bb.h
  %i.bh = load ptr, ptr %0, align 8, !tbaa !265
  %i.bi = getelementptr inbounds nuw i8, ptr %i.bh, i64 64
  %i.bj = load ptr, ptr %i.bi, align 8
  %i.bk = trunc i64 %i.as to i32
  %i.bl = or disjoint i32 %i.bk, 2
  tail call void %i.bj(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.bl, i32 noundef %i.w, i32 noundef 0), !call_target !391
  %.pre65 = load i8, ptr %i.aq, align 1, !tbaa !344
  %.pre73 = zext i8 %.pre65 to i32
  br label %bb.j

bb.j:                                             ; preds = %bb.i, %bb.h
  %.pre-phi74 = phi i32 [ %.pre73, %bb.i ], [ %.pre-phi72, %bb.h ] ; 2 uses
  %i.bm = and i32 %.pre-phi74, 8
  %.not.not.3 = icmp eq i32 %i.bm, 0
  br i1 %.not.not.3, label %bb.l, label %bb.k

bb.k:                                             ; preds = %bb.j
  %i.bn = load ptr, ptr %0, align 8, !tbaa !265
  %i.bo = getelementptr inbounds nuw i8, ptr %i.bn, i64 64
  %i.bp = load ptr, ptr %i.bo, align 8
  %i.bq = trunc i64 %i.as to i32
  %i.br = or disjoint i32 %i.bq, 3
  tail call void %i.bp(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.br, i32 noundef %i.x, i32 noundef 0), !call_target !391
  %.pre66 = load i8, ptr %i.aq, align 1, !tbaa !344
  %.pre75 = zext i8 %.pre66 to i32
  br label %bb.l

bb.l:                                             ; preds = %bb.k, %bb.j
  %.pre-phi76 = phi i32 [ %.pre75, %bb.k ], [ %.pre-phi74, %bb.j ] ; 2 uses
  %i.bs = and i32 %.pre-phi76, 16
  %.not.not.4 = icmp eq i32 %i.bs, 0
  br i1 %.not.not.4, label %bb.n, label %bb.m

bb.m:                                             ; preds = %bb.l
  %i.bt = load ptr, ptr %0, align 8, !tbaa !265
  %i.bu = getelementptr inbounds nuw i8, ptr %i.bt, i64 64
  %i.bv = load ptr, ptr %i.bu, align 8
  %i.bw = trunc i64 %i.as to i32
  %i.bx = or disjoint i32 %i.bw, 4
  tail call void %i.bv(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.bx, i32 noundef %i.y, i32 noundef 0), !call_target !391
  %.pre67 = load i8, ptr %i.aq, align 1, !tbaa !344
  %.pre77 = zext i8 %.pre67 to i32
  br label %bb.n

bb.n:                                             ; preds = %bb.m, %bb.l
  %.pre-phi78 = phi i32 [ %.pre77, %bb.m ], [ %.pre-phi76, %bb.l ] ; 2 uses
  %i.by = and i32 %.pre-phi78, 32
  %.not.not.5 = icmp eq i32 %i.by, 0
  br i1 %.not.not.5, label %bb.p, label %bb.o

bb.o:                                             ; preds = %bb.n
  %i.bz = load ptr, ptr %0, align 8, !tbaa !265
  %i.ca = getelementptr inbounds nuw i8, ptr %i.bz, i64 64
  %i.cb = load ptr, ptr %i.ca, align 8
  %i.cc = trunc i64 %i.as to i32
  %i.cd = or disjoint i32 %i.cc, 5
  tail call void %i.cb(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.cd, i32 noundef %i.z, i32 noundef 0), !call_target !391
  %.pre68 = load i8, ptr %i.aq, align 1, !tbaa !344
  %.pre79 = zext i8 %.pre68 to i32
  br label %bb.p

bb.p:                                             ; preds = %bb.o, %bb.n
  %.pre-phi80 = phi i32 [ %.pre79, %bb.o ], [ %.pre-phi78, %bb.n ] ; 2 uses
  %i.ce = and i32 %.pre-phi80, 64
  %.not.not.6 = icmp eq i32 %i.ce, 0
  br i1 %.not.not.6, label %bb.r, label %bb.q

bb.q:                                             ; preds = %bb.p
  %i.cf = load ptr, ptr %0, align 8, !tbaa !265
  %i.cg = getelementptr inbounds nuw i8, ptr %i.cf, i64 64
  %i.ch = load ptr, ptr %i.cg, align 8
  %i.ci = trunc i64 %i.as to i32
  %i.cj = or disjoint i32 %i.ci, 6
  tail call void %i.ch(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.cj, i32 noundef %i.aa, i32 noundef 0), !call_target !391
  %.pre69 = load i8, ptr %i.aq, align 1, !tbaa !344
  %.pre81 = zext i8 %.pre69 to i32
  br label %bb.r

bb.r:                                             ; preds = %bb.q, %bb.p
  %.pre-phi82 = phi i32 [ %.pre81, %bb.q ], [ %.pre-phi80, %bb.p ]
  %i.ck = and i32 %.pre-phi82, 128
  %.not.not.7 = icmp eq i32 %i.ck, 0
  br i1 %.not.not.7, label %bb.t, label %bb.s

bb.s:                                             ; preds = %bb.r
  %i.cl = load ptr, ptr %0, align 8, !tbaa !265
  %i.cm = getelementptr inbounds nuw i8, ptr %i.cl, i64 64
  %i.cn = load ptr, ptr %i.cm, align 8
  %i.co = trunc i64 %i.as to i32
  %i.cp = or disjoint i32 %i.co, 7
  tail call void %i.cn(ptr noundef nonnull align 8 dereferenceable(624) %0, i32 noundef %i.cp, i32 noundef %i.ab, i32 noundef 0), !call_target !391
  br label %bb.t

bb.t:                                             ; preds = %bb.s, %bb.r
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1 ; 2 uses
  %exitcond.not = icmp eq i64 %indvars.iv.next, 4
  br i1 %exitcond.not, label %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit.thread", label %.preheader, !llvm.loop !6921

"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit.thread": ; preds = %bb.t, %bb.d, %"_ZSt6all_ofIPhZN8rawspeed12RawImageData18fixBadPixelsThreadEiiE3$_0EbT_S4_T0_.exit"
  %indvars.iv.next58 = add nuw nsw i64 %indvars.iv57, 1 ; 2 uses
  %exitcond60.not = icmp eq i64 %indvars.iv.next58, %wide.trip.count
  br i1 %exitcond60.not, label %._crit_edge, label %.lr.ph.preheader.i.i.i.i, !llvm.loop !6922
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind memory(write, argmem: readwrite, target_mem: none) uwtable
define hidden void @_ZN8rawspeed12RawImageData9clearAreaENS_12iRectangle2DE(ptr nofree noundef nonnull readonly align 8 captures(none) dereferenceable(624) %0, i64 %1, i64 %2) local_unnamed_addr #8 align 2 personality ptr @__gxx_personality_v0 {
bb.a:
  %.sroa.012.0.extract.trunc14 = trunc i64 %1 to i32 ; 2 uses
  %.sroa.415.0.extract.shift16 = lshr i64 %1, 32
  %.sroa.415.0.extract.trunc17 = trunc nuw i64 %.sroa.415.0.extract.shift16 to i32 ; 2 uses
  %.sroa.7.8.extract.trunc22 = trunc i64 %2 to i32
  %.sroa.1223.8.extract.shift24 = lshr i64 %2, 32
  %.sroa.1223.8.extract.trunc25 = trunc nuw i64 %.sroa.1223.8.extract.shift24 to i32
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 40
  %i.b = load i64, ptr %i.a, align 8              ; 2 uses
end_hunk_0
