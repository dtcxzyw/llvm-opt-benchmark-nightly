...
bb.b:                                             ; preds = %.lr.ph, %bb.b
  ...
  %9 = fadd double %i.bn, %i.bp                   ; 2 uses
  %10 = fsub double %i.bn, %i.bp                  ; 2 uses
  %i.cx = fsub double %i.cn, %i.cp                ; 2 uses
  %i.cy = fsub double %i.cr, %i.ct                ; 2 uses
  %i.cz = insertelement <2 x double> poison, double %9, i64 0
  %i.da = insertelement <2 x double> %i.cz, double %10, i64 1
  ...
  %i.eq = fadd double %i.bk, %i.bl                ; 2 uses
  %i.er = fadd double %i.br, %i.ch                ; 2 uses
  %i.es = fadd double %i.bx, %i.cl                ; 2 uses
  %11 = fsub double %i.es, %i.er                  ; 2 uses
  %i.et = fadd double %i.er, %i.es                ; 2 uses
  %12 = fadd double %i.cx, %i.cy                  ; 2 uses
  %i.eu = fadd double %10, %12                    ; 2 uses
  %13 = fsub double %i.bt, %i.bv                  ; 2 uses
  %i.ev = fsub double %i.bz, %i.cb                ; 2 uses
  %i.ew = fadd double %i.cn, %i.cp                ; 2 uses
  %14 = fadd double %i.cr, %i.ct                  ; 2 uses
  %i.ex = fadd double %i.ew, %14                  ; 2 uses
  %i.ey = insertelement <2 x double> poison, double %i.ex, i64 0
  %15 = fsub double %14, %i.ew
  %16 = fadd double %9, %i.ex                     ; 2 uses
  %i.ez = fsub double %13, %i.ev
  %i.fa = insertelement <2 x double> poison, double %15, i64 0
  %i.fb = insertelement <2 x double> %i.fa, double %i.ez, i64 1
  %i.fc = fmul <2 x double> %i.fb, splat (double f0x3FEBB67AE8584CAA) ; 2 uses
  %i.fd = fadd double %13, %i.ev                  ; 2 uses
  %17 = fadd double %i.fd, %i.db
  %18 = fsub double %i.db, %i.fd
  %i.fe = insertelement <2 x double> poison, double %17, i64 0
  %i.ff = insertelement <2 x double> %i.fe, double %18, i64 1 ; 2 uses
  %i.fg = fmul <2 x double> %i.ff, <double f0xBFC105974D8DEBB7, double f0xBFD01CF9B20F3131>
  %i.fh = insertelement <2 x double> %i.ey, double %12, i64 1
  ...
  %i.fk = fsub double %i.et, %16
  %i.fl = fmul double %i.fk, f0x3FD33AC782EB914D  ; 2 uses
  %i.fm = fadd double %i.et, %16                  ; 2 uses
  ...
  %19 = fsub double %i.dj, %i.fj
  %20 = fadd double %i.dj, %i.fj
  %i.fr = insertelement <2 x double> poison, double %19, i64 0
  %i.fs = insertelement <2 x double> %i.fr, double %20, i64 1 ; 2 uses
  ...
  %i.gr = insertelement <2 x double> %i.fo, double %11, i64 0
  ...
  %21 = insertelement <2 x double> %i.dd, double %i.gv, i64 1
  %22 = fadd double %i.p, %i.ap                   ; 2 uses
  %i.gw = fadd double %i.ab, %i.aq                ; 2 uses
  %i.gx = fsub double %i.gw, %22                  ; 2 uses
  %i.gy = fadd double %22, %i.gw                  ; 2 uses
  %i.gz = fsub double %i.ax, %i.bb                ; 2 uses
  %23 = fadd double %i.de, %i.df                  ; 2 uses
  %i.ha = fadd double %i.gv, %23                  ; 2 uses
  %i.hb = fsub double %i.t, %i.x                  ; 2 uses
  %i.hc = fadd double %i.gy, %i.ha                ; 2 uses
  %i.hd = fsub double %i.gy, %i.ha
  %24 = fmul double %i.hd, f0x3FD33AC782EB914D    ; 2 uses
  %i.he = fadd double %i.c, %i.hc
  store double %i.he, ptr %.0465474, align 8, !tbaa !13
  %25 = fsub double %i.bf, %i.bj                  ; 2 uses
  %i.hf = fadd double %i.gz, %25                  ; 2 uses
  %i.hg = fadd double %i.dc, %i.hf                ; 2 uses
  %26 = fsub double %i.gz, %25                    ; 2 uses
  %27 = insertelement <2 x double> poison, double %i.hf, i64 0
  %28 = fsub double %i.af, %i.aj                  ; 2 uses
  %29 = fadd double %i.hb, %28                    ; 2 uses
  %30 = fsub double %28, %i.hb
  %i.hh = fsub double %29, %26                    ; 2 uses
  %31 = fadd double %29, %26
  %32 = insertelement <2 x double> %27, double %23, i64 1
  %33 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %32, <2 x double> splat (double -5.000000e-01), <2 x double> %21) ; 3 uses
  %i.hi = insertelement <2 x double> %i.di, double %30, i64 1
  %34 = fmul <2 x double> %i.hi, splat (double f0x3FEBB67AE8584CAA) ; 3 uses
  %35 = shufflevector <2 x double> %33, <2 x double> poison, <2 x i32> <i32 poison, i32 0>
  %i.hj = insertelement <2 x double> %35, double %i.dh, i64 0 ; 2 uses
  %36 = fadd <2 x double> %34, %i.hj              ; 3 uses
  %37 = shufflevector <2 x double> %34, <2 x double> %i.hj, <2 x i32> <i32 1, i32 2>
  %38 = shufflevector <2 x double> %33, <2 x double> %34, <2 x i32> <i32 0, i32 2>
  %39 = fsub <2 x double> %37, %38                ; 3 uses
  %i.hk = extractelement <2 x double> %33, i64 1  ; 2 uses
  %i.hl = fsub double %i.eq, %i.hk                ; 2 uses
  %i.hm = fadd double %i.eq, %i.hk                ; 2 uses
  %i.hn = insertelement <2 x double> %36, double %i.hg, i64 0
  %i.ho = fmul <2 x double> %i.hn, <double f0x3FC64A2C7675B5D5, double f0xBFC4150460CB959A>
  %i.hp = insertelement <2 x double> %39, double %i.gx, i64 0
  ...
  %i.hr = insertelement <2 x double> %36, double %i.gx, i64 1
  %i.hs = fmul <2 x double> %i.hr, <double f0x3FD3371C1C9E25A7, double f0xBFC64A2C7675B5D5>
  %i.ht = insertelement <2 x double> %39, double %i.hg, i64 1
  ...
  %i.hv = fmul <2 x double> %39, <double f0xBFD3371C1C9E25A7, double f0x3FC4150460CB959A>
  %i.hw = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %36, <2 x double> <double f0x3F87C14599EAC08D, double f0x3FD0665CA247FDBA>, <2 x double> %i.hv) ; 4 uses
  ...
  %i.if = tail call double @llvm.fmuladd.f64(double %i.hh, double f0x3FD8CB01E1D7263E, double %i.ie) ; 2 uses
  ...
  %i.ij = insertelement <2 x double> poison, double %31, i64 0 ; 2 uses
  %i.ik = insertelement <2 x double> %i.ij, double %11, i64 1
  ...
  %i.kg = insertelement <2 x double> %i.fo, double %i.hh, i64 1
  ...
  %i.ko = fsub double %24, %i.kn                  ; 2 uses
  %i.kp = fmul double %i.hm, f0x3FB36E60CAB2D064
  %i.kq = insertelement <2 x double> %i.ij, double %i.hc, i64 1
  ...
  %i.kx = insertelement <2 x double> %i.kt, double %24, i64 0
  ...
  %40 = fadd double %i.kf, %i.lj                  ; 2 uses
  ...
  %i.mc = fsub double %40, %i.io                  ; 2 uses
  ...
  %i.mi = fadd double %i.io, %40                  ; 2 uses
