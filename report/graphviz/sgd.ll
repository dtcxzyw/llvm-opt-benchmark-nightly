...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %i.of = tail call nsz <4 x float> @llvm.minnum.v4f32(<4 x float> %vec.phi317, <4 x float> %i.nw) ; 2 uses
  %i.og = tail call nsz <4 x float> @llvm.minnum.v4f32(<4 x float> %vec.phi318, <4 x float> %i.oe) ; 2 uses
  %i.oh = tail call nsz <4 x float> @llvm.maxnum.v4f32(<4 x float> %vec.phi, <4 x float> %i.nw) ; 2 uses
  %i.oi = tail call nsz <4 x float> @llvm.maxnum.v4f32(<4 x float> %vec.phi316, <4 x float> %i.oe) ; 2 uses
  %index.next = add nuw i64 %index, 8             ; 2 uses
  %i.oj = fcmp uno <4 x float> %i.nw, %i.oe
  %8 = freeze <4 x i1> %i.oj
  %i.ok = bitcast <4 x i1> %8 to i4
