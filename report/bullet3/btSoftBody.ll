...
.lr.ph347:                                        ; preds = %.lr.ph, %bb.d
  ...
  %i.be = phi float [ %i.ej, %bb.d ], [ %i.o, %.lr.ph ] ; 4 uses
  %i.bf = phi float [ %i.ay, %bb.d ], [ %i.x, %.lr.ph ] ; 4 uses
  %.0104309345 = phi float [ %i.el, %bb.d ], [ %i.s, %.lr.ph ] ; 2 uses
  %.0103310344 = phi float [ %i.eo, %bb.d ], [ %i.v, %.lr.ph ] ; 2 uses
  %.0102311343 = phi float [ %i.ax, %bb.d ], [ %i.w, %.lr.ph ] ; 3 uses
  %.0312342 = phi float [ %i.az, %bb.d ], [ %i.y, %.lr.ph ] ; 2 uses
  %18 = fmul float %i.be, %i.be
  %19 = call float @llvm.fmuladd.f32(float %i.bf, float %i.bf, float %18)
  %20 = fmul float %.0103310344, %.0103310344
  %21 = call float @llvm.fmuladd.f32(float %.0104309345, float %.0104309345, float %20) ; 3 uses
  %22 = fsub float %19, %21
  %i.bg = fmul float %22, 5.000000e-01            ; 4 uses
...
bb.e:                                             ; preds = %bb.d
  ...
  %i.j = load ptr, ptr %i.h, align 8, !tbaa !315  ; 3 uses
  %i.k = getelementptr inbounds nuw i8, ptr %i.j, i64 32
  %i.l = load <2 x float>, ptr %i.k, align 8      ; 2 uses
  %.sroa.937.0..sroa_idx = getelementptr inbounds nuw i8, ptr %i.j, i64 40
  %.sroa.937.0.copyload = load float, ptr %.sroa.937.0..sroa_idx, align 8 ; 3 uses
  ...
  %i.o = insertelement <2 x float> <float poison, float 0.000000e+00>, float %.sroa.937.0.copyload, i64 0
  br i1 %.not19, label %bb.f, label %.thread

.thread:                                          ; preds = %bb.d, %bb.e
  %.04057 = phi float [ %i.n, %bb.e ], [ 0.000000e+00, %bb.d ]
  %.sroa.937.055 = phi float [ %.sroa.937.0.copyload, %bb.e ], [ 0.000000e+00, %bb.d ] ; 2 uses
  ...
  %i.t = load <2 x float>, ptr %i.s, align 8
  ...
  %i.w = insertelement <2 x float> poison, float %.sroa.937.055, i64 0
  %1 = insertelement <2 x float> %i.w, float %.sroa.9.0.copyload, i64 1
  br label %bb.f

bb.f:                                             ; preds = %.thread, %bb.e
  %.04058 = phi float [ %i.n, %bb.e ], [ %.04057, %.thread ] ; 2 uses
  %.sroa.937.056 = phi float [ %.sroa.937.0.copyload, %bb.e ], [ %.sroa.937.055, %.thread ] ; 2 uses
  %.sroa.9.0 = phi float [ 0.000000e+00, %bb.e ], [ %.sroa.9.0.copyload, %.thread ] ; 2 uses
  %.0 = phi float [ 0.000000e+00, %bb.e ], [ %i.v, %.thread ] ; 2 uses
  %i.x = phi <2 x float> [ %i.l, %bb.e ], [ %i.q, %.thread ] ; 5 uses
  %i.y = phi <2 x float> [ zeroinitializer, %bb.e ], [ %i.t, %.thread ] ; 5 uses
  %2 = phi <2 x float> [ %i.o, %bb.e ], [ %1, %.thread ] ; 2 uses
  %i.z = fadd <2 x float> %i.x, %i.y              ; 4 uses
...
_ZN9btVector313safeNormalizeEv.exit:              ; preds = %bb.g, %bb.h
  ...
  %3 = extractelement <2 x float> %i.x, i64 0
  %4 = extractelement <2 x float> %i.y, i64 0
  %5 = shufflevector <2 x float> %i.x, <2 x float> %i.y, <2 x i32> <i32 1, i32 3> ; 2 uses
  %6 = fmul <2 x float> %5, %5
  %7 = shufflevector <2 x float> %i.x, <2 x float> %i.y, <2 x i32> <i32 0, i32 2> ; 2 uses
  %8 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %7, <2 x float> %7, <2 x float> %6)
  %i.ap = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %2, <2 x float> %2, <2 x float> %8) ; 2 uses
  ...
  %foldExtExtBinop63 = fmul <2 x float> %i.x, %i.y
  %i.as = extractelement <2 x float> %foldExtExtBinop63, i64 1
  %i.at = tail call float @llvm.fmuladd.f32(float %3, float %4, float %i.as)
