...
_Z18polarDecompositionRK11btMatrix2x2R14GivensRotationS1_.exit: ; preds = %bb.a, %bb.b
  ...
  %.sroa.0.0.copyload = load float, ptr %0, align 4, !tbaa !253 ; 4 uses
  ...
  %i.w = select i1 %.not.not.i.i.not, float %.sroa.13.0.copyload, float %.sroa.0.0.copyload
  %i.x = icmp ult i32 %i.t, 2
  %.not43.not.i.i.not = icmp eq i32 %i.v, 1       ; 4 uses
  %i.y = select i1 %.not43.not.i.i.not, float %.sroa.13.0.copyload, float %.sroa.0.0.copyload ; 2 uses
  %i.z = fneg float %i.y
  %i.aa = extractelement <2 x float> %storemerge, i64 1 ; 4 uses
  %i.ab = fmul float %i.aa, %i.z
  %i.ac = extractelement <2 x float> %storemerge, i64 0 ; 4 uses
  %i.ad = tail call float @llvm.fmuladd.f32(float %i.ac, float %.sroa.0.0.copyload, float %i.ab)
  %.sroa.0.1 = select i1 %.not.not.i.i.not, float %.sroa.0.0.copyload, float %i.ad
  %i.ae = fmul float %i.ac, %i.y
  %i.af = tail call float @llvm.fmuladd.f32(float %i.aa, float %i.w, float %i.ae) ; 2 uses
  %.sroa.0.0 = select i1 %.not43.not.i.i.not, float %.sroa.0.1, float %i.af ; 2 uses
  ...
  %.sroa.speculate.load.false.sroa.speculated = select i1 %.not43.not.i.i.not, float %.sroa.16.0.copyload, float %i.af
  %.sroa.speculated = select i1 %.not91, float %.sroa.10.0.copyload, float %.sroa.speculate.load.false.sroa.speculated ; 2 uses
  %i.ag = fneg float %.sroa.speculated
  %i.ah = fmul float %i.aa, %i.ag
  %i.ai = tail call float @llvm.fmuladd.f32(float %i.ac, float %.sroa.speculated88, float %i.ah) ; 3 uses
  %.sroa.0.3 = select i1 %i.x, float %.sroa.0.0, float %i.ai
  %.sroa.10.1 = select i1 %.not, float %i.ai, float %.sroa.10.0.copyload
  %i.aj = fmul float %i.ac, %.sroa.speculated
  %i.ak = tail call float @llvm.fmuladd.f32(float %i.aa, float %.sroa.speculated88, float %i.aj) ; 3 uses
  %i.al = icmp ult i32 %i.v, 2
  %.sroa.0.2 = select i1 %i.al, float %.sroa.0.3, float %i.ak ; 6 uses
  %.sroa.10.0 = select i1 %.not91, float %i.ak, float %.sroa.10.1 ; 5 uses
  %simplifycfg.merge = select i1 %.not.not.i.i.not, float %i.ai, float %.sroa.16.0.copyload
  %.sroa.16.0 = select i1 %.not43.not.i.i.not, float %i.ak, float %simplifycfg.merge ; 4 uses
