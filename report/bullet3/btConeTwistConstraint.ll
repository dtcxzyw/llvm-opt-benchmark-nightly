...
bb.c:                                             ; preds = %._crit_edge, %bb.b
  ...
  %i.y = tail call noundef float @cosf(float noundef %i.v) #21 ; 3 uses
  ...
  %i.ak = fneg <2 x float> %i.ac                  ; 5 uses
  ...
  %i.aq = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ak, <2 x float> zeroinitializer, <2 x float> %i.ao) ; 5 uses
  %i.ar = fmul float %i.ap, -0.000000e+00
  %i.as = extractelement <2 x float> %i.ak, i64 1 ; 2 uses
  %i.at = tail call float @llvm.fmuladd.f32(float %i.as, float %2, float %i.ar)
  %i.au = extractelement <2 x float> %i.ak, i64 0 ; 2 uses
  %i.av = tail call float @llvm.fmuladd.f32(float %i.au, float 0.000000e+00, float %i.at) ; 2 uses
  %i.aw = extractelement <2 x float> %i.aq, i64 0
  %i.ax = fmul float %i.y, %i.aw
  %i.ay = tail call float @llvm.fmuladd.f32(float %i.av, float %i.as, float %i.ax)
  %i.az = extractelement <2 x float> %i.aq, i64 1
  %3 = tail call float @llvm.fmuladd.f32(float %i.az, float %i.au, float %i.ay)
  %4 = fneg float %i.ap                           ; 3 uses
  %5 = tail call float @llvm.fmuladd.f32(float %i.y, float 0.000000e+00, float %i.aj)
  %i.ba = tail call float @llvm.fmuladd.f32(float %4, float %2, float %5) ; 3 uses
  %i.bb = tail call float @llvm.fmuladd.f32(float %i.ba, float %i.ap, float %3)
  %6 = insertelement <2 x float> %i.aq, float %i.ba, i64 0
  %7 = fmul <2 x float> %i.am, %6
  %8 = insertelement <2 x float> poison, float %i.av, i64 0
  %9 = shufflevector <2 x float> %8, <2 x float> poison, <2 x i32> zeroinitializer
  %10 = insertelement <2 x float> %i.ak, float %4, i64 1
  %11 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %9, <2 x float> %10, <2 x float> %7)
  %12 = shufflevector <2 x float> %i.aq, <2 x float> %i.ak, <2 x i32> <i32 0, i32 3>
  %i.bc = insertelement <2 x float> poison, float %4, i64 0
  %i.bd = insertelement <2 x float> %i.bc, float %i.ba, i64 1
  %13 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %12, <2 x float> %i.bd, <2 x float> %11)
  %14 = shufflevector <2 x float> %13, <2 x float> poison, <2 x i32> <i32 1, i32 0>
  %i.be = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.aq, <2 x float> %i.ac, <2 x float> %14) ; 2 uses
