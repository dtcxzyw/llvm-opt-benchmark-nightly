...
bb.a:
  ...
  %5 = load <2 x float>, ptr %2, align 4          ; 4 uses
  %.sroa.7.0.copyload.i.a = load float, ptr %.sroa.7.0..sroa_idx.i, align 4 ; 4 uses
  %6 = extractelement <2 x float> %5, i64 1       ; 3 uses
  %i.v = fmul float %6, %i.t
  %7 = extractelement <2 x float> %5, i64 0       ; 3 uses
  %i.w = tail call float @llvm.fmuladd.f32(float %7, float %i.s, float %i.v)
  %i.x = tail call noundef float @llvm.fmuladd.f32(float %.sroa.7.0.copyload.i.a, float %i.u, float %i.w)
  %i.y = fmul float %.sroa.564.0.copyload, %6
  %i.z = tail call float @llvm.fmuladd.f32(float %7, float %.sroa.063.0.copyload, float %i.y)
  %i.aa = tail call noundef float @llvm.fmuladd.f32(float %.sroa.7.0.copyload.i.a, float %.sroa.7.0.copyload, float %i.z)
...
.sink.split:                                      ; preds = %bb.b, %bb.a
  ...
  %i.bw = fmul <2 x float> %8, %i.bp
  %i.bx = insertelement <2 x float> poison, float %i.bl, i64 0
  %i.by = insertelement <2 x float> %i.bx, float %i.bk, i64 1
  %i.bz = shufflevector <2 x float> %5, <2 x float> poison, <2 x i32> zeroinitializer
  ...
  %i.cd = insertelement <2 x float> poison, float %.sroa.7.0.copyload.i.a, i64 0
  ...
  %i.cg = fmul float %6, %i.bt
  %i.ch = tail call float @llvm.fmuladd.f32(float %i.bs, float %7, float %i.cg)
  %i.ci = tail call noundef float @llvm.fmuladd.f32(float %i.bv, float %.sroa.7.0.copyload.i.a, float %i.ch)
