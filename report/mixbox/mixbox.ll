...
@_ZZL21compute_huffman_codesP4zbufE15length_dezigzag = internal unnamed_addr constant [19 x i8] c"\10\11\12\00\08\07\09\06\0A\05\0B\04\0C\03\0D\02\0E\01\0F", align 16
@_ZL12zlength_base = internal unnamed_addr constant [31 x i32] [i32 3, i32 4, i32 5, i32 6, i32 7, i32 8, i32 9, i32 10, i32 11, i32 13, i32 15, i32 17, i32 19, i32 23, i32 27, i32 31, i32 35, i32 43, i32 51, i32 59, i32 67, i32 83, i32 99, i32 115, i32 131, i32 163, i32 195, i32 227, i32 258, i32 0, i32 0], align 16
@_ZL13zlength_extra = internal unnamed_addr constant [31 x i32] [i32 0, i32 0, i32 0, i32 0, i32 0, i32 0, i32 0, i32 0, i32 1, i32 1, i32 1, i32 1, i32 2, i32 2, i32 2, i32 2, i32 3, i32 3, i32 3, i32 3, i32 4, i32 4, i32 4, i32 4, i32 5, i32 5, i32 5, i32 5, i32 0, i32 0, i32 0], align 16
@_ZL10zdist_base = internal unnamed_addr constant [32 x i32] [i32 1, i32 2, i32 3, i32 4, i32 5, i32 7, i32 9, i32 13, i32 17, i32 25, i32 33, i32 49, i32 65, i32 97, i32 129, i32 193, i32 257, i32 385, i32 513, i32 769, i32 1025, i32 1537, i32 2049, i32 3073, i32 4097, i32 6145, i32 8193, i32 12289, i32 16385, i32 24577, i32 0, i32 0], align 16
...
define dso_local void @mixbox_rgb_to_latent(i8 noundef zeroext %0, i8 noundef zeroext %1, i8 noundef zeroext %2, ptr nofree noundef writeonly captures(none) initializes((0, 28)) %3) local_unnamed_addr #0 personality ptr @__gxx_personality_v0 {
bb.a:
  %i.a = load atomic i8, ptr @_ZGVZL10mixbox_lutvE12decompressed acquire, align 8
  %i.b = icmp eq i8 %i.a, 0
  br i1 %i.b, label %bb.b, label %_ZL10mixbox_lutv.exit, !prof !9

bb.b:                                             ; preds = %bb.a
  %i.c = tail call i32 @__cxa_guard_acquire(ptr nonnull @_ZGVZL10mixbox_lutvE12decompressed) #12
  %.not.i = icmp eq i32 %i.c, 0
  br i1 %.not.i, label %_ZL10mixbox_lutv.exit, label %bb.c

bb.c:                                             ; preds = %bb.b
  tail call fastcc void @_ZZL10mixbox_lutvEN13mixbox_init_tC2Ev()
  %i.d = tail call ptr @llvm.invariant.start.p0(i64 799107, ptr nonnull @_ZZL10mixbox_lutvE12decompressed) ; 0 uses
...
_ZL10mixbox_lutv.exit:                            ; preds = %bb.a, %bb.b, %bb.c
  %i.e = uitofp i8 %2 to float
  %i.f = fdiv float %i.e, 2.550000e+02            ; 2 uses
  %i.g = fcmp ogt float %i.f, 1.000000e+00
  %i.h = select i1 %i.g, float 1.000000e+00, float %i.f ; 2 uses
  %i.i = fmul nnan float %i.h, 6.300000e+01       ; 2 uses
  %i.j = fptosi float %i.i to i32                 ; 2 uses
  %i.k = uitofp nneg i32 %i.j to float
  %i.l = fsub float %i.i, %i.k                    ; 5 uses
  %i.m = insertelement <2 x i8> poison, i8 %0, i64 0
  %i.n = insertelement <2 x i8> %i.m, i8 %1, i64 1
  %i.o = uitofp <2 x i8> %i.n to <2 x float>
  %i.p = shl nuw nsw i32 %i.j, 12
  %i.q = fsub float 1.000000e+00, %i.l            ; 4 uses
  ...
  %i.v = fdiv <2 x float> %i.o, splat (float 2.550000e+02) ; 2 uses
  %i.w = fcmp ogt <2 x float> %i.v, splat (float 1.000000e+00)
  %i.x = select <2 x i1> %i.w, <2 x float> splat (float 1.000000e+00), <2 x float> %i.v ; 2 uses
  %i.y = fmul nnan <2 x float> %i.x, splat (float 6.300000e+01) ; 2 uses
  %i.z = fptosi <2 x float> %i.y to <2 x i32>     ; 3 uses
  %i.aa = uitofp nneg <2 x i32> %i.z to <2 x float>
  %i.ab = fsub <2 x float> %i.y, %i.aa            ; 2 uses
  %i.ac = extractelement <2 x i32> %i.z, i64 1
  %i.ad = shl nuw nsw i32 %i.ac, 6
  %i.ae = extractelement <2 x i32> %i.z, i64 0
  %i.af = add nuw nsw i32 %i.ad, %i.ae
  %i.ag = add nuw nsw i32 %i.af, %i.p
  %i.ah = and i32 %i.ag, 262143
  %i.ai = mul nuw nsw i32 %i.ah, 3
  %i.aj = zext nneg i32 %i.ai to i64
  %i.ak = getelementptr inbounds nuw i8, ptr @_ZZL10mixbox_lutvE12decompressed, i64 %i.aj ; 16 uses
  %i.al = extractelement <2 x float> %i.ab, i64 0 ; 3 uses
  %i.am = fsub float 1.000000e+00, %i.al          ; 2 uses
  %i.an = extractelement <2 x float> %i.ab, i64 1 ; 3 uses
  %i.ao = fsub float 1.000000e+00, %i.an          ; 2 uses
  %i.ap = fmul float %i.am, %i.ao                 ; 2 uses
  %i.aq = fmul float %i.ap, %i.q                  ; 2 uses
  %i.ar = getelementptr inbounds nuw i8, ptr %i.ak, i64 192
  %i.as = load i8, ptr %i.ar, align 1, !tbaa !10
  %i.at = uitofp i8 %i.as to float
  %i.au = tail call float @llvm.fmuladd.f32(float %i.aq, float %i.at, float 0.000000e+00)
  %i.av = getelementptr inbounds nuw i8, ptr %i.ak, i64 193
  %i.aw = load <2 x i8>, ptr %i.av, align 1, !tbaa !10
  %i.ax = shufflevector <2 x i8> %i.aw, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.ay = uitofp <2 x i8> %i.ax to <2 x float>
  ...
  %i.bb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ba, <2 x float> %i.ay, <2 x float> zeroinitializer)
  %i.bc = fmul float %i.al, %i.ao                 ; 2 uses
  %i.bd = fmul float %i.bc, %i.q                  ; 2 uses
  %i.be = getelementptr inbounds nuw i8, ptr %i.ak, i64 195
  %i.bf = load i8, ptr %i.be, align 1, !tbaa !10
  %i.bg = uitofp i8 %i.bf to float
  %i.bh = tail call float @llvm.fmuladd.f32(float %i.bd, float %i.bg, float %i.au)
  %i.bi = getelementptr inbounds nuw i8, ptr %i.ak, i64 196
  %i.bj = load <2 x i8>, ptr %i.bi, align 1, !tbaa !10
  %i.bk = shufflevector <2 x i8> %i.bj, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.bl = uitofp <2 x i8> %i.bk to <2 x float>
  ...
  %i.bo = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bn, <2 x float> %i.bl, <2 x float> %i.bb)
  %i.bp = fmul float %i.am, %i.an                 ; 2 uses
  %i.bq = fmul float %i.bp, %i.q                  ; 2 uses
  %i.br = getelementptr inbounds nuw i8, ptr %i.ak, i64 384
  %i.bs = load i8, ptr %i.br, align 1, !tbaa !10
  %i.bt = uitofp i8 %i.bs to float
  %i.bu = tail call float @llvm.fmuladd.f32(float %i.bq, float %i.bt, float %i.bh)
  %i.bv = getelementptr inbounds nuw i8, ptr %i.ak, i64 385
  %i.bw = load <2 x i8>, ptr %i.bv, align 1, !tbaa !10
  %i.bx = shufflevector <2 x i8> %i.bw, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.by = uitofp <2 x i8> %i.bx to <2 x float>
  ...
  %i.cb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ca, <2 x float> %i.by, <2 x float> %i.bo)
  %i.cc = fmul float %i.al, %i.an                 ; 2 uses
  %i.cd = fmul float %i.cc, %i.q                  ; 2 uses
  %i.ce = getelementptr inbounds nuw i8, ptr %i.ak, i64 387
  %i.cf = load i8, ptr %i.ce, align 1, !tbaa !10
  %i.cg = uitofp i8 %i.cf to float
  %i.ch = tail call float @llvm.fmuladd.f32(float %i.cd, float %i.cg, float %i.bu)
  %i.ci = getelementptr inbounds nuw i8, ptr %i.ak, i64 388
  %i.cj = load <2 x i8>, ptr %i.ci, align 1, !tbaa !10
  %i.ck = shufflevector <2 x i8> %i.cj, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.cl = uitofp <2 x i8> %i.ck to <2 x float>
  %i.cm = insertelement <2 x float> poison, float %i.cd, i64 0
  %i.cn = shufflevector <2 x float> %i.cm, <2 x float> poison, <2 x i32> zeroinitializer
  %i.co = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cn, <2 x float> %i.cl, <2 x float> %i.cb)
  %i.cp = fmul float %i.l, %i.ap                  ; 2 uses
  %i.cq = getelementptr inbounds nuw i8, ptr %i.ak, i64 12480
  %i.cr = load i8, ptr %i.cq, align 1, !tbaa !10
  %i.cs = uitofp i8 %i.cr to float
  %i.ct = tail call float @llvm.fmuladd.f32(float %i.cp, float %i.cs, float %i.ch)
  %i.cu = getelementptr inbounds nuw i8, ptr %i.ak, i64 12481
  %i.cv = load <2 x i8>, ptr %i.cu, align 1, !tbaa !10
  %i.cw = shufflevector <2 x i8> %i.cv, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.cx = uitofp <2 x i8> %i.cw to <2 x float>
  ...
  %i.da = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cz, <2 x float> %i.cx, <2 x float> %i.co)
  %i.db = fmul float %i.l, %i.bc                  ; 2 uses
  %i.dc = getelementptr inbounds nuw i8, ptr %i.ak, i64 12483
  %i.dd = load i8, ptr %i.dc, align 1, !tbaa !10
  %i.de = uitofp i8 %i.dd to float
  %i.df = tail call float @llvm.fmuladd.f32(float %i.db, float %i.de, float %i.ct)
  %i.dg = getelementptr inbounds nuw i8, ptr %i.ak, i64 12484
  %i.dh = load <2 x i8>, ptr %i.dg, align 1, !tbaa !10
  %i.di = shufflevector <2 x i8> %i.dh, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.dj = uitofp <2 x i8> %i.di to <2 x float>
  ...
  %i.dm = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.dl, <2 x float> %i.dj, <2 x float> %i.da)
  %i.dn = fmul float %i.bp, %i.l                  ; 2 uses
  %i.do = getelementptr inbounds nuw i8, ptr %i.ak, i64 12672
  %i.dp = load i8, ptr %i.do, align 1, !tbaa !10
  %i.dq = uitofp i8 %i.dp to float
  %i.dr = tail call float @llvm.fmuladd.f32(float %i.dn, float %i.dq, float %i.df)
  %i.ds = getelementptr inbounds nuw i8, ptr %i.ak, i64 12673
  %i.dt = load <2 x i8>, ptr %i.ds, align 1, !tbaa !10
  %i.du = shufflevector <2 x i8> %i.dt, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.dv = uitofp <2 x i8> %i.du to <2 x float>
  ...
  %i.dy = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.dx, <2 x float> %i.dv, <2 x float> %i.dm)
  %i.dz = fmul float %i.cc, %i.l                  ; 2 uses
  %i.ea = getelementptr inbounds nuw i8, ptr %i.ak, i64 12675
  %i.eb = load i8, ptr %i.ea, align 1, !tbaa !10
  %i.ec = uitofp i8 %i.eb to float
  %i.ed = tail call float @llvm.fmuladd.f32(float %i.dz, float %i.ec, float %i.dr)
  %i.ee = getelementptr inbounds nuw i8, ptr %i.ak, i64 12676
  %i.ef = load <2 x i8>, ptr %i.ee, align 1, !tbaa !10
  %i.eg = shufflevector <2 x i8> %i.ef, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  ...
  %i.ek = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ej, <2 x float> %i.eh, <2 x float> %i.dy)
  %i.el = fmul <2 x float> %i.ek, splat (float f0x3B808081) ; 9 uses
  %i.em = extractelement <2 x float> %i.el, i64 1 ; 4 uses
  %i.en = fmul <2 x float> %i.el, %i.el           ; 4 uses
  %i.eo = fmul float %i.ed, f0x3B808081           ; 7 uses
  %i.ep = fadd float %i.eo, %i.em
  %i.eq = extractelement <2 x float> %i.el, i64 0 ; 4 uses
  %i.er = fadd float %i.ep, %i.eq
  %4 = fmul float %i.eo, %i.eo                    ; 4 uses
  %5 = shufflevector <2 x float> %i.el, <2 x float> poison, <4 x i32> <i32 1, i32 0, i32 poison, i32 1>
  %6 = insertelement <4 x float> %5, float 1.000000e+00, i64 2 ; 2 uses
  %7 = shufflevector <2 x float> %i.el, <2 x float> poison, <4 x i32> <i32 0, i32 poison, i32 poison, i32 poison>
  %8 = insertelement <4 x float> %7, float %i.eo, i64 1
  %9 = insertelement <4 x float> %8, float %i.er, i64 2
  %10 = shufflevector <4 x float> %9, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 2, i32 1> ; 2 uses
  %11 = fmul <4 x float> %6, %10                  ; 3 uses
  %12 = fsub <4 x float> %6, %10                  ; 2 uses
  %13 = shufflevector <4 x float> %11, <4 x float> %12, <4 x i32> <i32 0, i32 1, i32 6, i32 3> ; 2 uses
  %14 = extractelement <4 x float> %12, i64 2     ; 5 uses
  %i.es = fmul float %14, %14                     ; 3 uses
  %i.et = fmul float %i.eo, %4                    ; 2 uses
  %foldExtExtBinop = fmul <2 x float> %i.el, %i.en ; 2 uses
  %i.eu = extractelement <2 x float> %foldExtExtBinop, i64 1
  %foldExtExtBinop4 = fmul <2 x float> %i.el, %i.en ; 2 uses
  %i.ev = extractelement <2 x float> %foldExtExtBinop4, i64 0
  %i.ew = fmul float %14, %i.es                   ; 2 uses
  %i.ex = fmul float %4, %i.em                    ; 2 uses
  %15 = extractelement <4 x float> %11, i64 3
  %i.ey = fmul float %i.em, %15                   ; 2 uses
  %i.ez = fmul float %4, %i.eq                    ; 2 uses
  %16 = extractelement <4 x float> %11, i64 1
  %i.fa = fmul float %i.eq, %16                   ; 2 uses
  %i.fb = fmul float %4, %14                      ; 2 uses
  %i.fc = fmul float %i.eo, %i.es                 ; 2 uses
  ...
  %i.ff = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fe, <2 x float> <float f0x3D9E0B95, float f0x3CE79607>, <2 x float> zeroinitializer)
  %i.fg = tail call float @llvm.fmuladd.f32(float %i.et, float f0x3E7E4A33, float 0.000000e+00)
  %i.fh = shufflevector <2 x float> %foldExtExtBinop, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.fi = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fh, <2 x float> <float 9.591230e-01, float f0x3F4D74EB>, <2 x float> %i.ff)
  %i.fj = tail call float @llvm.fmuladd.f32(float %i.eu, float f0x3D11E497, float %i.fg)
  %i.fk = shufflevector <2 x float> %foldExtExtBinop4, <2 x float> poison, <2 x i32> zeroinitializer
  %i.fl = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fk, <2 x float> <float f0x3F3F30C2, float f0x3D476AD3>, <2 x float> %i.fi)
  %i.fm = tail call float @llvm.fmuladd.f32(float %i.ev, float 0.000000e+00, float %i.fj)
  ...
  %i.fp = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fo, <2 x float> <float f0x3F7EC435, float f0x3F7FF1AE>, <2 x float> %i.fl)
  %i.fq = tail call float @llvm.fmuladd.f32(float %i.ew, float 9.970480e-01, float %i.fm)
  ...
  %i.ft = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fs, <2 x float> <float f0x3D456469, float f0x3F55694A>, <2 x float> %i.fp)
  %i.fu = tail call float @llvm.fmuladd.f32(float %i.ex, float f0x3EA67A8E, float %i.fq)
  ...
  %i.fx = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fw, <2 x float> <float f0xBF2E74C9, float f0x3FBB049B>, <2 x float> %i.ft)
  %i.fy = tail call float @llvm.fmuladd.f32(float %i.ey, float f0x3F88EF83, float %i.fu)
  %i.fz = insertelement <2 x float> poison, float %i.ez, i64 0
  %i.ga = shufflevector <2 x float> %i.fz, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ga, <2 x float> <float f0x3E8A8A03, float f0xBE1CED3A>, <2 x float> %i.fx)
  %i.gc = tail call float @llvm.fmuladd.f32(float %i.ez, float f0x3FFE6181, float %i.fy)
  %i.gd = insertelement <2 x float> poison, float %i.fa, i64 0
  %i.ge = shufflevector <2 x float> %i.gd, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gf = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ge, <2 x float> <float f0x3F4E0630, float f0x3F2BC289>, <2 x float> %i.gb)
  %i.gg = tail call float @llvm.fmuladd.f32(float %i.fa, float 1.842450e-01, float %i.gc)
  %i.gh = insertelement <2 x float> poison, float %i.fb, i64 0
  %i.gi = shufflevector <2 x float> %i.gh, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gj = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gi, <2 x float> <float f0xBEB35BD6, float f0x3FB07499>, <2 x float> %i.gf)
  %i.gk = tail call float @llvm.fmuladd.f32(float %i.fb, float 3.688650e+00, float %i.gg)
  %i.gl = insertelement <2 x float> poison, float %i.fc, i64 0
  %i.gm = shufflevector <2 x float> %i.gl, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gn = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gm, <2 x float> <float f0x3F86905C, float f0x3FFD3419>, <2 x float> %i.gj)
  %i.go = tail call float @llvm.fmuladd.f32(float %i.fc, float f0x40351CEE, float %i.gk)
  %i.gp = shufflevector <2 x float> %i.en, <2 x float> poison, <10 x i32> <i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 0, i32 poison, i32 1, i32 poison, i32 poison>
  %17 = shufflevector <4 x float> %13, <4 x float> poison, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %18 = shufflevector <10 x float> %17, <10 x float> %i.gp, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 poison, i32 15, i32 poison, i32 17, i32 poison, i32 poison>
  %i.gq = insertelement <10 x float> %18, float %i.es, i64 4
  %19 = shufflevector <10 x float> %i.gq, <10 x float> poison, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 4, i32 7, i32 poison, i32 poison>
  %20 = shufflevector <2 x float> %i.en, <2 x float> poison, <10 x i32> <i32 0, i32 1, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %21 = shufflevector <10 x float> %19, <10 x float> %20, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7, i32 10, i32 11>
  %22 = shufflevector <4 x float> %13, <4 x float> poison, <10 x i32> <i32 2, i32 3, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %23 = shufflevector <2 x float> %i.el, <2 x float> poison, <10 x i32> <i32 0, i32 1, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %24 = shufflevector <10 x float> %22, <10 x float> %23, <10 x i32> <i32 0, i32 1, i32 10, i32 11, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %25 = shufflevector <10 x float> %24, <10 x float> poison, <10 x i32> <i32 0, i32 0, i32 1, i32 2, i32 2, i32 0, i32 3, i32 0, i32 3, i32 2>
  %i.gr = fmul <10 x float> %21, %25              ; 20 uses
  %i.gs = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 9, i32 9>
  %i.gt = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gs, <2 x float> <float f0x404DD41D, float f0x3F500D42>, <2 x float> %i.gn)
  %i.gu = extractelement <10 x float> %i.gr, i64 9
  %i.gv = tail call float @llvm.fmuladd.f32(float %i.gu, float f0x3F84550C, float %i.go)
  %i.gw = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 8, i32 8>
  %i.gx = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gw, <2 x float> <float f0x40327DE4, float f0x3ED4D0CC>, <2 x float> %i.gt)
  %i.gy = extractelement <10 x float> %i.gr, i64 8
  %i.gz = tail call float @llvm.fmuladd.f32(float %i.gy, float f0xBD37CCB3, float %i.gv)
  %i.ha = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 7, i32 7>
  %i.hb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ha, <2 x float> <float f0x40416251, float f0x4023707E>, <2 x float> %i.gx)
  %i.hc = extractelement <10 x float> %i.gr, i64 7
  %i.hd = tail call float @llvm.fmuladd.f32(float %i.hc, float f0x3EA7C333, float %i.gz)
  %i.he = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 6, i32 6>
  %i.hf = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.he, <2 x float> <float f0x403CE13B, float f0x4033F7FD>, <2 x float> %i.hb)
  %i.hg = extractelement <10 x float> %i.gr, i64 6
  %i.hh = tail call float @llvm.fmuladd.f32(float %i.hg, float f0x3F96801B, float %i.hd)
  %i.hi = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 5, i32 5>
  %i.hj = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hi, <2 x float> <float f0x4034E9CF, float f0x3F4CA0EA>, <2 x float> %i.hf)
  %i.hk = extractelement <10 x float> %i.gr, i64 5
  %i.hl = tail call float @llvm.fmuladd.f32(float %i.hk, float f0x3FE89875, float %i.hh)
  %i.hm = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 4, i32 4>
  %i.hn = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hm, <2 x float> <float f0x403FCD64, float f0x3F9CEB4B>, <2 x float> %i.hj)
  %i.ho = extractelement <10 x float> %i.gr, i64 4
  %i.hp = tail call float @llvm.fmuladd.f32(float %i.ho, float f0x3FE73C97, float %i.hl)
  %i.hq = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 3, i32 3>
  %i.hr = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hq, <2 x float> <float f0x3FEFDD4D, float f0x400337A7>, <2 x float> %i.hn)
  %i.hs = extractelement <10 x float> %i.gr, i64 3
  %i.ht = tail call float @llvm.fmuladd.f32(float %i.hs, float f0xBE98C2A3, float %i.hp)
  %i.hu = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 2, i32 2>
  %i.hv = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hu, <2 x float> <float f0x40243AE9, float f0x40E118D7>, <2 x float> %i.hr)
  %i.hw = extractelement <10 x float> %i.gr, i64 2
  %i.hx = tail call float @llvm.fmuladd.f32(float %i.hw, float f0x3F203166, float %i.ht)
  %i.hy = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.hz = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hy, <2 x float> <float f0x4082AA5A, float f0xBFB3B903>, <2 x float> %i.hv)
  %i.ia = extractelement <10 x float> %i.gr, i64 1
  %i.ib = tail call float @llvm.fmuladd.f32(float %i.ia, float f0x400998DE, float %i.hx)
  %i.ic = shufflevector <10 x float> %i.gr, <10 x float> poison, <2 x i32> zeroinitializer
  %i.id = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ic, <2 x float> <float f0x40C00672, float f0x40238DA6>, <2 x float> %i.hz)
  %i.ie = extractelement <10 x float> %i.gr, i64 0
  %i.if = tail call float @llvm.fmuladd.f32(float %i.ie, float f0x3FF42585, float %i.ib)
  store float %i.eo, ptr %3, align 4, !tbaa !12
  store float %i.em, ptr %i.r, align 4, !tbaa !12
  store float %i.eq, ptr %i.s, align 4, !tbaa !12
  store float %14, ptr %i.t, align 4, !tbaa !12
  %i.ig = fsub <2 x float> %i.x, %i.id
  store <2 x float> %i.ig, ptr %i.u, align 4, !tbaa !12
  %i.ih = fsub float %i.h, %i.if
  %i.ii = getelementptr inbounds nuw i8, ptr %3, i64 24
  store float %i.ih, ptr %i.ii, align 4, !tbaa !12
  ret void
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(argmem: readwrite) uwtable
define dso_local void @mixbox_latent_to_rgb(ptr nofree noundef readonly captures(none) %0, ptr nofree noundef writeonly captures(none) initializes((0, 1)) %1, ptr nofree noundef writeonly captures(none) initializes((0, 1)) %2, ptr nofree noundef writeonly captures(none) initializes((0, 1)) %3) local_unnamed_addr #1 {
bb.a:
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 4 ; 2 uses
  %i.b = getelementptr inbounds nuw i8, ptr %0, i64 16
  %i.c = load <3 x float>, ptr %i.a, align 4, !tbaa !12 ; 4 uses
  %i.d = shufflevector <3 x float> %i.c, <3 x float> poison, <10 x i32> <i32 2, i32 2, i32 2, i32 1, i32 1, i32 2, i32 0, i32 2, i32 0, i32 1>
  %i.e = load <4 x float>, ptr %0, align 4, !tbaa !12 ; 2 uses
  %i.f = shufflevector <4 x float> %i.e, <4 x float> poison, <10 x i32> <i32 1, i32 0, i32 0, i32 0, i32 3, i32 2, i32 3, i32 1, i32 2, i32 1>
  %i.g = load float, ptr %i.a, align 4, !tbaa !12 ; 3 uses
  %i.h = shufflevector <3 x float> %i.c, <3 x float> poison, <10 x i32> <i32 1, i32 1, i32 0, i32 0, i32 2, i32 1, i32 2, i32 0, i32 1, i32 0>
  %i.i = fmul <10 x float> %i.f, %i.h             ; 6 uses
  %i.j = extractelement <10 x float> %i.i, i64 5
  %i.k = extractelement <10 x float> %i.i, i64 4  ; 2 uses
  %i.l = extractelement <10 x float> %i.i, i64 1
  %i.m = extractelement <4 x float> %i.e, i64 0   ; 4 uses
  %i.n = extractelement <10 x float> %i.i, i64 7
  %i.o = extractelement <3 x float> %i.c, i64 1   ; 3 uses
  %i.p = extractelement <3 x float> %i.c, i64 2   ; 2 uses
  %i.q = extractelement <10 x float> %i.i, i64 2
  %i.r = fmul float %i.m, %i.m                    ; 4 uses
  %i.s = fmul float %i.m, %i.r                    ; 2 uses
  %i.t = fmul float %i.g, %i.n                    ; 2 uses
  %i.u = fmul float %i.o, %i.j                    ; 2 uses
  %i.v = fmul float %i.p, %i.k                    ; 2 uses
  %i.w = fmul float %i.r, %i.g                    ; 2 uses
  %i.x = fmul float %i.g, %i.q                    ; 2 uses
  %i.y = fmul float %i.r, %i.o                    ; 2 uses
  %i.z = fmul float %i.o, %i.l                    ; 2 uses
  %i.aa = fmul float %i.r, %i.p                   ; 2 uses
  %i.ab = fmul float %i.m, %i.k                   ; 2 uses
  ...
  %i.ae = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ad, <2 x float> <float f0x3CE79607, float f0x3D9E0B95>, <2 x float> zeroinitializer)
  %i.af = tail call float @llvm.fmuladd.f32(float %i.s, float f0x3E7E4A33, float 0.000000e+00)
  ...
  %i.ai = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ah, <2 x float> <float f0x3F4D74EB, float 9.591230e-01>, <2 x float> %i.ae)
  %i.aj = tail call float @llvm.fmuladd.f32(float %i.t, float f0x3D11E497, float %i.af)
  ...
  %i.am = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.al, <2 x float> <float f0x3D476AD3, float f0x3F3F30C2>, <2 x float> %i.ai)
  %i.an = tail call float @llvm.fmuladd.f32(float %i.u, float 0.000000e+00, float %i.aj)
  ...
  %i.aq = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ap, <2 x float> <float f0x3F7FF1AE, float f0x3F7EC435>, <2 x float> %i.am)
  %i.ar = tail call float @llvm.fmuladd.f32(float %i.v, float 9.970480e-01, float %i.an)
  ...
  %i.au = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.at, <2 x float> <float f0x3F55694A, float f0x3D456469>, <2 x float> %i.aq)
  %i.av = tail call float @llvm.fmuladd.f32(float %i.w, float f0x3EA67A8E, float %i.ar)
  %i.aw = insertelement <2 x float> poison, float %i.x, i64 0
  %i.ax = shufflevector <2 x float> %i.aw, <2 x float> poison, <2 x i32> zeroinitializer
  %i.ay = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ax, <2 x float> <float f0x3FBB049B, float f0xBF2E74C9>, <2 x float> %i.au)
  %i.az = tail call float @llvm.fmuladd.f32(float %i.x, float f0x3F88EF83, float %i.av)
  %i.ba = insertelement <2 x float> poison, float %i.y, i64 0
  %i.bb = shufflevector <2 x float> %i.ba, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bc = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bb, <2 x float> <float f0xBE1CED3A, float f0x3E8A8A03>, <2 x float> %i.ay)
  %i.bd = tail call float @llvm.fmuladd.f32(float %i.y, float f0x3FFE6181, float %i.az)
  %i.be = insertelement <2 x float> poison, float %i.z, i64 0
  %i.bf = shufflevector <2 x float> %i.be, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bg = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bf, <2 x float> <float f0x3F2BC289, float f0x3F4E0630>, <2 x float> %i.bc)
  %i.bh = tail call float @llvm.fmuladd.f32(float %i.z, float 1.842450e-01, float %i.bd)
  %i.bi = insertelement <2 x float> poison, float %i.aa, i64 0
  %i.bj = shufflevector <2 x float> %i.bi, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bk = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bj, <2 x float> <float f0x3FB07499, float f0xBEB35BD6>, <2 x float> %i.bg)
  %i.bl = tail call float @llvm.fmuladd.f32(float %i.aa, float 3.688650e+00, float %i.bh)
  %i.bm = insertelement <2 x float> poison, float %i.ab, i64 0
  %i.bn = shufflevector <2 x float> %i.bm, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bo = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bn, <2 x float> <float f0x3FFD3419, float f0x3F86905C>, <2 x float> %i.bk)
  %i.bp = tail call float @llvm.fmuladd.f32(float %i.ab, float f0x40351CEE, float %i.bl)
  %i.bq = fmul <10 x float> %i.d, %i.i            ; 10 uses
  %i.br = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 9, i32 9, i32 9, i32 9>
  %i.bs = insertelement <4 x float> poison, float %i.bp, i64 0
  %i.bt = shufflevector <2 x float> %i.bo, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
  %i.bu = shufflevector <4 x float> %i.bs, <4 x float> %i.bt, <4 x i32> <i32 0, i32 4, i32 5, i32 0>
  %i.bv = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.br, <4 x float> <float f0x3F84550C, float f0x3F500D42, float f0x404DD41D, float f0x3F84550C>, <4 x float> %i.bu)
  %i.bw = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 8, i32 8, i32 8, i32 8>
  %i.bx = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.bw, <4 x float> <float f0xBD37CCB3, float f0x3ED4D0CC, float f0x40327DE4, float f0xBD37CCB3>, <4 x float> %i.bv)
  %i.by = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 7, i32 7, i32 7, i32 7>
  %i.bz = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.by, <4 x float> <float f0x3EA7C333, float f0x4023707E, float f0x40416251, float f0x3EA7C333>, <4 x float> %i.bx)
  %i.ca = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 6, i32 6, i32 6, i32 6>
  %i.cb = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.ca, <4 x float> <float f0x3F96801B, float f0x4033F7FD, float f0x403CE13B, float f0x3F96801B>, <4 x float> %i.bz)
  %i.cc = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 5, i32 5, i32 5, i32 5>
  %i.cd = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.cc, <4 x float> <float f0x3FE89875, float f0x3F4CA0EA, float f0x4034E9CF, float f0x3FE89875>, <4 x float> %i.cb)
  %i.ce = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 4, i32 4, i32 4, i32 4>
  %i.cf = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.ce, <4 x float> <float f0x3FE73C97, float f0x3F9CEB4B, float f0x403FCD64, float f0x3FE73C97>, <4 x float> %i.cd)
  %i.cg = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 3, i32 3, i32 3, i32 3>
  %i.ch = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.cg, <4 x float> <float f0xBE98C2A3, float f0x400337A7, float f0x3FEFDD4D, float f0xBE98C2A3>, <4 x float> %i.cf)
  %i.ci = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 2, i32 2, i32 2, i32 2>
  %i.cj = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.ci, <4 x float> <float f0x3F203166, float f0x40E118D7, float f0x40243AE9, float f0x3F203166>, <4 x float> %i.ch)
  %i.ck = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> <i32 1, i32 1, i32 1, i32 1>
  %i.cl = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.ck, <4 x float> <float f0x400998DE, float f0xBFB3B903, float f0x4082AA5A, float f0x400998DE>, <4 x float> %i.cj)
  %i.cm = shufflevector <10 x float> %i.bq, <10 x float> poison, <4 x i32> zeroinitializer
  %i.cn = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.cm, <4 x float> <float f0x3FF42585, float f0x40238DA6, float f0x40C00672, float f0x3FF42585>, <4 x float> %i.cl)
  %i.co = load <3 x float>, ptr %i.b, align 4, !tbaa !12
  %i.cp = shufflevector <3 x float> %i.co, <3 x float> poison, <4 x i32> <i32 2, i32 1, i32 0, i32 2>
  %i.cq = fadd <4 x float> %i.cp, %i.cn           ; 5 uses
  %i.cr = extractelement <4 x float> %i.cq, i64 2 ; 2 uses
  %i.cs = fcmp ogt float %i.cr, 1.000000e+00
  %i.ct = select i1 %i.cs, float 1.000000e+00, float %i.cr
  %i.cu = extractelement <4 x float> %i.cq, i64 1
  %i.cv = fcmp ogt float %i.cu, 1.000000e+00
  %i.cw = fcmp olt <4 x float> %i.cq, <float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 1.000000e+00> ; 2 uses
  %i.cx = fcmp ogt <4 x float> %i.cq, <float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 1.000000e+00>
  %i.cy = tail call float @llvm.fmuladd.f32(float %i.ct, float 2.550000e+02, float 5.000000e-01)
  %i.cz = extractelement <4 x i1> %i.cw, i64 2
  %i.da = fptosi float %i.cy to i32
  %i.db = trunc i32 %i.da to i8
  %i.dc = select i1 %i.cz, i8 0, i8 %i.db
  store i8 %i.dc, ptr %1, align 1, !tbaa !10
  %i.dd = shufflevector <4 x i1> %i.cx, <4 x i1> poison, <2 x i32> <i32 3, i32 poison>
  %i.de = insertelement <2 x i1> %i.dd, i1 %i.cv, i64 1
  %i.df = shufflevector <4 x float> %i.cq, <4 x float> poison, <2 x i32> <i32 0, i32 1>
  %i.dg = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.df, <2 x float> splat (float 2.550000e+02), <2 x float> splat (float 5.000000e-01))
  %i.dh = select <2 x i1> %i.de, <2 x float> splat (float 2.555000e+02), <2 x float> %i.dg
  %i.di = shufflevector <4 x i1> %i.cw, <4 x i1> poison, <2 x i32> <i32 0, i32 1>
  %i.dj = select <2 x i1> %i.di, <2 x float> splat (float 5.000000e-01), <2 x float> %i.dh ; 2 uses
  %i.dk = extractelement <2 x float> %i.dj, i64 1
  %i.dl = fptosi float %i.dk to i32
  %i.dm = trunc i32 %i.dl to i8
  store i8 %i.dm, ptr %2, align 1, !tbaa !10
  %i.dn = extractelement <2 x float> %i.dj, i64 0
  %i.do = fptosi float %i.dn to i32
  %i.dp = trunc i32 %i.do to i8
  store i8 %i.dp, ptr %3, align 1, !tbaa !10
  ret void
}

; Function Attrs: mustprogress nounwind uwtable
define dso_local void @mixbox_float_rgb_to_latent(float noundef %0, float noundef %1, float noundef %2, ptr nofree noundef writeonly captures(none) initializes((0, 28)) %3) local_unnamed_addr #0 personality ptr @__gxx_personality_v0 {
bb.a:
  %i.a = load atomic i8, ptr @_ZGVZL10mixbox_lutvE12decompressed acquire, align 8
  %i.b = icmp eq i8 %i.a, 0
  br i1 %i.b, label %bb.b, label %_ZL10mixbox_lutv.exit, !prof !9
...
_ZL10mixbox_lutv.exit:                            ; preds = %bb.a, %bb.b, %bb.c
  ...
  %i.f = fcmp ogt float %2, 1.000000e+00
  %i.g = select i1 %i.f, float 1.000000e+00, float %2
  %i.h = select i1 %i.e, float 0.000000e+00, float %i.g ; 2 uses
  %i.i = fmul float %i.h, 6.300000e+01            ; 2 uses
  %i.j = fptosi float %i.i to i32                 ; 2 uses
  %i.k = sitofp i32 %i.j to float
  %i.l = fsub float %i.i, %i.k                    ; 5 uses
  %i.m = insertelement <2 x float> poison, float %0, i64 0
  %i.n = insertelement <2 x float> %i.m, float %1, i64 1 ; 3 uses
  %i.o = fcmp olt <2 x float> %i.n, zeroinitializer
  %i.p = fcmp ogt <2 x float> %i.n, splat (float 1.000000e+00)
  %i.q = shl nsw i32 %i.j, 12
  %i.r = fsub float 1.000000e+00, %i.l            ; 4 uses
  ...
  %i.w = select <2 x i1> %i.p, <2 x float> splat (float 1.000000e+00), <2 x float> %i.n
  %i.x = select <2 x i1> %i.o, <2 x float> zeroinitializer, <2 x float> %i.w ; 2 uses
  %i.y = fmul <2 x float> %i.x, splat (float 6.300000e+01) ; 2 uses
  %i.z = fptosi <2 x float> %i.y to <2 x i32>     ; 3 uses
  %i.aa = sitofp <2 x i32> %i.z to <2 x float>
  %i.ab = fsub <2 x float> %i.y, %i.aa            ; 2 uses
  %i.ac = extractelement <2 x i32> %i.z, i64 1
  %i.ad = shl nsw i32 %i.ac, 6
  %i.ae = extractelement <2 x i32> %i.z, i64 0
  ...
  %i.ao = fsub float 1.000000e+00, %i.an          ; 2 uses
  %i.ap = fmul float %i.am, %i.ao                 ; 2 uses
  %i.aq = fmul float %i.ap, %i.r                  ; 2 uses
  %i.ar = getelementptr inbounds nuw i8, ptr %i.ak, i64 192
  %i.as = load i8, ptr %i.ar, align 1, !tbaa !10
  %i.at = uitofp i8 %i.as to float
  %i.au = tail call float @llvm.fmuladd.f32(float %i.aq, float %i.at, float 0.000000e+00)
  ...
  %i.bb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ba, <2 x float> %i.ay, <2 x float> zeroinitializer)
  %i.bc = fmul float %i.al, %i.ao                 ; 2 uses
  %i.bd = fmul float %i.bc, %i.r                  ; 2 uses
  %i.be = getelementptr inbounds nuw i8, ptr %i.ak, i64 195
  %i.bf = load i8, ptr %i.be, align 1, !tbaa !10
  %i.bg = uitofp i8 %i.bf to float
  %i.bh = tail call float @llvm.fmuladd.f32(float %i.bd, float %i.bg, float %i.au)
  ...
  %i.bo = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bn, <2 x float> %i.bl, <2 x float> %i.bb)
  %i.bp = fmul float %i.am, %i.an                 ; 2 uses
  %i.bq = fmul float %i.bp, %i.r                  ; 2 uses
  %i.br = getelementptr inbounds nuw i8, ptr %i.ak, i64 384
  %i.bs = load i8, ptr %i.br, align 1, !tbaa !10
  %i.bt = uitofp i8 %i.bs to float
  %i.bu = tail call float @llvm.fmuladd.f32(float %i.bq, float %i.bt, float %i.bh)
  ...
  %i.cb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ca, <2 x float> %i.by, <2 x float> %i.bo)
  %i.cc = fmul float %i.al, %i.an                 ; 2 uses
  %i.cd = fmul float %i.cc, %i.r                  ; 2 uses
  %i.ce = getelementptr inbounds nuw i8, ptr %i.ak, i64 387
  %i.cf = load i8, ptr %i.ce, align 1, !tbaa !10
  %i.cg = uitofp i8 %i.cf to float
  %i.ch = tail call float @llvm.fmuladd.f32(float %i.cd, float %i.cg, float %i.bu)
  ...
  %foldExtExtBinop = fmul <2 x float> %i.el, %i.en ; 2 uses
  %i.eu = extractelement <2 x float> %foldExtExtBinop, i64 1
  %foldExtExtBinop5 = fmul <2 x float> %i.el, %i.en ; 2 uses
  %i.ev = extractelement <2 x float> %foldExtExtBinop5, i64 0
  %i.ew = fmul float %14, %i.es                   ; 2 uses
  %i.ex = fmul float %4, %i.em                    ; 2 uses
  %15 = extractelement <4 x float> %11, i64 3
  %i.ey = fmul float %i.em, %15                   ; 2 uses
  ...
  %i.fi = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fh, <2 x float> <float 9.591230e-01, float f0x3F4D74EB>, <2 x float> %i.ff)
  %i.fj = tail call float @llvm.fmuladd.f32(float %i.eu, float f0x3D11E497, float %i.fg)
  %i.fk = shufflevector <2 x float> %foldExtExtBinop5, <2 x float> poison, <2 x i32> zeroinitializer
  %i.fl = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fk, <2 x float> <float f0x3F3F30C2, float f0x3D476AD3>, <2 x float> %i.fi)
  %i.fm = tail call float @llvm.fmuladd.f32(float %i.ev, float 0.000000e+00, float %i.fj)
  %i.fn = insertelement <2 x float> poison, float %i.ew, i64 0
  %i.fo = shufflevector <2 x float> %i.fn, <2 x float> poison, <2 x i32> zeroinitializer
  ...
  %i.if = tail call float @llvm.fmuladd.f32(float %i.ie, float f0x3FF42585, float %i.ib)
  store float %i.eo, ptr %3, align 4, !tbaa !12
  store float %i.em, ptr %i.s, align 4, !tbaa !12
  store float %i.eq, ptr %i.t, align 4, !tbaa !12
  store float %14, ptr %i.u, align 4, !tbaa !12
  %i.ig = fsub <2 x float> %i.x, %i.id
  store <2 x float> %i.ig, ptr %i.v, align 4, !tbaa !12
  %i.ih = fsub float %i.h, %i.if
  %i.ii = getelementptr inbounds nuw i8, ptr %3, i64 24
  store float %i.ih, ptr %i.ii, align 4, !tbaa !12
  ret void
...
; Function Attrs: mustprogress nounwind uwtable
define dso_local void @mixbox_linear_float_rgb_to_latent(float noundef %0, float noundef %1, float noundef %2, ptr nofree noundef writeonly captures(none) initializes((0, 28)) %3) local_unnamed_addr #0 personality ptr @__gxx_personality_v0 {
bb.a:
  %i.a = fcmp ult float %0, 3.130800e-03
  br i1 %i.a, label %bb.c, label %bb.b

bb.b:                                             ; preds = %bb.a
  %i.b = tail call noundef float @powf(float noundef %0, float noundef f0x3ED55555) #12
  %i.c = tail call float @llvm.fmuladd.f32(float %i.b, float 1.055000e+00, float -5.500000e-02)
  br label %_ZL14linear_to_srgbf.exit5

bb.c:                                             ; preds = %bb.a
  %i.d = fmul float %0, 1.292000e+01
  br label %_ZL14linear_to_srgbf.exit5

_ZL14linear_to_srgbf.exit5:                       ; preds = %bb.b, %bb.c
  %i.e = phi float [ %i.c, %bb.b ], [ %i.d, %bb.c ]
  %i.f = fcmp ult float %1, 3.130800e-03
  br i1 %i.f, label %bb.e, label %bb.d

bb.d:                                             ; preds = %_ZL14linear_to_srgbf.exit5
  %i.g = tail call noundef float @powf(float noundef %1, float noundef f0x3ED55555) #12
  %i.h = tail call float @llvm.fmuladd.f32(float %i.g, float 1.055000e+00, float -5.500000e-02)
  br label %_ZL14linear_to_srgbf.exit4

bb.e:                                             ; preds = %_ZL14linear_to_srgbf.exit5
  %i.i = fmul float %1, 1.292000e+01
  br label %_ZL14linear_to_srgbf.exit4

_ZL14linear_to_srgbf.exit4:                       ; preds = %bb.d, %bb.e
  %i.j = phi float [ %i.h, %bb.d ], [ %i.i, %bb.e ]
  %i.k = fcmp ult float %2, 3.130800e-03
  br i1 %i.k, label %bb.g, label %bb.f

bb.f:                                             ; preds = %_ZL14linear_to_srgbf.exit4
  %i.l = tail call noundef float @powf(float noundef %2, float noundef f0x3ED55555) #12
  %i.m = tail call float @llvm.fmuladd.f32(float %i.l, float 1.055000e+00, float -5.500000e-02)
  br label %_ZL14linear_to_srgbf.exit

bb.g:                                             ; preds = %_ZL14linear_to_srgbf.exit4
  %i.n = fmul float %2, 1.292000e+01
  br label %_ZL14linear_to_srgbf.exit

_ZL14linear_to_srgbf.exit:                        ; preds = %bb.f, %bb.g
  %i.o = phi float [ %i.m, %bb.f ], [ %i.n, %bb.g ] ; 3 uses
  %i.p = load atomic i8, ptr @_ZGVZL10mixbox_lutvE12decompressed acquire, align 8
  %i.q = icmp eq i8 %i.p, 0
  br i1 %i.q, label %bb.h, label %_ZL10mixbox_lutv.exit, !prof !9

bb.h:                                             ; preds = %_ZL14linear_to_srgbf.exit
  %i.r = tail call i32 @__cxa_guard_acquire(ptr nonnull @_ZGVZL10mixbox_lutvE12decompressed) #12
  %.not.i = icmp eq i32 %i.r, 0
  br i1 %.not.i, label %_ZL10mixbox_lutv.exit, label %bb.i

bb.i:                                             ; preds = %bb.h
  tail call fastcc void @_ZZL10mixbox_lutvEN13mixbox_init_tC2Ev()
  %i.s = tail call ptr @llvm.invariant.start.p0(i64 799107, ptr nonnull @_ZZL10mixbox_lutvE12decompressed) ; 0 uses
  tail call void @__cxa_guard_release(ptr nonnull @_ZGVZL10mixbox_lutvE12decompressed) #12
  br label %_ZL10mixbox_lutv.exit

_ZL10mixbox_lutv.exit:                            ; preds = %_ZL14linear_to_srgbf.exit, %bb.h, %bb.i
  %i.t = fcmp olt float %i.o, 0.000000e+00
  %i.u = fcmp ogt float %i.o, 1.000000e+00
  %i.v = select i1 %i.u, float 1.000000e+00, float %i.o
  %i.w = select i1 %i.t, float 0.000000e+00, float %i.v ; 2 uses
  %i.x = fmul float %i.w, 6.300000e+01            ; 2 uses
  %i.y = fptosi float %i.x to i32                 ; 2 uses
  %i.z = sitofp i32 %i.y to float
  %i.aa = fsub float %i.x, %i.z                   ; 5 uses
  %i.ab = insertelement <2 x float> poison, float %i.e, i64 0
  %i.ac = insertelement <2 x float> %i.ab, float %i.j, i64 1 ; 3 uses
  %i.ad = fcmp olt <2 x float> %i.ac, zeroinitializer
  %i.ae = fcmp ogt <2 x float> %i.ac, splat (float 1.000000e+00)
  %i.af = shl nsw i32 %i.y, 12
  %i.ag = fsub float 1.000000e+00, %i.aa          ; 4 uses
  ...
  %i.al = select <2 x i1> %i.ae, <2 x float> splat (float 1.000000e+00), <2 x float> %i.ac
  %i.am = select <2 x i1> %i.ad, <2 x float> zeroinitializer, <2 x float> %i.al ; 2 uses
  %i.an = fmul <2 x float> %i.am, splat (float 6.300000e+01) ; 2 uses
  %i.ao = fptosi <2 x float> %i.an to <2 x i32>   ; 3 uses
  %i.ap = sitofp <2 x i32> %i.ao to <2 x float>
  %i.aq = fsub <2 x float> %i.an, %i.ap           ; 2 uses
  %i.ar = extractelement <2 x i32> %i.ao, i64 1
  %i.as = shl nsw i32 %i.ar, 6
  %i.at = extractelement <2 x i32> %i.ao, i64 0
  %i.au = add nsw i32 %i.as, %i.at
  %i.av = add nsw i32 %i.au, %i.af
  %i.aw = and i32 %i.av, 262143
  %i.ax = mul nuw nsw i32 %i.aw, 3
  %i.ay = zext nneg i32 %i.ax to i64
  %i.az = getelementptr inbounds nuw i8, ptr @_ZZL10mixbox_lutvE12decompressed, i64 %i.ay ; 16 uses
  %i.ba = extractelement <2 x float> %i.aq, i64 0 ; 3 uses
  %i.bb = fsub float 1.000000e+00, %i.ba          ; 2 uses
  %i.bc = extractelement <2 x float> %i.aq, i64 1 ; 3 uses
  %i.bd = fsub float 1.000000e+00, %i.bc          ; 2 uses
  %i.be = fmul float %i.bb, %i.bd                 ; 2 uses
  %i.bf = fmul float %i.be, %i.ag                 ; 2 uses
  %i.bg = getelementptr inbounds nuw i8, ptr %i.az, i64 192
  %i.bh = load i8, ptr %i.bg, align 1, !tbaa !10
  %i.bi = uitofp i8 %i.bh to float
  %i.bj = tail call float @llvm.fmuladd.f32(float %i.bf, float %i.bi, float 0.000000e+00)
  %i.bk = getelementptr inbounds nuw i8, ptr %i.az, i64 193
  %i.bl = load <2 x i8>, ptr %i.bk, align 1, !tbaa !10
  %i.bm = shufflevector <2 x i8> %i.bl, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.bn = uitofp <2 x i8> %i.bm to <2 x float>
  %i.bo = insertelement <2 x float> poison, float %i.bf, i64 0
  %i.bp = shufflevector <2 x float> %i.bo, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bq = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bp, <2 x float> %i.bn, <2 x float> zeroinitializer)
  %i.br = fmul float %i.ba, %i.bd                 ; 2 uses
  %i.bs = fmul float %i.br, %i.ag                 ; 2 uses
  %i.bt = getelementptr inbounds nuw i8, ptr %i.az, i64 195
  %i.bu = load i8, ptr %i.bt, align 1, !tbaa !10
  %i.bv = uitofp i8 %i.bu to float
  %i.bw = tail call float @llvm.fmuladd.f32(float %i.bs, float %i.bv, float %i.bj)
  %i.bx = getelementptr inbounds nuw i8, ptr %i.az, i64 196
  %i.by = load <2 x i8>, ptr %i.bx, align 1, !tbaa !10
  %i.bz = shufflevector <2 x i8> %i.by, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.ca = uitofp <2 x i8> %i.bz to <2 x float>
  %i.cb = insertelement <2 x float> poison, float %i.bs, i64 0
  %i.cc = shufflevector <2 x float> %i.cb, <2 x float> poison, <2 x i32> zeroinitializer
  %i.cd = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cc, <2 x float> %i.ca, <2 x float> %i.bq)
  %i.ce = fmul float %i.bb, %i.bc                 ; 2 uses
  %i.cf = fmul float %i.ce, %i.ag                 ; 2 uses
  %i.cg = getelementptr inbounds nuw i8, ptr %i.az, i64 384
  %i.ch = load i8, ptr %i.cg, align 1, !tbaa !10
  %i.ci = uitofp i8 %i.ch to float
  %i.cj = tail call float @llvm.fmuladd.f32(float %i.cf, float %i.ci, float %i.bw)
  %i.ck = getelementptr inbounds nuw i8, ptr %i.az, i64 385
  %i.cl = load <2 x i8>, ptr %i.ck, align 1, !tbaa !10
  %i.cm = shufflevector <2 x i8> %i.cl, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.cn = uitofp <2 x i8> %i.cm to <2 x float>
  ...
  %i.cq = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cp, <2 x float> %i.cn, <2 x float> %i.cd)
  %i.cr = fmul float %i.ba, %i.bc                 ; 2 uses
  %i.cs = fmul float %i.cr, %i.ag                 ; 2 uses
  %i.ct = getelementptr inbounds nuw i8, ptr %i.az, i64 387
  %i.cu = load i8, ptr %i.ct, align 1, !tbaa !10
  %i.cv = uitofp i8 %i.cu to float
  %i.cw = tail call float @llvm.fmuladd.f32(float %i.cs, float %i.cv, float %i.cj)
  %i.cx = getelementptr inbounds nuw i8, ptr %i.az, i64 388
  %i.cy = load <2 x i8>, ptr %i.cx, align 1, !tbaa !10
  %i.cz = shufflevector <2 x i8> %i.cy, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.da = uitofp <2 x i8> %i.cz to <2 x float>
  ...
  %i.dd = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.dc, <2 x float> %i.da, <2 x float> %i.cq)
  %i.de = fmul float %i.be, %i.aa                 ; 2 uses
  %i.df = getelementptr inbounds nuw i8, ptr %i.az, i64 12480
  %i.dg = load i8, ptr %i.df, align 1, !tbaa !10
  %i.dh = uitofp i8 %i.dg to float
  %i.di = tail call float @llvm.fmuladd.f32(float %i.de, float %i.dh, float %i.cw)
  %i.dj = getelementptr inbounds nuw i8, ptr %i.az, i64 12481
  %i.dk = load <2 x i8>, ptr %i.dj, align 1, !tbaa !10
  %i.dl = shufflevector <2 x i8> %i.dk, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.dm = uitofp <2 x i8> %i.dl to <2 x float>
  ...
  %i.dp = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.do, <2 x float> %i.dm, <2 x float> %i.dd)
  %i.dq = fmul float %i.br, %i.aa                 ; 2 uses
  %i.dr = getelementptr inbounds nuw i8, ptr %i.az, i64 12483
  %i.ds = load i8, ptr %i.dr, align 1, !tbaa !10
  %i.dt = uitofp i8 %i.ds to float
  %i.du = tail call float @llvm.fmuladd.f32(float %i.dq, float %i.dt, float %i.di)
  %i.dv = getelementptr inbounds nuw i8, ptr %i.az, i64 12484
  %i.dw = load <2 x i8>, ptr %i.dv, align 1, !tbaa !10
  %i.dx = shufflevector <2 x i8> %i.dw, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.dy = uitofp <2 x i8> %i.dx to <2 x float>
  %i.dz = insertelement <2 x float> poison, float %i.dq, i64 0
  %i.ea = shufflevector <2 x float> %i.dz, <2 x float> poison, <2 x i32> zeroinitializer
  %i.eb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ea, <2 x float> %i.dy, <2 x float> %i.dp)
  %i.ec = fmul float %i.ce, %i.aa                 ; 2 uses
  %i.ed = getelementptr inbounds nuw i8, ptr %i.az, i64 12672
  %i.ee = load i8, ptr %i.ed, align 1, !tbaa !10
  %i.ef = uitofp i8 %i.ee to float
  %i.eg = tail call float @llvm.fmuladd.f32(float %i.ec, float %i.ef, float %i.du)
  %i.eh = getelementptr inbounds nuw i8, ptr %i.az, i64 12673
  %i.ei = load <2 x i8>, ptr %i.eh, align 1, !tbaa !10
  %i.ej = shufflevector <2 x i8> %i.ei, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  ...
  %i.en = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.em, <2 x float> %i.ek, <2 x float> %i.eb)
  %i.eo = fmul float %i.cr, %i.aa                 ; 2 uses
  %i.ep = getelementptr inbounds nuw i8, ptr %i.az, i64 12675
  %i.eq = load i8, ptr %i.ep, align 1, !tbaa !10
  %i.er = uitofp i8 %i.eq to float
  %i.es = tail call float @llvm.fmuladd.f32(float %i.eo, float %i.er, float %i.eg)
  %i.et = getelementptr inbounds nuw i8, ptr %i.az, i64 12676
  %i.eu = load <2 x i8>, ptr %i.et, align 1, !tbaa !10
  %i.ev = shufflevector <2 x i8> %i.eu, <2 x i8> poison, <2 x i32> <i32 1, i32 0>
  %i.ew = uitofp <2 x i8> %i.ev to <2 x float>
  %i.ex = insertelement <2 x float> poison, float %i.eo, i64 0
  %i.ey = shufflevector <2 x float> %i.ex, <2 x float> poison, <2 x i32> zeroinitializer
  %i.ez = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ey, <2 x float> %i.ew, <2 x float> %i.en)
  %i.fa = fmul <2 x float> %i.ez, splat (float f0x3B808081) ; 9 uses
  %i.fb = extractelement <2 x float> %i.fa, i64 1 ; 4 uses
  %i.fc = fmul <2 x float> %i.fa, %i.fa           ; 4 uses
  %i.fd = fmul float %i.es, f0x3B808081           ; 7 uses
  %i.fe = fadd float %i.fd, %i.fb
  %i.ff = extractelement <2 x float> %i.fa, i64 0 ; 4 uses
  %i.fg = fadd float %i.fe, %i.ff
  %4 = fmul float %i.fd, %i.fd                    ; 4 uses
  %5 = shufflevector <2 x float> %i.fa, <2 x float> poison, <4 x i32> <i32 1, i32 0, i32 poison, i32 1>
  %6 = insertelement <4 x float> %5, float 1.000000e+00, i64 2 ; 2 uses
  %7 = shufflevector <2 x float> %i.fa, <2 x float> poison, <4 x i32> <i32 0, i32 poison, i32 poison, i32 poison>
  %8 = insertelement <4 x float> %7, float %i.fd, i64 1
  %9 = insertelement <4 x float> %8, float %i.fg, i64 2
  %10 = shufflevector <4 x float> %9, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 2, i32 1> ; 2 uses
  %11 = fmul <4 x float> %6, %10                  ; 3 uses
  %12 = fsub <4 x float> %6, %10                  ; 2 uses
  %13 = shufflevector <4 x float> %11, <4 x float> %12, <4 x i32> <i32 0, i32 1, i32 6, i32 3> ; 2 uses
  %14 = extractelement <4 x float> %12, i64 2     ; 5 uses
  %i.fh = fmul float %14, %14                     ; 3 uses
  %i.fi = fmul float %i.fd, %4                    ; 2 uses
  %foldExtExtBinop = fmul <2 x float> %i.fa, %i.fc ; 2 uses
  %i.fj = extractelement <2 x float> %foldExtExtBinop, i64 1
  %foldExtExtBinop11 = fmul <2 x float> %i.fa, %i.fc ; 2 uses
  %i.fk = extractelement <2 x float> %foldExtExtBinop11, i64 0
  %i.fl = fmul float %14, %i.fh                   ; 2 uses
  %i.fm = fmul float %4, %i.fb                    ; 2 uses
  %15 = extractelement <4 x float> %11, i64 3
  %i.fn = fmul float %i.fb, %15                   ; 2 uses
  %i.fo = fmul float %4, %i.ff                    ; 2 uses
  %16 = extractelement <4 x float> %11, i64 1
  %i.fp = fmul float %i.ff, %16                   ; 2 uses
  %i.fq = fmul float %4, %14                      ; 2 uses
  %i.fr = fmul float %i.fd, %i.fh                 ; 2 uses
  %i.fs = insertelement <2 x float> poison, float %i.fi, i64 0
  %i.ft = shufflevector <2 x float> %i.fs, <2 x float> poison, <2 x i32> zeroinitializer
  %i.fu = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ft, <2 x float> <float f0x3D9E0B95, float f0x3CE79607>, <2 x float> zeroinitializer)
  %i.fv = tail call float @llvm.fmuladd.f32(float %i.fi, float f0x3E7E4A33, float 0.000000e+00)
  %i.fw = shufflevector <2 x float> %foldExtExtBinop, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.fx = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fw, <2 x float> <float 9.591230e-01, float f0x3F4D74EB>, <2 x float> %i.fu)
  %i.fy = tail call float @llvm.fmuladd.f32(float %i.fj, float f0x3D11E497, float %i.fv)
  %i.fz = shufflevector <2 x float> %foldExtExtBinop11, <2 x float> poison, <2 x i32> zeroinitializer
  %i.ga = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.fz, <2 x float> <float f0x3F3F30C2, float f0x3D476AD3>, <2 x float> %i.fx)
  %i.gb = tail call float @llvm.fmuladd.f32(float %i.fk, float 0.000000e+00, float %i.fy)
  %i.gc = insertelement <2 x float> poison, float %i.fl, i64 0
  %i.gd = shufflevector <2 x float> %i.gc, <2 x float> poison, <2 x i32> zeroinitializer
  %i.ge = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gd, <2 x float> <float f0x3F7EC435, float f0x3F7FF1AE>, <2 x float> %i.ga)
  %i.gf = tail call float @llvm.fmuladd.f32(float %i.fl, float 9.970480e-01, float %i.gb)
  %i.gg = insertelement <2 x float> poison, float %i.fm, i64 0
  %i.gh = shufflevector <2 x float> %i.gg, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gi = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gh, <2 x float> <float f0x3D456469, float f0x3F55694A>, <2 x float> %i.ge)
  %i.gj = tail call float @llvm.fmuladd.f32(float %i.fm, float f0x3EA67A8E, float %i.gf)
  %i.gk = insertelement <2 x float> poison, float %i.fn, i64 0
  %i.gl = shufflevector <2 x float> %i.gk, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gm = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gl, <2 x float> <float f0xBF2E74C9, float f0x3FBB049B>, <2 x float> %i.gi)
  %i.gn = tail call float @llvm.fmuladd.f32(float %i.fn, float f0x3F88EF83, float %i.gj)
  %i.go = insertelement <2 x float> poison, float %i.fo, i64 0
  %i.gp = shufflevector <2 x float> %i.go, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gq = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gp, <2 x float> <float f0x3E8A8A03, float f0xBE1CED3A>, <2 x float> %i.gm)
  %i.gr = tail call float @llvm.fmuladd.f32(float %i.fo, float f0x3FFE6181, float %i.gn)
  %i.gs = insertelement <2 x float> poison, float %i.fp, i64 0
  %i.gt = shufflevector <2 x float> %i.gs, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gu = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gt, <2 x float> <float f0x3F4E0630, float f0x3F2BC289>, <2 x float> %i.gq)
  %i.gv = tail call float @llvm.fmuladd.f32(float %i.fp, float 1.842450e-01, float %i.gr)
  %i.gw = insertelement <2 x float> poison, float %i.fq, i64 0
  %i.gx = shufflevector <2 x float> %i.gw, <2 x float> poison, <2 x i32> zeroinitializer
  %i.gy = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gx, <2 x float> <float f0xBEB35BD6, float f0x3FB07499>, <2 x float> %i.gu)
  %i.gz = tail call float @llvm.fmuladd.f32(float %i.fq, float 3.688650e+00, float %i.gv)
  %i.ha = insertelement <2 x float> poison, float %i.fr, i64 0
  %i.hb = shufflevector <2 x float> %i.ha, <2 x float> poison, <2 x i32> zeroinitializer
  %i.hc = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hb, <2 x float> <float f0x3F86905C, float f0x3FFD3419>, <2 x float> %i.gy)
  %i.hd = tail call float @llvm.fmuladd.f32(float %i.fr, float f0x40351CEE, float %i.gz)
  %i.he = shufflevector <2 x float> %i.fc, <2 x float> poison, <10 x i32> <i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 0, i32 poison, i32 1, i32 poison, i32 poison>
  %17 = shufflevector <4 x float> %13, <4 x float> poison, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %18 = shufflevector <10 x float> %17, <10 x float> %i.he, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 poison, i32 15, i32 poison, i32 17, i32 poison, i32 poison>
  %i.hf = insertelement <10 x float> %18, float %i.fh, i64 4
  %19 = shufflevector <10 x float> %i.hf, <10 x float> poison, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 4, i32 7, i32 poison, i32 poison>
  %20 = shufflevector <2 x float> %i.fc, <2 x float> poison, <10 x i32> <i32 0, i32 1, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %21 = shufflevector <10 x float> %19, <10 x float> %20, <10 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7, i32 10, i32 11>
  %22 = shufflevector <4 x float> %13, <4 x float> poison, <10 x i32> <i32 2, i32 3, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %23 = shufflevector <2 x float> %i.fa, <2 x float> poison, <10 x i32> <i32 0, i32 1, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %24 = shufflevector <10 x float> %22, <10 x float> %23, <10 x i32> <i32 0, i32 1, i32 10, i32 11, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
  %25 = shufflevector <10 x float> %24, <10 x float> poison, <10 x i32> <i32 0, i32 0, i32 1, i32 2, i32 2, i32 0, i32 3, i32 0, i32 3, i32 2>
  %i.hg = fmul <10 x float> %21, %25              ; 20 uses
  %i.hh = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 9, i32 9>
  %i.hi = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hh, <2 x float> <float f0x404DD41D, float f0x3F500D42>, <2 x float> %i.hc)
  %i.hj = extractelement <10 x float> %i.hg, i64 9
  %i.hk = tail call float @llvm.fmuladd.f32(float %i.hj, float f0x3F84550C, float %i.hd)
  %i.hl = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 8, i32 8>
  %i.hm = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hl, <2 x float> <float f0x40327DE4, float f0x3ED4D0CC>, <2 x float> %i.hi)
  %i.hn = extractelement <10 x float> %i.hg, i64 8
  %i.ho = tail call float @llvm.fmuladd.f32(float %i.hn, float f0xBD37CCB3, float %i.hk)
  %i.hp = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 7, i32 7>
  %i.hq = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hp, <2 x float> <float f0x40416251, float f0x4023707E>, <2 x float> %i.hm)
  %i.hr = extractelement <10 x float> %i.hg, i64 7
  %i.hs = tail call float @llvm.fmuladd.f32(float %i.hr, float f0x3EA7C333, float %i.ho)
  %i.ht = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 6, i32 6>
  %i.hu = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ht, <2 x float> <float f0x403CE13B, float f0x4033F7FD>, <2 x float> %i.hq)
  %i.hv = extractelement <10 x float> %i.hg, i64 6
  %i.hw = tail call float @llvm.fmuladd.f32(float %i.hv, float f0x3F96801B, float %i.hs)
  %i.hx = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 5, i32 5>
  %i.hy = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.hx, <2 x float> <float f0x4034E9CF, float f0x3F4CA0EA>, <2 x float> %i.hu)
  %i.hz = extractelement <10 x float> %i.hg, i64 5
  %i.ia = tail call float @llvm.fmuladd.f32(float %i.hz, float f0x3FE89875, float %i.hw)
  %i.ib = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 4, i32 4>
  %i.ic = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ib, <2 x float> <float f0x403FCD64, float f0x3F9CEB4B>, <2 x float> %i.hy)
  %i.id = extractelement <10 x float> %i.hg, i64 4
  %i.ie = tail call float @llvm.fmuladd.f32(float %i.id, float f0x3FE73C97, float %i.ia)
  %i.if = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 3, i32 3>
  %i.ig = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.if, <2 x float> <float f0x3FEFDD4D, float f0x400337A7>, <2 x float> %i.ic)
  %i.ih = extractelement <10 x float> %i.hg, i64 3
  %i.ii = tail call float @llvm.fmuladd.f32(float %i.ih, float f0xBE98C2A3, float %i.ie)
  %i.ij = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 2, i32 2>
  %i.ik = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ij, <2 x float> <float f0x40243AE9, float f0x40E118D7>, <2 x float> %i.ig)
  %i.il = extractelement <10 x float> %i.hg, i64 2
  %i.im = tail call float @llvm.fmuladd.f32(float %i.il, float f0x3F203166, float %i.ii)
  %i.in = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.io = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.in, <2 x float> <float f0x4082AA5A, float f0xBFB3B903>, <2 x float> %i.ik)
  %i.ip = extractelement <10 x float> %i.hg, i64 1
  %i.iq = tail call float @llvm.fmuladd.f32(float %i.ip, float f0x400998DE, float %i.im)
  %i.ir = shufflevector <10 x float> %i.hg, <10 x float> poison, <2 x i32> zeroinitializer
  %i.is = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ir, <2 x float> <float f0x40C00672, float f0x40238DA6>, <2 x float> %i.io)
  %i.it = extractelement <10 x float> %i.hg, i64 0
  %i.iu = tail call float @llvm.fmuladd.f32(float %i.it, float f0x3FF42585, float %i.iq)
  store float %i.fd, ptr %3, align 4, !tbaa !12
  store float %i.fb, ptr %i.ah, align 4, !tbaa !12
  store float %i.ff, ptr %i.ai, align 4, !tbaa !12
  store float %14, ptr %i.aj, align 4, !tbaa !12
  %i.iv = fsub <2 x float> %i.am, %i.is
  store <2 x float> %i.iv, ptr %i.ak, align 4, !tbaa !12
  %i.iw = fsub float %i.w, %i.iu
  %i.ix = getelementptr inbounds nuw i8, ptr %3, i64 24
  store float %i.iw, ptr %i.ix, align 4, !tbaa !12
  ret void
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(argmem: readwrite) uwtable
define dso_local void @mixbox_latent_to_float_rgb(ptr nofree noundef readonly captures(none) %0, ptr nofree noundef writeonly captures(none) initializes((0, 4)) %1, ptr nofree noundef writeonly captures(none) initializes((0, 4)) %2, ptr nofree noundef writeonly captures(none) initializes((0, 4)) %3) local_unnamed_addr #1 {
bb.a:
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 4
  %i.b = getelementptr inbounds nuw i8, ptr %0, i64 8
  %i.c = getelementptr inbounds nuw i8, ptr %0, i64 12
  %i.d = load float, ptr %0, align 4, !tbaa !12   ; 6 uses
  %i.e = load float, ptr %i.a, align 4, !tbaa !12 ; 9 uses
  %i.f = load float, ptr %i.b, align 4, !tbaa !12 ; 10 uses
  %i.g = load float, ptr %i.c, align 4, !tbaa !12 ; 9 uses
  %i.h = fmul float %i.d, %i.d                    ; 4 uses
  %i.i = fmul float %i.e, %i.e                    ; 3 uses
  %i.j = fmul float %i.f, %i.f                    ; 3 uses
  %i.k = fmul float %i.g, %i.g                    ; 4 uses
  %i.l = fmul float %i.d, %i.e                    ; 3 uses
  %i.m = fmul float %i.d, %i.f                    ; 2 uses
  %i.n = fmul float %i.e, %i.f
  %i.o = fmul float %i.d, %i.h                    ; 3 uses
  %i.p = tail call float @llvm.fmuladd.f32(float %i.o, float f0x3D9E0B95, float 0.000000e+00)
  %i.q = tail call float @llvm.fmuladd.f32(float %i.o, float f0x3CE79607, float 0.000000e+00)
  %i.r = tail call float @llvm.fmuladd.f32(float %i.o, float f0x3E7E4A33, float 0.000000e+00)
  %i.s = fmul float %i.e, %i.i                    ; 3 uses
  %i.t = tail call float @llvm.fmuladd.f32(float %i.s, float 9.591230e-01, float %i.p)
  %i.u = tail call float @llvm.fmuladd.f32(float %i.s, float f0x3F4D74EB, float %i.q)
  %i.v = tail call float @llvm.fmuladd.f32(float %i.s, float f0x3D11E497, float %i.r)
  %i.w = fmul float %i.f, %i.j                    ; 3 uses
  %i.x = tail call float @llvm.fmuladd.f32(float %i.w, float f0x3F3F30C2, float %i.t)
  %i.y = tail call float @llvm.fmuladd.f32(float %i.w, float f0x3D476AD3, float %i.u)
  %i.z = tail call float @llvm.fmuladd.f32(float %i.w, float 0.000000e+00, float %i.v)
  %i.aa = fmul float %i.g, %i.k                   ; 3 uses
  %i.ab = tail call float @llvm.fmuladd.f32(float %i.aa, float f0x3F7EC435, float %i.x)
  %i.ac = tail call float @llvm.fmuladd.f32(float %i.aa, float f0x3F7FF1AE, float %i.y)
  %i.ad = tail call float @llvm.fmuladd.f32(float %i.aa, float 9.970480e-01, float %i.z)
  %i.ae = fmul float %i.h, %i.e                   ; 3 uses
  %i.af = tail call float @llvm.fmuladd.f32(float %i.ae, float f0x3D456469, float %i.ab)
  %i.ag = tail call float @llvm.fmuladd.f32(float %i.ae, float f0x3F55694A, float %i.ac)
  %i.ah = tail call float @llvm.fmuladd.f32(float %i.ae, float f0x3EA67A8E, float %i.ad)
  %i.ai = fmul float %i.e, %i.l                   ; 3 uses
  %i.aj = tail call float @llvm.fmuladd.f32(float %i.ai, float f0xBF2E74C9, float %i.af)
  %i.ak = tail call float @llvm.fmuladd.f32(float %i.ai, float f0x3FBB049B, float %i.ag)
  %i.al = tail call float @llvm.fmuladd.f32(float %i.ai, float f0x3F88EF83, float %i.ah)
  %i.am = fmul float %i.h, %i.f                   ; 3 uses
  %i.an = tail call float @llvm.fmuladd.f32(float %i.am, float f0x3E8A8A03, float %i.aj)
  %i.ao = tail call float @llvm.fmuladd.f32(float %i.am, float f0xBE1CED3A, float %i.ak)
  %i.ap = tail call float @llvm.fmuladd.f32(float %i.am, float f0x3FFE6181, float %i.al)
  %i.aq = fmul float %i.f, %i.m                   ; 3 uses
  %i.ar = tail call float @llvm.fmuladd.f32(float %i.aq, float f0x3F4E0630, float %i.an)
  %i.as = tail call float @llvm.fmuladd.f32(float %i.aq, float f0x3F2BC289, float %i.ao)
  %i.at = tail call float @llvm.fmuladd.f32(float %i.aq, float 1.842450e-01, float %i.ap)
  %i.au = fmul float %i.h, %i.g                   ; 2 uses
  %i.av = fmul float %i.d, %i.k                   ; 2 uses
  %i.aw = fmul float %i.i, %i.f                   ; 2 uses
  %i.ax = fmul float %i.e, %i.j                   ; 2 uses
  %i.ay = fmul float %i.i, %i.g                   ; 2 uses
  %i.az = fmul float %i.e, %i.k                   ; 2 uses
  %i.ba = fmul float %i.j, %i.g                   ; 2 uses
  %i.bb = fmul float %i.f, %i.k                   ; 2 uses
  %i.bc = fmul float %i.l, %i.f                   ; 2 uses
  %i.bd = fmul float %i.l, %i.g                   ; 2 uses
  %i.be = fmul float %i.m, %i.g                   ; 2 uses
  %i.bf = fmul float %i.n, %i.g                   ; 2 uses
  %i.bg = tail call float @llvm.fmuladd.f32(float %i.au, float f0xBEB35BD6, float %i.ar)
  %i.bh = insertelement <2 x float> poison, float %i.au, i64 0
  ...
  %i.bk = insertelement <2 x float> %i.bj, float %i.at, i64 1
  %i.bl = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bi, <2 x float> <float f0x3FB07499, float 3.688650e+00>, <2 x float> %i.bk)
  %i.bm = tail call float @llvm.fmuladd.f32(float %i.av, float f0x3F86905C, float %i.bg)
  ...
  %i.bp = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bo, <2 x float> <float f0x3FFD3419, float f0x40351CEE>, <2 x float> %i.bl)
  %i.bq = tail call float @llvm.fmuladd.f32(float %i.aw, float f0x404DD41D, float %i.bm)
  %i.br = insertelement <2 x float> poison, float %i.aw, i64 0
  %i.bs = shufflevector <2 x float> %i.br, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bt = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bs, <2 x float> <float f0x3F500D42, float f0x3F84550C>, <2 x float> %i.bp)
  %i.bu = tail call float @llvm.fmuladd.f32(float %i.ax, float f0x40327DE4, float %i.bq)
  %i.bv = insertelement <2 x float> poison, float %i.ax, i64 0
  %i.bw = shufflevector <2 x float> %i.bv, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bx = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bw, <2 x float> <float f0x3ED4D0CC, float f0xBD37CCB3>, <2 x float> %i.bt)
  %i.by = tail call float @llvm.fmuladd.f32(float %i.ay, float f0x40416251, float %i.bu)
  %i.bz = insertelement <2 x float> poison, float %i.ay, i64 0
  %i.ca = shufflevector <2 x float> %i.bz, <2 x float> poison, <2 x i32> zeroinitializer
  %i.cb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ca, <2 x float> <float f0x4023707E, float f0x3EA7C333>, <2 x float> %i.bx)
  %i.cc = tail call float @llvm.fmuladd.f32(float %i.az, float f0x403CE13B, float %i.by)
  %i.cd = insertelement <2 x float> poison, float %i.az, i64 0
  %i.ce = shufflevector <2 x float> %i.cd, <2 x float> poison, <2 x i32> zeroinitializer
  %i.cf = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ce, <2 x float> <float f0x4033F7FD, float f0x3F96801B>, <2 x float> %i.cb)
  %i.cg = tail call float @llvm.fmuladd.f32(float %i.ba, float f0x4034E9CF, float %i.cc)
  %i.ch = insertelement <2 x float> poison, float %i.ba, i64 0
  %i.ci = shufflevector <2 x float> %i.ch, <2 x float> poison, <2 x i32> zeroinitializer
  %i.cj = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ci, <2 x float> <float f0x3F4CA0EA, float f0x3FE89875>, <2 x float> %i.cf)
  %i.ck = tail call float @llvm.fmuladd.f32(float %i.bb, float f0x403FCD64, float %i.cg)
  %i.cl = insertelement <2 x float> poison, float %i.bb, i64 0
  %i.cm = shufflevector <2 x float> %i.cl, <2 x float> poison, <2 x i32> zeroinitializer
  %i.cn = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cm, <2 x float> <float f0x3F9CEB4B, float f0x3FE73C97>, <2 x float> %i.cj)
  %i.co = tail call float @llvm.fmuladd.f32(float %i.bc, float f0x3FEFDD4D, float %i.ck)
  ...
  %i.cr = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cq, <2 x float> <float f0x400337A7, float f0xBE98C2A3>, <2 x float> %i.cn)
  %i.cs = tail call float @llvm.fmuladd.f32(float %i.bd, float f0x40243AE9, float %i.co)
  %i.ct = insertelement <2 x float> poison, float %i.bd, i64 0
  %i.cu = shufflevector <2 x float> %i.ct, <2 x float> poison, <2 x i32> zeroinitializer
  %i.cv = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cu, <2 x float> <float f0x40E118D7, float f0x3F203166>, <2 x float> %i.cr)
  %i.cw = tail call float @llvm.fmuladd.f32(float %i.be, float f0x4082AA5A, float %i.cs)
  %i.cx = insertelement <2 x float> poison, float %i.be, i64 0
  %i.cy = shufflevector <2 x float> %i.cx, <2 x float> poison, <2 x i32> zeroinitializer
  %i.cz = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cy, <2 x float> <float f0xBFB3B903, float f0x400998DE>, <2 x float> %i.cv)
  %i.da = tail call float @llvm.fmuladd.f32(float %i.bf, float f0x40C00672, float %i.cw)
  %i.db = insertelement <2 x float> poison, float %i.bf, i64 0
  %i.dc = shufflevector <2 x float> %i.db, <2 x float> poison, <2 x i32> zeroinitializer
  %i.dd = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.dc, <2 x float> <float f0x40238DA6, float f0x3FF42585>, <2 x float> %i.cz) ; 2 uses
  %i.de = getelementptr inbounds nuw i8, ptr %0, i64 16
  %i.df = load float, ptr %i.de, align 4, !tbaa !12
  %i.dg = fadd float %i.df, %i.da                 ; 3 uses
  %i.dh = fcmp olt float %i.dg, 0.000000e+00
  %i.di = fcmp ogt float %i.dg, 1.000000e+00
  %i.dj = select i1 %i.di, float 1.000000e+00, float %i.dg
  %i.dk = select i1 %i.dh, float 0.000000e+00, float %i.dj
  store float %i.dk, ptr %1, align 4, !tbaa !12
  %i.dl = getelementptr inbounds nuw i8, ptr %0, i64 20
  %i.dm = load float, ptr %i.dl, align 4, !tbaa !12
  %i.dn = extractelement <2 x float> %i.dd, i64 0
  %i.do = fadd float %i.dm, %i.dn                 ; 3 uses
  ...
  store float %i.ds, ptr %2, align 4, !tbaa !12
  %i.dt = getelementptr inbounds nuw i8, ptr %0, i64 24
  %i.du = load float, ptr %i.dt, align 4, !tbaa !12
  %i.dv = extractelement <2 x float> %i.dd, i64 1
  %i.dw = fadd float %i.du, %i.dv                 ; 3 uses
  ...
  store float %i.ea, ptr %3, align 4, !tbaa !12
  ret void
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(argmem: readwrite, errnomem: write) uwtable
define dso_local void @mixbox_latent_to_linear_float_rgb(ptr nofree noundef readonly captures(none) %0, ptr nofree noundef writeonly captures(none) initializes((0, 4)) %1, ptr nofree noundef writeonly captures(none) initializes((0, 4)) %2, ptr nofree noundef writeonly captures(none) initializes((0, 4)) %3) local_unnamed_addr #2 {
bb.a:
  %i.a = getelementptr inbounds nuw i8, ptr %0, i64 4 ; 2 uses
  %i.b = getelementptr inbounds nuw i8, ptr %0, i64 16
  %i.c = load <3 x float>, ptr %i.a, align 4, !tbaa !12 ; 4 uses
