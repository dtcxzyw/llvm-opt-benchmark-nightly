...
begin_hunk_1_@opj_t1_cblk_encode_processor:bb.a
  ...
  %i.jn = icmp slt <4 x i32> %wide.load786, zeroinitializer ; 5 uses
  %i.jo = tail call <4 x i32> @llvm.umax.v4i32(<4 x i32> %wide.load, <4 x i32> splat (i32 -2147483647))
  %i.jp = tail call <4 x i32> @llvm.umax.v4i32(<4 x i32> %wide.load786, <4 x i32> splat (i32 -2147483647))
...
pred.store.continue:                              ; preds = %pred.store.if, %vector.body
  ...
  br i1 %i.jv, label %pred.store.if787, label %pred.store.continue788

pred.store.if787:                                 ; preds = %pred.store.continue
  %i.jw = extractelement <4 x i32> %i.jq, i64 1
  %i.jx = or disjoint i32 %i.jw, -2147483648
  store i32 %i.jx, ptr %next.gep779, align 4
  br label %pred.store.continue788

pred.store.continue788:                           ; preds = %pred.store.if787, %pred.store.continue
  %i.jy = extractelement <4 x i1> %i.jm, i64 2
  br i1 %i.jy, label %pred.store.if789, label %pred.store.continue790

pred.store.if789:                                 ; preds = %pred.store.continue788
  %i.jz = extractelement <4 x i32> %i.jq, i64 2
  %i.ka = or disjoint i32 %i.jz, -2147483648
  store i32 %i.ka, ptr %next.gep780, align 4
...
pred.store.continue800:                           ; preds = %pred.store.if799, %pred.store.continue798
  ...
  %minmaxop803 = select <4 x i1> %i.jn, <4 x i32> %i.jr, <4 x i32> %wide.load786
  %predphi801 = tail call <4 x i32> @llvm.umax.v4i32(<4 x i32> %minmaxop803, <4 x i32> %vec.phi778) ; 2 uses
  %index.next = add nuw i64 %index, 8             ; 2 uses
  %i.kq = icmp eq i64 %index.next, %n.vec
  br i1 %i.kq, label %middle.block, label %vector.body, !llvm.loop !233

middle.block:                                     ; preds = %pred.store.continue800
  %rdx.minmax = tail call <4 x i32> @llvm.umax.v4i32(<4 x i32> %predphi, <4 x i32> %predphi801)
  %i.kr = tail call i32 @llvm.vector.reduce.umax.v4i32(<4 x i32> %rdx.minmax) ; 2 uses
  %cmp.n = icmp eq i64 %n.vec, %i.iz
  br i1 %cmp.n, label %._crit_edge.loopexit.i, label %.lr.ph.i.preheader898

.lr.ph.i.preheader898:                            ; preds = %.lr.ph.i.preheader, %middle.block
...
.lr.ph.i:                                         ; preds = %.lr.ph.i.preheader898, %bb.s
  %.1138389.i = phi ptr [ %i.la, %bb.s ], [ %.1138389.i.ph, %.lr.ph.i.preheader898 ] ; 3 uses
  %.1140388.i = phi i32 [ %.2141.i, %bb.s ], [ %.1140388.i.ph, %.lr.ph.i.preheader898 ] ; 2 uses
  %.0143387.i = phi i32 [ %i.kz, %bb.s ], [ %.0143387.i.ph, %.lr.ph.i.preheader898 ]
...
._crit_edge.loopexit.i:                           ; preds = %bb.s, %middle.block
  ...
  %.lcssa771 = phi ptr [ %i.jb, %middle.block ], [ %i.la, %bb.s ]
...
._crit_edge.i:                                    ; preds = %._crit_edge.loopexit.i, %.lr.ph395.split.i
  ...
  %.1138.lcssa.i = phi ptr [ %.0137393.i, %.lr.ph395.split.i ], [ %.lcssa771, %._crit_edge.loopexit.i ]
