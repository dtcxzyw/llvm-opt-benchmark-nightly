begin_hunk_0
  ...
  %i.ie = call ptr @ft_mem_qalloc(ptr noundef %i.fc, i64 noundef %.282.i, ptr noundef nonnull %i.a) #20 ; 47 uses
...
vector.body:                                      ; preds = %pred.store.continue523, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %pred.store.continue523 ] ; 34 uses
  %i.ih = getelementptr inbounds nuw i8, ptr %i.ie, i64 %index ; 2 uses
...
pred.store.if:                                    ; preds = %vector.body
  %8 = getelementptr inbounds nuw i8, ptr %i.ie, i64 %index
  store i8 45, ptr %8, align 1, !tbaa !41
...
pred.store.continue463:                           ; preds = %pred.store.if462, %pred.store.continue
...
pred.store.continue483:                           ; preds = %pred.store.if482, %pred.store.continue481
...
vec.epilog.vector.body:                           ; preds = %pred.store.continue543, %vec.epilog.ph
  %index526 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next544, %pred.store.continue543 ] ; 10 uses
  %i.mc = getelementptr inbounds nuw i8, ptr %i.ie, i64 %index526
...
pred.store.if528:                                 ; preds = %vec.epilog.vector.body
  %9 = getelementptr inbounds nuw i8, ptr %i.ie, i64 %index526
  store i8 45, ptr %9, align 1, !tbaa !41
  br label %pred.store.continue529

pred.store.continue529:                           ; preds = %pred.store.if528, %vec.epilog.vector.body
...
bb.cj:                                            ; preds = %bb.ci, %bb.ch
  %.179.3.i = phi ptr [ %i.nn, %bb.ci ], [ %.2.2.i, %bb.ch ] ; 45 uses
...
vector.body554:                                   ; preds = %pred.store.continue621, %vector.ph551
  %index555 = phi i64 [ 0, %vector.ph551 ], [ %index.next622, %pred.store.continue621 ] ; 34 uses
  %i.no = getelementptr inbounds nuw i8, ptr %.179.3.i, i64 %index555 ; 2 uses
...
pred.store.if558:                                 ; preds = %vector.body554
  %10 = getelementptr inbounds nuw i8, ptr %.179.3.i, i64 %index555
  store i8 45, ptr %10, align 1, !tbaa !41
  br label %pred.store.continue559

pred.store.continue559:                           ; preds = %pred.store.if558, %vector.body554
...
pred.store.continue585:                           ; preds = %pred.store.if584, %pred.store.continue583
...
pred.store.continue587:                           ; preds = %pred.store.if586, %pred.store.continue585
...
pred.store.continue589:                           ; preds = %pred.store.if588, %pred.store.continue587
...
pred.store.continue591:                           ; preds = %pred.store.if590, %pred.store.continue589
...
pred.store.continue593:                           ; preds = %pred.store.if592, %pred.store.continue591
...
pred.store.continue595:                           ; preds = %pred.store.if594, %pred.store.continue593
...
vec.epilog.vector.body632:                        ; preds = %pred.store.continue650, %vec.epilog.ph629
  %index633 = phi i64 [ %vec.epilog.resume.val625, %vec.epilog.ph629 ], [ %index.next651, %pred.store.continue650 ] ; 10 uses
  %i.rj = getelementptr inbounds nuw i8, ptr %.179.3.i, i64 %index633
...
pred.store.if635:                                 ; preds = %vec.epilog.vector.body632
  %11 = getelementptr inbounds nuw i8, ptr %.179.3.i, i64 %index633
  store i8 45, ptr %11, align 1, !tbaa !41
  br label %pred.store.continue636

pred.store.continue636:                           ; preds = %pred.store.if635, %vec.epilog.vector.body632
