...
bb.e:                                             ; preds = %bb.c
  ...
  %2 = load <2 x float>, ptr %1, align 4, !tbaa !77 ; 3 uses
  %i.ce = load float, ptr %i.cd, align 4, !tbaa !77
  %3 = fneg float %i.ce                           ; 2 uses
  %4 = load <2 x float>, ptr %i.d, align 4, !tbaa !77 ; 4 uses
  %5 = extractelement <2 x float> %4, i64 0       ; 2 uses
  %6 = fneg float %5
  %i.cf = fmul float %5, %3
  %i.cg = insertelement <2 x float> %2, float 0.000000e+00, i64 1
  %7 = shufflevector <2 x float> <float poison, float -0.000000e+00>, <2 x float> %4, <2 x i32> <i32 3, i32 1>
  %i.ch = insertelement <2 x float> <float poison, float 1.000000e+00>, float %i.cf, i64 0
  %i.ci = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cg, <2 x float> %7, <2 x float> %i.ch) ; 2 uses
...
.preheader67:                                     ; preds = %bb.e
  ...
  %8 = shufflevector <2 x float> %4, <2 x float> %2, <4 x i32> <i32 1, i32 3, i32 0, i32 2>
  %i.cq = tail call <4 x float> @llvm.fabs.v4f32(<4 x float> %8)
...
.critedge63:                                      ; preds = %.preheader67, %bb.e
  %9 = shufflevector <2 x float> %4, <2 x float> poison, <4 x i32> <i32 1, i32 poison, i32 poison, i32 poison>
  %i.cx = insertelement <4 x float> %9, float 0.000000e+00, i64 2
  %10 = insertelement <4 x float> %i.cx, float %3, i64 1
  %11 = insertelement <4 x float> %10, float %6, i64 3
  %i.cy = fdiv <4 x float> %11, %i.cj             ; 3 uses
  ...
  %.sroa.22.0 = fdiv float %12, %i.ck             ; 2 uses
