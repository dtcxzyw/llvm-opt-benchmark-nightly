...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %i.bbo = call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %broadcast.splat3341, <2 x double> %i.bbn, <2 x double> splat (double 5.000000e-01))
  %i.bbp = fptrunc <2 x double> %i.bbo to <2 x float> ; 2 uses
  %i.bbq = fcmp ogt <2 x float> %i.bbp, zeroinitializer
...
bb.ox:                                            ; preds = %bb.ow, %bb.ov
  ...
  %i.bgj = call double @llvm.fmuladd.f64(double %i.awy, double %i.bgi, double 5.000000e-01)
  %i.bgk = fptrunc double %i.bgj to float         ; 2 uses
  %i.bgl = fcmp ogt float %i.bgk, 0.000000e+00
