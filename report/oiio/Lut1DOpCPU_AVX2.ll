...
vector.ph130:                                     ; preds = %vector.main.loop.iter.check
  ...
  %n.vec132 = and i64 %i.hq, 1073741808           ; 4 uses
  %i.hs = shl nuw nsw i64 %n.vec132, 2            ; 2 uses
  %i.ht = getelementptr i8, ptr %.050.lcssa, i64 %i.hs
  br label %vector.body133

vector.body133:                                   ; preds = %vector.body133, %vector.ph130
  %index134 = phi i64 [ 0, %vector.ph130 ], [ %index.next144, %vector.body133 ] ; 2 uses
  %offset.idx136 = shl i64 %index134, 2           ; 2 uses
  %next.gep137 = getelementptr i8, ptr %.050.lcssa, i64 %offset.idx136
  %i.hu = getelementptr inbounds nuw i8, ptr %i.b, i64 %offset.idx136
...
vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  ...
  %n.vec150 = and i64 %i.hq, 1073741820           ; 3 uses
  %i.hw = shl nuw nsw i64 %n.vec150, 2            ; 2 uses
  %i.hx = getelementptr i8, ptr %.050.lcssa, i64 %i.hw
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index151 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next159, %vec.epilog.vector.body ] ; 2 uses
  %i.hy = shl i64 %index151, 2                    ; 2 uses
  %next.gep152 = getelementptr i8, ptr %.050.lcssa, i64 %i.hy
  %i.hz = getelementptr inbounds nuw i8, ptr %i.b, i64 %i.hy
...
.lr.ph103.preheader:                              ; preds = %vector.memcheck119, %iter.check, %vec.epilog.iter.check, %vec.epilog.middle.block
  %indvars.iv107.ph = phi i64 [ 0, %iter.check ], [ 0, %vector.memcheck119 ], [ %i.hs, %vec.epilog.iter.check ], [ %i.hw, %vec.epilog.middle.block ]
