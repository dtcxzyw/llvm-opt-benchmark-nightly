...
_ZN4pstd5arrayIN4pbrt6Point3IfEELi3EEC2ESt16initializer_listIS3_E.exit:
  ...
  %5 = load i32, ptr @_ZN4pbrt6PrimesE, align 16, !tbaa !9 ; 2 uses
  %i.d = zext i32 %5 to i64                       ; 5 uses
  %i.e = udiv i64 -1, %i.d
  %i.f = sub nuw i64 %i.e, %i.d
  %6 = uitofp i32 %5 to float
  %7 = fdiv nnan float 1.000000e+00, %6
  %8 = load i32, ptr getelementptr inbounds nuw (i8, ptr @_ZN4pbrt6PrimesE, i64 4), align 4, !tbaa !9 ; 2 uses
  %9 = zext i32 %8 to i64                         ; 5 uses
  %10 = udiv i64 -1, %9
  %11 = sub nuw i64 %10, %9
  %12 = uitofp i32 %8 to float
  %13 = fdiv nnan float 1.000000e+00, %12
  br label %bb.b
...
.lr.ph.i257:                                      ; preds = %bb.b, %.lr.ph.i257
  ...
  %i.m = udiv i64 %.023.i, %i.d                   ; 2 uses
  %reass.add.i = sub i64 %.01922.i, %i.m
  %reass.mul.i = mul i64 %reass.add.i, %i.d
  %i.n = add i64 %reass.mul.i, %.023.i            ; 3 uses
  %i.o = fmul float %7, %.02021.i                 ; 2 uses
  %i.p = icmp samesign uge i64 %.023.i, %i.d
  %i.q = icmp ult i64 %i.n, %i.f
...
.lr.ph.i259:                                      ; preds = %.lr.ph.i259.preheader, %.lr.ph.i259
  ...
  %i.v = udiv i64 %.023.i260, %9                  ; 2 uses
  %reass.add.i263 = sub i64 %.01922.i261, %i.v
  %reass.mul.i264 = mul i64 %reass.add.i263, %9
  %i.w = add i64 %reass.mul.i264, %.023.i260      ; 3 uses
  %i.x = fmul float %13, %.02021.i262             ; 2 uses
  %i.y = icmp samesign uge i64 %.023.i260, %9
  %i.z = icmp ult i64 %i.w, %11
...
_ZN4pbrt14RadicalInverseEim.exit267:              ; preds = %bb.b, %._crit_edge.loopexit.i265
  ...
  %foldExtExtBinop404 = fmul <2 x float> %i.as, %shift
  %i.au = extractelement <2 x float> %foldExtExtBinop404, i64 0
  ...
  %14 = insertelement <2 x float> poison, float %.0.i, i64 0
  %15 = shufflevector <2 x float> %14, <2 x float> poison, <2 x i32> zeroinitializer
  %16 = fmul <2 x float> %.sroa.0123.0.copyload, %15
  %i.bg = fmul float %.sroa.2124.0.copyload, %.0.i
  %i.bh = insertelement <2 x float> poison, float %.08.i, i64 0
  %i.bi = shufflevector <2 x float> %i.bh, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bj = fmul <2 x float> %i.bi, %.sroa.0113.0.copyload
  %17 = fmul float %.08.i, %.sroa.2114.0.copyload
  %18 = fadd <2 x float> %16, %i.bj
  %19 = fadd float %i.bg, %17
  ...
  %20 = fmul float %i.bf, %.sroa.298.0.copyload
  %i.bn = fadd <2 x float> %18, %i.bm             ; 4 uses
  %i.bo = fadd float %19, %20                     ; 3 uses
  %shift407 = shufflevector <2 x float> %i.bn, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop408 = fmul <2 x float> %i.bn, %shift407
  %21 = extractelement <2 x float> %foldExtExtBinop408, i64 0
  %i.bp = fmul float %i.bo, %21
  %i.bq = fadd float %i.bo, f0xBF333333           ; 3 uses
  %i.br = fmul float %i.bq, %i.bq
  %i.bs = fadd <2 x float> %i.bn, <float -5.000000e-01, float 4.000000e-01> ; 3 uses
  %i.bt = fmul <2 x float> %i.bs, %i.bs           ; 2 uses
  %shift410 = shufflevector <2 x float> %i.bt, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop411 = fadd <2 x float> %i.bt, %shift410
  %i.bu = extractelement <2 x float> %foldExtExtBinop411, i64 0
  %i.bv = fadd float %i.br, %i.bu
  %sqrt.i.i340 = call noundef float @llvm.sqrt.f32(float %i.bv) ; 2 uses
  %22 = insertelement <2 x float> poison, float %sqrt.i.i340, i64 0
  %23 = shufflevector <2 x float> %i.bs, <2 x float> poison, <2 x i32> <i32 1, i32 0>
  %i.bw = shufflevector <2 x float> %22, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bx = fdiv <2 x float> %23, %i.bw
  %24 = fdiv float %i.bq, %sqrt.i.i340
  %i.by = fmul <2 x float> %i.bx, <float f0x3F1FD749, float f0x3E49E785> ; 2 uses
  %shift413 = shufflevector <2 x float> %i.by, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop414 = fadd <2 x float> %shift413, %i.by
  %i.bz = extractelement <2 x float> %foldExtExtBinop414, i64 0
  %25 = fmul float %24, f0x3F417DE0
  %i.ca = fadd float %25, %i.bz
  %i.cb = call noundef float @llvm.fabs.f32(float %i.ca)
  %i.cc = fmul float %i.bp, %i.cb
  %26 = fsub <2 x float> <float 5.000000e-01, float -4.000000e-01>, %i.bn ; 2 uses
  %27 = fsub float f0x3F333333, %i.bo             ; 2 uses
  %i.cd = fmul <2 x float> %26, %26               ; 2 uses
  %shift406 = shufflevector <2 x float> %i.cd, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop407 = fadd <2 x float> %i.cd, %shift406
  %i.ce = extractelement <2 x float> %foldExtExtBinop407, i64 0
  %i.cf = fmul float %27, %27
  %i.cg = fadd float %i.cf, %i.ce
  %i.ch = fmul float %i.cg, f0x3C337876
