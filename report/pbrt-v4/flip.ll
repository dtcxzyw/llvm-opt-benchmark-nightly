...
.lr.ph:                                           ; preds = %_ZN11flip_detail5imageC2ERKS0_.exit, %bb.l
  ...
  %3 = fcmp olt float %.sroa.speculated.i, 0.000000e+00
  %4 = shufflevector <2 x i1> %i.be, <2 x i1> poison, <3 x i32> <i32 poison, i32 1, i32 0>
  %5 = insertelement <3 x i1> %4, i1 %3, i64 0
  %6 = shufflevector <2 x float> %i.bb, <2 x float> poison, <3 x i32> <i32 poison, i32 1, i32 0>
  %7 = insertelement <3 x float> %6, float %.sroa.speculated.i, i64 0
  %8 = select <3 x i1> %5, <3 x float> zeroinitializer, <3 x float> %7 ; 6 uses
  %9 = extractelement <3 x float> %8, i64 2
  %i.bf = fmul float %9, f0x3F37163B
  %10 = extractelement <3 x float> %8, i64 1
  %i.bg = tail call float @llvm.fmuladd.f32(float %10, float f0x3E59BD78, float %i.bf)
  %11 = extractelement <3 x float> %8, i64 0
  %12 = tail call float @llvm.fmuladd.f32(float %11, float 7.218020e-02, float %i.bg)
  %13 = shufflevector <3 x float> %8, <3 x float> poison, <2 x i32> <i32 2, i32 2>
  %14 = fmul <2 x float> %13, <float f0x3EB7163B, float f0x3DF41DA4>
  %15 = shufflevector <3 x float> %8, <3 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.bh = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %15, <2 x float> <float f0x3ED32455, float f0x3C9E5B3F>, <2 x float> %14)
  %i.bi = shufflevector <3 x float> %8, <3 x float> poison, <2 x i32> zeroinitializer
  %i.bj = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bi, <2 x float> <float f0x3E38C803, float f0x3F734B9E>, <2 x float> %i.bh)
  %i.bk = tail call float @llvm.fabs.f32(float %12) ; 3 uses
