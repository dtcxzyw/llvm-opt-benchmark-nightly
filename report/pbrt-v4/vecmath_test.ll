...
bb.a:                                             ; preds = %_ZN7testing15AssertionResultD2Ev.exit189, %_ZN7testing15AssertionResultD2Ev.exit228
  ...
  %i.az = fmul float %sqrt.i.i191, %i.ah          ; 3 uses
  %i.ba = fmul float %sqrt.i.i191, %i.ag          ; 3 uses
  ...
  %i.bi = fmul float %i.bg, %i.bf
  %i.bj = fmul float %i.aw, %i.bh                 ; 2 uses
  %i.bk = fmul float %i.ay, %i.bi                 ; 2 uses
  %i.bl = fadd float %i.bj, %i.bk
  %i.bm = fsub float %i.bk, %i.bj
  %i.bn = insertelement <2 x float> poison, float %i.bg, i64 0
  %i.bo = insertelement <2 x float> %i.bn, float %i.bf, i64 1 ; 2 uses
  ...
  %9 = fsub float %i.bx, %i.bz                    ; 2 uses
  %10 = fadd float %i.ca, %i.by                   ; 2 uses
  %11 = fsub float %i.by, %i.ca                   ; 2 uses
  %12 = fadd float %i.bz, %i.bx                   ; 2 uses
  %13 = fmul float %i.bh, %i.bh                   ; 2 uses
  %14 = fsub float 1.000000e+00, %13
  %15 = fmul float %i.ax, %14
  %16 = fadd float %13, %15                       ; 2 uses
  %17 = insertelement <2 x float> poison, float %10, i64 0
  %18 = insertelement <2 x float> %17, float %9, i64 1
  %i.cb = fmul <2 x float> %18, zeroinitializer   ; 2 uses
  %19 = fmul float %12, 0.000000e+00              ; 2 uses
  %i.cc = fadd float %11, %19
  %20 = fmul float %16, 0.000000e+00              ; 2 uses
  %i.cd = fadd float %i.cc, %20                   ; 3 uses
  %21 = fmul <2 x float> %i.bu, zeroinitializer   ; 3 uses
  %22 = insertelement <2 x float> poison, float %i.bm, i64 0
  %23 = insertelement <2 x float> %22, float %i.bl, i64 1 ; 2 uses
  %24 = fadd <2 x float> %23, %21
  %25 = fadd <2 x float> %i.cb, %24               ; 3 uses
  %i.ce = fmul <2 x float> %23, zeroinitializer   ; 3 uses
  %i.cf = fadd <2 x float> %i.bu, %i.ce
  %i.cg = fadd <2 x float> %i.cb, %i.cf           ; 3 uses
  %i.ch = fmul float %11, 0.000000e+00            ; 2 uses
  %26 = fadd float %12, %i.ch
  %i.ci = fadd float %26, %20                     ; 5 uses
  %foldExtExtBinop = fadd <2 x float> %i.ce, %21
  %27 = extractelement <2 x float> %foldExtExtBinop, i64 0
  %i.cj = fadd float %10, %27                     ; 5 uses
  %i.ck = fadd <2 x float> %i.ce, %21
  %28 = extractelement <2 x float> %i.ck, i64 1
  %i.cl = fadd float %9, %28                      ; 5 uses
  %i.cm = fadd float %i.ch, %19
  %29 = fadd float %16, %i.cm                     ; 5 uses
  %i.cn = fmul float %i.ci, %i.cl                 ; 2 uses
  %i.co = fneg float %i.cn
  %i.cp = extractelement <2 x float> %i.cg, i64 1 ; 4 uses
  %i.cq = call noundef float @llvm.fma.f32(float %i.cp, float %29, float %i.co)
  %i.cr = fneg float %i.ci
  %i.cs = call noundef float @llvm.fma.f32(float %i.cr, float %i.cl, float %i.cn)
  %i.ct = fadd float %i.cq, %i.cs
  %30 = extractelement <2 x float> %25, i64 0     ; 4 uses
  %31 = fmul float %29, %30                       ; 2 uses
  %32 = fneg float %31
  %i.cu = call noundef float @llvm.fma.f32(float %i.ci, float %i.cj, float %32)
  %i.cv = fneg float %30
  %i.cw = call noundef float @llvm.fma.f32(float %i.cv, float %29, float %31)
  %i.cx = fadd float %i.cu, %i.cw
  %i.cy = fmul float %i.cp, %i.cj                 ; 2 uses
  %i.cz = fneg float %i.cy
  %i.da = call noundef float @llvm.fma.f32(float %30, float %i.cl, float %i.cz)
  %i.db = fneg float %i.cp
  %i.dc = call noundef float @llvm.fma.f32(float %i.db, float %i.cj, float %i.cy)
  %i.dd = fadd float %i.da, %i.dc
  %i.de = extractelement <2 x float> %i.cg, i64 0 ; 2 uses
  %i.df = fmul float %i.de, %i.ct
  %33 = extractelement <2 x float> %25, i64 1     ; 2 uses
  %i.dg = fmul float %33, %i.cx
  %i.dh = fadd float %i.df, %i.dg
  %i.di = fmul float %i.cd, %i.dd
  %i.dj = fadd float %i.di, %i.dh
  %34 = fmul <2 x float> %i.cg, %25               ; 2 uses
  %shift = shufflevector <2 x float> %34, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop486 = fadd <2 x float> %34, %shift
  %35 = extractelement <2 x float> %foldExtExtBinop486, i64 0
  %36 = fmul float %i.cd, %i.ci
  %37 = fadd float %36, %35
  %38 = fadd float %37, 1.000000e+00
  %39 = fmul float %i.de, %i.cj
  %40 = fmul float %33, %i.cl
  %41 = fadd float %39, %40
  %42 = fmul float %i.cd, %29
  %43 = fadd float %42, %41
  %44 = fadd float %43, %38
  %45 = fmul float %30, %i.cj
  %46 = fmul float %i.cp, %i.cl
  %47 = fadd float %45, %46
  %48 = fmul float %i.ci, %29
  %49 = fadd float %48, %47
  %i.dk = fadd float %49, %44
  %i.dl = call noundef float @atan2f(float noundef %i.dj, float noundef %i.dk) #25
