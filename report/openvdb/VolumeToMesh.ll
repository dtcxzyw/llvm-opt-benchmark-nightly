...
.critedge:                                        ; preds = %bb.d, %bb.f, %bb.e, %bb.g
  ...
  %i.ar = getelementptr inbounds nuw [12 x i8], ptr %i.aq, i64 %i.y ; 2 uses
  %i.as = getelementptr inbounds nuw [12 x i8], ptr %i.aq, i64 %i.ad ; 2 uses
  %i.at = getelementptr inbounds nuw i8, ptr %i.v, i64 8
  %i.au = load i32, ptr %i.at, align 4, !tbaa !381
  %i.av = zext i32 %i.au to i64
  %i.aw = getelementptr inbounds nuw [12 x i8], ptr %i.aq, i64 %i.av ; 2 uses
  %i.ax = getelementptr inbounds nuw i8, ptr %i.v, i64 12
  %i.ay = load i32, ptr %i.ax, align 4, !tbaa !381
  %i.az = zext i32 %i.ay to i64
  %i.ba = getelementptr inbounds nuw [12 x i8], ptr %i.aq, i64 %i.az ; 2 uses
  ...
  %i.be = load float, ptr %i.as, align 4, !tbaa !761
  %i.bf = fpext float %i.be to double             ; 3 uses
  %i.bg = getelementptr inbounds nuw i8, ptr %i.as, i64 4
  %i.bh = load float, ptr %i.aw, align 4, !tbaa !761
  %i.bi = fpext float %i.bh to double             ; 3 uses
  %2 = getelementptr inbounds nuw i8, ptr %i.aw, i64 4
  %i.bj = load float, ptr %i.ba, align 4, !tbaa !761
  %i.bk = fpext float %i.bj to double             ; 3 uses
  %i.bl = getelementptr inbounds nuw i8, ptr %i.ba, i64 4
  %3 = fsub double %i.bi, %i.bc                   ; 2 uses
  %4 = fsub double %i.bf, %i.bk                   ; 2 uses
  %5 = fneg double %4
  %6 = fadd double %i.bc, %i.bf
  %7 = fadd double %6, %i.bi
  %8 = fadd double %7, %i.bk
  %9 = load <2 x float>, ptr %i.bd, align 4, !tbaa !761
  %10 = fpext <2 x float> %9 to <2 x double>      ; 4 uses
  %11 = load <2 x float>, ptr %i.bg, align 4, !tbaa !761
  %12 = fpext <2 x float> %11 to <2 x double>     ; 4 uses
  %13 = load <2 x float>, ptr %2, align 4, !tbaa !761
  %14 = fpext <2 x float> %13 to <2 x double>     ; 4 uses
  %15 = load <2 x float>, ptr %i.bl, align 4, !tbaa !761
  %16 = fpext <2 x float> %15 to <2 x double>     ; 4 uses
  %17 = extractelement <2 x double> %10, i64 0
  %18 = extractelement <2 x double> %14, i64 0
  %foldExtExtBinop = fsub <2 x double> %14, %10
  %19 = extractelement <2 x double> %foldExtExtBinop, i64 0 ; 2 uses
  %20 = extractelement <2 x double> %10, i64 1    ; 2 uses
  %21 = extractelement <2 x double> %14, i64 1    ; 2 uses
  %22 = fsub double %21, %20                      ; 2 uses
  %23 = extractelement <2 x double> %16, i64 0
  %foldExtExtBinop65 = fsub <2 x double> %12, %16
  %24 = extractelement <2 x double> %foldExtExtBinop65, i64 0 ; 2 uses
  %25 = extractelement <2 x double> %12, i64 1    ; 2 uses
  %26 = extractelement <2 x double> %16, i64 1    ; 2 uses
  %i.bm = fsub double %25, %26                    ; 2 uses
  %i.bn = fneg double %24
  %i.bo = fmul double %22, %i.bn
  %i.bp = tail call double @llvm.fmuladd.f64(double %19, double %i.bm, double %i.bo) ; 4 uses
  %i.bq = fneg double %i.bm
  %i.br = fmul double %3, %i.bq
  %i.bs = tail call double @llvm.fmuladd.f64(double %22, double %4, double %i.br) ; 4 uses
  %i.bt = fmul double %19, %5
  %i.bu = tail call double @llvm.fmuladd.f64(double %3, double %24, double %i.bt) ; 4 uses
  ...
  %27 = fadd <2 x double> %10, %12
  %28 = fadd <2 x double> %27, %14
  %29 = fadd <2 x double> %28, %16                ; 2 uses
  %30 = extractelement <2 x double> %29, i64 0
  %31 = fmul double %30, %.sroa.1056.0.i
  %32 = tail call double @llvm.fmuladd.f64(double %8, double %.sroa.050.0.i, double %31)
  %33 = extractelement <2 x double> %29, i64 1
  %i.ce = tail call noundef double @llvm.fmuladd.f64(double %33, double %.sroa.18.0.i, double %32)
  %i.cf = fmul double %i.ce, 2.500000e-01         ; 4 uses
  %i.cg = fmul double %.sroa.1056.0.i, %17
  %i.ch = tail call double @llvm.fmuladd.f64(double %i.bc, double %.sroa.050.0.i, double %i.cg)
  %i.ci = tail call noundef double @llvm.fmuladd.f64(double %20, double %.sroa.18.0.i, double %i.ch)
...
bb.h:                                             ; preds = %.critedge
  %34 = extractelement <2 x double> %12, i64 0
  %i.cm = fmul double %.sroa.1056.0.i, %34
  %i.cn = tail call double @llvm.fmuladd.f64(double %i.bf, double %.sroa.050.0.i, double %i.cm)
  %i.co = tail call noundef double @llvm.fmuladd.f64(double %25, double %.sroa.18.0.i, double %i.cn)
...
bb.i:                                             ; preds = %bb.h
  %i.cs = fmul double %.sroa.1056.0.i, %18
  %i.ct = tail call double @llvm.fmuladd.f64(double %i.bi, double %.sroa.050.0.i, double %i.cs)
  %i.cu = tail call noundef double @llvm.fmuladd.f64(double %21, double %.sroa.18.0.i, double %i.ct)
...
_ZN7openvdb5v13_05tools23volume_to_mesh_internal12isPlanarQuadERKNS0_4math4Vec3IdEES7_S7_S7_d.exit: ; preds = %bb.i
  %i.cy = fmul double %.sroa.1056.0.i, %23
  %i.cz = tail call double @llvm.fmuladd.f64(double %i.bk, double %.sroa.050.0.i, double %i.cy)
  %i.da = tail call noundef double @llvm.fmuladd.f64(double %26, double %.sroa.18.0.i, double %i.cz)
