...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %i.ca = add <2 x i32> %broadcast.splat, %vec.ind191
  %interleaved.vec192 = shufflevector <2 x i32> %i.by, <2 x i32> %i.ca, <4 x i32> <i32 0, i32 2, i32 1, i32 3>
  store <4 x i32> %interleaved.vec192, ptr %i.bx, align 4, !tbaa !35
  %i.cb = shufflevector <2 x i64> %i.bq, <2 x i64> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
  %i.cc = trunc nuw <4 x i64> %i.cb to <4 x i32>
  %interleaved.vec193 = shufflevector <4 x i32> %i.cc, <4 x i32> poison, <4 x i32> <i32 0, i32 2, i32 1, i32 3>
  ...
  br i1 %i.cd, label %middle.block, label %vector.body, !llvm.loop !25
...
scalar.ph:                                        ; preds = %scalar.ph.preheader, %scalar.ph
  ...
  %.174.us = phi i32 [ %i.dc, %scalar.ph ], [ %.174.us.ph, %scalar.ph.preheader ] ; 2 uses
  %i.ce = trunc nuw nsw i64 %indvars.iv to i32
  %i.cf = uitofp nneg i32 %i.ce to double
  %i.cg = fdiv double %i.cf, %i.l
  %i.ch = mul nuw nsw i64 %indvars.iv, %i.r       ; 2 uses
  %i.ci = add nuw nsw i64 %i.ch, %indvars.iv85    ; 2 uses
  %i.cj = getelementptr [8 x i8], ptr %i.p, i64 %i.ci ; 3 uses
  store double %i.aq, ptr %i.cj, align 8, !tbaa !30
  %i.ck = getelementptr [8 x i8], ptr %i.cj, i64 %i.q
  store double %i.ar, ptr %i.ck, align 8, !tbaa !30
  %i.cl = getelementptr i8, ptr %i.cj, i64 %.idx.us
  store double %i.cg, ptr %i.cl, align 8, !tbaa !30
  %i.cm = add nsw i64 %indvars.iv, -1
  %i.cn = mul nuw nsw i64 %i.cm, %i.s             ; 2 uses
  %i.co = add nsw i64 %indvars.iv85, %i.cn
  %i.cp = sext i32 %.174.us to i64
  %i.cq = getelementptr [4 x i8], ptr %i.av, i64 %i.cp ; 4 uses
  %i.cr = trunc nsw i64 %i.co to i32
  store i32 %i.cr, ptr %i.cq, align 4, !tbaa !35
  %i.cs = getelementptr [4 x i8], ptr %i.cq, i64 %i.ay
  %i.ct = trunc i64 %i.cn to i32
  %i.cu = add i32 %iv.rem, %i.ct                  ; 2 uses
  store i32 %i.cu, ptr %i.cs, align 4, !tbaa !35
  %i.cv = getelementptr [4 x i8], ptr %i.cq, i64 %i.az
  %i.cw = trunc nuw i64 %i.ci to i32              ; 2 uses
  store i32 %i.cw, ptr %i.cv, align 4, !tbaa !35
  %i.cx = getelementptr i8, ptr %i.cq, i64 4      ; 3 uses
  store i32 %i.cu, ptr %i.cx, align 4, !tbaa !35
  %i.cy = getelementptr [4 x i8], ptr %i.cx, i64 %i.ay
  %i.cz = trunc i64 %i.ch to i32
  %i.da = add i32 %iv.rem, %i.cz
  store i32 %i.da, ptr %i.cy, align 4, !tbaa !35
  %i.db = getelementptr [4 x i8], ptr %i.cx, i64 %i.az
  store i32 %i.cw, ptr %i.db, align 4, !tbaa !35
  %i.dc = add nsw i32 %.174.us, 2                 ; 2 uses
...
._crit_edge.us:                                   ; preds = %scalar.ph, %middle.block, %bb.a
  %.2.us.lcssa = phi i32 [ %.07276.us, %bb.a ], [ %i.bh, %middle.block ], [ %i.dc, %scalar.ph ]
  %exitcond89.not = icmp eq i64 %indvars.iv.next86, %i.r
  br i1 %exitcond89.not, label %._crit_edge79, label %bb.a, !llvm.loop !27

._crit_edge79.loopexit197.unr-lcssa:              ; preds = %.lr.ph78.split
  %lcmp.mod.not = icmp eq i32 %xtraiter, 0
  br i1 %lcmp.mod.not, label %._crit_edge79, label %.lr.ph78.split.epil.preheader

.lr.ph78.split.epil.preheader:                    ; preds = %._crit_edge79.loopexit197.unr-lcssa, %.lr.ph78.split.preheader
  %.07177.epil.init = phi i32 [ 0, %.lr.ph78.split.preheader ], [ %i.eg, %._crit_edge79.loopexit197.unr-lcssa ]
  %lcmp.mod198 = icmp ne i32 %xtraiter, 0
...
.lr.ph78.split.epil:                              ; preds = %.lr.ph78.split.epil, %.lr.ph78.split.epil.preheader
  %.07177.epil = phi i32 [ %i.di, %.lr.ph78.split.epil ], [ %.07177.epil.init, %.lr.ph78.split.epil.preheader ] ; 2 uses
  %epil.iter = phi i32 [ %epil.iter.next, %.lr.ph78.split.epil ], [ 0, %.lr.ph78.split.epil.preheader ]
  %i.dd = uitofp nneg i32 %.07177.epil to double
  %i.de = fmul nnan double %i.dd, f0x401921FB54442D18
  %i.df = fdiv double %i.de, %i.j                 ; 2 uses
  %i.dg = tail call double @cos(double noundef %i.df) #7 ; 0 uses
  %i.dh = tail call double @sin(double noundef %i.df) #7 ; 0 uses
  %i.di = add nuw nsw i32 %.07177.epil, 1
  %epil.iter.next = add i32 %epil.iter, 1         ; 2 uses
  %epil.iter.cmp.not = icmp eq i32 %epil.iter.next, %xtraiter
  br i1 %epil.iter.cmp.not, label %._crit_edge79, label %.lr.ph78.split.epil, !llvm.loop !28

._crit_edge79:                                    ; preds = %._crit_edge79.loopexit197.unr-lcssa, %.lr.ph78.split.epil, %._crit_edge.us, %_ZN5Eigen15PlainObjectBaseINS_6MatrixIdLin1ELin1ELi0ELin1ELin1EEEE6resizeEll.exit
  ret void

.lr.ph78.split:                                   ; preds = %.lr.ph78.split, %.lr.ph78.split.preheader.new
  %.07177 = phi i32 [ 0, %.lr.ph78.split.preheader.new ], [ %i.eg, %.lr.ph78.split ] ; 5 uses
  %niter = phi i32 [ 0, %.lr.ph78.split.preheader.new ], [ %niter.next.3, %.lr.ph78.split ]
  %i.dj = uitofp nneg i32 %.07177 to double
  %i.dk = fmul nnan double %i.dj, f0x401921FB54442D18
  %i.dl = fdiv double %i.dk, %i.j                 ; 2 uses
  %i.dm = tail call double @cos(double noundef %i.dl) #7 ; 0 uses
  %i.dn = tail call double @sin(double noundef %i.dl) #7 ; 0 uses
  %i.do = or disjoint i32 %.07177, 1
  %i.dp = uitofp nneg i32 %i.do to double
  %i.dq = fmul nnan double %i.dp, f0x401921FB54442D18
  %i.dr = fdiv double %i.dq, %i.j                 ; 2 uses
  %i.ds = tail call double @cos(double noundef %i.dr) #7 ; 0 uses
  %i.dt = tail call double @sin(double noundef %i.dr) #7 ; 0 uses
  %i.du = or disjoint i32 %.07177, 2
  %i.dv = uitofp nneg i32 %i.du to double
  %i.dw = fmul nnan double %i.dv, f0x401921FB54442D18
  %i.dx = fdiv double %i.dw, %i.j                 ; 2 uses
  %i.dy = tail call double @cos(double noundef %i.dx) #7 ; 0 uses
  %i.dz = tail call double @sin(double noundef %i.dx) #7 ; 0 uses
  %i.ea = or disjoint i32 %.07177, 3
  %i.eb = uitofp nneg i32 %i.ea to double
  %i.ec = fmul nnan double %i.eb, f0x401921FB54442D18
  %i.ed = fdiv double %i.ec, %i.j                 ; 2 uses
  %i.ee = tail call double @cos(double noundef %i.ed) #7 ; 0 uses
  %i.ef = tail call double @sin(double noundef %i.ed) #7 ; 0 uses
  %i.eg = add nuw nsw i32 %.07177, 4              ; 2 uses
  %niter.next.3 = add nuw nsw i32 %niter, 4       ; 2 uses
  %niter.ncmp.3 = icmp eq i32 %niter.next.3, %unroll_iter
  br i1 %niter.ncmp.3, label %._crit_edge79.loopexit197.unr-lcssa, label %.lr.ph78.split, !llvm.loop !27
}
