...
bb.i:                                             ; preds = %bb.h
  ...
  %4 = extractelement <2 x double> %i.cu, i64 0
  %5 = fsub double %i.ca, %4
  %i.cw = extractelement <2 x double> %i.ci, i64 0
  %6 = fsub double %i.cw, %i.cv
  %i.cx = fsub double %i.ct, %i.cv
  ...
  %i.dz = extractelement <2 x double> %i.dy, i64 1
  ...
  %i.ed = insertelement <2 x double> %i.de, double %i.cx, i64 0 ; 2 uses
  %i.ee = fmul <2 x double> %i.ed, %i.ec
  %i.ef = shufflevector <2 x double> %i.de, <2 x double> poison, <2 x i32> <i32 poison, i32 0>
  %i.eg = insertelement <2 x double> %i.ef, double %6, i64 0 ; 2 uses
  ...
  %i.eo = insertelement <2 x double> %i.en, double %5, i64 0 ; 2 uses
...
bb.a:
  ...
  %6 = shufflevector <2 x double> %i.bl, <2 x double> poison, <2 x i32> <i32 poison, i32 0>
  %7 = insertelement <2 x double> %6, double %5, i64 0
  %8 = tail call <2 x double> @llvm.sqrt.v2f64(<2 x double> %7) ; 2 uses
  %i.bm = extractelement <2 x double> %8, i64 0   ; 2 uses
  %9 = fcmp olt double %i.bm, 1.000000e+30
  %.133 = select i1 %9, double %i.bm, double 1.000000e+30 ; 2 uses
  %10 = extractelement <2 x double> %8, i64 1     ; 2 uses
  %i.bn = fcmp olt double %10, %.133              ; 2 uses
  %.133.1 = select i1 %i.bn, double %10, double %.133 ; 2 uses
...
_ZN10tetgenmesh9makepointEPPdNS_8verttypeE.exit:  ; preds = %bb.s, %bb.t, %bb.u, %bb.v
  ...
  %13 = shufflevector <2 x double> %i.ld, <2 x double> poison, <2 x i32> <i32 poison, i32 0>
  %14 = insertelement <2 x double> %13, double %12, i64 0
  %15 = tail call <2 x double> @llvm.sqrt.v2f64(<2 x double> %14) ; 2 uses
  %i.le = extractelement <2 x double> %15, i64 0  ; 2 uses
  %16 = fcmp olt double %i.le, 1.000000e+30
  %.1 = select i1 %16, double %i.le, double 1.000000e+30 ; 2 uses
  %17 = extractelement <2 x double> %15, i64 1    ; 2 uses
  %i.lf = fcmp olt double %17, %.1
  %.1.1 = select i1 %i.lf, double %17, double %.1 ; 2 uses
