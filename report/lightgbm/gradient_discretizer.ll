...
.lr.ph.epil.preheader:                            ; preds = %._crit_edge.loopexit.unr-lcssa, %.lr.ph.preheader
  ...
  %i.cb = fcmp olt double %.05963.epil.init, %i.ca
  %7 = fcmp olt double %.05864.epil.init, %i.by
  %.1.epil = select i1 %7, double %i.by, double %.05864.epil.init
  %.160.epil = select i1 %i.cb, double %i.ca, double %.05963.epil.init
  br label %._crit_edge
...
.lr.ph:                                           ; preds = %.lr.ph, %.lr.ph.preheader.new
  ...
  %i.cn = fcmp olt double %.05963, %i.cm
  %8 = fcmp olt double %.05864, %i.ck
  %.1 = select i1 %8, double %i.ck, double %.05864 ; 2 uses
  %.160 = select i1 %i.cn, double %i.cm, double %.05963 ; 2 uses
  ...
  %i.cs = fcmp olt double %.160, %i.cr
  %9 = fcmp olt double %.1, %i.cp
  %.1.1 = select i1 %9, double %i.cp, double %.1  ; 3 uses
  %.160.1 = select i1 %i.cs, double %i.cr, double %.160 ; 3 uses
