...
bb.qb:                                            ; preds = %bb.qa, %bb.px
  ...
  %i.bwg = load <2 x float>, ptr %.sroa.61280.0..sroa_idx, align 4 ; 10 uses
  %i.bwh = load <2 x float>, ptr %.sroa.81282.0..sroa_idx, align 4 ; 9 uses
  %.sroa.01289.0.vec.extract = extractelement <2 x float> %i.bwg, i64 0 ; 3 uses
  %.sroa.01289.4.vec.extract = extractelement <2 x float> %i.bwg, i64 1 ; 3 uses
  %.sroa.51290.8.vec.extract = extractelement <2 x float> %i.bwh, i64 0 ; 2 uses
  %.sroa.51290.12.vec.extract = extractelement <2 x float> %i.bwh, i64 1 ; 3 uses
  ...
  %foldExtExtBinop20467 = fmul <2 x float> %i.bwh, %i.bwh
  %25 = fmul float %.sroa.51290.8.vec.extract, %.sroa.51290.12.vec.extract ; 2 uses
  %26 = fmul float %.sroa.01289.0.vec.extract, %.sroa.01289.4.vec.extract ; 2 uses
  %27 = shufflevector <2 x float> %foldExtExtBinop20467, <2 x float> poison, <2 x i32> zeroinitializer
  %i.bwn = fadd <2 x float> %i.bwm, %27
  %i.bwo = fmul <2 x float> %i.bwn, splat (float 2.000000e+00)
  %i.bwp = fadd float %26, %25
  %i.bwq = fsub float %26, %25
