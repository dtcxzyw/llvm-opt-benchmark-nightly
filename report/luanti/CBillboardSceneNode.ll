...
_ZN4core5arrayINS_8vector3dIfEEEixEj.exit:        ; preds = %bb.f
  ...
  %2 = load <3 x float>, ptr %i.be, align 4, !tbaa !16 ; 5 uses
  %i.bg = load float, ptr %i.be, align 4, !tbaa !113
  ...
  %i.bj = fmul <2 x float> %3, %i.bi
  %i.bk = shufflevector <3 x float> %2, <3 x float> poison, <2 x i32> zeroinitializer
  ...
  %i.bn = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %4, <2 x float> %i.bm, <2 x float> %i.bl)
  ...
  %i.bs = fmul float %5, %i.br
  %i.bt = tail call float @llvm.fmuladd.f32(float %i.bg, float %i.bq, float %i.bs)
  %i.bu = load float, ptr %i.av, align 8, !tbaa !16
  %6 = extractelement <3 x float> %2, i64 2
  %i.bv = tail call float @llvm.fmuladd.f32(float %6, float %i.bu, float %i.bt)
