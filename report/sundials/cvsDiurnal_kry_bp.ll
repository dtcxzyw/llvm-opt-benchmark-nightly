Download link: https://huggingface.co/buckets/llvm-opt-benchmark/llvm-opt-benchmark/resolve/sundials/original/cvsDiurnal_kry_bp?download=true
...
bb.a:
  %i.a = tail call ptr @N_VGetArrayPointer(ptr noundef %1) #10 ; 14 uses
...
.peel.next:                                       ; preds = %.loopexit.peel.begin, %bb.c
  ...
  %4 = load <2 x double>, ptr %i.bx, align 8, !tbaa !14 ; 5 uses
  %5 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.ax, <2 x double> splat (double -2.000000e+00), <2 x double> %4) ; 2 uses
  %foldExtExtBinop = fadd <2 x double> %4, %5
  %i.by = extractelement <2 x double> %foldExtExtBinop, i64 0
  %6 = fmul double %i.u, %i.by
  %foldExtExtBinop140 = fadd <2 x double> %4, %5
  %i.bz = extractelement <2 x double> %foldExtExtBinop140, i64 1
  %7 = fmul double %i.u, %i.bz
  %8 = fsub <2 x double> %4, %4                   ; 2 uses
  %9 = extractelement <2 x double> %8, i64 0
  %i.ca = fmul double %i.w, %9
  %10 = extractelement <2 x double> %8, i64 1
  %i.cb = fmul double %i.w, %10
  %i.cc = extractelement <2 x double> %i.bv, i64 0
  %i.cd = fadd double %i.cc, %6
  ...
  %invariant.gep = getelementptr [8 x i8], ptr %i.a, i64 %i.at ; 2 uses
  ...
  %11 = trunc nuw nsw i64 %i.ar to i32
...
bb.d:                                             ; preds = %.peel.next, %bb.d
  ...
  %gep = getelementptr [8 x i8], ptr %invariant.gep, i64 %i.cr
  ...
  %i.ek = add nuw nsw i32 %i.eg, %11
  %i.el = sext i32 %i.ek to i64
  %i.em = getelementptr inbounds [8 x i8], ptr %i.a, i64 %i.el
  %i.en = load double, ptr %i.em, align 8, !tbaa !14 ; 2 uses
  %i.eo = shufflevector <2 x double> %i.bt, <2 x double> %i.cx, <2 x i32> <i32 0, i32 2>
  %i.ep = shufflevector <2 x double> <double poison, double -2.000000e+00>, <2 x double> %i.dl, <2 x i32> <i32 3, i32 1>
  %i.eq = insertelement <2 x double> poison, double %i.dw, i64 0
  %i.er = insertelement <2 x double> %i.eq, double %i.ej, i64 1
  %i.es = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.eo, <2 x double> %i.ep, <2 x double> %i.er) ; 2 uses
  %i.et = extractelement <2 x double> %i.es, i64 1
  %i.eu = fadd double %i.ec, %i.et
  %i.ev = fmul double %i.u, %i.eu
  %i.ew = tail call double @llvm.fmuladd.f64(double %i.cy, double -2.000000e+00, double %i.en)
  %i.ex = fadd double %i.ee, %i.ew
  %i.ey = fmul double %i.u, %i.ex
  %i.ez = fsub double %i.ej, %i.ec
  %i.fa = fmul double %i.w, %i.ez
  %i.fb = fsub double %i.en, %i.ee
  %i.fc = fmul double %i.w, %i.fb
  %i.fd = extractelement <2 x double> %i.dp, i64 1
  %i.fe = fadd double %i.fd, %i.ev
  %i.ff = fadd double %i.fa, %i.fe
  %i.fg = fadd double %i.dr, %i.ff
  %i.fh = getelementptr inbounds nuw [8 x i8], ptr %i.b, i64 %i.cs
  store double %i.fg, ptr %i.fh, align 8, !tbaa !14
  %i.fi = extractelement <2 x double> %i.es, i64 0
  %i.fj = fadd double %i.fi, %i.ey
  %i.fk = fadd double %i.fc, %i.fj
  %i.fl = fadd double %i.dt, %i.fk
  %gep143 = getelementptr inbounds [8 x i8], ptr %invariant.gep142, i64 %i.cu
  store double %i.fl, ptr %gep143, align 8, !tbaa !14
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1 ; 2 uses
  %exitcond.not = icmp eq i64 %indvars.iv.next, 9
  br i1 %exitcond.not, label %.loopexit.peel.begin, label %bb.d, !llvm.loop !25

.loopexit.peel.begin:                             ; preds = %bb.d
  %i.fm = add nuw nsw i64 %i.ao, 18               ; 2 uses
  %i.fn = getelementptr inbounds nuw [8 x i8], ptr %i.a, i64 %i.fm
  %gep.peel = getelementptr i8, ptr %invariant.gep, i64 144
  %i.fo = load double, ptr %gep.peel, align 8, !tbaa !14
  %gep135.peel = getelementptr i8, ptr %invariant.gep134, i64 152
  %i.fp = load double, ptr %gep135.peel, align 8, !tbaa !14
  %gep137.peel = getelementptr i8, ptr %invariant.gep136, i64 144
  %i.fq = load <2 x double>, ptr %i.fn, align 8, !tbaa !14 ; 4 uses
  %i.fr = extractelement <2 x double> %i.fq, i64 1 ; 4 uses
  %i.fs = fmul double %storemerge, %i.fr          ; 2 uses
  %i.ft = load <2 x double>, ptr %gep137.peel, align 8, !tbaa !14
  %i.fu = extractelement <2 x double> %i.fq, i64 0 ; 3 uses
  %i.fv = fmul double %i.fu, 1.630000e-16
  %i.fw = fmul double %i.fu, 4.660000e-16
  %i.fx = fmul double %i.fw, %i.fr                ; 2 uses
  %i.fy = fsub double %i.fu, %i.fo
  %i.fz = fmul double %i.fv, 3.700000e+16         ; 2 uses
  %i.ga = fneg double %i.fy
  %i.gb = fneg double %i.fz
  %i.gc = fmul double %i.ag, %i.ga
  %i.gd = fsub double %i.gb, %i.fx
  %i.ge = fsub <2 x double> %i.ft, %i.fq          ; 2 uses
  %i.gf = shufflevector <2 x double> <double 2.000000e+00, double poison>, <2 x double> %i.ge, <2 x i32> <i32 0, i32 2>
  %i.gg = insertelement <2 x double> poison, double %i.gd, i64 0
  %i.gh = insertelement <2 x double> %i.gg, double %i.gc, i64 1
  %i.gi = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.cq, <2 x double> %i.gf, <2 x double> %i.gh) ; 2 uses
  %i.gj = extractelement <2 x double> %i.gi, i64 0
  %i.gk = fadd double %i.fs, %i.gj
  %i.gl = fsub double %i.fz, %i.fx
  %i.gm = fsub double %i.gl, %i.fs
  %i.gn = fsub double %i.fr, %i.fp
  %i.go = fneg double %i.gn
  %i.gp = fmul double %i.ag, %i.go
  %i.gq = getelementptr i8, ptr %i.cn, i64 128
  %i.gr = load double, ptr %i.gq, align 8, !tbaa !14 ; 2 uses
  %i.gs = getelementptr i8, ptr %i.co, i64 128
  %i.gt = load double, ptr %i.gs, align 8, !tbaa !14 ; 2 uses
  %i.gu = getelementptr i8, ptr %i.cp, i64 128
  %i.gv = load double, ptr %i.gu, align 8, !tbaa !14 ; 2 uses
  %i.gw = mul i64 %indvars.iv128, 85899345920
  %sext141 = add i64 %i.gw, 73014444032
  %i.gx = ashr exact i64 %sext141, 29
  %i.gy = getelementptr inbounds i8, ptr %i.a, i64 %i.gx
  %i.gz = load double, ptr %i.gy, align 8, !tbaa !14 ; 2 uses
  %i.ha = shufflevector <2 x double> %i.bt, <2 x double> %i.fq, <2 x i32> <i32 0, i32 2>
  %i.hb = shufflevector <2 x double> <double poison, double -2.000000e+00>, <2 x double> %i.ge, <2 x i32> <i32 3, i32 1>
  %i.hc = insertelement <2 x double> poison, double %i.gp, i64 0
  %i.hd = insertelement <2 x double> %i.hc, double %i.gv, i64 1
  %i.he = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.ha, <2 x double> %i.hb, <2 x double> %i.hd) ; 2 uses
  %i.hf = extractelement <2 x double> %i.he, i64 1
  %i.hg = fadd double %i.gr, %i.hf
  %i.hh = fmul double %i.u, %i.hg
  %i.hi = tail call double @llvm.fmuladd.f64(double %i.fr, double -2.000000e+00, double %i.gz)
  %i.hj = fadd double %i.gt, %i.hi
  %i.hk = fmul double %i.u, %i.hj
  %i.hl = fsub double %i.gv, %i.gr
  %i.hm = fmul double %i.w, %i.hl
  %i.hn = fsub double %i.gz, %i.gt
  %i.ho = fmul double %i.w, %i.hn
  %i.hp = extractelement <2 x double> %i.gi, i64 1
  %i.hq = fadd double %i.hp, %i.hh
  %i.hr = fadd double %i.hm, %i.hq
  %i.hs = fadd double %i.gk, %i.hr
  %i.ht = getelementptr inbounds nuw [8 x i8], ptr %i.b, i64 %i.fm
  store double %i.hs, ptr %i.ht, align 8, !tbaa !14
  %i.hu = extractelement <2 x double> %i.he, i64 0
  %i.hv = fadd double %i.hu, %i.hk
  %i.hw = fadd double %i.ho, %i.hv
  %i.hx = fadd double %i.gm, %i.hw
  %i.hy = getelementptr inbounds nuw [8 x i8], ptr %i.y, i64 %i.ao
  store double %i.hx, ptr %i.hy, align 8, !tbaa !14
