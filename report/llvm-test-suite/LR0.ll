...
.lr.ph.preheader:                                 ; preds = %bb.a
  %i.n = add i64 %i.i, 2
  %i.o = tail call i64 @llvm.umax.i64(i64 %i.h, i64 %i.n)
  %i.p = xor i64 %i.i, -1
  %i.q = add i64 %i.o, %i.p                       ; 2 uses
  %i.r = lshr i64 %i.q, 1
  %i.s = add nuw i64 %i.r, 1                      ; 2 uses
  %min.iters.check = icmp ult i64 %i.q, 14
  br i1 %min.iters.check, label %.lr.ph.preheader156, label %vector.ph
...
middle.block:                                     ; preds = %vector.body
  ...
  br i1 %cmp.n, label %._crit_edge.loopexit, label %.lr.ph.preheader156

.lr.ph.preheader156:                              ; preds = %.lr.ph.preheader, %middle.block
...
.lr.ph:                                           ; preds = %.lr.ph.preheader156, %.lr.ph
  %.050 = phi i32 [ %i.ag, %.lr.ph ], [ %.050.ph, %.lr.ph.preheader156 ]
  %.03249 = phi ptr [ %i.ad, %.lr.ph ], [ %.03249.ph, %.lr.ph.preheader156 ] ; 2 uses
...
vector.body95:                                    ; preds = %vector.body95, %vector.ph92
  ...
  %i.cp = shl i64 %index96, 1                     ; 2 uses
  %next.gep97 = getelementptr i8, ptr %i.bk, i64 %i.cp ; 2 uses
  %next.gep98 = getelementptr i8, ptr %i.ce, i64 %i.cp ; 2 uses
  %i.cq = getelementptr i8, ptr %next.gep97, i64 16
  %wide.load99 = load <8 x i16>, ptr %next.gep97, align 2, !tbaa !11
  %wide.load100 = load <8 x i16>, ptr %i.cq, align 2, !tbaa !11
  %i.cr = getelementptr i8, ptr %next.gep98, i64 16
  store <8 x i16> %wide.load99, ptr %next.gep98, align 2, !tbaa !11
  store <8 x i16> %wide.load100, ptr %i.cr, align 2, !tbaa !11
  %index.next101 = add nuw i64 %index96, 16       ; 2 uses
  %i.cs = icmp eq i64 %index.next101, %n.vec94
  br i1 %i.cs, label %middle.block102, label %vector.body95, !llvm.loop !86

middle.block102:                                  ; preds = %vector.body95
  %cmp.n103 = icmp eq i64 %i.cj, %n.vec94
  br i1 %cmp.n103, label %new_state.exit, label %vec.epilog.iter.check

vec.epilog.iter.check:                            ; preds = %middle.block102
  %min.epilog.iters.check = icmp eq i64 %n.mod.vf93, 0
...
vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  ...
  %n.vec107 = and i64 %i.cj, -4                   ; 3 uses
  %i.ct = shl i64 %n.vec107, 1                    ; 2 uses
  %i.cu = getelementptr i8, ptr %i.bk, i64 %i.ct
  %i.cv = getelementptr i8, ptr %i.ce, i64 %i.ct
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index108 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next112, %vec.epilog.vector.body ] ; 2 uses
  %i.cw = shl i64 %index108, 1                    ; 2 uses
  %next.gep109 = getelementptr i8, ptr %i.bk, i64 %i.cw
  %next.gep110 = getelementptr i8, ptr %i.ce, i64 %i.cw
  %wide.load111 = load <4 x i16>, ptr %next.gep109, align 2, !tbaa !11
  store <4 x i16> %wide.load111, ptr %next.gep110, align 2, !tbaa !11
  %index.next112 = add nuw i64 %index108, 4       ; 2 uses
  %i.cx = icmp eq i64 %index.next112, %n.vec107
  br i1 %i.cx, label %vec.epilog.middle.block, label %vec.epilog.vector.body, !llvm.loop !87

vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  %cmp.n113 = icmp eq i64 %i.cj, %n.vec107
  br i1 %cmp.n113, label %new_state.exit, label %.lr.ph.i.preheader

.lr.ph.i.preheader:                               ; preds = %vector.memcheck, %iter.check, %vec.epilog.iter.check, %vec.epilog.middle.block
  %.019.i.ph = phi ptr [ %i.bk, %iter.check ], [ %i.bk, %vector.memcheck ], [ %i.cn, %vec.epilog.iter.check ], [ %i.cu, %vec.epilog.middle.block ]
  %.01718.i.ph = phi ptr [ %i.ce, %iter.check ], [ %i.ce, %vector.memcheck ], [ %i.co, %vec.epilog.iter.check ], [ %i.cv, %vec.epilog.middle.block ]
  br label %.lr.ph.i

.lr.ph.i:                                         ; preds = %.lr.ph.i.preheader, %.lr.ph.i
  %.019.i = phi ptr [ %i.cy, %.lr.ph.i ], [ %.019.i.ph, %.lr.ph.i.preheader ] ; 2 uses
  %.01718.i = phi ptr [ %i.da, %.lr.ph.i ], [ %.01718.i.ph, %.lr.ph.i.preheader ] ; 2 uses
  %i.cy = getelementptr inbounds nuw i8, ptr %.019.i, i64 2 ; 2 uses
  %i.cz = load i16, ptr %.019.i, align 2, !tbaa !11
  %i.da = getelementptr inbounds nuw i8, ptr %.01718.i, i64 2
  store i16 %i.cz, ptr %.01718.i, align 2, !tbaa !11
  %i.db = icmp ult ptr %i.cy, %i.bn
  br i1 %i.db, label %.lr.ph.i, label %new_state.exit, !llvm.loop !88

new_state.exit:                                   ; preds = %.lr.ph.i, %middle.block102, %vec.epilog.middle.block, %bb.f
  %i.dc = load ptr, ptr @last_state, align 8, !tbaa !24
  store ptr %i.bv, ptr %i.dc, align 8, !tbaa !53
  store ptr %i.bv, ptr @last_state, align 8, !tbaa !24
  %i.dd = add nsw i32 %i.by, 1
  store i32 %i.dd, ptr @nstates, align 4, !tbaa !4
  store ptr %i.bv, ptr %i.bf, align 8, !tbaa !47
  br label %.loopexit

bb.g:                                             ; preds = %._crit_edge
  %i.de = load i32, ptr @nstates, align 4, !tbaa !4
  %i.df = icmp sgt i32 %i.de, 32766
...
bb.i:                                             ; preds = %bb.h, %bb.g
  ...
  %i.dm = tail call ptr (i32, ...) @mallocate(i32 noundef %i.dl) #8 ; 9 uses
  %i.dn = ptrtoaddr ptr %i.dm to i64
  %i.do = trunc i32 %0 to i16
  %i.dp = getelementptr inbounds nuw i8, ptr %i.dm, i64 18
  store i16 %i.do, ptr %i.dp, align 2, !tbaa !49
  %i.dq = load i32, ptr @nstates, align 4, !tbaa !4 ; 2 uses
  %i.dr = trunc i32 %i.dq to i16
  %i.ds = getelementptr inbounds nuw i8, ptr %i.dm, i64 16
  store i16 %i.dr, ptr %i.ds, align 8, !tbaa !29
  %i.dt = trunc i64 %.pre-phi69 to i16
  %i.du = getelementptr inbounds nuw i8, ptr %i.dm, i64 20
  store i16 %i.dt, ptr %i.du, align 4, !tbaa !26
  %i.dv = icmp ult ptr %i.dh, %i.dg
  br i1 %i.dv, label %iter.check138, label %new_state.exit46

iter.check138:                                    ; preds = %bb.i
  %1 = ptrtoaddr ptr %i.dg to i64
  %i.dw = getelementptr inbounds nuw i8, ptr %i.dm, i64 22 ; 6 uses
  %i.dx = add i64 %i.di, 2
  %umax118 = tail call i64 @llvm.umax.i64(i64 %1, i64 %i.dx)
  %i.dy = xor i64 %i.di, -1
  %i.dz = add i64 %umax118, %i.dy                 ; 3 uses
  %i.ea = lshr i64 %i.dz, 1
  %i.eb = add nuw i64 %i.ea, 1                    ; 5 uses
  %min.iters.check120 = icmp ult i64 %i.dz, 6
  br i1 %min.iters.check120, label %.lr.ph.i43.preheader, label %vector.memcheck116

vector.memcheck116:                               ; preds = %iter.check138
  %i.ec = sub i64 %i.dn, %i.di
  %i.ed = add i64 %i.ec, 21
  %diff.check117 = icmp ult i64 %i.ed, 31
  br i1 %diff.check117, label %.lr.ph.i43.preheader, label %vector.main.loop.iter.check121

vector.main.loop.iter.check121:                   ; preds = %vector.memcheck116
  %min.iters.check122 = icmp ult i64 %i.dz, 30
  br i1 %min.iters.check122, label %vec.epilog.ph142, label %vector.ph123

vector.ph123:                                     ; preds = %vector.main.loop.iter.check121
  %n.mod.vf124 = and i64 %i.eb, 12
  %n.vec125 = and i64 %i.eb, -16                  ; 4 uses
  %i.ee = shl i64 %n.vec125, 1                    ; 2 uses
  %i.ef = getelementptr i8, ptr %i.dh, i64 %i.ee
  %i.eg = getelementptr i8, ptr %i.dw, i64 %i.ee
...
vector.body126:                                   ; preds = %vector.body126, %vector.ph123
  ...
  %i.eh = shl i64 %index127, 1                    ; 2 uses
  %next.gep128 = getelementptr i8, ptr %i.dh, i64 %i.eh ; 2 uses
  %next.gep129 = getelementptr i8, ptr %i.dw, i64 %i.eh ; 2 uses
  %i.ei = getelementptr i8, ptr %next.gep128, i64 16
  %wide.load130 = load <8 x i16>, ptr %next.gep128, align 2, !tbaa !11
  %wide.load131 = load <8 x i16>, ptr %i.ei, align 2, !tbaa !11
  %i.ej = getelementptr i8, ptr %next.gep129, i64 16
  store <8 x i16> %wide.load130, ptr %next.gep129, align 2, !tbaa !11
  store <8 x i16> %wide.load131, ptr %i.ej, align 2, !tbaa !11
  %index.next132 = add nuw i64 %index127, 16      ; 2 uses
  %i.ek = icmp eq i64 %index.next132, %n.vec125
  br i1 %i.ek, label %middle.block133, label %vector.body126, !llvm.loop !89

middle.block133:                                  ; preds = %vector.body126
  %cmp.n134 = icmp eq i64 %i.eb, %n.vec125
  br i1 %cmp.n134, label %new_state.exit46, label %vec.epilog.iter.check140

vec.epilog.iter.check140:                         ; preds = %middle.block133
  %min.epilog.iters.check141 = icmp eq i64 %n.mod.vf124, 0
  br i1 %min.epilog.iters.check141, label %.lr.ph.i43.preheader, label %vec.epilog.ph142, !prof !37

vec.epilog.ph142:                                 ; preds = %vector.main.loop.iter.check121, %vec.epilog.iter.check140
  %vec.epilog.resume.val135 = phi i64 [ %n.vec125, %vec.epilog.iter.check140 ], [ 0, %vector.main.loop.iter.check121 ]
  %n.vec144 = and i64 %i.eb, -4                   ; 3 uses
  %i.el = shl i64 %n.vec144, 1                    ; 2 uses
  %i.em = getelementptr i8, ptr %i.dh, i64 %i.el
  %i.en = getelementptr i8, ptr %i.dw, i64 %i.el
  br label %vec.epilog.vector.body145

vec.epilog.vector.body145:                        ; preds = %vec.epilog.vector.body145, %vec.epilog.ph142
  %index146 = phi i64 [ %vec.epilog.resume.val135, %vec.epilog.ph142 ], [ %index.next150, %vec.epilog.vector.body145 ] ; 2 uses
  %i.eo = shl i64 %index146, 1                    ; 2 uses
  %next.gep147 = getelementptr i8, ptr %i.dh, i64 %i.eo
  %next.gep148 = getelementptr i8, ptr %i.dw, i64 %i.eo
  %wide.load149 = load <4 x i16>, ptr %next.gep147, align 2, !tbaa !11
  store <4 x i16> %wide.load149, ptr %next.gep148, align 2, !tbaa !11
  %index.next150 = add nuw i64 %index146, 4       ; 2 uses
  %i.ep = icmp eq i64 %index.next150, %n.vec144
  br i1 %i.ep, label %vec.epilog.middle.block151, label %vec.epilog.vector.body145, !llvm.loop !90

vec.epilog.middle.block151:                       ; preds = %vec.epilog.vector.body145
  %cmp.n152 = icmp eq i64 %i.eb, %n.vec144
  br i1 %cmp.n152, label %new_state.exit46, label %.lr.ph.i43.preheader

.lr.ph.i43.preheader:                             ; preds = %vector.memcheck116, %iter.check138, %vec.epilog.iter.check140, %vec.epilog.middle.block151
  %.019.i44.ph = phi ptr [ %i.dh, %iter.check138 ], [ %i.dh, %vector.memcheck116 ], [ %i.ef, %vec.epilog.iter.check140 ], [ %i.em, %vec.epilog.middle.block151 ]
  %.01718.i45.ph = phi ptr [ %i.dw, %iter.check138 ], [ %i.dw, %vector.memcheck116 ], [ %i.eg, %vec.epilog.iter.check140 ], [ %i.en, %vec.epilog.middle.block151 ]
  br label %.lr.ph.i43

.lr.ph.i43:                                       ; preds = %.lr.ph.i43.preheader, %.lr.ph.i43
  %.019.i44 = phi ptr [ %i.eq, %.lr.ph.i43 ], [ %.019.i44.ph, %.lr.ph.i43.preheader ] ; 2 uses
  %.01718.i45 = phi ptr [ %i.es, %.lr.ph.i43 ], [ %.01718.i45.ph, %.lr.ph.i43.preheader ] ; 2 uses
  %i.eq = getelementptr inbounds nuw i8, ptr %.019.i44, i64 2 ; 2 uses
  %i.er = load i16, ptr %.019.i44, align 2, !tbaa !11
  %i.es = getelementptr inbounds nuw i8, ptr %.01718.i45, i64 2
  store i16 %i.er, ptr %.01718.i45, align 2, !tbaa !11
  %i.et = icmp ult ptr %i.eq, %i.dg
  br i1 %i.et, label %.lr.ph.i43, label %new_state.exit46, !llvm.loop !91

new_state.exit46:                                 ; preds = %.lr.ph.i43, %middle.block133, %vec.epilog.middle.block151, %bb.i
  %i.eu = load ptr, ptr @last_state, align 8, !tbaa !24
  store ptr %i.dm, ptr %i.eu, align 8, !tbaa !53
  store ptr %i.dm, ptr @last_state, align 8, !tbaa !24
  %i.ev = add nsw i32 %i.dq, 1
  store i32 %i.ev, ptr @nstates, align 4, !tbaa !4
  %i.ew = load ptr, ptr @state_table, align 8, !tbaa !22
  %i.ex = getelementptr inbounds [8 x i8], ptr %i.ew, i64 %.0.lcssa
  store ptr %i.dm, ptr %i.ex, align 8, !tbaa !24
  br label %.loopexit

.loopexit:                                        ; preds = %bb.c, %._crit_edge55, %new_state.exit, %new_state.exit46
  %.231 = phi ptr [ %i.dm, %new_state.exit46 ], [ %i.bv, %new_state.exit ], [ %.02957, %._crit_edge55 ], [ %.02957, %bb.c ]
  %i.ey = getelementptr inbounds nuw i8, ptr %.231, i64 16
  %i.ez = load i16, ptr %i.ey, align 8, !tbaa !29
  %i.fa = sext i16 %i.ez to i32
  ret i32 %i.fa
}
