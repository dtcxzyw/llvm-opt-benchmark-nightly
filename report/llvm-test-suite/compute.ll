...
.critedge:                                        ; preds = %.critedge.backedge, %bb.a
  %i.e = phi <2 x double> [ %i.b, %bb.a ], [ %i.dl, %.critedge.backedge ] ; 4 uses
...
bb.d:                                             ; preds = %make_orthogonal.exit, %.critedge
  ...
  %i.bd = fsub <2 x double> %i.bc, %i.d           ; 5 uses
  %2 = shufflevector <2 x double> %i.bd, <2 x double> poison, <2 x i32> <i32 1, i32 0> ; 2 uses
  %3 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.bd, <2 x double> %i.bd, <2 x double> <double 0.000000e+00, double -0.000000e+00>)
  %i.be = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %2, <2 x double> %2, <2 x double> %3)
  %i.bf = tail call <2 x double> @llvm.sqrt.v2f64(<2 x double> %i.be) ; 3 uses
  %4 = shufflevector <2 x double> %i.bf, <2 x double> poison, <2 x i32> zeroinitializer
  %i.bg = fdiv <2 x double> %i.bd, %4             ; 4 uses
  %i.bh = fneg <2 x double> %i.bc
  %i.bi = fmul <2 x double> %i.bc, %i.bh
  %i.bj = fmul <2 x double> %i.bi, %i.bd
  %5 = shufflevector <2 x double> %i.bf, <2 x double> poison, <2 x i32> <i32 1, i32 1>
  %i.bk = fdiv <2 x double> %i.bj, %5             ; 2 uses
  %i.bl = extractelement <2 x double> %i.bg, i64 0
  %i.bm = extractelement <2 x double> %i.bk, i64 0
  %i.bn = extractelement <2 x double> %i.bg, i64 1
  %i.bo = tail call double @llvm.fmuladd.f64(double %i.bl, double %i.bm, double 0.000000e+00)
  %i.bp = insertelement <2 x double> %i.bk, double f0x3FC91A556151761C, i64 0
  %i.bq = insertelement <2 x double> <double 0.000000e+00, double poison>, double %i.bo, i64 1
  %i.br = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.bg, <2 x double> %i.bp, <2 x double> %i.bq) ; 2 uses
  %i.bs = extractelement <2 x double> %i.br, i64 1
  %i.bt = tail call double @llvm.fabs.f64(double %i.bs)
  %i.bu = extractelement <2 x double> %i.bf, i64 0
  %i.bv = fdiv double %i.bu, %i.bt
  %i.bw = extractelement <2 x double> %i.br, i64 0
  %i.bx = tail call double @llvm.fmuladd.f64(double %i.bn, double f0xBFEF60EAB9A5D3A3, double %i.bw) ; 2 uses
  ...
  %i.cb = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.ca, <2 x double> <double f0x3FC91A556151761C, double f0xBFEF60EAB9A5D3A3>, <2 x double> %i.bg) ; 3 uses
  %i.cc = extractelement <2 x double> %i.cb, i64 0 ; 2 uses
  %i.cd = tail call double @llvm.fmuladd.f64(double %i.cc, double %i.cc, double 0.000000e+00)
  %i.ce = extractelement <2 x double> %i.cb, i64 1 ; 2 uses
  %i.cf = tail call double @llvm.fmuladd.f64(double %i.ce, double %i.ce, double %i.cd)
  %sqrt61 = tail call double @llvm.sqrt.f64(double %i.cf)
  %i.cg = insertelement <2 x double> poison, double %sqrt61, i64 0
  %i.ch = shufflevector <2 x double> %i.cg, <2 x double> poison, <2 x i32> zeroinitializer
  %i.ci = fdiv <2 x double> %i.cb, %i.ch          ; 3 uses
  %i.cj = tail call double @llvm.fmuladd.f64(double %i.by, double %i.bx, double 1.000000e+00) ; 2 uses
  %i.ck = fcmp olt double %i.cj, 0.000000e+00
  br i1 %i.ck, label %make_orthogonal.exit50, label %bb.e

bb.e:                                             ; preds = %bb.d
  %i.cl = tail call double @sqrt(double noundef %i.cj) #10, !tbaa !4
  br label %make_orthogonal.exit50

make_orthogonal.exit50:                           ; preds = %bb.d, %bb.e
  %.031.i49 = phi double [ %i.cl, %bb.e ], [ 0.000000e+00, %bb.d ]
  %i.cm = fmul <2 x double> %i.ba, splat (double 2.000000e+00) ; 3 uses
  %i.cn = extractelement <2 x double> %i.cm, i64 0 ; 2 uses
  %i.co = tail call double @llvm.fmuladd.f64(double %i.cn, double %i.cn, double 0.000000e+00)
  %i.cp = extractelement <2 x double> %i.cm, i64 1 ; 2 uses
  %i.cq = tail call double @llvm.fmuladd.f64(double %i.cp, double %i.cp, double %i.co)
  %sqrt62 = tail call double @llvm.sqrt.f64(double %i.cq)
  %i.cr = insertelement <2 x double> poison, double %sqrt62, i64 0
  %i.cs = shufflevector <2 x double> %i.cr, <2 x double> poison, <2 x i32> zeroinitializer
  %i.ct = fdiv <2 x double> %i.cm, %i.cs          ; 2 uses
  %i.cu = extractelement <2 x double> %i.ci, i64 0
  %i.cv = extractelement <2 x double> %i.ct, i64 0
  %i.cw = tail call double @llvm.fmuladd.f64(double %i.cu, double %i.cv, double 0.000000e+00)
  %i.cx = extractelement <2 x double> %i.ci, i64 1
  %i.cy = extractelement <2 x double> %i.ct, i64 1
  %i.cz = tail call double @llvm.fmuladd.f64(double %i.cx, double %i.cy, double %i.cw) ; 2 uses
  %i.da = fmul double %i.bv, %.031.i49            ; 3 uses
  %i.db = fcmp ogt double %i.cz, 0.000000e+00
  br i1 %i.db, label %bb.f, label %bb.h

bb.f:                                             ; preds = %make_orthogonal.exit50
  %foldExtExtBinop89 = fmul <2 x double> %i.ba, %i.ba
  %i.dc = extractelement <2 x double> %foldExtExtBinop89, i64 1
  %i.dd = extractelement <2 x double> %i.ba, i64 0 ; 2 uses
  %i.de = tail call double @llvm.fmuladd.f64(double %i.dd, double %i.dd, double %i.dc)
  %i.df = fadd double %i.de, -8.000000e-01
  %i.dg = fneg double %i.df
  %i.dh = fdiv double %i.dg, %i.cz                ; 2 uses
  %i.di = fcmp ogt double %i.da, %i.dh
...
bb.h:                                             ; preds = %bb.f, %bb.g, %make_orthogonal.exit50
  %.044 = phi double [ %i.dh, %bb.g ], [ %i.da, %bb.f ], [ %i.da, %make_orthogonal.exit50 ]
  %i.dj = insertelement <2 x double> poison, double %.044, i64 0
  %i.dk = shufflevector <2 x double> %i.dj, <2 x double> poison, <2 x i32> zeroinitializer
  %i.dl = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.dk, <2 x double> %i.ci, <2 x double> %i.ba) ; 5 uses
  %i.dm = extractelement <2 x double> %i.dl, i64 0 ; 2 uses
  %i.dn = extractelement <2 x double> %i.dl, i64 1 ; 4 uses
  %i.do = tail call double @llvm.fmuladd.f64(double %i.dn, double -5.000000e+00, double %i.dm)
  %i.dp = fmul double %i.dn, %i.dn
  %i.dq = fadd <2 x double> %i.dl, splat (double 1.000000e+00)
  %i.dr = fdiv <2 x double> splat (double 1.000000e+00), %i.dq
  %i.ds = fsub <2 x double> %i.dr, %i.d           ; 3 uses
  %i.dt = shufflevector <2 x double> %i.dl, <2 x double> %i.ds, <2 x i32> <i32 0, i32 2> ; 2 uses
  %i.du = insertelement <2 x double> <double poison, double 0.000000e+00>, double %i.dp, i64 0
  %i.dv = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.dt, <2 x double> %i.dt, <2 x double> %i.du) ; 2 uses
  %i.dw = extractelement <2 x double> %i.dv, i64 0
  %i.dx = fadd double %i.dw, -8.000000e-01        ; 2 uses
  %i.dy = extractelement <2 x double> %i.ds, i64 1 ; 2 uses
  %i.dz = extractelement <2 x double> %i.dv, i64 1
  %i.ea = tail call double @llvm.fmuladd.f64(double %i.dy, double %i.dy, double %i.dz)
  %sqrt63 = tail call double @llvm.sqrt.f64(double %i.ea)
  %i.eb = insertelement <2 x double> poison, double %sqrt63, i64 0
  %i.ec = shufflevector <2 x double> %i.eb, <2 x double> poison, <2 x i32> zeroinitializer
  %i.ed = fdiv <2 x double> %i.ds, %i.ec          ; 2 uses
  %i.ee = tail call double @llvm.fabs.f64(double %i.do)
  %i.ef = fcmp ogt double %i.ee, f0x3EB0C6F7A0B5ED8D
  %i.eg = fcmp ogt double %i.dx, f0x3EB0C6F7A0B5ED8D
  %or.cond = select i1 %i.ef, i1 true, i1 %i.eg
  br i1 %or.cond, label %.critedge.backedge, label %bb.i

bb.i:                                             ; preds = %bb.h
  %i.eh = tail call double @llvm.fabs.f64(double %i.dx)
  %i.ei = fcmp ogt double %i.eh, f0x3EB0C6F7A0B5ED8D
  br i1 %i.ei, label %bb.j, label %.critedge3

bb.j:                                             ; preds = %bb.i
  %i.ej = extractelement <2 x double> %i.ed, i64 1
  %i.ek = fmul double %i.ej, f0xBFC91A556151761C
  %i.el = extractelement <2 x double> %i.ed, i64 0
  %i.em = tail call double @llvm.fmuladd.f64(double %i.el, double f0xBFEF60EAB9A5D3A3, double %i.ek)
  %i.en = tail call double @llvm.fabs.f64(double %i.em)
  %i.eo = fcmp ogt double %i.en, f0x3EB0C6F7A0B5ED8D
  br i1 %i.eo, label %.critedge.backedge, label %.critedge3

.critedge.backedge:                               ; preds = %bb.j, %bb.h
  br label %.critedge, !llvm.loop !34

.critedge3:                                       ; preds = %bb.i, %bb.j
  store double %i.dm, ptr @P, align 8, !tbaa !18
  store double %i.dn, ptr @Q, align 8, !tbaa !18
  ret void
}
