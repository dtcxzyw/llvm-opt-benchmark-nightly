...
bb.c:                                             ; preds = %bb.a, %bb.c
  ...
  %2 = load float, ptr %i.j, align 8, !tbaa !20, !noalias !29 ; 2 uses
  ...
  %i.ax = load float, ptr %i.y, align 4, !tbaa !20 ; 3 uses
  %i.ay = load float, ptr %i.z, align 8, !tbaa !20 ; 3 uses
  %i.az = load float, ptr %i.aa, align 4, !tbaa !20 ; 3 uses
  %3 = load float, ptr %i.ab, align 8, !tbaa !20  ; 2 uses
  %4 = load float, ptr %i.ac, align 4, !tbaa !20  ; 2 uses
  %i.ba = load float, ptr %i.ad, align 8, !tbaa !20 ; 2 uses
  ...
  %5 = tail call float @llvm.fmuladd.f32(float %i.ca, float %i.ar, float %i.bz)
  %i.cb = tail call noundef float @llvm.fmuladd.f32(float %2, float %i.at, float %5)
  %6 = fadd float %i.cb, %i.aw
  %7 = fsub float %i.br, %i.au                    ; 2 uses
  %i.cc = fsub float %i.bx, %i.av                 ; 2 uses
  %i.cd = fsub float %6, %i.aw                    ; 2 uses
  ...
  %i.cj = extractelement <2 x float> %i.ce, i64 0
  %i.ck = tail call float @llvm.fmuladd.f32(float %i.cj, float %i.ax, float %i.ci)
  %i.cl = tail call noundef float @llvm.fmuladd.f32(float %i.ao, float %i.az, float %i.ck)
  %i.cm = fadd float %3, %i.cl
  %8 = extractelement <2 x float> %i.cf, i64 1
  %9 = fmul float %8, %i.ay
  %10 = extractelement <2 x float> %i.cf, i64 0
  %11 = tail call float @llvm.fmuladd.f32(float %10, float %i.ax, float %9)
  %12 = tail call noundef float @llvm.fmuladd.f32(float %i.ap, float %i.az, float %11)
  %13 = fadd float %4, %12
  %14 = extractelement <2 x float> %i.cg, i64 1
  %15 = fmul float %14, %i.ay
  %16 = extractelement <2 x float> %i.cg, i64 0
  %17 = tail call float @llvm.fmuladd.f32(float %16, float %i.ax, float %15)
  %18 = tail call noundef float @llvm.fmuladd.f32(float %i.aq, float %i.az, float %17)
  %19 = fadd float %18, %i.ba
  %20 = fsub float %i.cm, %3                      ; 2 uses
  %21 = fsub float %13, %4                        ; 2 uses
  %22 = fsub float %19, %i.ba                     ; 2 uses
  ...
  %i.co = load float, ptr %i.bi, align 4, !tbaa !20 ; 3 uses
  %i.cp = fneg float %i.co                        ; 2 uses
  %i.cq = fmul float %i.cd, %i.cp
  %i.cr = tail call float @llvm.fmuladd.f32(float %i.cc, float %i.cn, float %i.cq) ; 2 uses
  %23 = load float, ptr %i.al, align 4, !tbaa !20 ; 3 uses
  %24 = fneg float %i.cn                          ; 2 uses
  %25 = fmul float %7, %24
  %i.cs = tail call float @llvm.fmuladd.f32(float %i.cd, float %23, float %25) ; 2 uses
  %i.ct = fneg float %23                          ; 2 uses
  %i.cu = fmul float %i.cc, %i.ct
  %i.cv = tail call float @llvm.fmuladd.f32(float %7, float %i.co, float %i.cu) ; 2 uses
  ...
  %26 = tail call noundef float @llvm.fmuladd.f32(float %2, float %i.cv, float %i.dg) ; 3 uses
  %.sroa.3.12.vec.insert.i18.i = insertelement <2 x float> <float poison, float 0.000000e+00>, float %26, i64 0
  store <2 x float> %i.de, ptr %i.bd, align 4
  store <2 x float> %.sroa.3.12.vec.insert.i18.i, ptr %.sroa.46.0..sroa_idx.i, align 4, !tbaa !23
  %27 = fmul float %22, %i.co
  %28 = tail call float @llvm.fmuladd.f32(float %21, float %24, float %27) ; 2 uses
  %29 = fmul float %20, %i.cn
  %30 = tail call float @llvm.fmuladd.f32(float %22, float %i.ct, float %29) ; 2 uses
  %31 = fmul float %21, %23
  %32 = tail call float @llvm.fmuladd.f32(float %20, float %i.cp, float %31) ; 2 uses
  %i.dh = insertelement <2 x float> poison, float %30, i64 0
  %33 = shufflevector <2 x float> %i.dh, <2 x float> poison, <2 x i32> zeroinitializer
  %34 = fmul <2 x float> %i.cf, %33
  %35 = insertelement <2 x float> poison, float %28, i64 0
  %36 = shufflevector <2 x float> %35, <2 x float> poison, <2 x i32> zeroinitializer
  %i.di = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ce, <2 x float> %36, <2 x float> %34)
  %37 = insertelement <2 x float> poison, float %32, i64 0
  %38 = shufflevector <2 x float> %37, <2 x float> poison, <2 x i32> zeroinitializer
  %39 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.cg, <2 x float> %38, <2 x float> %i.di) ; 4 uses
  %i.dj = fmul float %i.ap, %30
  %i.dk = tail call float @llvm.fmuladd.f32(float %i.ao, float %28, float %i.dj)
  %i.dl = tail call noundef float @llvm.fmuladd.f32(float %i.aq, float %32, float %i.dk) ; 3 uses
  %.sroa.3.12.vec.insert.i33.i = insertelement <2 x float> <float poison, float 0.000000e+00>, float %i.dl, i64 0
  store <2 x float> %39, ptr %i.be, align 4
  ...
  %i.dq = fmul float %26, %i.dp                   ; 2 uses
  ...
  %i.dr = extractelement <2 x float> %39, i64 0
  %i.ds = load <2 x float>, ptr %i.ag, align 4, !tbaa !20
  %i.dt = fmul <2 x float> %39, %i.ds             ; 3 uses
  ...
  %i.dz = tail call noundef float @llvm.fmuladd.f32(float %i.dq, float %26, float %i.dy)
  %i.ea = fadd float %i.bb, %i.dz
  %i.eb = fadd float %i.bc, %i.ea
  %foldExtExtBinop39 = fmul <2 x float> %39, %i.dt
