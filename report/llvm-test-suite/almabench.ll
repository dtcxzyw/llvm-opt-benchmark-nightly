...
bb.m:                                             ; preds = %bb.l, %bb.k, %bb.j, %bb.i, %bb.h, %bb.g, %bb.f, %bb.e, %bb.d, %anpm.exit182
  ...
  %3 = fadd double %.0.i, %i.ot                   ; 2 uses
  %4 = tail call double @llvm.fmuladd.f64(double %i.jc, double %i.al, double 1.000000e+00)
  ...
  %5 = tail call double @llvm.fmuladd.f64(double %i.of, double f0x3E7AD7F29ABCAF48, double %i.oc) ; 5 uses
  %6 = tail call double @llvm.fmuladd.f64(double %i.jc, double %i.ou, double 1.000000e+00)
  %7 = fmul double %5, %5
  %8 = fmul double %5, %7
  %9 = fdiv double %i.oy, %8
  %i.pd = tail call double @sqrt(double noundef %9) #7, !tbaa !7
  %i.pe = insertelement <2 x double> poison, double %5, i64 0
  %i.pf = insertelement <2 x double> %i.pe, double %i.pd, i64 1
  %10 = insertelement <2 x double> <double poison, double f0x3F919D6D51A6B69A>, double %6, i64 0
  %i.pg = fmul <2 x double> %i.pf, %10            ; 3 uses
  %i.ph = tail call double @sin(double noundef %i.oz) #7, !tbaa !7 ; 2 uses
  %i.pi = tail call double @cos(double noundef %.0.i181) #7, !tbaa !7
  %i.pj = tail call double @sin(double noundef %.0.i181) #7, !tbaa !7
  %i.pk = tail call double @sin(double noundef %3) #7, !tbaa !7 ; 3 uses
  %i.pl = tail call double @cos(double noundef %3) #7, !tbaa !7 ; 3 uses
  %i.pm = fneg double %i.pk
  %i.pn = tail call double @sqrt(double noundef %4) #7, !tbaa !7
  %i.po = fdiv double %5, %i.pn                   ; 2 uses
  %i.pp = tail call double @cos(double noundef %i.oz) #7, !tbaa !7 ; 2 uses
  %i.pq = tail call double @sin(double noundef %.0.i) #7, !tbaa !7
  %i.pr = tail call double @cos(double noundef %.0.i) #7, !tbaa !7
  %i.ps = fmul double %i.ph, %i.pj                ; 5 uses
  %i.pt = fmul double %i.ph, %i.pi                ; 6 uses
  %i.pu = fmul double %i.pt, %i.pm
  %i.pv = tail call double @llvm.fmuladd.f64(double %i.ps, double %i.pl, double %i.pu)
  %11 = fmul double %i.pv, 2.000000e+00           ; 2 uses
  %i.pw = tail call double @llvm.fmuladd.f64(double %i.al, double %i.pq, double %i.pk)
  %i.px = fmul double %i.ps, 2.000000e+00         ; 2 uses
  %i.py = fmul double %i.pt, %i.px                ; 2 uses
  %12 = fneg double %11                           ; 2 uses
  %13 = insertelement <2 x double> poison, double %i.al, i64 0
  %i.pz = insertelement <2 x double> %13, double %i.px, i64 1
  %i.qa = insertelement <2 x double> poison, double %i.pr, i64 0
  %i.qb = insertelement <2 x double> %i.qa, double %i.ps, i64 1
  ...
  %i.qg = fmul double %i.py, %i.qf
  %14 = fmul double %i.po, %i.pw                  ; 3 uses
  %i.qh = insertelement <2 x double> %i.qd, double %12, i64 0
  %i.qi = insertelement <2 x double> poison, double %i.ps, i64 0
  %i.qj = insertelement <2 x double> %i.qi, double %14, i64 1
  %15 = insertelement <2 x double> poison, double %i.pl, i64 0
  %i.qk = insertelement <2 x double> %15, double %i.qg, i64 1
  %16 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.qh, <2 x double> %i.qj, <2 x double> %i.qk)
  %i.ql = fmul <2 x double> %i.pg, %16            ; 2 uses
  %i.qm = extractelement <2 x double> %i.ql, i64 0
  store double %i.qm, ptr %2, align 8, !tbaa !9
  %i.qn = fmul double %i.pp, %12
  %17 = tail call double @llvm.fmuladd.f64(double %11, double %i.pt, double %i.pk)
  %i.qo = extractelement <2 x double> %i.pg, i64 0 ; 2 uses
  %18 = fmul double %i.qo, %i.qn
  %19 = fmul double %i.qo, %17
  %i.qp = insertelement <2 x double> poison, double %18, i64 0
  %20 = shufflevector <2 x double> %i.qp, <2 x double> poison, <2 x i32> zeroinitializer
  %i.qq = fmul <2 x double> %20, <double f0xBFD9752E50F4B399, double f0x3FED5C0357681EF3>
  %i.qr = insertelement <2 x double> poison, double %19, i64 0
  %i.qs = shufflevector <2 x double> %i.qr, <2 x double> poison, <2 x i32> zeroinitializer
  %21 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.qs, <2 x double> <double f0x3FED5C0357681EF3, double f0x3FD9752E50F4B399>, <2 x double> %i.qq)
  store <2 x double> %21, ptr %i.pa, align 8, !tbaa !9
  %22 = fmul double %i.pp, 2.000000e+00
  %23 = fmul double %i.pt, -2.000000e+00
  %i.qt = fneg double %14
  %i.qu = fmul double %i.py, %i.qt
  %24 = fmul double %i.pt, %i.qf
  %i.qv = tail call double @llvm.fmuladd.f64(double %i.ps, double %14, double %24)
  %25 = tail call double @llvm.fmuladd.f64(double %23, double %i.pt, double 1.000000e+00)
  %i.qw = fmul double %22, %i.qv
  %26 = tail call double @llvm.fmuladd.f64(double %25, double %i.qf, double %i.qu)
  %27 = extractelement <2 x double> %i.pg, i64 1  ; 2 uses
  %i.qx = fmul double %27, %i.qw
  %i.qy = fmul double %27, %26
  %i.qz = extractelement <2 x double> %i.ql, i64 1
  store double %i.qz, ptr %i.pb, align 8, !tbaa !9
  %i.ra = insertelement <2 x double> poison, double %i.qx, i64 0
  ...
  %i.rd = insertelement <2 x double> poison, double %i.qy, i64 0
