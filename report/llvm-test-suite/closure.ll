...
begin_hunk_1_@closure:bb.a
  ...
  %i.cl = add i64 %umax137, %i.ck                 ; 3 uses
  %i.cm = lshr i64 %i.cl, 1
  %i.cn = add nuw i64 %i.cm, 1                    ; 5 uses
  %min.iters.check139 = icmp ult i64 %i.cl, 6
  ...
  br i1 %or.cond, label %.lr.ph84.preheader156, label %vector.main.loop.iter.check

vector.main.loop.iter.check:                      ; preds = %.lr.ph84.preheader.a
  %min.iters.check140 = icmp ult i64 %i.cl, 30
  br i1 %min.iters.check140, label %vec.epilog.ph, label %vector.ph140

vector.ph140:                                     ; preds = %vector.main.loop.iter.check
  %n.mod.vf142 = and i64 %i.cn, 12
  %n.vec142 = and i64 %i.cn, -16                  ; 5 uses
...
begin_hunk_2_@closure:bb.a
  ...
  br i1 %i.cv, label %middle.block153, label %vector.body143, !llvm.loop !37

middle.block153:                                  ; preds = %vector.body143
  %cmp.n154 = icmp eq i64 %i.cn, %n.vec142
  br i1 %cmp.n154, label %._crit_edge, label %vec.epilog.iter.check

vec.epilog.iter.check:                            ; preds = %middle.block153
  %min.epilog.iters.check = icmp eq i64 %n.mod.vf142, 0
  br i1 %min.epilog.iters.check, label %.lr.ph84.preheader156, label %vec.epilog.ph, !prof !38

vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  %vec.epilog.resume.val = phi i64 [ %n.vec142, %vec.epilog.iter.check ], [ 0, %vector.main.loop.iter.check ]
  %n.vec158 = and i64 %i.cn, -4                   ; 4 uses
  %2 = shl i64 %n.vec158, 1
  %3 = getelementptr i8, ptr %.142.lcssa, i64 %2
  %4 = shl i64 %n.vec158, 1
  %5 = getelementptr i8, ptr %itemsetend.promoted82, i64 %4 ; 2 uses
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index159 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next163, %vec.epilog.vector.body ] ; 3 uses
  %6 = shl i64 %index159, 1
  %next.gep160 = getelementptr i8, ptr %.142.lcssa, i64 %6
  %7 = shl i64 %index159, 1
  %next.gep161 = getelementptr i8, ptr %itemsetend.promoted82, i64 %7
  %wide.load162 = load <4 x i16>, ptr %next.gep160, align 2, !tbaa !16
  store <4 x i16> %wide.load162, ptr %next.gep161, align 2, !tbaa !16
  %index.next163 = add nuw i64 %index159, 4       ; 2 uses
  %8 = icmp eq i64 %index.next163, %n.vec158
  br i1 %8, label %middle.block152, label %vec.epilog.vector.body, !llvm.loop !39

middle.block152:                                  ; preds = %vec.epilog.vector.body
  %cmp.n153 = icmp eq i64 %i.cn, %n.vec158
  br i1 %cmp.n153, label %._crit_edge, label %.lr.ph84.preheader156

.lr.ph84.preheader156:                            ; preds = %.lr.ph84.preheader.a, %vec.epilog.iter.check, %middle.block152
  %.683.ph = phi ptr [ %.142.lcssa, %.lr.ph84.preheader.a ], [ %i.cq, %vec.epilog.iter.check ], [ %3, %middle.block152 ]
  %.ph = phi ptr [ %itemsetend.promoted82, %.lr.ph84.preheader.a ], [ %i.cs, %vec.epilog.iter.check ], [ %5, %middle.block152 ]
  br label %.lr.ph84
...
begin_hunk_3_@closure:bb.a
  ...
  br i1 %i.dl, label %.lr.ph71, label %.critedge.loopexit, !llvm.loop !40
...
begin_hunk_5_@closure:bb.a
  ...
  br i1 %i.dq, label %.lr.ph80, label %.preheader.loopexit, !llvm.loop !42
...
begin_hunk_6_@closure:bb.a
  ...
  br i1 %i.dv, label %.lr.ph84, label %._crit_edge, !llvm.loop !43

._crit_edge:                                      ; preds = %.lr.ph84, %middle.block152, %middle.block153
  %.lcssa = phi ptr [ %5, %middle.block152 ], [ %i.cs, %middle.block153 ], [ %i.du, %.lr.ph84 ]
...
!39 = distinct !{!39, !19, !26, !27}
!40 = distinct !{!40, !19}
!41 = distinct !{!41, !19}
!42 = distinct !{!42, !19}
!43 = distinct !{!43, !19, !26}
end_hunk_7
