...
._crit_edge:                                      ; preds = %bb.c, %bb.a
  ...
  %.fca.0.extract = extractvalue { <2 x float>, float } %i.l, 0 ; 3 uses
  %.fca.1.extract = extractvalue { <2 x float>, float } %i.l, 1
  %.sroa.012.4.vec.extract = extractelement <2 x float> %.fca.0.extract, i64 0
  %puts17 = tail call i32 @puts(ptr nonnull dereferenceable(1) @str.3) ; 0 uses
  %i.m = fpext float %.sroa.012.4.vec.extract to double
  %i.n = tail call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.5, double noundef %i.m) ; 0 uses
  %2 = shufflevector <2 x float> %.fca.0.extract, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %3 = insertelement <2 x float> %2, float %.fca.1.extract, i64 1
  %4 = shufflevector <2 x float> %.fca.0.extract, <2 x float> poison, <2 x i32> zeroinitializer
  %5 = fdiv <2 x float> %3, %4                    ; 2 uses
  %6 = extractelement <2 x float> %5, i64 0
  %i.o = fpext float %6 to double
