...
._crit_edge:                                      ; preds = %.lr.ph, %.lr.ph.prol.loopexit
  %spec.select.lcssa = phi i32 [ %spec.select.lcssa.unr, %.lr.ph.prol.loopexit ], [ %spec.select.1, %.lr.ph ] ; 3 uses
  %.phi.trans.insert = sext i32 %spec.select.lcssa to i64 ; 2 uses
...
.lr.ph167.preheader:                              ; preds = %._crit_edge
  %2 = zext i32 %spec.select.lcssa to i64
  %i.gt = trunc i64 %indvars.iv205 to i32
  %i.gu = xor i32 %i.gt, -1
  %i.gv = add i32 %spec.select.lcssa, %i.gu
...
bb.ad:                                            ; preds = %bb.ac
  ...
  %.192 = select i1 %i.ip, i32 %spec.select126, i32 %i.ht ; 2 uses
...
bb.af:                                            ; preds = %._crit_edge168, %bb.ac, %bb.ab
  %.2107 = phi i32 [ %i.ht, %bb.ac ], [ %.1106170, %bb.ab ], [ %.1106170, %._crit_edge168 ]
  %.293 = phi i32 [ %i.ht, %bb.ac ], [ %i.ht, %bb.ab ], [ %.091172, %._crit_edge168 ] ; 2 uses
...
.loopexit:                                        ; preds = %bb.af, %transpose.exit.thread, %bb.ad, %bb.ae
  ...
  %i.kh = icmp eq i32 %.3, %.0.i133148.neg
  %spec.store.select = select i1 %i.kh, i32 3, i32 %.3 ; 5 uses
  %i.ki = add i64 %i.kb, 1
