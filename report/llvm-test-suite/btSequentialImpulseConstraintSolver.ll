...
bb.bb:                                            ; preds = %bb.ba
  ...
  %5 = tail call float @llvm.fmuladd.f32(float %i.adj, float %i.adj, float %i.adi) ; 2 uses
  %sqrt.i = tail call float @llvm.sqrt.f32(float %5)
  %6 = fneg float %i.adf
  %7 = fdiv float 1.000000e+00, %sqrt.i           ; 3 uses
  %8 = fmul float %7, %6                          ; 2 uses
  %i.adk = fmul float %i.adj, %7                  ; 2 uses
  %9 = fneg float %i.tz
  %i.adl = insertelement <2 x float> poison, float %7, i64 0
  %i.adm = insertelement <2 x float> %i.adl, float %i.adk, i64 1
  %i.adn = insertelement <2 x float> poison, float %5, i64 0
  %i.ado = insertelement <2 x float> %i.adn, float %9, i64 1
  %i.adp = fmul <2 x float> %i.adm, %i.ado
  %i.adq = fmul float %i.tz, %8
...
bb.bc:                                            ; preds = %bb.ba
  ...
  %i.adt = fdiv float 1.000000e+00, %sqrt46.i     ; 2 uses
  %10 = insertelement <2 x float> poison, float %i.adt, i64 0
  %11 = shufflevector <2 x float> %10, <2 x float> poison, <2 x i32> zeroinitializer
  %12 = insertelement <2 x float> poison, float %i.tz, i64 0
  %13 = fneg <2 x float> %i.ua
  %14 = shufflevector <2 x float> %12, <2 x float> %13, <2 x i32> <i32 0, i32 2>
  %15 = fmul <2 x float> %11, %14                 ; 3 uses
...
_Z13btPlaneSpace1RK9btVector3RS_S2_.exit:         ; preds = %bb.bb, %bb.bc
  %.sink631 = phi float [ 0.000000e+00, %bb.bb ], [ %19, %bb.bc ]
  %.sink630 = phi float [ %8, %bb.bb ], [ %18, %bb.bc ]
  %.sink = phi float [ %i.adk, %bb.bb ], [ 0.000000e+00, %bb.bc ]
