...
bb.d:                                             ; preds = %.lr.ph, %_ZN20btAlignedObjectArrayI18btQuantizedBvhNodeE9push_backERKS0_.exit
  ...
  %i.aa = load float, ptr %i.w, align 4, !tbaa !42 ; 2 uses
  %6 = load float, ptr %i.y, align 4, !tbaa !42   ; 2 uses
  %i.ab = load <2 x float>, ptr %i.i, align 4, !tbaa !42
  %i.ac = load <2 x float>, ptr %i.x, align 4, !tbaa !42 ; 2 uses
  %i.ad = load <2 x float>, ptr %i.z, align 4, !tbaa !42 ; 2 uses
  %7 = load <4 x float>, ptr %3, align 16
  %8 = load <2 x float>, ptr %i.h, align 4, !tbaa !42
  %9 = fsub float %5, %i.aa
  %i.ae = fmul float %6, %9
  %i.af = shufflevector <2 x float> %8, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
  %i.ag = shufflevector <4 x float> %7, <4 x float> %i.af, <4 x i32> <i32 0, i32 4, i32 5, i32 poison>
  %i.ah = insertelement <4 x float> %i.ag, float %i.ae, i64 3 ; 2 uses
  %10 = shufflevector <2 x float> %i.ac, <2 x float> poison, <4 x i32> <i32 poison, i32 0, i32 1, i32 poison>
  %i.ai = insertelement <4 x float> %10, float 1.000000e+00, i64 3
  %i.aj = insertelement <4 x float> %i.ai, float %i.aa, i64 0 ; 2 uses
  %11 = fsub <4 x float> %i.ah, %i.aj
  %12 = fadd <4 x float> %i.ah, %i.aj
  %13 = shufflevector <4 x float> %11, <4 x float> %12, <4 x i32> <i32 0, i32 1, i32 2, i32 7>
  %14 = shufflevector <2 x float> %i.ad, <2 x float> poison, <4 x i32> <i32 poison, i32 0, i32 1, i32 poison>
  %15 = insertelement <4 x float> %14, float 1.000000e+00, i64 3
  %16 = insertelement <4 x float> %15, float %6, i64 0
  %i.ak = fmul <4 x float> %13, %16
  %i.al = fptoui <4 x float> %i.ak to <4 x i16>   ; 2 uses
  %i.am = fsub <2 x float> %i.ab, %i.ac
  %i.an = fmul <2 x float> %i.ad, %i.am
