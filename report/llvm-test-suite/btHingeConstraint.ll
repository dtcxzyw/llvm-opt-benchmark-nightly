...
bb.h:                                             ; preds = %bb.g, %_Z13btPlaneSpace1RK9btVector3RS_S2_.exit.i
  %.sroa.020.0.i = phi <2 x float> [ %.sroa.020.4.vec.insert.i, %_Z13btPlaneSpace1RK9btVector3RS_S2_.exit.i ], [ %i.dp, %bb.g ] ; 4 uses
  ...
  %5 = shufflevector <2 x float> %i.bf, <2 x float> %i.bg, <2 x i32> <i32 1, i32 3>
  ...
  %i.eg = load <2 x float>, ptr %2, align 4, !tbaa !27 ; 3 uses
  %6 = shufflevector <2 x float> %i.eg, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %7 = fmul <2 x float> %5, %6
  %i.eh = shufflevector <2 x float> %i.eg, <2 x float> poison, <2 x i32> zeroinitializer
  %i.ei = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.eb, <2 x float> %i.eh, <2 x float> %7)
  ...
  %i.fn = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.fi, <4 x float> %i.fm, <4 x float> %i.ew) ; 6 uses
  ...
  %8 = extractelement <4 x float> %i.fn, i64 1
  %9 = extractelement <4 x float> %i.fn, i64 2    ; 2 uses
  %10 = extractelement <4 x float> %i.fn, i64 0   ; 2 uses
  %11 = shufflevector <2 x float> %.sroa.526.0.i, <2 x float> %.sroa.020.0.i, <4 x i32> <i32 1, i32 1, i32 1, i32 3>
  %12 = insertelement <4 x float> %i.fn, float %i.dx, i64 3
  %13 = fmul <4 x float> %11, %12                 ; 4 uses
  %i.fq = extractelement <4 x float> %13, i64 3
  %14 = tail call float @llvm.fmuladd.f32(float %i.fe, float %.sroa.075.0, float %i.fq)
  %i.fr = tail call float @llvm.fmuladd.f32(float %i.dv, float %.sroa.11.0, float %14) ; 3 uses
  %i.fs = extractelement <4 x float> %13, i64 1
  %i.ft = tail call float @llvm.fmuladd.f32(float %i.fr, float %i.fe, float %i.fs)
  %15 = tail call float @llvm.fmuladd.f32(float %9, float %i.dv, float %i.ft)
  %i.fu = tail call float @llvm.fmuladd.f32(float %10, float %.sroa.059.4.vec.extract, float %15) ; 3 uses
  %16 = extractelement <4 x float> %13, i64 2
  %i.fv = tail call float @llvm.fmuladd.f32(float %i.fr, float %i.dw, float %16)
  %17 = extractelement <4 x float> %13, i64 0
  %i.fw = tail call float @llvm.fmuladd.f32(float %i.fr, float %i.dv, float %17)
  %i.fx = tail call float @llvm.fmuladd.f32(float %8, float %i.dw, float %i.fw)
  %i.fy = tail call float @llvm.fmuladd.f32(float %9, float %.sroa.059.0.vec.extract, float %i.fx) ; 3 uses
  %i.fz = fneg float %i.fy
  %i.ga = fmul float %i.ch, %i.fz
  %i.gb = tail call float @llvm.fmuladd.f32(float %10, float %i.fe, float %i.fv)
