...
vector.body842:                                   ; preds = %vector.body842, %vector.ph840
  ...
  %vec.phi844 = phi <4 x i32> [ %i.azg, %vector.ph840 ], [ %i.azv, %vector.body842 ]
  %vec.phi845 = phi <4 x i32> [ zeroinitializer, %vector.ph840 ], [ %i.azw, %vector.body842 ]
  %vec.phi846 = phi <4 x i32> [ <i32 1, i32 2, i32 3, i32 4>, %vector.ph840 ], [ %vec.ind.next855, %vector.body842 ] ; 3 uses
  %step.add847 = add <4 x i32> %vec.phi846, splat (i32 4)
  ...
  %i.azt = mul nsw <4 x i32> %i.azr, %vec.phi846
  %i.azu = mul nsw <4 x i32> %i.azs, %step.add847
  %i.azv = add <4 x i32> %i.azt, %vec.phi844      ; 2 uses
  %i.azw = add <4 x i32> %i.azu, %vec.phi845      ; 2 uses
  %index.next854 = add nuw i64 %index843, 8       ; 2 uses
  %vec.ind.next855 = add <4 x i32> %vec.phi846, splat (i32 8)
...
vector.body825:                                   ; preds = %vector.body825, %vector.ph823
  ...
  %vec.phi = phi <4 x i32> [ %i.ban, %vector.ph823 ], [ %i.bbc, %vector.body825 ]
  %vec.phi.a = phi <4 x i32> [ zeroinitializer, %vector.ph823 ], [ %i.bbd, %vector.body825 ]
  %vec.phi828 = phi <4 x i32> [ <i32 1, i32 2, i32 3, i32 4>, %vector.ph823 ], [ %vec.ind.next834, %vector.body825 ] ; 3 uses
  %step.add = add <4 x i32> %vec.phi828, splat (i32 4)
  ...
  %i.bba = mul nsw <4 x i32> %i.bay, %vec.phi828
  %i.bbb = mul nsw <4 x i32> %i.baz, %step.add
  %i.bbc = add <4 x i32> %i.bba, %vec.phi         ; 2 uses
  %i.bbd = add <4 x i32> %i.bbb, %vec.phi.a       ; 2 uses
  %index.next833 = add nuw i64 %index826, 8       ; 2 uses
  %vec.ind.next834 = add <4 x i32> %vec.phi828, splat (i32 8)
