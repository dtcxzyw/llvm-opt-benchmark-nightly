...
.preheader274.i:                                  ; preds = %_ZN3jxl6detailL13WriteICCUint8EhmPNSt3__16vectorIhNS1_9allocatorIhEEEE.exit88.2.i
  ...
  %43 = extractelement <2 x double> %i.bdt, i64 1
  %44 = call double @llvm.fmuladd.f64(double %43, double f0x3FA3EAB360000000, double 0.000000e+00)
  %45 = fadd <2 x double> %i.bdv, <double f0x3FFB6A7F00000000, double f0x3FD10CB2A0000000>
  %46 = fadd double %44, f0xBFB1893740000000
  %47 = shufflevector <2 x double> %i.bdt, <2 x double> poison, <2 x i32> zeroinitializer
  %48 = call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %47, <2 x double> <double f0x3FA2CA57A0000000, double f0xBFC4A8C160000000>, <2 x double> %45)
  %49 = extractelement <2 x double> %i.bdt, i64 0
  %50 = call double @llvm.fmuladd.f64(double %49, double f0x3FF0793DE0000000, double %46)
  %51 = fptrunc <2 x double> %48 to <2 x float>   ; 3 uses
  %i.bdw = fptrunc double %50 to float            ; 2 uses
  %i.bdx = fcmp oeq <2 x float> %51, zeroinitializer
  ...
  %i.bea = extractelement <2 x float> %51, i64 0
  %i.beb = fdiv float f0x3F829D5E, %i.bea         ; 2 uses
  %52 = shufflevector <2 x float> %51, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %i.bec = insertelement <2 x float> %52, float %i.bdw, i64 1
  %i.bed = fdiv <2 x float> <float f0x3F7F0C7F, float f0x3F5192AD>, %i.bec ; 3 uses
  %53 = extractelement <2 x float> %i.bed, i64 0
  %54 = call float @llvm.fabs.f32(float %53)
  %55 = fcmp ueq float %54, +inf
  %i.bee = call float @llvm.fabs.f32(float %i.beb)
  %i.bef = fcmp ueq float %i.bee, +inf
  %or.cond56.i.i = or i1 %55, %i.bef
  %i.beg = extractelement <2 x float> %i.bed, i64 1
  %i.beh = call float @llvm.fabs.f32(float %i.beg)
  %i.bei = fcmp ueq float %i.beh, +inf
  %or.cond58.i.i = or i1 %i.bei, %or.cond56.i.i
  %56 = fpext float %i.beb to double              ; 3 uses
  %i.bej = fmul nnan double %56, f0x3FE801A360000000
  %i.bek = fmul nnan double %56, f0x3FFB6A7F00000000
  %i.bel = fadd nnan double %i.bek, 0.000000e+00
  %i.bem = fptrunc nnan double %i.bel to float
  %57 = fpext <2 x float> %i.bed to <2 x double>  ; 4 uses
  %58 = extractelement <2 x double> %57, i64 0
  %59 = call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %57, <2 x double> <double f0x3FD10CB2A0000000, double f0xBFB1893740000000>, <2 x double> zeroinitializer)
  %60 = fptrunc <2 x double> %59 to <2 x float>   ; 2 uses
  %61 = call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %57, <2 x double> <double f0xBFC4A8C160000000, double f0x3FA3EAB360000000>, <2 x double> zeroinitializer) ; 2 uses
  %62 = extractelement <2 x double> %61, i64 1
  %63 = fptrunc double %62 to float
  %64 = extractelement <2 x double> %61, i64 0
  %65 = fadd double %64, 0.000000e+00
  %66 = fptrunc double %65 to float
  %67 = fmul nnan double %56, f0x3FA2CA57A0000000
  %68 = fadd double %67, 0.000000e+00
  %69 = fptrunc double %68 to float
  %i.ben = fpext nnan float %i.bem to double
  %i.beo = fmul nnan double %58, f0x3FECA4A8C0000000
  %70 = fsub double 0.000000e+00, %i.bej
  %i.bep = insertelement <2 x double> poison, double %70, i64 0
  %i.beq = insertelement <2 x double> %i.bep, double %i.beo, i64 1
  %71 = fadd <2 x double> %i.beq, <double -0.000000e+00, double 0.000000e+00>
  %i.ber = fptrunc <2 x double> %71 to <2 x float>
  %72 = fpext <2 x float> %i.ber to <2 x double>  ; 2 uses
  %73 = shufflevector <2 x double> %72, <2 x double> poison, <4 x i32> zeroinitializer
  %74 = fmul nnan <4 x double> %73, <double f0xBFC2D2ACE0000000, double f0x3FE0966860000000, double f0x3FA4807D60000000, double f0xBFC2D2ACE0000000>
  %75 = shufflevector <2 x double> %72, <2 x double> poison, <4 x i32> <i32 1, i32 1, i32 1, i32 1>
  %76 = call <4 x double> @llvm.fmuladd.v4f64(<4 x double> %75, <4 x double> <double f0x3FEF957220000000, double f0x3FDBAAE3E0000000, double f0xBF81777EC0000000, double f0x3FEF957220000000>, <4 x double> %74)
  %77 = extractelement <2 x float> %60, i64 0
  %78 = fpext float %77 to double
  %i.bes = extractelement <2 x float> %60, i64 1
  %79 = fpext float %i.bes to double
  %i.bet = fpext float %63 to double
  ...
  %i.bew = call <4 x double> @llvm.fmuladd.v4f64(<4 x double> %i.bev, <4 x double> <double f0x3FC479A860000000, double f0x3FA93CB240000000, double f0x3FEEFDD7E0000000, double f0x3FC479A860000000>, <4 x double> %76)
  %i.bex = fptrunc <4 x double> %i.bew to <4 x float>
  %80 = insertelement <4 x double> poison, double %i.ben, i64 0
  %81 = shufflevector <4 x double> %80, <4 x double> poison, <4 x i32> zeroinitializer
  %82 = fmul nnan <4 x double> %81, <double f0xBFC2D2ACE0000000, double f0x3FE0966860000000, double f0x3FA4807D60000000, double f0xBFC2D2ACE0000000>
  %i.bey = insertelement <4 x double> poison, double %78, i64 0
  %i.bez = shufflevector <4 x double> %i.bey, <4 x double> poison, <4 x i32> zeroinitializer
  %i.bfa = call <4 x double> @llvm.fmuladd.v4f64(<4 x double> %i.bez, <4 x double> <double f0x3FEF957220000000, double f0x3FDBAAE3E0000000, double f0xBF81777EC0000000, double f0x3FEF957220000000>, <4 x double> %82)
  %i.bfb = insertelement <4 x double> poison, double %79, i64 0
  ...
  %83 = fpext float %69 to double                 ; 3 uses
  %84 = fmul nnan double %83, f0xBFC2D2ACE0000000
  %85 = shufflevector <2 x double> %57, <2 x double> poison, <2 x i32> <i32 1, i32 poison>
  %86 = insertelement <2 x double> <double 0.000000e+00, double poison>, double %84, i64 1
  %i.bff = fmul nnan double %83, f0x3FE0966860000000
  %i.bfg = fmul nnan double %83, f0x3FA4807D60000000
  %i.bfh = fpext float %66 to double              ; 2 uses
  %i.bfi = insertelement <2 x double> %85, double %i.bfh, i64 1
  %i.bfj = call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.bfi, <2 x double> <double f0x3FF0793DE0000000, double f0x3FEF957220000000>, <2 x double> %86) ; 2 uses
