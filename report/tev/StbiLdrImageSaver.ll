...
.lr.ph.split.us.preheader:                        ; preds = %.lr.ph
  ...
  %min.iters.check299 = icmp ugt i32 %1, 11
  %ident.check256.not = icmp eq i32 %2, 1
  %or.cond316 = and i1 %min.iters.check299, %ident.check256.not
  br i1 %or.cond316, label %vector.memcheck257, label %.lr.ph.split.us.preheader322
...
vector.memcheck257:                               ; preds = %.lr.ph.split.us.preheader
  ...
  %i.ap = shl nuw nsw i64 %i.n, 2
  %i.aq = getelementptr i8, ptr %4, i64 %i.ap
  %scevgep261 = getelementptr i8, ptr %i.aq, i64 8 ; 4 uses
...
vector.body302:                                   ; preds = %vector.body302, %vector.ph300
  %index303 = phi i64 [ 0, %vector.ph300 ], [ %index.next311, %vector.body302 ] ; 6 uses
  %i.ar = getelementptr [4 x i8], ptr %4, i64 %index303 ; 3 uses
  %i.as = getelementptr i8, ptr %i.ar, i64 8
  %wide.load304 = load <4 x float>, ptr %i.as, align 4, !tbaa !61, !alias.scope !295 ; 3 uses
  %i.at = getelementptr i8, ptr %i.ar, i64 4
  %wide.load305 = load <4 x float>, ptr %i.at, align 4, !tbaa !61, !alias.scope !295 ; 3 uses
  %wide.load306 = load <4 x float>, ptr %i.ar, align 4, !tbaa !61, !alias.scope !295 ; 3 uses
  %i.au = fcmp ogt <4 x float> %wide.load305, %wide.load304
  %i.av = select <4 x i1> %i.au, <4 x float> %wide.load305, <4 x float> %wide.load304 ; 2 uses
  %i.aw = fcmp ogt <4 x float> %wide.load306, %i.av
  %i.ax = select <4 x i1> %i.aw, <4 x float> %wide.load306, <4 x float> %i.av ; 3 uses
  ...
  %i.be = fmul <4 x float> %wide.load306, %i.bd
  %i.bf = fptoui <4 x float> %i.be to <4 x i8>
  %i.bg = fmul <4 x float> %wide.load305, %i.bd
  %i.bh = fptoui <4 x float> %i.bg to <4 x i8>
  %i.bi = fmul <4 x float> %wide.load304, %i.bd
