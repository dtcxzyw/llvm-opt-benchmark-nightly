...
bb.e:                                             ; preds = %bb.a
  ...
  %i.m = fmul nnan double %i.h, 5.870000e-01      ; 2 uses
  %3 = fneg double %i.m
  %4 = uitofp nneg i32 %i.l to double
  %5 = insertelement <2 x i8> poison, i8 %i.e, i64 0
  %6 = insertelement <2 x i8> %5, i8 %i.i, i64 1
  %7 = uitofp <2 x i8> %6 to <2 x double>         ; 4 uses
  %8 = extractelement <2 x double> %7, i64 1
  %9 = tail call double @llvm.fmuladd.f64(double %8, double 2.990000e-01, double %i.m)
  %10 = extractelement <2 x double> %7, i64 0
  %11 = tail call double @llvm.fmuladd.f64(double %10, double 1.140000e-01, double %9)
  ...
  %i.n = insertelement <2 x double> poison, double %3, i64 0
  %15 = shufflevector <2 x double> %i.n, <2 x double> poison, <2 x i32> zeroinitializer
  %16 = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %14, <2 x double> <double -2.990000e-01, double 7.010000e-01>, <2 x double> %15)
  %i.o = shufflevector <2 x double> %7, <2 x double> poison, <2 x i32> zeroinitializer
  %i.p = tail call <2 x double> @llvm.fmuladd.v2f64(<2 x double> %i.o, <2 x double> <double 8.860000e-01, double -1.140000e-01>, <2 x double> %16)
  %i.q = fdiv <2 x double> %i.p, <double 1.772000e+00, double 1.402000e+00>
  %i.r = icmp slt i32 %13, 0
  %spec.select3.i = tail call i32 @llvm.umin.i32(i32 %13, i32 255)
  %spec.select.i = trunc nuw i32 %spec.select3.i to i8
  %i.s = select i1 %i.r, i8 0, i8 %spec.select.i
  store i8 %i.s, ptr %0, align 1, !tbaa !55
  ...
  %i.x = fadd <2 x double> %i.w, splat (double 5.000000e-01)
  %i.y = fptosi <2 x double> %i.x to <2 x i32>    ; 2 uses
  %i.z = icmp slt <2 x i32> %i.y, zeroinitializer
  %i.aa = tail call <2 x i32> @llvm.umin.v2i32(<2 x i32> %i.y, <2 x i32> splat (i32 255))
  %i.ab = trunc nuw <2 x i32> %i.aa to <2 x i8>
  %i.ac = select <2 x i1> %i.z, <2 x i8> zeroinitializer, <2 x i8> %i.ab ; 2 uses
  %i.ad = extractelement <2 x i8> %i.ac, i64 0
  store i8 %i.ad, ptr %i.t, align 1, !tbaa !55
  %i.ae = getelementptr inbounds nuw i8, ptr %0, i64 2
  %i.af = extractelement <2 x i8> %i.ac, i64 1
  store i8 %i.af, ptr %i.ae, align 1, !tbaa !55
  ret void
}
