...
bb.g:                                             ; preds = %If_Dec7Cofactor.exit28, %bb.e, %bb.f
  %.120 = phi i32 [ %i.fh, %bb.f ], [ %.01967, %bb.e ], [ %.01967, %If_Dec7Cofactor.exit28 ] ; 2 uses
...
.preheader:                                       ; preds = %.preheader.preheader, %bb.af
  ...
  %.02998 = phi i64 [ %0, %.preheader.preheader ], [ %.0.lcssa.i, %bb.af ] ; 2 uses
...
.lr.ph.i:                                         ; preds = %.preheader, %.lr.ph.i
  ...
  %.029.i = phi i64 [ %i.ah, %.lr.ph.i ], [ %.02998, %.preheader ] ; 3 uses
...
If_Dec6MoveTo.exit:                               ; preds = %.lr.ph.i, %.preheader
  %.0.lcssa.i = phi i64 [ %.02998, %.preheader ], [ %i.ah, %.lr.ph.i ] ; 3 uses
...
bb.a:                                             ; preds = %If_Dec6MoveTo.exit, %.loopexit
  %.197 = phi i32 [ 0, %If_Dec6MoveTo.exit ], [ %i.ct, %.loopexit ] ; 23 uses
  %i.ct = add nuw nsw i32 %.197, 1                ; 3 uses
  %i.cu = icmp samesign ult i32 %.197, 3
  br i1 %i.cu, label %.lr.ph, label %.loopexit

.lr.ph:                                           ; preds = %bb.a
  %i.cv = icmp eq i32 %.197, 0                    ; 3 uses
  %i.cw = lshr i32 2, %.197                       ; 2 uses
  %i.cx = lshr i32 3, %.197                       ; 2 uses
  %i.cy = lshr i32 4, %.197                       ; 2 uses
  %i.cz = lshr i32 5, %.197                       ; 2 uses
  %i.da = lshr i32 6, %.197                       ; 2 uses
  %i.db = lshr i32 7, %.197                       ; 2 uses
  %i.dc = lshr exact i32 8, %.197                 ; 2 uses
  %i.dd = lshr i32 9, %.197                       ; 2 uses
  %i.de = lshr i32 10, %.197                      ; 2 uses
  %i.df = lshr i32 11, %.197                      ; 2 uses
  %i.dg = lshr i32 12, %.197                      ; 2 uses
  %i.dh = lshr i32 13, %.197                      ; 2 uses
  %i.di = lshr i32 14, %.197                      ; 2 uses
...
vector.ph:                                        ; preds = %.lr.ph, %vector.early.exit
  %.02596 = phi i32 [ %i.ct, %.lr.ph ], [ %i.vu, %vector.early.exit ] ; 18 uses
  call void @llvm.lifetime.start.p0(ptr nonnull %i.a)
  store i64 %0, ptr %i.a, align 8, !tbaa !12
  %i.fb = icmp eq i32 %.02596, 1                  ; 3 uses
