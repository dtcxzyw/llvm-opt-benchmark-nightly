...
bb.g:                                             ; preds = %bb.f
  ...
  %i.cp = extractelement <2 x float> %i.cl, i64 1
  ...
  %i.ed = fdiv <4 x float> %i.dz, %i.ec           ; 2 uses
  %7 = shufflevector <4 x float> %i.ed, <4 x float> poison, <2 x i32> <i32 0, i32 1>
  %8 = shufflevector <2 x float> %i.ci, <2 x float> %i.cj, <2 x i32> <i32 0, i32 2>
  %9 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %7, <2 x float> %i.cl, <2 x float> %8) ; 2 uses
  %i.ee = extractelement <2 x float> %9, i64 0
  %10 = extractelement <2 x float> %9, i64 1
  %i.ef = select i1 %i.cx, float %i.ee, float %10
...
bb.b:                                             ; preds = %bb.a
  ...
  %i.q = load float, ptr %i.p, align 4, !tbaa !70 ; 4 uses
  %5 = load <2 x float>, ptr %i.f, align 8, !tbaa !70 ; 6 uses
  %6 = extractelement <2 x float> %5, i64 1       ; 13 uses
  %7 = extractelement <2 x float> %5, i64 0       ; 8 uses
  %8 = load <2 x float>, ptr %i.i, align 8, !tbaa !70 ; 4 uses
  %9 = load <2 x float>, ptr %i.l, align 8, !tbaa !70 ; 7 uses
...
bb.c:                                             ; preds = %bb.b
  %10 = insertelement <2 x float> poison, float %i.q, i64 0
  %11 = shufflevector <2 x float> %10, <2 x float> poison, <2 x i32> zeroinitializer
  %12 = fsub <2 x float> %11, %5                  ; 2 uses
  %i.v = fsub float %6, %7                        ; 2 uses
  %13 = extractelement <2 x float> %12, i64 0
  %i.w = fdiv float %13, %i.v                     ; 2 uses
  ...
  %i.z = tail call float @llvm.fmuladd.f32(float %i.y, float %15, float %14)
  %16 = extractelement <2 x float> %8, i64 0
  %i.aa = tail call float @llvm.fmuladd.f32(float %i.x, float %i.z, float %16)
  %i.ab = fcmp olt float %i.q, %7
  %17 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %12, <2 x float> %9, <2 x float> %8) ; 2 uses
  %18 = extractelement <2 x float> %17, i64 0
  %19 = select i1 %i.ab, float %18, float %i.aa
  %i.ac = fcmp olt float %i.q, %6
  %20 = extractelement <2 x float> %17, i64 1
  %i.ad = select i1 %i.ac, float %19, float %20
...
bb.e:                                             ; preds = %bb.d
  %..i = select i1 %3, float %6, float %7         ; 2 uses
  %i.af = fsub float %i.q, %..i
  %21 = extractelement <2 x float> %9, i64 0      ; 3 uses
  %22 = extractelement <2 x float> %9, i64 1      ; 3 uses
  %i.ag = fsub float %22, %21                     ; 2 uses
  %i.ah = fmul float %i.ag, 5.000000e-01
  %i.ai = fsub float %6, %7                       ; 6 uses
  %i.aj = fmul float %i.ai, %i.ah
  %i.ak = fmul float %i.ai, %21                   ; 3 uses
  %i.al = fmul float %i.aj, 4.000000e+00
  %23 = extractelement <2 x float> %8, i64 0      ; 3 uses
  %i.am = tail call float @llvm.fmuladd.f32(float %i.af, float %i.r, float %..i) ; 6 uses
  %i.an = fsub float %i.am, %23
  %i.ao = fsub float %23, %i.am                   ; 2 uses
  ...
  %i.av = insertelement <2 x float> %i.au, float %i.an, i64 1
  %i.aw = fmul <2 x float> %i.av, <float -2.000000e+00, float 1.000000e+00>
  %24 = shufflevector <2 x float> %9, <2 x float> poison, <2 x i32> <i32 poison, i32 0>
  %i.ax = insertelement <2 x float> %24, float %i.at, i64 0
  %i.ay = fdiv <2 x float> %i.aw, %i.ax           ; 2 uses
  %i.az = extractelement <2 x float> %i.ay, i64 0
  %i.ba = tail call float @llvm.fmuladd.f32(float %i.az, float %i.ai, float %7)
  %shift11 = shufflevector <2 x float> %i.ay, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop12 = fadd <2 x float> %5, %shift11
  %25 = extractelement <2 x float> %foldExtExtBinop12, i64 0
  %i.bb = fcmp olt float %i.am, %23
  %i.bc = select i1 %i.bb, float %25, float %i.ba ; 2 uses
  br i1 %3, label %bb.g, label %bb.f

bb.f:                                             ; preds = %bb.e
  %i.bd = fsub float %i.bc, %7
  ...
  %i.bj = shufflevector <2 x float> %5, <2 x float> poison, <2 x i32> zeroinitializer ; 2 uses
  %i.bk = fadd <2 x float> %i.bj, %i.bi           ; 2 uses
  %i.bl = tail call float @llvm.fmuladd.f32(float %i.ai, float 9.900000e-01, float %7) ; 2 uses
  %i.bm = fsub float %i.bl, %7
  %i.bn = fmul float %i.bm, %i.ag
  %i.bo = fdiv float %i.bn, %i.ai
  %i.bp = fadd float %21, %i.bo
  %i.bq = fdiv float 1.000000e+00, %i.bp
  %i.br = fdiv float 1.000000e+00, %22            ; 2 uses
  %i.bs = fsub float %6, %i.bl
  %i.bt = fneg float %6
  %i.bu = insertelement <2 x float> poison, float %i.br, i64 0
  %i.bv = insertelement <2 x float> %i.bu, float %i.am, i64 1
  %i.bw = insertelement <2 x float> %i.bj, float %i.bq, i64 0
  ...
  %i.ce = tail call float @llvm.fmuladd.f32(float %i.cd, float %6, float %i.br) ; 2 uses
  %i.cf = fneg float %i.ce
  %i.cg = extractelement <2 x float> %i.bk, i64 1
  %i.ch = tail call float @llvm.fmuladd.f32(float %i.cf, float %6, float %i.cg)
  %i.ci = fmul float %i.cc, %i.bt
  %i.cj = tail call float @llvm.fmuladd.f32(float %i.ci, float %6, float %i.ch)
  %shift14 = shufflevector <2 x float> %i.cb, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop15 = fadd <2 x float> %5, %shift14
  %26 = extractelement <2 x float> %foldExtExtBinop15, i64 0 ; 3 uses
  ...
  %i.cm = fcmp olt float %26, %6
  %i.cn = extractelement <2 x float> %i.bk, i64 0
  %i.co = select i1 %i.cm, float %i.cn, float %i.cl
  br label %.sink.split.i

bb.g:                                             ; preds = %bb.e
  %27 = extractelement <2 x float> %8, i64 1      ; 2 uses
  %i.cp = fsub float %i.am, %27
  %i.cq = fdiv float %i.cp, %22
  %i.cr = fadd float %6, %i.cq
  %i.cs = fcmp olt float %i.am, %27
  %i.ct = select i1 %i.cs, float %i.bc, float %i.cr
  %i.cu = fsub float %i.ct, %6
  %i.cv = fdiv float %i.cu, %i.r
  %i.cw = fadd float %6, %i.cv
  br label %.sink.split.i
...
bb.h:                                             ; preds = %bb.a
  %28 = getelementptr inbounds nuw i8, ptr %i.l, i64 4
  %29 = getelementptr inbounds nuw i8, ptr %i.i, i64 4
  %30 = getelementptr inbounds nuw i8, ptr %i.f, i64 4
...
bb.d:                                             ; preds = %bb.c
  ...
  %i.af = extractelement <2 x float> %i.ab, i64 1
  ...
  %i.bt = fdiv <4 x float> %i.bp, %i.bs           ; 2 uses
  %7 = shufflevector <4 x float> %i.bt, <4 x float> poison, <2 x i32> <i32 0, i32 1>
  %8 = shufflevector <2 x float> %i.y, <2 x float> %i.z, <2 x i32> <i32 0, i32 2>
  %9 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %7, <2 x float> %i.ab, <2 x float> %8) ; 2 uses
  %i.bu = extractelement <2 x float> %9, i64 0
  %10 = extractelement <2 x float> %9, i64 1
  %i.bv = select i1 %i.an, float %i.bu, float %10
