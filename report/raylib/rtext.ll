...
begin_hunk_0_@LoadFontDefault:bb.a
  ...
  store float 1.000000e+01, ptr %i.ap, align 4
  %i.aq = fadd float %i.an, 1.000000e+00
  %i.ar = fptosi float %i.aq to i32
  %i.as = add nsw i32 %.05166, %i.ar              ; 2 uses
  %i.at = icmp sgt i32 %i.as, 127
  br i1 %i.at, label %bb.k, label %bb.l

bb.k:                                             ; preds = %.lr.ph
  %i.au = getelementptr inbounds nuw i8, ptr %i.af, i64 4
  %i.av = add nsw i32 %.05465, 1                  ; 2 uses
  %i.aw = add nsw i32 %i.am, 2
  store float 1.000000e+00, ptr %i.af, align 4
  %i.ax = mul nsw i32 %i.av, 11
  %i.ay = add nsw i32 %i.ax, 1
  %i.az = sitofp i32 %i.ay to float
  store float %i.az, ptr %i.au, align 4
  br label %bb.l

bb.l:                                             ; preds = %.lr.ph, %bb.k
  %.155 = phi i32 [ %i.av, %bb.k ], [ %.05465, %.lr.ph ]
  %.1 = phi i32 [ %i.aw, %bb.k ], [ %i.as, %.lr.ph ]
  ...
  %i.bc = getelementptr inbounds nuw i8, ptr %i.ab, i64 12
  ...
  %i.be = load <2 x float>, ptr %i.af, align 4
  %i.bf = load <2 x float>, ptr %i.ao, align 4
  call void @ImageFromImage(ptr dead_on_unwind nonnull writable sret(%struct.Image) align 8 %2, ptr noundef nonnull byval(%struct.Image) align 8 %0, <2 x float> %i.be, <2 x float> %i.bf) #39
  ...
  %i.bg = load i32, ptr getelementptr inbounds nuw (i8, ptr @defaultFont, i64 4), align 4
  %i.bh = sext i32 %i.bg to i64
  %i.bi = icmp slt i64 %indvars.iv.next77, %i.bh
...
begin_hunk_1_@GenImageFontAtlas:bb.a
  br i1 %cmp.n377, label %._crit_edge281, label %vec.epilog.iter.check

vec.epilog.iter.check:                            ; preds = %middle.block376
  %min.epilog.iters.check = icmp eq i64 %i.ka, 0
  br i1 %min.epilog.iters.check, label %vec.epilog.scalar.ph.preheader, label %vec.epilog.ph, !prof !20

vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  %vec.epilog.resume.val = phi i64 [ %n.vec370, %vec.epilog.iter.check ], [ 0, %vector.main.loop.iter.check ]
  %n.vec379 = and i64 %wide.trip.count328, 2147483644 ; 4 uses
  %i.kj = shl nuw nsw i64 %n.vec379, 1
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index380 = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next383, %vec.epilog.vector.body ] ; 3 uses
  %i.kk = shl nuw i64 %index380, 1
  %i.kl = getelementptr inbounds nuw i8, ptr %i.jx, i64 %i.kk
  %i.km = getelementptr inbounds nuw i8, ptr %.pre332, i64 %index380
  %wide.load381 = load <4 x i8>, ptr %i.km, align 1, !alias.scope !52
  %interleaved.vec382 = shufflevector <4 x i8> splat (i8 -1), <4 x i8> %wide.load381, <8 x i32> <i32 0, i32 4, i32 1, i32 5, i32 2, i32 6, i32 3, i32 7>
  store <8 x i8> %interleaved.vec382, ptr %i.kl, align 1, !alias.scope !55, !noalias !52
  %index.next383 = add nuw i64 %index380, 4       ; 2 uses
  %i.kn = icmp eq i64 %index.next383, %n.vec379
  br i1 %i.kn, label %vec.epilog.middle.block, label %vec.epilog.vector.body, !llvm.loop !58

vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  %cmp.n384 = icmp eq i64 %n.vec379, %wide.trip.count328
  br i1 %cmp.n384, label %._crit_edge281, label %vec.epilog.scalar.ph.preheader

vec.epilog.scalar.ph.preheader:                   ; preds = %vector.memcheck, %iter.check, %vec.epilog.iter.check, %vec.epilog.middle.block
  %indvars.iv323.ph = phi i64 [ 0, %iter.check ], [ 0, %vector.memcheck ], [ %n.vec370, %vec.epilog.iter.check ], [ %n.vec379, %vec.epilog.middle.block ] ; 3 uses
  %indvars.iv321.ph = phi i64 [ 0, %iter.check ], [ 0, %vector.memcheck ], [ %i.kb, %vec.epilog.iter.check ], [ %i.kj, %vec.epilog.middle.block ] ; 2 uses
  %xtraiter408 = and i64 %wide.trip.count328, 3   ; 2 uses
  %lcmp.mod409.not = icmp eq i64 %xtraiter408, 0
  br i1 %lcmp.mod409.not, label %vec.epilog.scalar.ph.prol.loopexit, label %vec.epilog.scalar.ph.prol

vec.epilog.scalar.ph.prol:                        ; preds = %vec.epilog.scalar.ph.preheader, %vec.epilog.scalar.ph.prol
  %indvars.iv323.prol = phi i64 [ %indvars.iv.next324.prol, %vec.epilog.scalar.ph.prol ], [ %indvars.iv323.ph, %vec.epilog.scalar.ph.preheader ] ; 2 uses
  ...
  %prol.iter.next = add i64 %prol.iter, 1         ; 2 uses
  %prol.iter.cmp.not = icmp eq i64 %prol.iter.next, %xtraiter408
...
._crit_edge281:                                   ; preds = %vec.epilog.scalar.ph.prol.loopexit, %vec.epilog.scalar.ph, %middle.block376, %vec.epilog.middle.block, %.loopexit
...
vec.epilog.scalar.ph:                             ; preds = %vec.epilog.scalar.ph.prol.loopexit, %vec.epilog.scalar.ph
  ...
  br i1 %exitcond329.not.3, label %._crit_edge281, label %vec.epilog.scalar.ph, !llvm.loop !60
...
.lr.ph.prol:                                      ; preds = %.lr.ph.preheader
  ...
  %.2132.prol = add i32 %i.cs, %i.by              ; 2 uses
  %i.ct = add nsw i16 %i.bk, -1
  br label %.lr.ph.prol.loopexit

.lr.ph.prol.loopexit:                             ; preds = %.lr.ph.prol, %.lr.ph.preheader
  %.2132.lcssa.unr = phi i32 [ poison, %.lr.ph.preheader ], [ %.2132.prol, %.lr.ph.prol ]
  %.0122.in10.unr = phi i16 [ %i.ce, %.lr.ph.preheader ], [ %.0122.prol, %.lr.ph.prol ]
  %.01239.unr = phi i16 [ %i.bk, %.lr.ph.preheader ], [ %i.ct, %.lr.ph.prol ]
  %.11318.unr = phi i32 [ %i.by, %.lr.ph.preheader ], [ %.2132.prol, %.lr.ph.prol ]
...
.lr.ph:                                           ; preds = %.lr.ph.prol.loopexit, %.lr.ph
  ...
  %.2132 = add i32 %i.dh, %.11318                 ; 2 uses
  %.0122.1 = lshr i16 %.0122.in10, 2              ; 2 uses
  %i.di = zext i32 %.2132 to i64
  ...
  %.2132.1 = add i32 %i.du, %.2132                ; 2 uses
  %i.dv = add i16 %.01239, -2                     ; 2 uses
  %.not142.1 = icmp eq i16 %i.dv, 0
  br i1 %.not142.1, label %._crit_edge, label %.lr.ph

._crit_edge:                                      ; preds = %.lr.ph.prol.loopexit, %.lr.ph, %bb.h
  %.1131.lcssa = phi i32 [ %i.by, %bb.h ], [ %.2132.lcssa.unr, %.lr.ph.prol.loopexit ], [ %.2132.1, %.lr.ph ]
...
bb.l:                                             ; preds = %._crit_edge, %bb.k, %bb.j
  %.2.shrunk = phi i32 [ %i.gb, %bb.k ], [ %i.fn, %bb.j ], [ 0, %._crit_edge ]
...
bb.o:                                             ; preds = %.lr.ph14, %bb.r
  %.012412 = phi i32 [ %i.gg, %.lr.ph14 ], [ %.2126, %bb.r ] ; 2 uses
  %.012711 = phi i32 [ 0, %.lr.ph14 ], [ %.2129, %bb.r ] ; 3 uses
  %i.gj = sub nsw i32 %.012412, %.012711
  %i.gk = lshr i32 %i.gj, 1
  %i.gl = add nuw nsw i32 %i.gk, %.012711         ; 3 uses
...
bb.r:                                             ; preds = %bb.q, %bb.o
  %.2129 = phi i32 [ %.012711, %bb.o ], [ %i.gw, %bb.q ] ; 2 uses
  %.2126 = phi i32 [ %i.gl, %bb.o ], [ %.012412, %bb.q ] ; 2 uses
  %i.hc = icmp slt i32 %.2129, %.2126
  br i1 %i.hc, label %bb.o, label %.loopexit

.loopexit:                                        ; preds = %bb.r, %bb.n, %.thread, %bb.m, %bb.l, %bb.g, %bb.a, %bb.f, %bb.e, %bb.d, %bb.c, %bb.b
  %.8 = phi i32 [ 0, %bb.m ], [ 0, %bb.b ], [ 0, %bb.d ], [ 0, %bb.a ], [ 0, %bb.g ], [ %i.t, %bb.c ], [ %i.ar, %bb.f ], [ 0, %bb.e ], [ %.2, %bb.l ], [ %.5, %.thread ], [ 0, %bb.n ], [ 0, %bb.r ]
  ret i32 %.8
}
