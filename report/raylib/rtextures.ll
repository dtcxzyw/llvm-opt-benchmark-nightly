...
.preheader:                                       ; preds = %.preheader.preheader, %._crit_edge
  ...
  %6 = insertelement <2 x float> poison, float %.042, i64 1
  br label %bb.b
...
bb.b:                                             ; preds = %.preheader, %stb_perlin_fbm_noise3.exit
  ...
  %7 = insertelement <2 x float> %6, float %.041, i64 0
  br label %.lr.ph.i
...
.lr.ph.i:                                         ; preds = %.lr.ph.i, %bb.b
  ...
  %8 = shufflevector <2 x float> %i.ai, <2 x float> poison, <2 x i32> <i32 1, i32 1> ; 2 uses
  %9 = fmul <2 x float> %7, %8                    ; 2 uses
  %10 = shufflevector <2 x float> %8, <2 x float> %9, <2 x i32> <i32 0, i32 3> ; 3 uses
  %11 = fptosi <2 x float> %10 to <2 x i32>       ; 2 uses
  %12 = shufflevector <2 x float> %9, <2 x float> %i.ai, <3 x i32> <i32 0, i32 3, i32 1> ; 3 uses
  %13 = fptosi <3 x float> %12 to <3 x i32>       ; 2 uses
  %i.aj = sitofp <2 x i32> %11 to <2 x float>
  %14 = sitofp <3 x i32> %13 to <3 x float>
  %i.ak = fcmp olt <2 x float> %10, %i.aj
  %15 = fcmp olt <3 x float> %12, %14
  %i.al = sext <2 x i1> %i.ak to <2 x i32>
  %16 = sext <3 x i1> %15 to <3 x i32>
  %i.am = add nsw <2 x i32> %i.al, %11            ; 3 uses
  %17 = add nsw <3 x i32> %16, %13                ; 2 uses
  %18 = extractelement <3 x i32> %17, i64 0       ; 2 uses
  %i.an = and i32 %18, 255
  %i.ao = add nsw i32 %18, 1
  ...
  %19 = sitofp <3 x i32> %17 to <3 x float>
  %20 = fsub <3 x float> %12, %19                 ; 2 uses
  %21 = extractelement <3 x float> %20, i64 0     ; 5 uses
  %i.az = tail call float @llvm.fmuladd.f32(float %21, float 6.000000e+00, float -1.500000e+01)
  %i.ba = tail call float @llvm.fmuladd.f32(float %i.az, float %21, float 1.000000e+01)
  %i.bb = fmul float %21, %i.ba
  %i.bc = fmul float %21, %i.bb
  ...
  %i.ed = shufflevector <3 x float> %20, <3 x float> poison, <2 x i32> zeroinitializer
