...
bb.a:
  ...
  %foldExtExtBinop60 = fsub <2 x float> %2, %3
  ...
  %6 = fsub <2 x float> %4, %5                    ; 3 uses
  %7 = shufflevector <2 x float> %0, <2 x float> %1, <2 x i32> <i32 1, i32 3>
  %8 = shufflevector <2 x float> %3, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.a = fsub <2 x float> %7, %8                  ; 2 uses
  %9 = shufflevector <2 x float> %foldExtExtBinop, <2 x float> poison, <2 x i32> zeroinitializer
  %foldExtExtBinop60.a = fmul <2 x float> %9, %i.a
  %10 = shufflevector <2 x float> %foldExtExtBinop60, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %11 = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %10, <2 x float> %6, <2 x float> %foldExtExtBinop60.a) ; 2 uses
  %foldExtExtBinop62 = fsub <2 x float> %3, %1
  %12 = extractelement <2 x float> %foldExtExtBinop62, i64 1
  %shift = shufflevector <2 x float> %6, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop64 = fmul <2 x float> %shift, %i.a
  %13 = extractelement <2 x float> %foldExtExtBinop64, i64 0
  %14 = extractelement <2 x float> %6, i64 0
  %15 = tail call float @llvm.fmuladd.f32(float %12, float %14, float %13)
  %i.b = insertelement <2 x float> %11, float %15, i64 1
  %16 = shufflevector <2 x float> %11, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.c = fdiv <2 x float> %i.b, %16               ; 2 uses
  %i.d = extractelement <2 x float> %i.c, i64 0   ; 2 uses
  %i.e = fsub float 1.000000e+00, %i.d
  %i.f = fcmp ogt float %i.d, 0.000000e+00
  %i.g = extractelement <2 x float> %i.c, i64 1   ; 2 uses
  %i.h = fcmp ogt float %i.g, 0.000000e+00
  %i.i = fcmp ogt float %i.e, %i.g
  %i.j = and i1 %i.h, %i.i
  %or.cond3 = select i1 %i.f, i1 %i.j, i1 false
  ret i1 %or.cond3
}
