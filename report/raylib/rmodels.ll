...
MatrixRotate.exit:                                ; preds = %bb.a, %bb.b
  ...
  %10 = fmul float %i.co, 0.000000e+00            ; 2 uses
  %11 = extractelement <2 x float> %i.cj, i64 0   ; 3 uses
  %12 = fadd float %11, %10
  %i.cp = tail call float @llvm.fmuladd.f32(float %i.bv, float 0.000000e+00, float %12)
  %i.cq = fadd float %.sroa.030.0.vec.extract, %i.cp
  %i.cr = extractelement <2 x float> %i.cj, i64 1
  %i.cs = tail call float @llvm.fmuladd.f32(float %11, float 0.000000e+00, float %i.cr)
  %13 = tail call float @llvm.fmuladd.f32(float %i.bv, float 0.000000e+00, float %i.cs)
  %14 = fadd float %.sroa.030.4.vec.extract, %13
  %15 = tail call float @llvm.fmuladd.f32(float %11, float 0.000000e+00, float %10) ; 2 uses
  %i.ct = fadd float %i.bv, %15
  ...
  %i.ds = shufflevector <4 x float> %16, <4 x float> poison, <4 x i32> zeroinitializer
  ...
  %17 = insertelement <4 x float> %i.ek, float %15, i64 3
  %i.el = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.ej, <4 x float> zeroinitializer, <4 x float> %17)
