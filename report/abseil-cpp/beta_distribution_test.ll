...
bb.bp:                                            ; preds = %bb.f
  ...
  %25 = uitofp i64 %24 to double                  ; 2 uses
  %26 = load <2 x double>, ptr %i.y, align 8, !tbaa !90 ; 2 uses
  %.val65.i = load double, ptr %i.z, align 16, !tbaa !108 ; 5 uses
  %27 = extractelement <2 x double> %26, i64 0    ; 4 uses
  %28 = fmul double %27, %.val65.i                ; 2 uses
  %i.le = fadd double %27, 2.000000e+00
  %29 = fadd double %i.le, %.val65.i
  %30 = fneg double %29
  %31 = fmul double %28, %30
  %32 = fadd double %27, %.val65.i                ; 5 uses
  %33 = fadd double %32, 1.000000e+00             ; 2 uses
  %34 = fdiv double %28, %33
  %35 = fdiv double %34, %32
  %36 = fsub double %27, %.val65.i                ; 2 uses
  %i.lf = fmul double %36, %36
  %i.lg = call double @llvm.fmuladd.f64(double %i.lf, double %33, double %31)
  %i.lh = fmul double %i.lg, 6.000000e+00
  %i.li = fdiv double %35, %32                    ; 4 uses
  %i.lj = insertelement <2 x double> poison, double %i.li, i64 0
  %i.lk = insertelement <2 x double> %i.lj, double %i.lh, i64 1
  %37 = shufflevector <2 x double> %26, <2 x double> poison, <2 x i32> <i32 poison, i32 0>
  %i.ll = insertelement <2 x double> %37, double %25, i64 0
  %i.lm = fdiv <2 x double> %i.lk, %i.ll          ; 2 uses
  %i.ln = extractelement <2 x double> %i.lm, i64 0
  %i.lo = call double @sqrt(double noundef %i.ln) #39
  %i.lp = extractelement <2 x double> %i.lm, i64 1
  %i.lq = fdiv double %i.lp, %.val65.i
  %i.lr = fadd double %32, 2.000000e+00
  %i.ls = fdiv double %i.lq, %i.lr
  %i.lt = fadd double %32, 3.000000e+00
  %i.lu = fdiv double %i.ls, %i.lt
  %i.lv = fadd double %i.lu, 3.000000e+00
  %i.lw = fadd double %i.lv, -1.000000e+00
  %i.lx = fmul double %i.li, %i.lw
  %i.ly = fmul double %i.li, %i.lx
  %i.lz = fdiv double %i.ly, %25
  %i.ma = call double @sqrt(double noundef %i.lz) #39 ; 2 uses
  %i.mb = load double, ptr %i.ar, align 8, !tbaa !138
  %i.mc = fsub double %i.mb, %i.li
