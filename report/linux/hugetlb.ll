...
pgd_present.exit.thread.i.i:                      ; preds = %pgd_present.exit.i.i, %bb.g
  callbr void asm sideeffect "# ALT: oldinstr\0A771:\0A\09# ALT: oldinstr\0A771:\0A\09jmp 6f\0A772:\0A# ALT: padding\0A.skip -(((775f-774f)-(772b-771b)) > 0) * ((775f-774f)-(772b-771b)),0x90\0A773:\0A.pushsection .altinstructions, \22aM\22, @progbits, 14\0A .long 771b - .\0A .long 774f - .\0A .4byte ( 3*32+21)\0A .byte 773b-771b\0A .byte 775f-774f\0A.popsection\0A.pushsection .altinstr_replacement, \22ax\22\0A912: .pushsection .discard.annotate_data, \22M\22, @progbits, 8; .long 912b - ., 1; .popsection\0A# ALT: replacement\0A774:\0A\09jmp ${4:l}\0A775:\0A.popsection\0A\0A772:\0A# ALT: padding\0A.skip -(((775f-774f)-(772b-771b)) > 0) * ((775f-774f)-(772b-771b)),0x90\0A773:\0A.pushsection .altinstructions, \22aM\22, @progbits, 14\0A .long 771b - .\0A .long 774f - .\0A .4byte ${0:c}\0A .byte 773b-771b\0A .byte 775f-774f\0A.popsection\0A.pushsection .altinstr_replacement, \22ax\22\0A912: .pushsection .discard.annotate_data, \22M\22, @progbits, 8; .long 912b - ., 1; .popsection\0A# ALT: replacement\0A774:\0A\09\0A775:\0A.popsection\0A.pushsection .altinstr_aux,\22ax\22\0A6:\0A912: .pushsection .discard.annotate_data, \22M\22, @progbits, 8; .long 912b - ., 1; .popsection\0A testb $1, ${2:a}\0A jnz ${3:l}\0A jmp ${4:l}\0A.popsection\0A", "i,i,i,!i,!i,~{dirflag},~{fpsr},~{flags}"(i16 528, i32 1, ptr nonnull getelementptr inbounds nuw (i8, ptr @boot_cpu_data, i64 114)) #28
          to label %bb.h [label %bb.h, label %p4d_offset.exit.i.i], !srcloc !47
...
softleaf_from_pte.exit:                           ; preds = %bb.v
  ...
  br i1 %switch.i, label %bb.w, label %softleaf_from_pte.exit.thread, !prof !50
...
softleaf_from_pte.exit.thread:                    ; preds = %bb.v, %softleaf_from_pte.exit
  ...
  br i1 %spec.select.i, label %bb.z, label %bb.ab, !prof !21
...
bb.ab:                                            ; preds = %softleaf_from_pte.exit.thread
  ...
  call void asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock incl $0", "=*m,*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.gd, ptr elementtype(i32) %i.gd) #28, !srcloc !51
...
bb.ac:                                            ; preds = %bb.ab
  ...
  call void asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock incl $0", "=*m,*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.gj, ptr elementtype(i32) %i.gj) #28, !srcloc !51
  call void asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock incl $0", "=*m,*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.gi, ptr elementtype(i32) %i.gi) #28, !srcloc !51
  br label %bb.ba
...
.split.i:                                         ; preds = %bb.af
  ...
  br i1 %i.hh, label %bb.ag, label %folio_needs_cow_for_dma.exit.thread.i, !prof !50
...
folio_needs_cow_for_dma.exit.i:                   ; preds = %bb.af
  ...
  br i1 %i.hk, label %bb.ag, label %folio_needs_cow_for_dma.exit.thread.i, !prof !50
...
folio_needs_cow_for_dma.exit.thread.i:            ; preds = %folio_needs_cow_for_dma.exit.i, %.split.i, %mm_flags_test.exit.i.i
  ...
  call void asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock andb ${1:b},$0", "=*m,iq,*m,~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i8) %i.hl, i32 -9, ptr elementtype(i8) %i.hl) #28, !srcloc !30
...
hugetlb_try_dup_anon_rmap.exit:                   ; preds = %PageAnonExclusive.exit.i, %folio_needs_cow_for_dma.exit.thread.i
  ...
  call void asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock incl $0", "=*m,*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.hn, ptr elementtype(i32) %i.hn) #28, !srcloc !51
  call void asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock incl $0", "=*m,*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.hm, ptr elementtype(i32) %i.hm) #28, !srcloc !51
  br label %bb.ba
...
bb.ah:                                            ; preds = %bb.ag
  %i.hq = call i8 asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock decl $0", "=*m,={@cce},*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.gd, ptr elementtype(i32) %i.gd) #28, !srcloc !52 ; 2 uses
...
bb.aj:                                            ; preds = %bb.ag
  ...
  %i.hv = call i8 asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock decl $0", "=*m,={@cce},*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.gd, ptr elementtype(i32) %i.gd) #28, !srcloc !52 ; 2 uses
...
bb.al:                                            ; preds = %folio_put.exit217
  ...
  %i.hy = call i8 asm sideeffect ".pushsection .smp_locks,\22a\22\0A.balign 4\0A.long 671f - .\0A.popsection\0A671:\0A\09lock decl $0", "=*m,={@cce},*m,~{memory},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i32) %i.hx, ptr elementtype(i32) %i.hx) #28, !srcloc !52 ; 2 uses
