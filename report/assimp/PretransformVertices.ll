...
bb.h:                                             ; preds = %bb.g, %.loopexit117
  ...
  %4 = load float, ptr %3, align 4                ; 3 uses
  %5 = load float, ptr %i.fh, align 4             ; 3 uses
  ...
  %i.fr = fneg float %5                           ; 2 uses
  %i.fs = fmul float %i.fq, %i.fr
  %i.ft = tail call float @llvm.fmuladd.f32(float %i.fo, float %4, float %i.fs)
  ...
  %i.fz = tail call float @llvm.fmuladd.f32(float %i.fy, float %4, float %i.fv)
  %i.ga = fmul float %i.fc, %i.fw
  %i.gb = tail call float @llvm.fmuladd.f32(float %i.ga, float %5, float %i.fz)
...
bb.i:                                             ; preds = %bb.h
  ...
  %i.gs = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gr, <2 x float> %7, <2 x float> %i.gl)
  ...
  %i.ha = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.gz, <2 x float> %7, <2 x float> %i.gy)
  ...
  %i.he = shufflevector <2 x float> %8, <2 x float> poison, <2 x i32> zeroinitializer
