...
bb.v:                                             ; preds = %bb.u, %bb.t, %bb.s, %bb.r
  ...
  %.198.i.a = select i1 %i.fj, double %.097.i, double %.2.i
  %.3.i = select i1 %i.fj, double %.2.i, double %.097.i
  %2 = insertelement <2 x double> poison, double %.198.i.a, i64 0
  %3 = insertelement <2 x double> %2, double %.3.i, i64 1 ; 2 uses
  %4 = fcmp ogt <2 x double> %3, zeroinitializer
  %5 = select <2 x i1> %4, <2 x double> %3, <2 x double> zeroinitializer ; 2 uses
  %6 = fcmp olt <2 x double> %5, splat (double 1.000000e+00)
  %7 = select <2 x i1> %6, <2 x double> %5, <2 x double> splat (double 1.000000e+00) ; 4 uses
  %shift = shufflevector <2 x double> %7, <2 x double> poison, <2 x i32> <i32 1, i32 poison>
  %foldExtExtBinop = fsub <2 x double> %shift, %7
  %8 = extractelement <2 x double> %foldExtExtBinop, i64 0
  %i.fk = tail call double @llvm.fabs.f64(double %8)
