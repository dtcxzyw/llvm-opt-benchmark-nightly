...
.lr.ph302:                                        ; preds = %.lr.ph302.preheader, %_ZN13aiQuaterniontIfEC2ERK12aiMatrix3x3tIfE.exit162
  ...
  %3 = shufflevector <2 x float> %i.oj, <2 x float> poison, <2 x i32> <i32 1, i32 0> ; 2 uses
  %4 = extractelement <2 x float> %i.oj, i64 0    ; 4 uses
  %i.om = tail call float @llvm.fmuladd.f32(float %4, float %4, float %i.ol)
  %5 = tail call float @llvm.fmuladd.f32(float %i.om, float -2.000000e+00, float 1.000000e+00) ; 5 uses
  %i.on = fmul <2 x float> %3, %i.of
  ...
  %6 = insertelement <2 x float> poison, float %i.oc, i64 0
  %7 = shufflevector <2 x float> %6, <2 x float> poison, <2 x i32> zeroinitializer
  %i.os = fmul <2 x float> %i.oj, %7
  %i.ot = shufflevector <2 x float> %i.og, <2 x float> poison, <2 x i32> zeroinitializer ; 3 uses
  %i.ou = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ot, <2 x float> %3, <2 x float> %i.os)
  %8 = extractelement <2 x float> %i.oj, i64 1    ; 2 uses
  %9 = tail call float @llvm.fmuladd.f32(float %4, float %8, float %i.oh)
  ...
  %i.oz = tail call float @llvm.fmuladd.f32(float %4, float %8, float %i.oi)
  %i.pa = fmul float %i.oz, 2.000000e+00          ; 4 uses
  %i.pb = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ot, <2 x float> %i.ot, <2 x float> %i.ok)
...
bb.ak:                                            ; preds = %.lr.ph302
  %i.px = insertelement <2 x float> poison, float %5, i64 0
...
bb.al:                                            ; preds = %bb.ak
  %i.qb = fadd float %5, 1.000000e+00
