...
.lr.ph1414:                                       ; preds = %.lr.ph1414.preheader, %.lr.ph1414
  ...
  %5 = load <3 x float>, ptr %i.bgm, align 4      ; 5 uses
  %i.bgo = load float, ptr %i.bgm, align 4
  %6 = shufflevector <3 x float> %5, <3 x float> poison, <2 x i32> <i32 1, i32 1>
  %i.bgp = fmul <2 x float> %i.bfq, %6
  %i.bgq = shufflevector <3 x float> %5, <3 x float> poison, <2 x i32> zeroinitializer
  ...
  %8 = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.bfs, <2 x float> %7, <2 x float> %i.bgr)
  %9 = fadd <2 x float> %i.bft, %8
  %10 = extractelement <3 x float> %5, i64 1
  %i.bgs = fmul float %.sroa.12.0.copyload, %10
  %i.bgt = call float @llvm.fmuladd.f32(float %.sroa.11.0.copyload, float %i.bgo, float %i.bgs)
  %11 = extractelement <3 x float> %5, i64 2
  %i.bgu = call float @llvm.fmuladd.f32(float %.sroa.13.0.copyload, float %11, float %i.bgt)
  %i.bgv = fadd float %.sroa.14.0.copyload, %i.bgu
  store <2 x float> %9, ptr %i.bgm, align 4
