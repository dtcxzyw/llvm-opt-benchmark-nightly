...
_ZN10aiVector3tIfE9NormalizeEv.exit:              ; preds = %bb.b
  ...
  %5 = fmul float %i.y, %i.ag
  %6 = fmul float %i.aa, %i.ag
  %.sroa.01301.0 = select i1 %i.af, float %i.w, float %i.ah ; 5 uses
  %.sroa.01301.0.a = select i1 %i.af, float %i.y, float %5 ; 5 uses
  %.sroa.81306.0 = select i1 %i.af, float %i.aa, float %6 ; 5 uses
  %i.ai = fmul float %.sroa.01301.0.a, 0.000000e+00
  %i.aj = fadd float %.sroa.01301.0, %i.ai
  %i.ak = tail call noundef float @llvm.fmuladd.f32(float %.sroa.81306.0, float 0.000000e+00, float %i.aj)
  ...
  %7 = fmul float %.sroa.81306.0, %i.ao
  %8 = tail call float @llvm.fmuladd.f32(float %.sroa.01301.0.a, float 0.000000e+00, float %7) ; 4 uses
  %9 = fmul float %.sroa.01301.0, -0.000000e+00
  %10 = tail call float @llvm.fmuladd.f32(float %.sroa.81306.0, float %.sroa.01295.0, float %9) ; 4 uses
  %i.ap = fneg float %.sroa.01295.0
  %i.aq = fmul float %.sroa.01301.0.a, %i.ap
  %i.ar = tail call float @llvm.fmuladd.f32(float %.sroa.01301.0, float %.sroa.61297.0, float %i.aq) ; 4 uses
  ...
  %11 = fmul float %10, %10
  %i.as = tail call float @llvm.fmuladd.f32(float %8, float %8, float %11)
...
_ZN10aiVector3tIfEdVEf.exit.i181:                 ; preds = %_ZN10aiVector3tIfE9NormalizeEv.exit
  ...
  %i.av = fdiv float 1.000000e+00, %sqrt.i.i182   ; 3 uses
  %12 = fmul float %8, %i.av
  %i.aw = insertelement <2 x float> poison, float %12, i64 0
  %13 = fmul float %10, %i.av
  %.sroa.01269.4.vec.insert = insertelement <2 x float> %i.aw, float %13, i64 1
...
_ZN10aiVector3tIfE9NormalizeEv.exit183:           ; preds = %_ZN10aiVector3tIfE9NormalizeEv.exit, %_ZN10aiVector3tIfEdVEf.exit.i181
  ...
  %i.ay = fneg float %.sroa.01301.0.a
  %i.az = fmul float %.sroa.81272.0, %i.ay
  %i.ba = tail call float @llvm.fmuladd.f32(float %.sroa.01273.4.vec.extract, float %.sroa.81306.0, float %i.az) ; 4 uses
  %.sroa.01273.0.vec.extract = extractelement <2 x float> %.sroa.01269.0, i64 0 ; 2 uses
  %i.bb = fneg float %.sroa.81306.0
  %i.bc = fmul float %.sroa.01273.0.vec.extract, %i.bb
  %i.bd = tail call float @llvm.fmuladd.f32(float %.sroa.81272.0, float %.sroa.01301.0, float %i.bc) ; 4 uses
  %i.be = fneg float %.sroa.01301.0
  %i.bf = fmul float %.sroa.01273.4.vec.extract, %i.be
  %i.bg = tail call float @llvm.fmuladd.f32(float %.sroa.01273.0.vec.extract, float %.sroa.01301.0.a, float %i.bf) ; 4 uses
