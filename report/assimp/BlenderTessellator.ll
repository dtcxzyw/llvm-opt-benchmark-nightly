...
begin_hunk_1_@_ZNK6Assimp21BlenderTessellatorP2T13FindLLSQPlaneERKSt6vectorINS_7Blender8PointP2TESaIS3_EE:bb.a
  ...
  %i.aa = fmul <4 x float> %i.x, %i.z             ; 3 uses
...
._crit_edge90:                                    ; preds = %.lr.ph89, %._crit_edge
  %4 = phi <4 x float> [ zeroinitializer, %._crit_edge ], [ %i.cm, %.lr.ph89 ] ; 10 uses
  %5 = phi <2 x float> [ zeroinitializer, %._crit_edge ], [ %i.ck, %.lr.ph89 ] ; 4 uses
  %i.bg = extractelement <2 x float> %5, i64 0    ; 6 uses
  %i.bh = extractelement <4 x float> %4, i64 0    ; 3 uses
  %i.bi = extractelement <2 x float> %5, i64 1    ; 3 uses
  %i.bj = fmul float %i.bh, %i.bi
  %i.bk = extractelement <4 x float> %4, i64 3    ; 4 uses
  ...
  %i.bq = extractelement <4 x float> %4, i64 2    ; 4 uses
...
.lr.ph89:                                         ; preds = %._crit_edge, %.lr.ph89
  %.06081 = phi i64 [ %niter143.next.1, %.lr.ph89 ], [ 0, %._crit_edge ] ; 2 uses
  %i.cc = phi <4 x float> [ %i.cm, %.lr.ph89 ], [ zeroinitializer, %._crit_edge ]
  %i.cd = phi <2 x float> [ %i.ck, %.lr.ph89 ], [ zeroinitializer, %._crit_edge ]
  %i.ce = getelementptr inbounds nuw [64 x i8], ptr %i.d, i64 %.06081 ; 2 uses
  %i.cf = getelementptr inbounds nuw i8, ptr %i.ce, i64 4
  %6 = load float, ptr %i.ce, align 4
  %i.cg = load <2 x float>, ptr %i.cf, align 4
  %7 = shufflevector <2 x float> %i.cg, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
  %8 = insertelement <4 x float> %7, float %6, i64 2
  %9 = shufflevector <4 x float> %8, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 2, i32 0>
  %i.ch = fsub <4 x float> %9, %i.aa              ; 4 uses
  ...
  %i.ck = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.ci, <2 x float> %i.cj, <2 x float> %i.cd) ; 2 uses
  %i.cl = shufflevector <4 x float> %i.ch, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
  %i.cm = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.ch, <4 x float> %i.cl, <4 x float> %i.cc) ; 2 uses
  %niter143.next.1 = add nuw i64 %.06081, 1       ; 2 uses
  %niter143.ncmp.1 = icmp eq i64 %niter143.next.1, %i.h
  br i1 %niter143.ncmp.1, label %._crit_edge90, label %.lr.ph89, !llvm.loop !17
...
begin_hunk_4_@_ZNK6Assimp21BlenderTessellatorP2T13FindLLSQPlaneERKSt6vectorINS_7Blender8PointP2TESaIS3_EE:bb.a
  %i.cp = fneg float %i.bg                        ; 2 uses
  %i.cq = tail call float @llvm.fmuladd.f32(float %i.bi, float %i.bh, float %i.bu)
  %i.cr = fmul float %i.cq, %i.cn
  %i.cs = shufflevector <4 x float> %4, <4 x float> poison, <4 x i32> <i32 3, i32 poison, i32 3, i32 poison>
  %i.ct = insertelement <4 x float> %i.cs, float %i.bg, i64 1
  %i.cu = insertelement <4 x float> %i.ct, float %i.bz, i64 3
  %i.cv = insertelement <4 x float> <float poison, float poison, float poison, float 1.000000e+00>, float %i.bm, i64 0
  %i.cw = insertelement <4 x float> %i.cv, float %i.cp, i64 2
  %i.cx = shufflevector <4 x float> %i.cw, <4 x float> poison, <4 x i32> <i32 0, i32 0, i32 2, i32 3>
  %i.cy = fmul <4 x float> %i.cu, %i.cx
  %i.cz = shufflevector <4 x float> %4, <4 x float> poison, <4 x i32> <i32 1, i32 2, i32 1, i32 2>
  %i.da = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %4, <4 x float> %i.cz, <4 x float> %i.cy)
  %i.db = insertelement <4 x float> poison, float %i.cn, i64 0
  %i.dc = insertelement <4 x float> %i.db, float %i.co, i64 1 ; 2 uses
  %i.dd = shufflevector <4 x float> %i.da, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 3, i32 2>
  %i.de = shufflevector <4 x float> %i.dc, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
  %i.df = fmul <4 x float> %i.dd, %i.de
  store <4 x float> %i.df, ptr %3, align 16
  %i.dg = shufflevector <2 x float> %5, <2 x float> poison, <4 x i32> <i32 0, i32 poison, i32 poison, i32 poison>
  %i.dh = shufflevector <4 x float> %i.dg, <4 x float> %4, <4 x i32> <i32 0, i32 0, i32 4, i32 6>
  %i.di = insertelement <4 x float> poison, float %i.cp, i64 0
  %i.dj = insertelement <4 x float> %i.di, float %i.bt, i64 1
  %i.dk = shufflevector <4 x float> %i.dj, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 0>
  %i.dl = fmul <4 x float> %i.dh, %i.dk
  %i.dm = shufflevector <4 x float> %4, <4 x float> poison, <4 x i32> <i32 1, i32 3, i32 3, i32 poison>
  %i.dn = shufflevector <2 x float> %5, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison> ; 2 uses
  %i.do = shufflevector <4 x float> %i.dm, <4 x float> %i.dn, <4 x i32> <i32 0, i32 1, i32 2, i32 5>
  %i.dp = shufflevector <4 x float> %i.dn, <4 x float> %4, <4 x i32> <i32 1, i32 1, i32 6, i32 7>
  %i.dq = tail call <4 x float> @llvm.fmuladd.v4f32(<4 x float> %i.do, <4 x float> %i.dp, <4 x float> %i.dl)
  %i.dr = shufflevector <4 x float> %i.dc, <4 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
