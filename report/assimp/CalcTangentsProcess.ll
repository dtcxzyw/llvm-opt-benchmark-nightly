...
.lr.ph.preheader:                                 ; preds = %bb.n
  ...
  %i.eg = shufflevector <2 x float> %7, <2 x float> poison, <2 x i32> zeroinitializer
  %i.eh = fmul <2 x float> %i.eg, %i.dt
  %8 = insertelement <2 x float> poison, float %6, i64 0
  %9 = shufflevector <2 x float> %8, <2 x float> poison, <2 x i32> zeroinitializer
  %i.ei = tail call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %9, <2 x float> %i.dr, <2 x float> %i.eh) ; 2 uses
