...
bb.ni:                                            ; preds = %bb.nh, %_ZN10aiVector3tIfEdVEf.exit.i
  ...
  %.sroa.0905.0.vec.extract908 = extractelement <2 x float> %.sroa.0905.0, i64 1
  %i.axb = fneg float %.sroa.10.0
  ...
  %10 = shufflevector <2 x float> %.sroa.0905.0, <2 x float> poison, <2 x i32> <i32 poison, i32 0>
  %i.axe = insertelement <2 x float> %10, float %.sroa.10.0, i64 0
  %11 = fneg <2 x float> %.sroa.0905.0            ; 2 uses
  %12 = shufflevector <2 x float> %11, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %i.axf = insertelement <2 x float> %12, float %i.axb, i64 1
  %13 = fmul <2 x float> %i.axd, %i.axf
  %14 = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.awi, <2 x float> %i.axe, <2 x float> %13)
  %foldExtExtBinop1968 = fmul <2 x float> %i.awi, %11
  %15 = extractelement <2 x float> %foldExtExtBinop1968, i64 0
  %i.axg = call float @llvm.fmuladd.f32(float %i.awd, float %.sroa.0905.0.vec.extract908, float %15)
  br label %bb.nj
