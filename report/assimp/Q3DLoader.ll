...
bb.nm:                                            ; preds = %bb.nl, %_ZN10aiVector3tIfEdVEf.exit.i
  ...
  %.sroa.0906.4.vec.extract912.a = extractelement <2 x float> %.sroa.0906.0, i64 1
  ...
  %10 = shufflevector <2 x float> %.sroa.0906.0, <2 x float> poison, <2 x i32> <i32 poison, i32 0>
  %i.ayb = insertelement <2 x float> %10, float %.sroa.10.0, i64 0
  %11 = fneg <2 x float> %.sroa.0906.0            ; 2 uses
  %12 = shufflevector <2 x float> %11, <2 x float> poison, <2 x i32> <i32 1, i32 poison>
  %i.ayc = insertelement <2 x float> %12, float %i.axy, i64 1
  %13 = fmul <2 x float> %i.aya, %i.ayc
  %14 = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %i.axf, <2 x float> %i.ayb, <2 x float> %13)
  %foldExtExtBinop1974 = fmul <2 x float> %i.axf, %11
  %15 = extractelement <2 x float> %foldExtExtBinop1974, i64 0
  %i.ayd = call float @llvm.fmuladd.f32(float %i.axa, float %.sroa.0906.4.vec.extract912.a, float %15)
  br label %bb.nn
