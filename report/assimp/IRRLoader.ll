...
begin_hunk_0_@_ZN6Assimp11IRRImporter13GenerateGraphEPNS0_4NodeEP6aiNodeP7aiSceneRNS_11BatchLoaderERSt6vectorIP6aiMeshSaISB_EERS9_IP10aiNodeAnimSaISG_EERS9_INS_14AttachmentInfoESaISK_EERS9_IP10aiMaterialSaISP_EERj:bb.a
  ...
  %14 = load float, ptr %i.mv, align 4
  %15 = fmul float %14, 0x3F91DF46A0000000        ; 2 uses
  %16 = getelementptr inbounds nuw i8, ptr %1, i64 24
  %17 = load float, ptr %16, align 4
  %18 = fmul float %17, 0x3F91DF46A0000000        ; 2 uses
  %19 = call noundef float @cosf(float noundef %i.mu) #27 ; 5 uses
  %i.mw = call noundef float @sinf(float noundef %i.mu) #27 ; 5 uses
  %i.mx = call noundef float @cosf(float noundef %15) #27 ; 4 uses
  %20 = call noundef float @sinf(float noundef %15) #27 ; 3 uses
  %i.my = call noundef float @cosf(float noundef %18) #27 ; 4 uses
  %i.mz = call noundef float @sinf(float noundef %18) #27 ; 4 uses
  %i.na = fmul float %i.mx, %i.my                 ; 2 uses
  store float %i.na, ptr %i.mr, align 4
  %i.nb = fmul float %20, %i.my                   ; 2 uses
  %i.nc = fneg float %19
  %i.nd = fmul float %i.mz, %i.nc
  %i.ne = call float @llvm.fmuladd.f32(float %i.nb, float %i.mw, float %i.nd) ; 2 uses
  %i.nf = getelementptr inbounds nuw i8, ptr %2, i64 1032 ; 2 uses
  store float %i.ne, ptr %i.nf, align 4
  %i.ng = fmul float %19, %i.nb
  %i.nh = call float @llvm.fmuladd.f32(float %i.mz, float %i.mw, float %i.ng) ; 2 uses
  %i.ni = getelementptr inbounds nuw i8, ptr %2, i64 1036 ; 2 uses
  store float %i.nh, ptr %i.ni, align 4
  %i.nj = fmul float %i.mx, %i.mz                 ; 2 uses
  %i.nk = getelementptr inbounds nuw i8, ptr %2, i64 1044 ; 2 uses
  store float %i.nj, ptr %i.nk, align 4
  %i.nl = fmul float %20, %i.mz                   ; 2 uses
  %i.nm = fmul float %i.mw, %i.nl
  %i.nn = call float @llvm.fmuladd.f32(float %i.my, float %19, float %i.nm) ; 2 uses
  %i.no = getelementptr inbounds nuw i8, ptr %2, i64 1048 ; 2 uses
  store float %i.nn, ptr %i.no, align 4
  %i.np = fneg float %i.mw
  %i.nq = fmul float %i.my, %i.np
  %i.nr = call float @llvm.fmuladd.f32(float %i.nl, float %19, float %i.nq) ; 2 uses
  %i.ns = getelementptr inbounds nuw i8, ptr %2, i64 1052 ; 2 uses
  store float %i.nr, ptr %i.ns, align 4
  %i.nt = fneg float %20                          ; 2 uses
  %i.nu = getelementptr inbounds nuw i8, ptr %2, i64 1060 ; 2 uses
  store float %i.nt, ptr %i.nu, align 4
  %i.nv = fmul float %i.mw, %i.mx                 ; 2 uses
  %i.nw = getelementptr inbounds nuw i8, ptr %2, i64 1064 ; 2 uses
  store float %i.nv, ptr %i.nw, align 4
  %i.nx = fmul float %19, %i.mx                   ; 2 uses
