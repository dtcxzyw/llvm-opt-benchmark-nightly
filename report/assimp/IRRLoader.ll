...
bb.bm:                                            ; preds = %.lr.ph566, %bb.bm
  ...
  %i.sf = fmul float %i.rr, 2.000000e+00          ; 6 uses
  ...
  %4 = tail call float @llvm.fmuladd.f32(float %i.sf, float -2.000000e+00, float %i.si) ; 2 uses
  %5 = tail call float @llvm.fmuladd.f32(float %i.sf, float -2.000000e+00, float %i.sf) ; 2 uses
  ...
  %8 = fmul <2 x float> %i.sl, %7
  %i.sz = fmul float %i.rt, %4
  %i.ta = fadd <2 x float> %8, %i.sx
  %i.tb = fadd float %i.sz, %i.sy
  %9 = insertelement <2 x float> poison, float %5, i64 0
  %10 = shufflevector <2 x float> %9, <2 x float> poison, <2 x i32> zeroinitializer
  %11 = fmul <2 x float> %10, %i.sq
  %i.tc = fmul float %5, %i.ry
  %i.td = fadd <2 x float> %11, %i.ta
