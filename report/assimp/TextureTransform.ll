...
bb.cz:                                            ; preds = %bb.cx, %_ZNK6Assimp17STransformVecInfo15IsUntransformedEv.exit338.thread
  ...
  %i.up = extractelement <4 x float> %.fr847, i64 3
  %3 = fmul float %i.up, 0.000000e+00
  %4 = extractelement <4 x float> %.fr847, i64 2
  %5 = call float @llvm.fmuladd.f32(float %4, float 0.000000e+00, float 0.000000e+00)
  %6 = shufflevector <4 x float> %.fr847, <4 x float> poison, <2 x i32> <i32 2, i32 3> ; 2 uses
  %i.uq = insertelement <2 x float> %6, float %5, i64 1
  %i.ur = fadd <2 x float> %i.uq, zeroinitializer ; 4 uses
  %7 = insertelement <2 x float> %6, float %3, i64 0
  %i.us = fadd <2 x float> %7, zeroinitializer    ; 4 uses
