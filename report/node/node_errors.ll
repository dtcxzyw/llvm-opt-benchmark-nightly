...
bb.bk:                                            ; preds = %bb.bj, %_ZNO2v85MaybeIiE8FromJustEv.exit
  %.063 = phi i32 [ %i.iq, %bb.bj ], [ %.sroa.speculated110, %_ZNO2v85MaybeIiE8FromJustEv.exit ] ; 8 uses
  %.062 = phi i32 [ %i.ir, %bb.bj ], [ %.sroa.speculated, %_ZNO2v85MaybeIiE8FromJustEv.exit ] ; 5 uses
...
bb.bm:                                            ; preds = %bb.bk
  ...
  %i.iu = icmp sgt i32 %.063, %.062
  %i.iv = icmp slt i32 %.063, 0
  %or.cond3 = or i1 %i.iv, %i.iu
  %21 = zext nneg i32 %.062 to i64
...
._crit_edge:                                      ; preds = %bb.br, %._crit_edge.loopexit.split.loop.exit, %bb.bp
  %.059.lcssa = phi i32 [ 0, %bb.bp ], [ %i.jm, %._crit_edge.loopexit.split.loop.exit ], [ %.063, %bb.br ] ; 3 uses
  %i.jn = icmp slt i32 %.063, %.062
  br i1 %i.jn, label %.lr.ph153, label %._crit_edge154

.lr.ph153:                                        ; preds = %._crit_edge
  %i.jo = load ptr, ptr %11, align 8
  %i.jp = zext nneg i32 %.063 to i64
  %i.jq = zext i32 %.059.lcssa to i64
  %i.jr = add i32 %.062, %.059.lcssa
  %i.js = sub i32 %i.jr, %.063
  br label %bb.bs
