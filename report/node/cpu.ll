begin_hunk_0
  %i.r = getelementptr inbounds nuw i8, ptr %0, i64 76
  %i.s = getelementptr inbounds nuw i8, ptr %0, i64 77
  %i.t = getelementptr inbounds nuw i8, ptr %0, i64 78
  %i.u = getelementptr inbounds nuw i8, ptr %0, i64 79 ; 7 uses
  %i.v = getelementptr inbounds nuw i8, ptr %0, i64 81 ; 2 uses
  %i.w = getelementptr inbounds nuw i8, ptr %0, i64 82
  %i.x = getelementptr inbounds nuw i8, ptr %0, i64 83
end_hunk_0
begin_hunk_1
  br i1 %i.ds, label %bb.f, label %bb.n

bb.f:                                             ; preds = %bb.e
  %trunc = trunc nuw i32 %i.bo to i8
  switch i8 %trunc, label %bb.n [
    i8 28, label %bb.g
    i8 38, label %bb.g
end_hunk_1
begin_hunk_2
    i8 94, label %bb.j
    i8 -114, label %bb.k
    i8 -98, label %bb.l
    i8 -90, label %bb.m
    i8 -82, label %.sink.split
  ]

bb.g:                                             ; preds = %bb.f, %bb.f, %bb.f, %bb.f, %bb.f, %bb.f, %bb.f, %bb.f, %bb.f, %bb.f
  store i8 1, ptr %i.t, align 2
  br label %bb.n

bb.h:                                             ; preds = %bb.f
  %i.dt = icmp eq i32 %i.bj, 3
  %1 = zext i1 %i.dt to i8
  store i8 %1, ptr %i.u, align 1
  br label %bb.n

bb.i:                                             ; preds = %bb.f
  %i.du = icmp eq i32 %i.bj, 4
  %i.dv = icmp eq i32 %i.bj, 7
  %narrow49 = or i1 %i.du, %i.dv
  %2 = zext i1 %narrow49 to i8
  store i8 %2, ptr %i.u, align 1
  br label %bb.n

bb.j:                                             ; preds = %bb.f
  %i.dw = icmp eq i32 %i.bj, 3
  %3 = zext i1 %i.dw to i8
  store i8 %3, ptr %i.u, align 1
  br label %bb.n

bb.k:                                             ; preds = %bb.f
  %i.dx = add nsw i32 %i.bj, -9
  %narrow48 = icmp ult i32 %i.dx, 4
  %4 = zext i1 %narrow48 to i8
  store i8 %4, ptr %i.u, align 1
  br label %bb.n

bb.l:                                             ; preds = %bb.f
  %.off21 = add nsw i32 %i.bj, -9
  %switch22 = icmp ult i32 %.off21, 3
  %i.dy = icmp eq i32 %i.bj, 13
  %narrow = or i1 %i.dy, %switch22
  %5 = zext i1 %narrow to i8
  store i8 %5, ptr %i.u, align 1
  br label %bb.n

bb.m:                                             ; preds = %bb.f
  %i.dz = icmp eq i32 %i.bj, 0
  %6 = zext i1 %i.dz to i8
  store i8 %6, ptr %i.u, align 1
  br label %bb.n

.sink.split:                                      ; preds = %bb.f
  %7 = icmp eq i32 %i.bj, 10
  %i.ea = zext i1 %7 to i8
  store i8 %i.ea, ptr %i.u, align 1
  br label %bb.n

bb.n:                                             ; preds = %bb.g, %bb.f, %bb.e, %.sink.split, %bb.m, %bb.l, %bb.k, %bb.j, %bb.i, %bb.h
  br i1 %i.au, label %bb.o, label %.thread

bb.o:                                             ; preds = %bb.n
end_hunk_2
